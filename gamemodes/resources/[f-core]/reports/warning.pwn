...
stock JailUser(playerid, targetid, minutes, fee, reason[])
{
    ...
    new string[192];
    mysql_format(dcrp_query, string, sizeof(string), "INSERT INTO `warninglogs` (`pID`, `WarnType`, `WarnReason`, `WarnSender`, `WarnTime`) VALUES('%d', '%d', '%e', '%e', '%d')", AccountData[targetid][pID], TYPE_JAIL, reason, AccountData[playerid][pAdminname], gettime());
...
stock GetTotalWarning(playerid)
{
    ...
    mysql_format(dcrp_query, query, sizeof(query), "SELECT * FROM `warninglogs` WHERE `pID` = '%d' AND `WarnType` = '%d'", AccountData[playerid][pID], TYPE_WARN);
...
{
    new string[192];
    mysql_format(dcrp_query, string, sizeof(string), "INSERT INTO `warninglogs` (`pID`, `WarnType`, `WarnReason`, `WarnSender`, `WarnTime`) VALUES('%d', '%d', '%e', '%e', '%d')", AccountData[targetid][pID], TYPE_WARN, reason, AccountData[playerid][pAdminname], gettime());
    mysql_tquery(dcrp_query, string, "OnWarnUser", "dds", playerid, targetid, reason);
    return 1;
}
...
stock OfflineWarnUser(playerid, name[], reason[])
{
    ...
    mysql_format(dcrp_query, string, sizeof(string), "SELECT * FROM `player_characters` WHERE `Char_Name`='%e' LIMIT 1", name);
    mysql_query(dcrp_query, string, true);
    if(cache_num_rows())
    {
        cache_get_value_name_int(0, "pID", sqlid);
        mysql_format(dcrp_query, string, sizeof(string), "INSERT INTO `warninglogs` (`pID`, `WarnType`, `WarnReason`, `WarnSender`, `WarnTime`) VALUES('%d', '%d', '%e', '%e', '%d')", sqlid, TYPE_WARN, reason, AccountData[playerid][pAdminname], gettime());
        mysql_query(dcrp_query, string, false);
...
stock ShowPlayerWarning(playerid, targetid)
{
    ...
    if(cache_num_rows())
    {
        ...
        ShowPlayerDialog(targetid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Riwayat Peringatan", shstr, "Tutup", "");
    }
    else
    {
...
CMD:unwarn(playerid, params[])
{
    ...
    SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah menghapus 1 point warning player "YELLOW"%s(%d).", AccountData[playerid][pAdminname], ReturnName(userid), userid);
    return 1;
}
...
CMD:clearwarn(playerid, params[])
{
    ...
    new query[178];
    mysql_format(dcrp_query, query, sizeof(query), "DELETE FROM `warninglogs` WHERE `pID`='%d'", AccountData[otherid][pID]);
    mysql_query(dcrp_query, query, false);
    return 1;
}
...
CMD:ajail(playerid, params[])
{
    ...
    if(count == 0)
		...
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Player Jail", list, "Close", "");
	return 1;
}
...
public OnOfflineJailed(playerid, name[], reason[], fee, time)
{
    ...
    if(rows)
    {
        ...
        new cQuery[256];
        mysql_format(dcrp_query, cQuery, sizeof(cQuery), "UPDATE `player_characters` SET `Char_Jail` = 1, `Char_JailTime` = '%d', `Char_JailReason` = '%e', `Char_Money` = `Char_Money` - '%d' WHERE `pID` = '%d'", time * 60, reason, fee, sqlid);
        mysql_query(dcrp_query, cQuery, false);

        new string[192];
        mysql_format(dcrp_query, cQuery, sizeof(cQuery), "INSERT INTO `warninglogs` (`pID`, `WarnType`, `WarnReason`, `WarnSender`, `WarnTime`) VALUES('%d', '%d', '%e', '%e', '%d')", sqlid, TYPE_JAIL, reason, AccountData[playerid][pAdminname], gettime());
        mysql_query(dcrp_query, string, false);
...
CMD:ojail(playerid, params[])
{
    ...
    mysql_format(dcrp_query, characterQuery, sizeof(characterQuery), "SELECT * FROM `player_characters` WHERE `Char_Name`='%s' LIMIT 1", name);
    mysql_tquery(dcrp_query, characterQuery, "OnOfflineJailed", "dssdd", playerid, name, reason, fee, minutes);
    return 1;
}