...
Tags_Create(playerid)
{
	...
	if((index = Iter_Free(Tags)) != cellmin)
	{
		...
		new output[700];
		mysql_format(dcrp_query, output, sizeof(output), "INSERT INTO `tags`(`tagText`, `tagFont`, `tagCreated`, `tagColor`, `tagFontsize`, `tagBold`, `tagOwner`, `tagPosx`, `tagPosy`, `tagPosz`, `tagRotx`, `tagRoty`, `tagRotz`, `tagExpired`) VALUES ('%s','%s','%s','%d','%d','%d','%d','%.3f','%.3f','%.3f','%.3f','%.3f','%.3f','%d')", 
		...
		mysql_tquery(dcrp_query, output, "OnTagsCreated", "d", index);
		return index;
	}
	return -1;
}
...
Tags_Delete(index)
{
	...
	{
		mysql_tquery(dcrp_query, sprintf("DELETE FROM `tags` WHERE `tagId`='%d';", TagsData[index][tagID]));
	...
	if(response)
	{
		if(IsPlayerEditingTags(playerid))
		{
			switch(listitem)
			{
				...
				case 1: // Editing Text
				{
                    Dialog_Show(playerid, TagsText, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Text", 
				...
				case 2: // Font Name
				{
					Dialog_Show(playerid, TagsFont, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Font Name", object_font, "Change", "Back");
				}
				case 3: // Font Name
				{
					Dialog_Show(playerid, TagsFontSize, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Font Size", WHITE"Ukuran sekarang: "YELLOW"%d\n\n"WHITE"Masukkan ukuran font mulai dari angka 1 sampai "#TAGS_DEFAULT_MAX_SIZE":", "Update", "Back", GetPVarInt(playerid, "TagsSize"));
				}
				case 4: // Font Color
				{
					Dialog_Show(playerid, TagsColor, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Font Color", color_string, "Change", "Back");
				}
				case 5: // Toggle bold
				{
...
Dialog:TagsFont(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		if(listitem == sizeof(FontNames) - 1)
			return Dialog_Show(playerid, TagsFontCustom, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Custom Font", "Masukkan nama font yang akan kamu ubah:", "Input", "Back");
...
Dialog:TagsFontCustom(playerid, response, listitem, inputtext[])
{
	...
	{
		if(!strlen(inputtext))
			return Dialog_Show(playerid, TagsFontCustom, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Custom Font", "error: tidak boleh kosong, silahkan masukkan nama font yang benar!\n\nMasukkan nama font yang akan kamu ubah:", "Input", "Back");
...
Dialog:TagsFontSize(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		if(!(0 < strval(inputtext) <= TAGS_DEFAULT_MAX_SIZE))
			return Dialog_Show(playerid, TagsFontSize, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Font Size", WHITE"error: ukuran dibatasi mulai dari 1 sampai "#TAGS_DEFAULT_MAX_SIZE"!\n\nUkuran sekarang: "YELLOW"%d\n\n"WHITE"Masukkan ukuran font mulai dari angka 1 sampai "#TAGS_DEFAULT_MAX_SIZE":", "Update", "Back", GetPVarInt(playerid, "TagsSize"));
...
Dialog:TagsColor(playerid, response, listitem, inputtext[])
{
	if(response)
	{
        if(!(0 <= strval(inputtext) <= sizeof(ColorList)-1))
			return Dialog_Show(playerid, TagsColor, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Font Color", color_string, "Change", "Back");
