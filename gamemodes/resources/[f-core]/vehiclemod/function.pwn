...
JadenDCRP: Vehicle_ObjectLoad(vehicleid)
{
...
Vehicle_ObjectAddObjects(playerid, vehicleid, model, type)
{
    ...
    { 
        if(VehicleObjects[vehicleid][slot][vehObjectExists] == false)
        {
            ...
            Dialog_Show(playerid, DIALOG_MODSHOPMOVE, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Modshop", "Position\nPosition (Manual)", "Select", "Close");
            ...
            mysql_tquery(dcrp_query, query_string, "Vehicle_ObjectDB", "dd", vehicleid, slot);
            return 1;
        }
    }
	return 0;
...
Vehicle_SpotLightAdd(playerid, vehicleid, model, type)
{
    ...
    { 
        if(VehicleObjects[vehicleid][slot][vehObjectExists] == false)
        {
            ...
            Dialog_Show(playerid, DIALOG_SPOTLIGHT, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Modshop", "Position\nPosition (Manual)\nLight Color", "Select", "Close");
...
Vehicle_ObjectDelete(vehicleid, slot)
{
    ...
    mysql_tquery(dcrp_query, query_string);
    return 1;
}
    ...
    if(dialogid == DIALOG_MODSHOP)
    {
        if(response)
        {
            switch(listitem)
            {
                ...
                case 5:
                {
                    Dialog_Show(playerid, MODSHOP_CUSTOM, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Modshop",
