...
CMD:unblacklist(playerid, params[])
{
    ...
    switch(AccountData[playerid][pFaction])
    {
        case 1:// Polisi
        {
            ...
            new query[512];
            mysql_format(dcrp_query, query, sizeof(query), "UPDATE `blacklist_players` SET `PoliceIssuer`='', `PoliceIssuerRank`='', `PoliceReason`='', `PoliceDuration`=0 WHERE `PID`=%d", AccountData[otherid][pID]);
        ...
        case 2:// pemerintah
        {
            ...
            new query[512];
            mysql_format(dcrp_query, query, sizeof(query), "UPDATE `blacklist_players` SET `PemerintahIssuer`='', `PemerintahIssuerRank`='', `PemerintahReason`='', `PemerintahDuration`=0 WHERE `PID`=%d", AccountData[otherid][pID]);
            mysql_tquery(dcrp_query, query);
        }
        case 3:// ems
        {
            ...
            new query[512];
            mysql_format(dcrp_query, query, sizeof(query), "UPDATE `blacklist_players` SET `EMSIssuer`='', `EMSIssuerRank`='', `EMSReason`='', `EMSDuration`=0 WHERE `PID`=%d", AccountData[otherid][pID]);
            mysql_tquery(dcrp_query, query);
        }
        case 5:// bengkel
        {
            ...
            new query[512];
            mysql_format(dcrp_query, query, sizeof(query), "UPDATE `blacklist_players` SET `BengkelIssuer`='', `BengkelIssuerRank`='', `BengkelReason`='', `BengkelDuration`=0 WHERE `PID`=%d", AccountData[otherid][pID]);
            mysql_tquery(dcrp_query, query);
        }
        case 6:// pedagang
        {
            ...
            new query[512];
            mysql_format(dcrp_query, query, sizeof(query), "UPDATE `blacklist_players` SET `PedagangIssuer`='', `PedagangIssuerRank`='', `PedagangReason`='', `PedagangDuration`=0 WHERE `PID`=%d", AccountData[otherid][pID]);
            mysql_tquery(dcrp_query, query);
        }
    }
    return 1;
...
CMD:blacklist(playerid, params[])
{
    ...
    switch(AccountData[playerid][pFaction])
    {
        case 1:// polisi
        {
            ...
            mysql_format(dcrp_query, query, sizeof(query), "UPDATE `blacklist_players` SET `PoliceIssuer`='%s', `PoliceIssuerRank`='%s', `PoliceReason`='%s', `PoliceDuration`='%d 'WHERE `PID`='%d'", BlackListInfo[otherid][PDIssuer], BlackListInfo[otherid][PDIssuerRank], BlackListInfo[otherid][PDReason], BlackListInfo[otherid][PDDur], AccountData[otherid][pID]);
            mysql_tquery(dcrp_query, query);
        }
        case 2:// pemerintah
        {
            ...
            mysql_format(dcrp_query, query, sizeof(query), "UPDATE `blacklist_players` SET `PemerintahIssuer`='%s', `PemerintahIssuerRank`='%s', `PemerintahReason`='%s', `PemerintahDuration`='%d' WHERE `PID`='%d'", BlackListInfo[otherid][PEMERIssuer], BlackListInfo[otherid][PEMERIssuerRank], BlackListInfo[otherid][PEMERReason], BlackListInfo[otherid][PEMERDur], AccountData[otherid][pID]);
            mysql_tquery(dcrp_query, query);
        }
        case 3:// EMS
        {
            ...
            mysql_format(dcrp_query, query, sizeof(query), "UPDATE `blacklist_players` SET `EMSIssuer`='%s', `EMSIssuerRank`='%s', `EMSReason`='%s', `EMSDuration`='%d' WHERE `PID`='%d'", BlackListInfo[otherid][EMSIssuer], BlackListInfo[otherid][EMSIssuerRank], BlackListInfo[otherid][EMSReason], BlackListInfo[otherid][EMSDur], AccountData[otherid][pID]);
            mysql_tquery(dcrp_query, query);
        }
        case 5:// Bengkel
        {
            ...
            mysql_format(dcrp_query, query, sizeof(query), "UPDATE `blacklist_players` SET `BengkelIssuer`='%s', `BengkelIssuerRank`='%s', `BengkelReason`='%s', `BengkelDuration`='%d' WHERE `PID`='%d'", BlackListInfo[otherid][BENGKELIssuer], BlackListInfo[otherid][BENGKELIssuerRank], BlackListInfo[otherid][BENGKELReason], BlackListInfo[otherid][BENGKELDur], AccountData[otherid][pID]);
            mysql_tquery(dcrp_query, query);
        }
        case 6:// Pedagang
        {
            ...
            mysql_format(dcrp_query, query, sizeof(query), "UPDATE `blacklist_players` SET `PedagangIssuer`='%s', `PedagangIssuerRank`='%s', `PedagangReason`='%s', `PedagangDuration`='%d' WHERE `PID`='%d'", BlackListInfo[otherid][PEDAIssuer], BlackListInfo[otherid][PEDAIssuerRank], BlackListInfo[otherid][PEDAReason], BlackListInfo[otherid][PEDADur], AccountData[otherid][pID]);
            mysql_tquery(dcrp_query, query);
        }
    }
    return 1;
...
ShowDurringBlacklist(playerid, targetid)
{
    ...
    \n\n"GRAY"- [Kepolisian Daerah DCRP] -\
    \nStatus: "YELLOW"%s\
    \n"GRAY"Petugas: %s\
    \nAlasan: %s\
    \n\n- [Pemerintah Daerah DCRP] -\
    \nStatus: "YELLOW"%s\
    \n"GRAY"Petugas: %s\
    \nAlasan: %s\
    \n\n- [EMS Kota DCRP] -\
    ...
    ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Cek Blacklist", strgbg, "Tutup", "");
    return 1;
}
...
/*ptask BlacklistDelay[1000](playerid)
{
    ...
    if(BlackListInfo[playerid][PDDur] != 0 && BlackListInfo[playerid][PDDur] <= gettime())
    {
        ...
        new query[512];
        mysql_format(dcrp_query, query, sizeof(query), "UPDATE `blacklist_players` SET `PoliceIssuer`='', `PoliceIssuerRank`='', `PoliceReason`='', `PoliceDuration`=0 WHERE `PID`=%d", AccountData[playerid][pID]);
        mysql_tquery(dcrp_query, query);
    }
    if(BlackListInfo[playerid][PEMERDur] != 0 && BlackListInfo[playerid][PEMERDur] <= gettime())
    {
        ...
        new query[512];
        mysql_format(dcrp_query, query, sizeof(query), "UPDATE `blacklist_players` SET `PemerintahIssuer`='', `PemerintahIssuerrank`='', `PemerintahReason`='', `PemerintahDuration`=0 WHERE `PID`=%d", AccountData[playerid][pID]);
        mysql_tquery(dcrp_query, query);
    }
    if(BlackListInfo[playerid][EMSDur] != 0 && BlackListInfo[playerid][EMSDur] <= gettime())
    {
        ...
        new query[512];
        mysql_format(dcrp_query, query, sizeof(query), "UPDATE `blacklist_players` SET `EMSIssuer`='', `EMSIssuerrank`='', `EMSReason`='', `EMSDuration`=0 WHERE `PID`=%d", AccountData[playerid][pID]);
        mysql_tquery(dcrp_query, query);
    }
    if(BlackListInfo[playerid][BENGKELDur] != 0 && BlackListInfo[playerid][BENGKELDur] <= gettime())
    {
        ...
        new query[512];
        mysql_format(dcrp_query, query, sizeof(query), "UPDATE `blacklist_players` SET `BengkelIssuer`='', `BengkelIssuerrank`='', `BengkelReason`='', `BengkelDuration`=0 WHERE `PID`=%d", AccountData[playerid][pID]);
        mysql_tquery(dcrp_query, query);
    }
    if(BlackListInfo[playerid][PEDADur] != 0 && BlackListInfo[playerid][PEDADur] <= gettime())
    {
        ...
        new query[512];
        mysql_format(dcrp_query, query, sizeof(query), "UPDATE `blacklist_players` SET `PedagangIssuer`='', `PedagangIssuerrank`='', `PedagangReason`='', `PedagangDuration`=0 WHERE `PID`=%d", AccountData[playerid][pID]);
        mysql_tquery(dcrp_query, query);
    }
    return 1;
}*/