...
RefreshLabelPoms(playerid)
{
    ...
    for(new x = 0; x < sizeof(RefillArea); x++)
    {
        if(!IsPlayerInAnyVehicle(playerid))
        {
            ...
            {
                if(!DurringRefill[playerid]) UpdateDynamic3DTextLabelText(pomLabel[playerid][x], -1, "Tekan "GREEN"ALT"WHITE" untuk mengisi BBM");
            }
            else 
            {
                UpdateDynamic3DTextLabelText(pomLabel[playerid][x], -1, "Tekan "GREEN"ALT"WHITE" untuk membeli jerigen\nHarga:"DARKGREEN" $100");
            }
        }
        else 
        {
        ...
        if(IsRefillArea(playerid))
        {
            if(carid != INVALID_VEHICLE_ID)
            {
                if(!DurringRefill[playerid])
                {
                    ...
                    SetPlayerAttachedObject(playerid, 9, 19621, 6, 0.115, 0.045, 0.039, 0.000, 0.000, 70.599, 1,1,1);
            ...
            {
                if(AccountData[playerid][pMoney] < 100) return ShowTDN(playerid, NOTIFICATION_ERROR, "Uang anda tidak mencukupi!");
                ...
                ItemAdd(playerid, "Jerigen");
                ShowItemBox(playerid, "Jerigen", "Recieved_1x", 2);
                ShowItemBox(playerid, "Uang", "Removed_$100", 2);
            }
        }   
    }
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
...
public VehicleRefill(playerid, vehicleid)
{
    ...
    if(AccountData[playerid][pTempVehID] == vehicleid)
    {
        ...
        for(new i = 0; i < sizeof(RefillArea); i ++) {
            UpdateDynamic3DTextLabelText(pomLabel[playerid][i], -1, sprintf("Tekan "GREEN"ALT"WHITE" untuk membatalkan isi bbm\n"WHITE"Harga: "DARKGREEN"$%d", RefillPrice[playerid]));
        }
        VehicleCore[AccountData[playerid][pTempVehID]][vCoreFuel] += value;
