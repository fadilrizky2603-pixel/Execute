...
SQL_FactionWeapons(playerid)
{
    if(AccountData[playerid][pDutyPD] && AccountData[playerid][pDutyPemerintah] && AccountData[playerid][pDutyEms]
        && AccountData[playerid][pDutyBengkel] && AccountData[playerid][pDutyGojek])
    {
        mysql_tquery(dcrp_query, sprintf("SELECT * FROM `weapon_factions` WHERE `Owner_ID`=%d", AccountData[playerid][pID]), "OnLoadPlayerFactWeapons", "d", playerid);
    }
    return 1;
}
...
GiveFactionWeapon(playerid, weaponid, ammo)
{
        ...
        && AccountData[playerid][pDutyBengkel] && AccountData[playerid][pDutyGojek])
    {
        ...
        mysql_tquery(dcrp_query, sprintf("DELETE FROM `weapon_factions` WHERE `Slot`=%d", g_aWeaponSlots[weaponid]));

        new 
            query[165]
        ;
        mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `weapon_factions` VALUES ('%d', '%d', '%d', '%d', '%d') ON DUPLICATE KEY UPDATE ammo = %d, created %d", GetPlayerSQLID(playerid), weaponid, ammo, gettime(), g_aWeaponSlots[weaponid], ammo, gettime());
...
ResetFactionWeapon(playerid) {
        ...
        && AccountData[playerid][pDutyBengkel] && AccountData[playerid][pDutyGojek])
    {
        ...
        return mysql_tquery(dcrp_query, sprintf("DELETE FROM `weapon_factions` WHERE `Owner_ID`=%d", AccountData[playerid][pID]));
    }
    return 0;
}
