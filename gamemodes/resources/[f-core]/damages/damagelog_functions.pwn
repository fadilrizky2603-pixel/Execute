...
hook OnPlayerTakeDamage(playerid, issuerid, Float:amount, weaponid, bodypart)
{
	...
	new query[255];
	mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `damagelogs` (`IDs`, `dWeapon`, `dBodyPart`, `dAmount`, `dIssuer`, `dTime`) VALUES ('%d', '%d', '%d', '%.1f', '%s', CURRENT_TIMESTAMP())", AccountData[playerid][pID], weaponid, bodypart, Float:amount, issuer);
...
hook OnPlayerTakeDamage(playerid, issuerid, Float:amount, weaponid, bodypart)
{
	...
	new query[300];
	mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `damagelogs` SET `IDs`=%d, `dWeapon`=%d, `dBodyPart`=%d, `dAmount`='%.1f', `dIssuer`='%s', `dTime`=CURRENT_TIMESTAMP()", AccountData[playerid][pID], weaponid, bodypart, Float:amount, issuer);
	mysql_tquery(dcrp_query, query);
	return 1;
}
...
Damage_Show(playerid)
{
	...
	mysql_format(dcrp_query, dbstr, sizeof(dbstr), "SELECT * FROM `damagelogs` WHERE `IDs`=%d ORDER BY `dTime` ASC", AccountData[playerid][pID]);
	execute = mysql_query(dcrp_query, dbstr, true);
	new rows = cache_num_rows();
	if(rows)
	{
		if(rows >= 50)
		{
			mysql_tquery(dcrp_query, sprintf("DELETE FROM `damagelogs` WHERE `IDs`=%d", AccountData[playerid][pID]));
		}
		...
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Damage Log", list, "Tutup", ""); 
	}
	else 
	{
		new list[108];
...
{
	new dbstr[255], Cache:execute;
	mysql_format(dcrp_query, dbstr, sizeof(dbstr), "SELECT * FROM `damagelogs` WHERE `IDs`=%d ORDER BY `dTime` ASC", AccountData[player][pID]);
	execute = mysql_query(dcrp_query, dbstr, true);
	new rows = cache_num_rows();
	if(rows)
	{
		if(rows >= 50)
		{
			mysql_tquery(dcrp_query, sprintf("DELETE FROM `damagelogs` WHERE `IDs`=%d", AccountData[player][pID]));
		}
...
public DamageShowOffline(playerid, const NamePlayer[])
{
	...
	else 
	{
		...
		mysql_query(dcrp_query, sprintf("SELECT * FROM `damagelogs` WHERE `IDs`=%d", playerdbID));
		new rows = cache_num_rows();
		if(rows)
		{
...
CMD:dlogoff(playerid, params[])
{
	...
	new query[512];
	mysql_format(dcrp_query, query, sizeof(query), "SELECT `pID` FROM `player_characters` WHERE `Char_Name`='%s'", playerName);
	mysql_tquery(dcrp_query, query, "DamageShowOffline", "ds", playerid, playerName);
	return 1;
}
