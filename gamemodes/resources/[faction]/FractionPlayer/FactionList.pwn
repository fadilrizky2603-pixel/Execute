// All Callbacks
JadenDCRP: DisplayFactionMemberList(playerid, pageindex)
{
    ...
    mysql_tquery(dcrp_query, sprintf("SELECT `pID` FROM `player_characters` WHERE `Char_Faction` = %i", MemberBrowsing[playerid]), "GetTotalFactionMemberCount", "isi", playerid, string, onlineCount);
    return 1;
}

JadenDCRP: GetTotalFactionMemberCount(playerid, string[], onlineCount)
{
    new sha[300];
...
Dialog:FactionMemberList(playerid, response, listitem, inputtext[])
{
    ...
    if(strcmp(inputtext, "> Halaman Selanjutnya", inputtext, true) == 0)
    {
        ...
        mysql_tquery(dcrp_query, string, "DisplayFactionMemberList", "dd", playerid, MemberListIndex[playerid]);
    }
    ...
    else
    {
        ...
        format(sha, sizeof(sha), ""PINK1"DCRP"WHITE"- Kelola "YELLOW"%s", playerName);
