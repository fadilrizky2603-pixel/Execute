...
ShowFamiliesWeapon(playerid, famid)
{
    ...
    ShowPlayerDialog(playerid, DIALOG_FAMS_WEAPON, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Weapon Chest", shstr, "Pilih", "Batal");
    return 1;
}
...
stock Families_GedelahFiture(playerid, targetid)
{
    ...
    Dialog_Show(playerid, FamiliesGeledah, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Geledah", str, "Select", "Close");
    return 1;
}

NERO::OnFamiliesCreated(id)
{
    Families_Save(id);
    Families_Refresh(id);
    return 1;
...
CMD:famcreate(playerid, params[])
{
    ...
    new icsr[200];
    mysql_format(dcrp_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_Family`=%d, `Char_FamilyRank`=6 WHERE `pID`=%d", id, AccountData[otherid][pID]);
    mysql_tquery(dcrp_query, icsr);

    mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `families` SET `F_ID` = '%d', `F_Name` = '%s', `F_Leader` = '%s'", id, name, AccountData[otherid][pName]);
	mysql_tquery(dcrp_query, query, "OnFamiliesCreated", "d", id);
	return 1;
}
...
CMD:famdelete(playerid, params[])
{
    ...
    mysql_format(dcrp_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_Family`=-1, `Char_FamilyRank`=0 WHERE `Char_Family`=%d", fid);
    mysql_tquery(dcrp_query, query);

    mysql_format(dcrp_query, query, sizeof(query), "DELETE FROM families WHERE F_ID ='%d'", fid);
    mysql_tquery(dcrp_query, query);
...
{
    new id, famname[126], famleader[MAX_PLAYER_NAME], famonline, Cache:execute;
    execute = mysql_query(dcrp_query, sprintf("SELECT `F_ID`, `F_Name`, `F_Leader` FROM `families` ORDER BY `F_ID` ASC"), true);
    if(cache_num_rows())
    {
        new list[2500];

        format(list, sizeof(list), "FID\tName\tOnline\tLeader\n");
        for(new i = 0; i < cache_num_rows(); i ++)
        {
            ...
            format(list, sizeof(list), "%s%d\t%s\t%d\t%s\n", list, id, famname, famonline, famleader);
        }
        ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE" - Fam List", list, "Tutup", "");
    }
    else 
        ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Official Family", "Tidak ada Families Official Kota untuk saat ini.", "Tutup", "");
...
hook OnPlayerEnterDynArea(playerid, STREAMER_TAG_AREA:areaid)
{
    ...
    {
        ShowKey(playerid, "Jual Marijuana");
    }

    if(areaid == Families_Stuffs[WashRedMoney])
    {
        ShowKey(playerid, "Cuci Uang");
    }
    ...
    foreach(new i : Families) if (areaid == FamData[i][famgarageArea])
    {
        if(IsPlayerInAnyVehicle(playerid))
        {
            ShowPlayerFooter(playerid, "~y~Garasi Families~n~~w~Gunakan Klakson!", 2000);
        }
        else
        {
            ShowKey(playerid, "Garasi Families");
        }
    }
    return 1;
}
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    ...
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        ...
        if(gid > -1)
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_FAMGARAGE_OUT, DIALOG_STYLE_TABLIST_HEADERS, sprintf(""PINK1"DCRP"WHITE" - Garasi %s", FamData[gid][famName]), list, "Pilih", "Batal");
            HideShortKey(playerid);
        }
        ...
        if(IsPlayerInDynamicArea(playerid, Families_Stuffs[WashRedMoney]))
        {
            ...
            PlayerTextDrawSetString(playerid, ProgressBar[playerid][1], "WASHING");
        ...
        if(IsPlayerInDynamicArea(playerid, Families_Stuffs[BlackmarketArea]))
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_BLACKMARKET, DIALOG_STYLE_TABLIST, ""PINK1"DCRP"WHITE"- BlackMarket Store",
        ...
        if(IsPlayerInDynamicArea(playerid, Families_Stuffs[JualMarijuana]))
        {
            ...
            AccountData[playerid][ActivityTime] = 1;
            PlayerTextDrawSetString(playerid, ProgressBar[playerid][1], "MENJUAL MARIJUANA");
            ShowProgressBar(playerid);
        ...
        if(bosid > -1)
        {
            if(IsPlayerInRangeOfPoint(playerid, 2.0, FamData[bosid][famBosDeskPos][0], FamData[bosid][famBosDeskPos][1], FamData[bosid][famBosDeskPos][2]))
            {
                if(AccountData[playerid][pFamily] == bosid)
                {
                    ...
                    {
                        Dialog_Show(playerid, Bosdesk_Family, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Bos Desk", ""WHITE"Invite\n"VERONA_ARWIN"Kelola Jabatan\n"WHITE"Kick\n"WHITE"Cek Keuangan\n"VERONA_ARWIN"Taruh/Ambil Uang", "Pilih", "Batal");
                    }
        ...
        if(fid > -1)
        {
            if(IsPlayerInRangeOfPoint(playerid, 2.0, FamData[fid][famBrankasPos][0], FamData[fid][famBrankasPos][1], FamData[fid][famBrankasPos][2]))
            {
                if(AccountData[playerid][pFamily] == fid)
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_FAMSBRANKAS, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Brankas Families",
    ...
    switch(dialogid)
    {
        ...
        case DIALOG_FAMSBRANKAS:
        {
            ...
            switch(listitem)
            {
                case 0: // Uang Merah
                {       
                    ShowPlayerDialog(playerid, DIALOG_FAMSRM_VAULT, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Red Money Storage",
                    "Cek Uang Merah\
                    \n"GRAY"Deposit\
                ...
                case 2: //ambil
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_FAMSVAULT, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Brankas Families",
        ...
        case DIALOG_FAMSRM_VAULT:
        {
            ...
            switch(listitem)
            {
                case 0: // cek
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Red Money", frmtmoney, "Tutup", "");
                }
                case 1: //
                {
                    ShowPlayerDialog(playerid, DIALOG_FAMSRM_DEPOSIT, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Deposit",
                    "Mohon masukkan jumlah uang merah yang ingin anda simpan dibrankas families:", "Input", "Batal");
                }
                case 2: //
                {
                    ShowPlayerDialog(playerid, DIALOG_FAMSRM_WITHDRAW, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Deposit",
        ...
        case DIALOG_FAMS_WEAPON:
        {
            ...
            if(FamData[famid][famWeapon][listitem] != 0)
            {
                ...
                new str[256];
                format(str, sizeof(str), "Senjata: %s\nAmmo tersedia: %d\n\nMasukkan jumlah ammo yang ingin diambil:", 
                    ReturnWeaponName(FamData[famid][famWeapon][listitem]), 
                    FamData[famid][famAmmo][listitem]
                );
                ShowPlayerDialog(playerid, DIALOG_FAMS_WEAPON_AMMO, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Ambil Senjata", str, "Ambil", "Batal");
            }
            else 
            {
                new 
        ...
        case DIALOG_FAMS_WEAPON_AMMO:
        {
            if(!response) return ShowFamiliesWeapon(playerid, AccountData[playerid][pFamily]);
            
            new 
                famid = AccountData[playerid][pFamily],
                slot = GetPVarInt(playerid, "SelectedWeaponSlot"),
                ammo = strval(inputtext);
            
            if(ammo < 1 || ammo > FamData[famid][famAmmo][slot])
                return ShowTDN(playerid, NOTIFICATION_ERROR, "Jumlah ammo tidak valid!");
            
            GivePlayerWeaponEx(playerid, FamData[famid][famWeapon][slot], ammo);
            ShowTDN(playerid, NOTIFICATION_SUKSES, sprintf("Anda mengambil %s dengan %d ammo dari chest!", 
                ReturnWeaponName(FamData[famid][famWeapon][slot]), ammo));
            
            // Update remaining ammo
            FamData[famid][famAmmo][slot] -= ammo;
            
            // If no ammo left, clear the weapon slot
            if(FamData[famid][famAmmo][slot] <= 0)
            {
                FamData[famid][famWeapon][slot] = 0;
                FamData[famid][famAmmo][slot] = 0;
            }
            
            Families_Save(famid);
            ShowFamiliesWeapon(playerid, famid);
            DeletePVar(playerid, "SelectedWeaponSlot");
        }
        case DIALOG_BLACKMARKET:
        {
            ...
            switch(listitem)
            {
                case 0: // DE
                {
                    ...
                    ShowItemBox(playerid, "Uang Merah", "Removed_$80,000", 2);
                    ShowItemBox(playerid, "Desert Eagle", "Received_1x", 2);
                }
                case 1: // Shotgun
                {
                    ...
                    ShowItemBox(playerid, "Uang Merah", "Removed_$100,000", 2);
                    ShowItemBox(playerid, "Shotgun", "Received_1x", 2);
                }
                case 2: //TEC 9
                {
                    ...
                    ShowItemBox(playerid, "Uang Merah", "Removed_$145,000", 2);
                    ShowItemBox(playerid, "TEC 9", "Received_1x", 2);
                }
                case 3: //MP 5
                {
                    ...
                    ShowItemBox(playerid, "Uang Merah", "Removed_$180,000", 2);
                    ShowItemBox(playerid, "MP 5", "Received_1x", 2);
                }
                case 4: //AK 47
                {
                    ...
                    ShowItemBox(playerid, "Uang Merah", "Removed_$250,000", 2);
                    ShowItemBox(playerid, "AK-47", "Received_1x", 2);
                }
            }
        }
        case DIALOG_FAMSTAKE_MONEY:
        {
            ...
            if(isnull(inputtext)) 
            {
                ...
                return ShowPlayerDialog(playerid, DIALOG_FAMSTAKE_MONEY, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Ambil Uang", frmxt, "Input", "Batal");
            }
            ...
            Warning(targetid, "(Player ID: %d) telah mengambil uang anda sebanyak "RED"%s", FormatMoney(value));
        ...
        case DIALOG_FAMSTAKE_REDMONEY:
        {
            ...
            if(isnull(inputtext)) 
            {
                ...
                return ShowPlayerDialog(playerid, DIALOG_FAMSTAKE_REDMONEY, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Ambil Uang Merah", frmxt, "Input", "Batal");
            }
            ...
            Warning(targetid, "(Player ID: %d) telah mengambil uang kotor anda sebanyak "RED"%s", FormatMoney(value));
        ...
        case DIALOG_FAMSVAULT:
        {
            ...
            switch(listitem)
            {
                ...
                case 1: //withdraw
                {
                    ...
                    mysql_format(dcrp_query, icsr, sizeof(icsr), "SELECT * FROM `badside_brankas` WHERE `ID`=%d", AccountData[playerid][pFamily]);
                    mysql_query(dcrp_query, icsr);
                    if(cache_num_rows() > 0)
                    {
        ...
        case DIALOG_FAMSVAULT_IN:
        {
            ...
            new invstr[1028];
            mysql_format(dcrp_query, shstr, sizeof(shstr), "SELECT * FROM `badside_brankas` WHERE `ID`=%d AND `fItemName`='%s'", AccountData[playerid][pFamily], FactionBrankas[playerid][factionBrankasTemp]);
            mysql_query(dcrp_query, shstr);
            new rows = cache_num_rows();
            if(rows > 0)
            {
                mysql_format(dcrp_query, invstr, sizeof(invstr), "UPDATE `badside_brankas` SET `fItemQuantity` = `fItemQuantity` + %d WHERE `ID`=%d AND `fItemName`='%s'", quantity, AccountData[playerid][pFamily], FactionBrankas[playerid][factionBrankasTemp]);
                mysql_tquery(dcrp_query, invstr, "OnBadsideDeposit", "i", playerid);
            }
            else 
            {
                mysql_format(dcrp_query, invstr, sizeof(invstr), "INSERT INTO `badside_brankas` SET `ID`=%d, `fItemName`='%s', `fItemModel`=%d, `fItemQuantity`=%d", AccountData[playerid][pFamily], FactionBrankas[playerid][factionBrankasTemp], FactionBrankas[playerid][factionBrankasModel], quantity);
                mysql_tquery(dcrp_query, invstr, "OnBadsideDeposit", "i", playerid);
            }
            AccountData[playerid][menuShowed] = false;
        }
        case DIALOG_FAMSVAULT_WITHDRAW:
        {
            ...
            new tss[188];
            mysql_format(dcrp_query, tss, sizeof(tss), "SELECT * FROM `badside_brankas` WHERE `ID`=%d", AccountData[playerid][pFamily]);
            mysql_query(dcrp_query, tss);
            if(cache_num_rows() > 0)
            {
        ...
        case DIALOG_FAMSVAULT_OUT:
        {
            ...
            {
                mysql_format(dcrp_query, jts, sizeof(jts), "UPDATE `badside_brankas` SET `fItemQuantity`=%d WHERE `fItemID`=%d", FactionBrankas[playerid][factionBrankasQuant], FactionBrankas[playerid][factionBrankasID]);
                mysql_tquery(dcrp_query, jts);
            }
            else 
            {
                mysql_format(dcrp_query, jts, sizeof(jts), "DELETE FROM `badside_brankas` WHERE `fItemID`=%d", FactionBrankas[playerid][factionBrankasID]);
                mysql_tquery(dcrp_query, jts);
            }
        ...
        case DIALOG_FAMILIESSETRANK:
        {
            ...
            mysql_query(dcrp_query, sprintf("SELECT * FROM `player_characters` WHERE `Char_Family` = %d ORDER BY `Char_FamilyRank` DESC", famid));
            new rows = cache_num_rows();
            if(rows)
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_RANK_SET_FAMILIES, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Set Jabatan", 
        ...
        case DIALOG_FAMILIESKICKMEMBER:
        {
            ...
            if(cache_num_rows())
            {
                ...
                if(AccountData[playerid][pID] == pidrow) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda tidak dapat kick diri sendiri!");
                if(fckrank >= AccountData[playerid][pFamilyRank]) return ShowTDN(playerid, NOTIFICATION_ERROR, "Tidak dapat kick pangkat diatasmu atau setara denganmu!");
                if(AccountData[playerid][pFamilyRank] < 5) return ShowTDN(playerid, NOTIFICATION_ERROR, "Minimal rank Wakil Ketua untuk kick anggota!");
                ...
                mysql_format(dcrp_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_Family`=-1, `Char_FamilyRank`=0 WHERE `pID`=%d", pidrow);
                mysql_tquery(dcrp_query, icsr);
        ...
        case DIALOG_RANK_SET_FAMILIES:
        {
            ...
            if(isnull(inputtext)) return ShowPlayerDialog(playerid, DIALOG_RANK_SET_FAMILIES, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Kelola Jabatan", \
            "Error: Tidak dapat diisi kosong!\n\
            Silahkan pilih jabatan untuk ditetapkan (masukkan angka saja):\n\
            1. Relasi\n\
            2. Outsider\n\
            ...
            if(!IsNumeric(inputtext)) return ShowPlayerDialog(playerid, DIALOG_RANK_SET_FAMILIES, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Kelola Jabatan", \
            ...
            if(strval(inputtext) < 1 || strval(inputtext) > AccountData[playerid][pFamilyRank]) return ShowPlayerDialog(playerid, DIALOG_RANK_SET_FAMILIES, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Kelola Jabatan", \
            ...
            new affah[128];
            mysql_format(dcrp_query, affah, sizeof(affah), "UPDATE `player_characters` SET `Char_FamilyRank`=%d WHERE `pID`=%d", strval(inputtext), AccountData[playerid][pTempSQLFamMemberID]);
        ...
        case DIALOG_FAMILY_PANEL:
        {
            ...
            switch(listitem)
            {
                ...
                case 9: // Ambil Uang Kotor
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_FAMSTAKE_REDMONEY, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Ambil Uang Merah", frmxt, "Input", "Batal");
                ...
                case 10: // ambil uang
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_FAMSTAKE_MONEY, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Ambil Uang", frmxt, "Input", "Batal");
                    SetPVarInt(playerid, "TargetFamsID", targetid);
                }
                case 11: // Cek Senjata
                {
...
Dialog:Bosdesk_Family(playerid, response, listitem, inputtext[])
{
    ...
    switch(listitem)
    {
        case 0:// Invite
        {
            ...
            if(!count) return Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Invite", "Tidak ada orang yang dekat dengan anda!", "Close", "");
            Dialog_Show(playerid, FamiliesInvite, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Invite", sstr, "Pilih", "Batal");
        }
        case 1:// Kelola Jabatan Online / Offline
        {
            mysql_query(dcrp_query, sprintf("SELECT * FROM player_characters WHERE Char_Family = %d ORDER BY Char_FamilyRank DESC", AccountData[playerid][pFamily]));
            ...
            if(rows)
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_FAMILIESSETRANK, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Set Jabatan", shstr, "Pilih", "Batal");
            }
            else 
            {
                PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
                return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Set Jabatan", "Tidak ada Anggota Families!", "Tutup", "");
            }
        }
        case 2:// Offline Kick / Online
        {
            ...
            if(rows)
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_FAMILIESKICKMEMBER, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Kick Member", shstr, "Kick", "Batal");
            }
            else 
            {
                PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
                return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Kick Member", "Tidak ada Anggota Families!", "Tutup", "");
            }
        }
        case 3:// cek keuangan
        {
            ...
            Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Keuangan Families", sstr, "Tutup", "");
        }
        case 4:// Ambil atau taruh uang
        {
            ...
            Dialog_Show(playerid, FamiliesKeuangan, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Keuangan Families", sstr, "Input", "Batal");
        }
    }
    return 1;
}
...
Dialog:FamiliesInvite(playerid, response, listitem, inputtext[])
{
    ...
    if(response) 
    {
        ...
        new icsr[200];
        mysql_format(dcrp_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_Family`=%d, `Char_FamilyRank`=1 WHERE `pID`=%d", AccountData[playerid][pFamily], AccountData[id][pID]);
        mysql_tquery(dcrp_query, icsr);
    }
    return 1;
}
...
Dialog:FamiliesGeledah(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        if(InventoryData[targetid][listitem][invExists])
        {
            ...
            Dialog_Show(playerid, FamiliesTakeBarang, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Geledah", str, "Input", "Back");
        }
    }
    return 1;
}
...
public SellMarjun(playerid)
{
    ...
    if(AccountData[playerid][ActivityTime] >= 15)
    {
        ...
        ItemDelete(playerid, "Marijuana", total);
        GiveMoney(playerid, pay);
        ShowItemBox(playerid, "Marijuana", sprintf("Removed_%dx", total), 2);
...
public WashingRedMoney(playerid)
{
    ...
    if(AccountData[playerid][ActivityTime] >= 15)
    {
        ...
        ShowItemBox(playerid, "Uang", sprintf("Received_$%s", FormatMoney(value)), 2);
