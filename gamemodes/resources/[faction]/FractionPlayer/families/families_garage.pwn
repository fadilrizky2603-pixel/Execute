...
CMD:editfamgarkot(playerid, params[])
{
    ...
    else if(!strcmp(type, "delete", true))
    {
        ...
        new query[200];
        mysql_format(dcrp_query, query, sizeof(query), "DELETE FROM `families_garage` WHERE `ID` = '%d'", id);
        ...
        foreach(new i : FamiliesGarkot)
        {
            if(IsPlayerInRangeOfPoint(playerid, 2.0, FamiliesGarage[i][gFamPOS][0], FamiliesGarage[i][gFamPOS][1], FamiliesGarage[i][gFamPOS][2]))
            {
                if(AccountData[playerid][pFamily] == FamiliesGarage[i][gFamID])
                {
                    ...
                    Dialog_Show(playerid, DialogFamiliesGarage, DIALOG_STYLE_TABLIST_HEADERS, sprintf(""PINK1"DCRP"WHITE"- Garasi %s", FamiliesGarage[i][gFamName]), twsmk, "Pilih", "Batal");
