...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
	{
		if(AccountData[playerid][pFaction] == FACTION_GOJEK)
		{
			...
			{
				ShowPlayerDialog(playerid, DIALOG_GOJVAULT, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Brankas Gojek",
				"Simpan Barang\n"GRAY"Ambil Barang", "Pilih", "Batal");
			}

			if(IsPlayerInDynamicArea(playerid, Gojek_Stuff[GojekLocker]) && AccountData[playerid][pDutyGojek])
			{
				ShowPlayerDialog(playerid, DIALOG_GOJEK_LOCKER, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Locker Gojek", 
			...
			if(IsPlayerInDynamicArea(playerid, Gojek_Stuff[GojekDesk]) && AccountData[playerid][pDutyGojek])
			{
				...
				ShowPlayerDialog(playerid, DIALOG_BOSDESK_GOJEK, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Bos Desk",
				"Invite\ 
				\n"GRAY"Kelola Jabatan\ 
				\nKick\ 
				\n"GRAY"Lihat Anggota\ 
			...
			{
				ShowPlayerDialog(playerid, DIALOG_GOJEK_GARAGE, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Garasi Gojek",
				"Keluarkan Kendaraan\ 
...
DialogPages:GojekSetRank(playerid, response, listitem, inputtext[])
{
    ...
    mysql_query(dcrp_query, "SELECT * FROM `player_characters` WHERE `Char_Faction` = 7 ORDER BY `Char_FactionRank` DESC");
    ...
    if(rows)
    {
        ...
        ShowPlayerDialog(playerid, DIALOG_RANK_SET_GOJEK, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Kelola Jabatan",
...
DialogPages:GojekKickMember(playerid, response, listitem, inputtext[])
{
    ...
    if(cache_num_rows())
    {
        ...
        new strgbg[158];
        mysql_format(dcrp_query, strgbg, sizeof(strgbg), "DELETE FROM `player_vehicles` WHERE `PVeh_OwnerID`=%d AND `PVeh_Faction` = 7", pidrow);
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	switch(dialogid)
	{
		case DIALOG_BOSDESK_GOJEK:
		{
			...
			switch(listitem)
			{
				case 0:
				{
					...
					ShowPlayerDialog(playerid, DIALOG_GOJEK_INVITECONF, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Invite", frmxt, "Pilih", "Batal");
				}
				case 1:
				{
					mysql_query(dcrp_query, "SELECT * FROM player_characters WHERE Char_Faction = 7 ORDER BY Char_FactionRank DESC");
					...
					if(rows)
					{
						...
						ShowPlayerDialogPages(playerid, "GojekSetRank", DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Set Jabatan", "Pilih", "Batal", 15, ""GREEN">> Lanjut", ""ORANGE"<< Kembali");
					}
					else 
					{
						PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
						return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Set Jabatan", "Tidak ada Anggota Gojek!", "Tutup", "");
					}
				}
				case 2:
				{
					...
					if(rows)
					{
						...
						ShowPlayerDialogPages(playerid, "GojekKickMember", DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Kick Anggota", "Kick", "Batal", 15, ""GREEN">> Lanjut", ""ORANGE"<< Kembali");
					}
					else 
					{
						PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
						return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Kick Anggota", "Tidak ada Anggota Gojek!", "Tutup", "");
					}
				}
				case 3:
				{
				...
				case 4:
				{
					...
					ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Gojek Money", frmxt, "Tutup", "");
				}
				case 5:
				{
					ShowPlayerDialog(playerid, DIALOG_DEPOSIT_GOJEK, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Gojek Deposit",
					"Mohon masukkan nominal deposit untuk saldo perusahaan:", "Input", "Batal");
				}
				case 6:
				{
					ShowPlayerDialog(playerid, DIALOG_WITHDRAW_GOJEK, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Gojek Withdraw",
		...
		case DIALOG_GOJEK_INVITECONF:
		{
			...
			mysql_format(dcrp_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_Faction`= 7, `Char_FactionRank` = 1 WHERE `pID`=%d", AccountData[targetid][pID]);
			mysql_tquery(dcrp_query, icsr);
		...
		case DIALOG_RANK_SET_GOJEK:
		{
			...
			if(isnull(inputtext)) return ShowPlayerDialog(playerid, DIALOG_RANK_SET_GOJEK, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Kelola Jabatan", \
			...
			if(!IsNumeric(inputtext)) return ShowPlayerDialog(playerid, DIALOG_RANK_SET_GOJEK, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Kelola Jabatan", \
			...
			if(strval(inputtext) < 1 || strval(inputtext) > AccountData[playerid][pFactionRank]) return ShowPlayerDialog(playerid, DIALOG_RANK_SET_GOJEK, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Kelola Jabatan", \
		...
		case DIALOG_DEPOSIT_GOJEK:
		{
			...
			mysql_format(dcrp_query, frmtmny, sizeof(frmtmny), "UPDATE `stuffs` SET `gojekmoneyvault` = %d WHERE `ID` = 0", GojekMoneyVault);
		...
		case DIALOG_GOJEK_GARAGE:
		{
			...
			switch(listitem)
			{
				case 0: // Keluarkan
				{
					...
					ShowPlayerDialog(playerid, DIALOG_GOJEK_GARAGE_TAKEOUT, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Garasi Gojek", list, "Pilih", "Batal");
				}
				...
				{
					ShowPlayerDialog(playerid, DIALOG_GOJEK_GARAGE_BUY, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Beli Kendaraan",
					"Model\tHarga\ 
					\nWayfarer\t$4000\ 
				...
				{
					new frmtdel[158];
					mysql_format(dcrp_query, frmtdel, sizeof(frmtdel), "SELECT * FROM `player_vehicles` WHERE `PVeh_Faction` = 7 AND `PVeh_OwnerID` = %d", AccountData[playerid][pID]);
					...
					if(rows)
					{
						...
						ShowPlayerDialog(playerid, DIALOG_GOJEK_GARAGE_DELETE, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Hapus Kendaraan", list, "Hapus", "Batal");
					}
					else 
					{
						PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
						return Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Hapus Kendaraan", "Anda tidak memiliki kendaraan Gojek", "Tutup", "");
					}
				}
			}
		}
			...
			switch(listitem)
			{
				case 0: //deposit
                {
                    ...
                    if(rows) 
                    {
                        ...
                        ShowPlayerDialog(playerid, DIALOG_GOJVAULT_DEPOSIT, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Brankas Gojek", str, "Pilih", "Batal");
                    }
                    else 
                    {
                        PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
                        return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Brankas Gojek",
                ...
                case 1: //withdraw
                {
                    ...
                    format(str, sizeof(str), "Nama Item\tJumlah\tBerat (-/-)\n");
                    mysql_query(dcrp_query, "SELECT * FROM `gojek_brankas` WHERE `PID`=0");
                    if(cache_num_rows() > 0)
                    {
                        ...
                        ShowPlayerDialog(playerid, DIALOG_GOJVAULT_WITHDRAW, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Brankas Gojek", str, "Pilih", "Batal");
                    }
                    else 
                    {
                        return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Brankas Gojek",
		...
		case DIALOG_GOJVAULT_DEPOSIT:
		{
            ...
            if(cache_num_rows() > 0)
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_GOJVAULT_IN, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Gojek", 
                shstr, "Input", "Batal");
            }
		}
		case DIALOG_GOJVAULT_IN:
		{
            ...
            new invstr[1028];
            mysql_format(dcrp_query, shstr, sizeof(shstr), "SELECT * FROM `gojek_brankas` WHERE `PID`=0 AND `Item` = '%s'", FactionBrankas[playerid][factionBrankasTemp]);
            mysql_query(dcrp_query, shstr);
            new rows = cache_num_rows();
            if(rows > 0)
            {
                mysql_format(dcrp_query, invstr, sizeof(invstr), "UPDATE `gojek_brankas` SET `Quantity` = `Quantity` + %d WHERE `PID` = 0 AND `Item`='%s'", quantity, FactionBrankas[playerid][factionBrankasTemp]);
                mysql_tquery(dcrp_query, invstr, "OnGojekDeposit", "i", playerid);
            }
            else 
            {
                mysql_format(dcrp_query, invstr, sizeof(invstr), "INSERT INTO `gojek_brankas` SET `PID`=0, `Item`='%s', `Model`=%d, `Quantity`=%d", FactionBrankas[playerid][factionBrankasTemp], FactionBrankas[playerid][factionBrankasModel], quantity);
                mysql_tquery(dcrp_query, invstr, "OnGojekDeposit", "i", playerid);
            }
		}
		case DIALOG_GOJVAULT_WITHDRAW:
		{
            ...
            if(cache_num_rows() > 0)
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_GOJVAULT_OUT, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Gojek",
                shstr, "Input", "Batal");
            }
            else 
            {
                ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Brankas Gojek",
		...
		case DIALOG_GOJVAULT_OUT:
		{
            ...
            {
                mysql_format(dcrp_query, jts, sizeof(jts), "UPDATE `gojek_brankas` SET `Quantity`=%d WHERE `ID`=%d", FactionBrankas[playerid][factionBrankasQuant], FactionBrankas[playerid][factionBrankasID]);
                mysql_tquery(dcrp_query, jts);
            }
            else 
            {
                mysql_format(dcrp_query, jts, sizeof(jts), "DELETE FROM `gojek_brankas` WHERE `ID`=%d", FactionBrankas[playerid][factionBrankasID]);
                mysql_tquery(dcrp_query, jts);
            }
