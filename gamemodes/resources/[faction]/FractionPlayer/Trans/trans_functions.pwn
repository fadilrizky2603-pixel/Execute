    ...
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        if(AccountData[playerid][pFaction] == FACTION_TRANS)
        {
            ...
            if(IsPlayerInDynamicArea(playerid, Trans_Stuff[TransStorage]) && AccountData[playerid][pDutyTrans])
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_TRANSVAULT, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Brankas Trans",
                "Simpan Barang\n"GRAY"Ambil Barang", "Pilih", "Batal");
            }

            if(IsPlayerInDynamicArea(playerid, Trans_Stuff[TransLocker]) && AccountData[playerid][pDutyTrans])
            {
                ShowPlayerDialog(playerid, DIALOG_TRANS_LOCKER, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Locker Trans",
                "Baju Biasa\n"GRAY"Baju 1\nBaju 2", "Pilih", "Batal");
            }
            ...
            if(IsPlayerInDynamicArea(playerid, Trans_Stuff[TransDesk]) && AccountData[playerid][pDutyTrans])
            {
				...
				ShowPlayerDialog(playerid, DIALOG_TRANS_DESK, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Bos Desk",
				"Invite\
				\n"GRAY"Kelola Jabatan\
				\nKick\
				\n"GRAY"Lihat Anggota\
            ...
            {
                ShowPlayerDialog(playerid, DIALOG_TRANS_GARAGE, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Garasi Trans",
                "Keluarkan Kendaraan\
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    switch(dialogid)
    {
        case DIALOG_TRANS_DESK:
        {
            ...
            if(AccountData[playerid][pFaction] != FACTION_TRANS) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan anggota Transportasi DCRP!");
            if(AccountData[playerid][pFactionRank] < 4) return ShowTDN(playerid, NOTIFICATION_ERROR, "Minimal rank Kepala Transport untuk akses desk!");
            switch(listitem)
            {
                case 0:
                {
					...
					ShowPlayerDialog(playerid, DIALOG_TRANS_INVITECONF, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Invite", frmxt, "Pilih", "Batal");
                }
                case 1:
                {
                    mysql_query(dcrp_query, "SELECT * FROM `player_characters` WHERE `Char_Faction` = 4 ORDER BY `Char_FactionRank` DESC");
					...
					if(rows)
					{
						...
						ShowPlayerDialog(playerid, DIALOG_TRANSSETRANK, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Set Jabatan", shstr, "Pilih", "Batal");
					}
					else 
					{
						PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
						return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Set Jabatan", "Tidak ada Anggota Trans!", "Tutup", "");
					}
                }
                case 2:
                {
					...
					if(rows)
					{
						...
						ShowPlayerDialog(playerid, DIALOG_TRANSKICKMEMBER, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Kick Anggota", shstr, "Kick", "Batal");
					}
					else 
					{
						PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
						return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Kick Anggota", "Tidak ada Anggota Trans!", "Tutup", "");
					}
                }
                case 3:
                {
					...
					ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Anggota", list, "Tutup", "");
                }
                case 4:
                {
                    new frmxt[158];
					format(frmxt, sizeof(frmxt), "Perusahaan Transportasi DCRP saat ini memiliki saldo sebesar\
					\n"DARKGREEN"%s", FormatMoney(TransMoneyVault));
					ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Trans Money", frmxt, "Tutup", "");
                }
				case 5:
				{
					ShowPlayerDialog(playerid, DIALOG_DEPOSIT_TRANS, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Trans Deposit",
					"Mohon masukkan nominal deposit untuk saldo perusahaan:", "Input", "Batal");
				}
				case 6:
				{
					ShowPlayerDialog(playerid, DIALOG_WITHDRAW_TRANS, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Trans Withdraw",
        ...
        case DIALOG_TRANS_INVITECONF:
        {
            ...
            mysql_format(dcrp_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_Faction`=4, `Char_FactionRank`=1 WHERE `pID`=%d", AccountData[targetid][pID]);
        ...
        {
            if(!response) return 1;
            if(AccountData[playerid][pFaction] != FACTION_TRANS) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan anggota Tansportasi DCRP!");
            if(AccountData[playerid][pFactionRank] < 4) return ShowTDN(playerid, NOTIFICATION_ERROR, "Minimal rank Kepala Transport untuk akses desk!");
            ...
            if(rows)
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_RANK_SET_TRANS, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Kelola Jabatan",
        ...
        case DIALOG_TRANSKICKMEMBER:
        {
            ...
            if(cache_num_rows())
            {
                ...
                new strgbg[158];
                mysql_format(dcrp_query, strgbg, sizeof(strgbg), "DELETE FROM `player_vehicles` WHERE `PVeh_OwnerID`=%d AND `PVeh_Faction` = 4", pidrow);
                ...
                foreach(new i : Player)
                {
                    if(AccountData[i][IsLoggedIn] && AccountData[i][pSpawned] && pidrow == AccountData[i][pID])
                    {
                        ...
                        RefreshFactionMap(i);
                        ShowTDN(i, NOTIFICATION_WARNING, "Anda telah dikeluarkan dari faction Transportasi DCRP!");
                    }
                }
        ...
        case DIALOG_RANK_SET_TRANS:
		{
			...
			if(isnull(inputtext)) return ShowPlayerDialog(playerid, DIALOG_RANK_SET_TRANS, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Kelola Jabatan", \
			...
			if(!IsNumeric(inputtext)) return ShowPlayerDialog(playerid, DIALOG_RANK_SET_TRANS, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Kelola Jabatan", \
			...
			if(strval(inputtext) < 1 || strval(inputtext) > AccountData[playerid][pFactionRank]) return ShowPlayerDialog(playerid, DIALOG_RANK_SET_TRANS, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Kelola Jabatan", \
        ...
        case DIALOG_DEPOSIT_TRANS:
		{
			...
			mysql_format(dcrp_query, frmtmny, sizeof(frmtmny), "UPDATE `stuffs` SET `transmoneyvault` = %d WHERE `ID` = 0", TransMoneyVault);
			mysql_tquery(dcrp_query, frmtmny);
			ShowTDN(playerid, NOTIFICATION_SUKSES, sprintf("Anda berhasil deposit %s untuk Trans DCRP", FormatMoney(depocash)));
		}
		case DIALOG_WITHDRAW_TRANS:
		{
			...
			ShowTDN(playerid, NOTIFICATION_SUKSES, sprintf("Anda berhasil withdraw %s dari Trans DCRP", FormatMoney(withdrawcash)));
		}
        case DIALOG_TRANS_GARAGE:
        {
            ...
            switch(listitem)
            {
                case 0: //takeout
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_TRANS_GARAGE_TAKEOUT, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Garasi Trans", list, "Pilih", "Batal");
                }
                case 1:// in
				{
					...
					if((carid = Vehicle_Nearest(playerid, 10.0)) != -1)
					{
						...
						if(PlayerVehicle[carid][pVehFaction] != FACTION_TRANS) return ShowTDN(playerid, NOTIFICATION_ERROR, "Kendaraan tersebut bukan kendaraan Trans DCRP!");
					...
					if(!foundnearby) return ShowTDN(playerid, NOTIFICATION_ERROR, "Tidak ada kendaraan dari Transportasi DCRP milik anda di sekitar!");
				}
                case 2: //buy
                {
                    ShowPlayerDialog(playerid, DIALOG_TRANS_GARAGE_BUY, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Beli Kendaraan",
                    "Model\tHarga\
                    \nTaxi\t$8500\
				...
				{
					new frmtdel[158];
					mysql_format(dcrp_query, frmtdel, sizeof(frmtdel), "SELECT * FROM `player_vehicles` WHERE `PVeh_Faction` = 4 AND `PVeh_OwnerID` = %d", AccountData[playerid][pID]);
					...
					if(rows)
					{
						...
						ShowPlayerDialog(playerid, DIALOG_TRANS_GARAGE_DELETE, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Hapus Kendaraan", list, "Hapus", "Batal");
					}
					else 
					{
						PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
						return Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Hapus Kendaraan", "Anda tidak memiliki kendaraan Trans", "Tutup", "");
					}
				}
            }
        }
		...
		{
			if(!response) return 1;
			if(AccountData[playerid][pFaction] != FACTION_TRANS) return ShowTDN(playerid, NOTIFICATION_INFO, "Anda bukan anggota Transportasi DCRP");
        ...
        case DIALOG_TRANSVAULT:
		{
            ...
            {
                AccountData[playerid][menuShowed] = false;   
                return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan anggota Transportasi DCRP!");
            }
			switch(listitem)
			{
				case 0: //deposit
                {
                    ...
                    if(rows) 
                    {
                        ...
                        ShowPlayerDialog(playerid, DIALOG_TRANSVAULT_DEPOSIT, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Brankas Trans", str, "Pilih", "Batal");
                    }
                    else 
                    {
                        AccountData[playerid][menuShowed] = false;
                        PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
                        return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Brankas Trans",
                ...
                case 1: //withdraw
                {
                    ...
                    format(str, sizeof(str), "Nama Item\tJumlah\tBerat (-/-)\n");
                    mysql_query(dcrp_query, "SELECT * FROM `trans_brankas` WHERE `PID`=0");
                    if(cache_num_rows() > 0)
                    {
                        ...
                        ShowPlayerDialog(playerid, DIALOG_TRANSVAULT_WITHDRAW, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Brankas Trans", str, "Pilih", "Batal");
                    }
                    else 
                    {
                        AccountData[playerid][menuShowed] = false;
        ...
        case DIALOG_TRANSVAULT_DEPOSIT:
		{
            ...
            if(cache_num_rows() > 0)
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_TRANSVAULT_IN, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Trans", 
                shstr, "Input", "Batal");
            }
		}
		case DIALOG_TRANSVAULT_IN:
		{
            ...
            new invstr[1028];
            mysql_format(dcrp_query, shstr, sizeof(shstr), "SELECT * FROM `trans_brankas` WHERE `PID`=0 AND `Item` = '%s'", FactionBrankas[playerid][factionBrankasTemp]);
            mysql_query(dcrp_query, shstr);
            new rows = cache_num_rows();
            if(rows > 0)
            {
                mysql_format(dcrp_query, invstr, sizeof(invstr), "UPDATE `trans_brankas` SET `Quantity` = `Quantity` + %d WHERE `PID` = 0 AND `Item`='%s'", quantity, FactionBrankas[playerid][factionBrankasTemp]);
                mysql_tquery(dcrp_query, invstr, "OnTransDeposit", "i", playerid);
            }
            else 
            {
                mysql_format(dcrp_query, invstr, sizeof(invstr), "INSERT INTO `trans_brankas` SET `PID`=0, `Item`='%s', `Model`=%d, `Quantity`=%d", FactionBrankas[playerid][factionBrankasTemp], FactionBrankas[playerid][factionBrankasModel], quantity);
                mysql_tquery(dcrp_query, invstr, "OnTransDeposit", "i", playerid);
            }
            AccountData[playerid][menuShowed] = false;
		}
        case DIALOG_TRANSVAULT_WITHDRAW:
		{
            ...
            if(cache_num_rows() > 0)
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_TRANSVAULT_OUT, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Trans",
                shstr, "Input", "Batal");
            }
            else 
            {
                AccountData[playerid][menuShowed] = false;
                ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Brankas Trans",
		...
		case DIALOG_TRANSVAULT_OUT:
		{
            ...
            {
                mysql_format(dcrp_query, jts, sizeof(jts), "UPDATE `trans_brankas` SET `Quantity`=%d WHERE `ID`=%d", FactionBrankas[playerid][factionBrankasQuant], FactionBrankas[playerid][factionBrankasID]);
                mysql_tquery(dcrp_query, jts);
            }
            else 
            {
                mysql_format(dcrp_query, jts, sizeof(jts), "DELETE FROM `trans_brankas` WHERE `ID`=%d", FactionBrankas[playerid][factionBrankasID]);
                mysql_tquery(dcrp_query, jts);
            }
