...
stock Pemerintah_OpenStorage(playerid, facid)
{
    ...
    if(!count3) return Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Brankas Pemerintah", "Tidak ada barang apapun dibrankas saat ini!", "Tutup", "");
    Dialog_Show(playerid, PemerintahBrankasItem, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Brankas Pemerintah", str, "Pilih", "Batal");
    return 1;
}
...
stock PemerintahItem_Add(facid, item[], model, quantity = 1)
{
	...
	if (itemid == -1)
	{
	    ...
	    if (itemid != -1)
	    {
			...
			mysql_format(dcrp_query, string, sizeof(string), "INSERT INTO `brankas_pemerintah` (`ID`, `ItemName`, `ItemModel`, `ItemQuantity`) VALUES('%d', '%s', '%d', '%d')", facid, item, model, quantity);
			mysql_tquery(dcrp_query, string, "OnPemerintahAddItem", "dd", facid, itemid);
	...
	{
	    mysql_format(dcrp_query, string, sizeof(string), "UPDATE `brankas_pemerintah` SET `ItemQuantity` = `ItemQuantity` + %d WHERE `ID` = '%d' AND `ItemID` = '%d'", quantity, facid, PemerintahStorage[facid][itemid][ItemID]);
...
NERO::OnPemerintahAddItem(facid, itemid)
{
...
stock PemerintahItem_Remove(facid, item[], quantity = 1)
{
	...
	if (itemid != -1)
	{
		...
		if (quantity == -1 || PemerintahStorage[facid][itemid][ItemQuantity] < 1)
		{
		    ...
		    mysql_format(dcrp_query, string, sizeof(string), "DELETE FROM `brankas_pemerintah` WHERE `ID` = '%d' AND `ItemID` = '%d'", facid, PemerintahStorage[facid][itemid][ItemID]);
	        mysql_tquery(dcrp_query, string);
		}
...
NERO::LoadBrankasPemerintah(facid)
{
    if(facid != 2) return false;
...
Dialog:PemerintahBrankasDeposit(playerid, response, listitem, inputtext[])
{
    ...
    if(response)
    {
        ...
        ItemDelete(playerid, string, amount);
        
        ShowItemBox(playerid, string, sprintf("Removed_%dx", amount), 2);
    }
    return 1;
}
...
Dialog:PemerintahBrankasItem(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        if(PemerintahStorage[facid][AccountData[playerid][pListItemGudang]][ItemExists])
        {
            ...
            Dialog_Show(playerid, PemerintahWithdraw, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Pemerintah", str, "Submit", "Back");
        }
    }
    return 1;
}
