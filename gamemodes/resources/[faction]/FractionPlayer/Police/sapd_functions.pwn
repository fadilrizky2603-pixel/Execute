...
VarsDoorPolice()
{
    PoliceStuff[PoldaDuty] = CreateDynamicSphere(1507.0428, -2858.4790, 1586.4999, 2.0, 6, 6, -1);
    PoliceStuff[PoldaDesk] = CreateDynamicSphere(1496.9266, -2877.3750, 1591.5604, 2.0, 6, 6, -1);
    PoliceStuff[PoldaGun] = CreateDynamicSphere(1509.1433, -2845.8010, 1586.4999, 2.0, 6, 6, -1);
    PoliceStuff[PoldaBrankas] = CreateDynamicSphere(1495.0739, -2868.8235, 1586.4999, 2.0, 6, 6, -1);
    PoliceStuff[PoldaLocker] = CreateDynamicSphere(1511.0513, -2848.9338, 1586.4999, 2.0, 6, 6, -1);
    PoliceStuff[PoldaGarage] = CreateDynamicSphere(939.3791,-1733.5276,14.5444, 2.0, 0, 0, -1);
    PoliceStuff[PoldaImpound] = CreateDynamicSphere(944.2418,-1733.6715,14.5444, 2.0, 0, 0, -1);
    PoliceStuff[PoldaHelipad] = CreateDynamicSphere(959.1901,-1720.0372,34.7359, 2.0, 0, 0, -1);
    ...
    PoliceStuff[PoldaGaragePOS][0] = 939.3791;
    PoliceStuff[PoldaGaragePOS][1] = -1733.5276;
    PoliceStuff[PoldaGaragePOS][2] = 14.5444;
    PoliceStuff[PoldaSpawnPOS][0] = 943.3786;
    PoliceStuff[PoldaSpawnPOS][1] = -1746.1252;
    PoliceStuff[PoldaSpawnPOS][2] = 14.5503;
    PoliceStuff[PoldaSpawnPOS][3] = 91.1420;
...
{
    new query[255];
    mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `criminallogs` (`OwnerID`, `Sender`, `History`, `Time`) VALUES ('%d', '%s', '%s', CURRENT_TIMESTAMP())", AccountData[otherid][pID], AccountData[playerid][pName], string);
...
stock Sapd_GedelahFiture(playerid, targetid)
{
    ...
    Dialog_Show(playerid, SapdGeledah, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Geledah", str, "Pilih", "Batal");
    return 1;
}

Dialog:SapdGeledah(playerid, response, listitem, inputtext[])
{
    if(AccountData[playerid][pFaction] != FACTION_POLISI) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan bagian dari Kepolisian Kota DCRP!");
    if(response)
    {
        ...
        if(InventoryData[targetid][listitem][invExists])
        {
            ...
            Dialog_Show(playerid, SapdTakeBarang, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Geledah", str, "Input", "Batal");
        }
    }
    return 1;
}
    ...
    if(response)
    {
        ...
        if(!strcmp(string, "Radio", true))
        {
            if(ToggleRadio[targetid] || RadioMicOn[targetid])
            {
                ...
                PlayerTextDrawSetString(targetid, RadioTD[targetid][12], "0");
            }
        }
        
        ItemAdd(playerid, string, amount);
        ItemDelete(targetid, string, amount);
        
        ShowItemBox(playerid, string, sprintf("Received_%dx", amount), 2);
        ShowItemBox(targetid, string, sprintf("Removed_%dx", amount), 2);
...
hook OnPlayerEnterDynArea(playerid, STREAMER_TAG_AREA:areaid)
{
    if(AccountData[playerid][pFaction] == FACTION_POLISI)
    {
        ...
        {
            ShowKey(playerid, "Garasi Polisi");
        }
        ...
        if(areaid == PoliceStuff[PoldaDuty])
        {
            ...
            {
                ShowKey(playerid, "~g~On Duty");
            }
            else
            {
                ShowKey(playerid, "~r~Off Duty");
            }
        }
    ...
    else 
    {
        ...
        {
            ShowKey(playerid, "Cek Kendaraanmu");
        }
    }
    return 1;
}
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        if(AccountData[playerid][pFaction] == FACTION_POLISI)
        {
            ...
            {
                ShowPlayerDialog(playerid, DIALOG_POLICE_IMPOUND, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Impound",
                "Masukkan ID Pemain yang ingin dicek kendaraannya", "Submit", "Batal");
                // Dialog_Show(playerid, DialogCekPlayerImpound, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Impound", 
            ...
            {
                Dialog_Show(playerid, DialogLockerSapd, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Pakaian", 
                "Baju Biasa\
            ...
            if(IsPlayerInRangeOfPoint(playerid, 2.5, 939.3791,-1733.5276,14.5444) && AccountData[playerid][pDutyPD]) // Polda Garage
            {
                ShowPlayerDialog(playerid, DIALOG_POLICE_GARAGE, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Garasi Polda", 
            ...
            {
                ShowPlayerDialog(playerid, DIALOG_FEDERAL_GARAGE, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Garasi Federal", 
                "Keluarkan Kendaraan\ 
                \n"GRAY"Simpan Kendaraan\ 
                \nambil kendaraan\ 
                \n"GRAY"Hapus Kendaraan", "Pilih", "Batal");
            ...
            if(IsPlayerInRangeOfPoint(playerid, 2.5, 959.1901, -1720.0372, 34.7359) && AccountData[playerid][pDutyPD]) // Helipad
            {
                ShowPlayerDialog(playerid, DIALOG_POLICE_HELI_GARAGE, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Garasi Polda",
                "Keluarkan Kendaraan\
                \n"GRAY"Simpan Kendaraan\
                \nambil kendaraan\
                \n"GRAY"Hapus Kendaraan", "Pilih", "Batal");
            ...
            {   
                ...
                Dialog_Show(playerid, BrankasPolice, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Brankas Kepolisian", "ambil senjata", "Pilih", "Batal");
            }
            ...
            if(IsPlayerInDynamicArea(playerid, PoliceStuff[PoldaDesk]))
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_POLICE_BOSDESK, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Bos Desk", 
                "Invite\
                \n"GRAY"Kelola Jabatan\
                \nKick\
            ...
            if(IsPlayerInDynamicArea(playerid, PoliceStuff[PoldaBrankas]) && AccountData[playerid][pDutyPD])
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_POLVAULT, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Brankas Polisi", 
...
Dialog:DIALOG_TENTARA_PANEL(playerid, response, listitem, inputtext[])
{
    ...
    switch(listitem)
    {
        case 0: //revive
        {
            ...
            ItemDelete(playerid, "Medkit");
            ShowItemBox(playerid, "Medkit", "Removed_1x", 2);
            ...
            PlayerTextDrawSetString(playerid, ProgressBar[playerid][1], "MEREVIVE");
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    {
        if(!response) return 1;
        if(AccountData[playerid][pFaction] != FACTION_POLISI) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan bagian dari anggota Kepolisian DCRP");
        new targetid = AccountData[playerid][pTarget];
        if(!IsPlayerConnected(targetid)) return ShowTDN(playerid, NOTIFICATION_ERROR, "Pemain tersebut tidak terkoneksi kedalam server!");
        if(!IsPlayerNearPlayer(playerid, targetid, 3.0)) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda tidak dekat dengan player tersebut!");
        if(IsPlayerInjured(playerid)) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda sedang pingsan, tidak dapat menggunakan Panel!");
        switch(listitem)
        {
            ...
            case 11:// Penjarakan
            {
                ...
                Dialog_Show(playerid, Dialog_PenjaraFederal, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Penjarakan", "Masukan jumlah menit yang ingin diberikan:", "Input", "Batal");
            }
            ...
            case 17: // Cek Riwayat Kriminal
            {
                ...
                mysql_format(dcrp_query, query, sizeof(query), "SELECT * FROM `criminallogs` WHERE `OwnerID`=%d ORDER BY `Time` ASC", AccountData[targetid][pID]);
                executee = mysql_query(dcrp_query, query, true);
                if(cache_num_rows())
                {
            ...
            case 18: // Buat Catatan Kriminal
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_ADD_HKRIMINAL, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Buat Catatan Kriminal",
            ...
            {
                SetPVarInt(playerid, "IDTarget", targetid);
                ShowPlayerDialog(playerid, DIALOG_REMOVE_HKRIMINAL, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Hapus Catatan Kriminal",
    ...
    else if(dialogid == DIALOG_REMOVE_HKRIMINAL)
    {
        ...
        if(AccountData[playerid][pFaction] != FACTION_POLISI) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan bagian dari Kepolisian DCRP!");
        ...
        new query[200];
        mysql_format(dcrp_query, query, sizeof(query), "DELETE FROM `criminallogs` WHERE `OwnerID` = %d", AccountData[targetid][pID]);
        mysql_tquery(dcrp_query, query);
    ...
    else if(dialogid == DIALOG_ADD_HKRIMINAL)
    {
        ...
        if(isnull(inputtext)) return ShowPlayerDialog(playerid, DIALOG_ADD_HKRIMINAL, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Buat Catatan Kriminal",
        "Error: Tidak dapat diisi kosong!\nMohon masukkan deskripsi catatan kriminal yang akan diberikan kepada orang tersebut dibawah ini:", "Buat", "Batal");

        if(IsNumeric(inputtext)) return ShowPlayerDialog(playerid, DIALOG_ADD_HKRIMINAL, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Buat Catatan Kriminal",
        "Error: Tidak dapat diisi angka!\nMohon masukkan deskripsi catatan kriminal yang akan diberikan kepada orang tersebut dibawah ini:", "Buat", "Batal");

        if(strlen(inputtext) > 128) return ShowPlayerDialog(playerid, DIALOG_ADD_HKRIMINAL, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Buat Catatan Kriminal",
        ...
        switch(listitem)
        {
            case 0:// Invite
            {
                ...
                if(count == 0)
                {
                    ...
                    return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Invite",
                    "Tidak ada orang di sekitar anda!", "Tutup", "");
                }

                ShowPlayerDialog(playerid, DIALOG_POLICE_INVITE, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Invite", frmxt, "Pilih", "Batal");
            }
            case 1:// Kelola Jabatan:
            {
                new dbquery[258];
                mysql_format(dcrp_query, dbquery, sizeof(dbquery), "SELECT * FROM `player_characters` WHERE `Char_Faction` = 1 ORDER BY `Char_FactionRank` DESC");
                mysql_query(dcrp_query, dbquery);
                ...
                if(rows)
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_POLICESETRANK, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Set Jabatan", shstr, "Pilih", "Batal");
                    // ShowPlayerDialogPages(playerid, "PolisiSetRank", DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Set Jabatan", "Pilih", "Batal", 15, ""GREEN">> Lanjut", ""ORANGE"<< Kembali");
                }
                else 
                {
                    PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
                    return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Set Jabatan", "Tidak ada Anggota Polisi!", "Tutup", "");
                }
            }
            case 2://
            {
                mysql_query(dcrp_query, "SELECT * FROM player_characters WHERE Char_Faction = 1 ORDER BY Char_FactionRank DESC");
                ...
                if(rows)
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_POLICEKICKMEMBER, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Set Jabatan", shstr, "Kick", "Batal");
                    // ShowPlayerDialogPages(playerid, "PolisiKickMember", DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Set Jabatan", "Kick", "Batal", 15, ""GREEN">> Lanjut", ""ORANGE"<< Kembali");
                }
                else 
                {
                    PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
                    return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Set Jabatan", "Tidak ada Anggota Kepolisian!", "Tutup", "");
                }
            }
            case 3:// Saldo Finansial
            {
                new frmxt[255];
                format(frmxt, sizeof(frmxt), "Perusahaan Kepolisian DCRP saat ini memiliki saldo sebesar:\
                \n"DARKGREEN"%s", FormatMoney(PoliceMoneyVault));
                ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Police Money", frmxt, "Tutup", "");
            }
            case 4:// Depo Saldo 
            {
                ShowPlayerDialog(playerid, DIALOG_DEPOSIT_POLICE, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Polisi Deposit",
                "Mohon masukkan nominal deposit untuk saldo perusahaan:", "Input", "Batal");
            }
            case 5:// Withdraw Saldo
            {
                ShowPlayerDialog(playerid, DIALOG_WITHDRAW_POLICE, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Polisi Withdraw", 
    ...
    else if(dialogid == DIALOG_POLICE_INVITE)
    {
        ...
        mysql_format(dcrp_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_Faction` = 1, `Char_FactionRank` = 1 WHERE `pID` = %d", AccountData[targetid][pID]);
    ...
    else if(dialogid == DIALOG_POLICESETRANK)
    {
        ...
        if(AccountData[playerid][pFaction] != FACTION_POLISI) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan anggota Polda DCRP!");
        ...
        if(rows)
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_RANK_SET_POLISI, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Kelola Jabatan", \
    ...
    else if(dialogid == DIALOG_POLICEKICKMEMBER)
    {
        ...
        if(cache_num_rows())
        {
            ...
            new strgbg[158];
            mysql_format(dcrp_query, strgbg, sizeof(strgbg), "DELETE FROM `player_vehicles` WHERE `PVeh_OwnerID`=%d AND `PVeh_Faction` = 1", pidrow);
            ...
            foreach(new i : Player)
            {
                if(AccountData[i][IsLoggedIn] && AccountData[i][pSpawned] && pidrow == AccountData[i][pID])
                {
                    ...
                    RefreshFactionMap(i);
                    ShowTDN(i, NOTIFICATION_WARNING, "Anda telah dikeluarkan dari faction Kepolisian DCRP!");
                }
            }
            mysql_format(dcrp_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_Faction`=0, `Char_FactionRank`=0, `Char_UsingUniform`=0 WHERE `pID`=%d", pidrow);
            mysql_tquery(dcrp_query, icsr);
            format(kckstr, sizeof(kckstr), "Anda telah berhasil menendang anggota:\n\
            Nama: %s\n\
            Rank: %s\n\
            Last Online: %s", fckname, PolisiRank[fckrank], fcklastlogin);
            ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Kick Anggota",
            kckstr, "Tutup", "");
    ...
    else if(dialogid == DIALOG_RANK_SET_POLISI)
    {
        ...
        if(isnull(inputtext)) return ShowPlayerDialog(playerid, DIALOG_RANK_SET_POLISI, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Kelola Jabatan", \
        "Error: Tidak dapat diisi kosong!\
        \nSilahkan pilih jabatan untuk ditetapkan (masukkan angka saja):\
        \n1. BRIPDA\
        \n2. BRIPTU\
        ...
        if(!IsNumeric(inputtext)) return ShowPlayerDialog(playerid, DIALOG_RANK_SET_POLISI, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Kelola Jabatan", \
        ...
        if(strval(inputtext) < 1 || strval(inputtext) > AccountData[playerid][pFactionRank]) return ShowPlayerDialog(playerid, DIALOG_RANK_SET_POLISI, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Kelola Jabatan", \
        ...
        new affah[200];
        mysql_format(dcrp_query, affah, sizeof(affah), "UPDATE `player_characters` SET `Char_FactionRank`=%d WHERE `pID`=%d", strval(inputtext), AccountData[playerid][pTempSQLFactMemberID]);
        mysql_tquery(dcrp_query, affah);
    ...
    else if(dialogid == DIALOG_DEPOSIT_POLICE)
    {
        ...
        mysql_format(dcrp_query, frmtmny, sizeof(frmtmny), "UPDATE `stuffs` SET `policemoneyvault` = %d WHERE `ID` = 0", PoliceMoneyVault);
        mysql_tquery(dcrp_query, frmtmny);
        ShowTDN(playerid, NOTIFICATION_SUKSES, sprintf("Anda berhasil deposit %s untuk Kepolisian DCRP", FormatMoney(depocash)));
    }
    else if(dialogid == DIALOG_WITHDRAW_POLICE)
    {
        ...
        ShowTDN(playerid, NOTIFICATION_SUKSES, sprintf("Anda berhasil withdraw %s dari Kepolisian DCRP", FormatMoney(withdrawcash)));
    }
    else if(dialogid == DIALOG_POLVAULT)
    {
        ...
        switch(listitem)
        {
            case 0: //deposit
            {
                ...
                mysql_format(dcrp_query, tss, sizeof(tss), "SELECT * FROM `inventory` WHERE `ID`=%d", AccountData[playerid][pID]);
                mysql_query(dcrp_query, tss);
                new rows = cache_num_rows();
                if(rows)
                {
                    for(new x; x < rows; ++x)
                    ...
                    ShowPlayerDialog(playerid, DIALOG_POLVAULT_DEPOSIT, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Brankas Polisi", str, "Pilih", "Batal");
                }
                else 
                {
                    AccountData[playerid][menuShowed] = false;
                    PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
                    return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Brankas Polisi",
            ...
            case 1: //withdraw
            {
                ...
                format(str, sizeof(str), "Nama Item\tJumlah\tBerat (-/-)\n");
                mysql_query(dcrp_query, "SELECT * FROM `polisi_brankas` WHERE `PID`=0");
                if(cache_num_rows() > 0)
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_POLVAULT_WITHDRAW, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Brankas Polisi", str, "Pilih", "Batal");
                }
                else 
                {
                    AccountData[playerid][menuShowed] = false;
    ...
    else if(dialogid == DIALOG_POLVAULT_DEPOSIT)
    {
        ...
        if(cache_num_rows() > 0)
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_POLVAULT_IN, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Polisi", 
            shstr, "Input", "Batal");
        }
    }
    else if(dialogid == DIALOG_POLVAULT_IN)
    {
        ...
        ItemDelete(playerid, FactionBrankas[playerid][factionBrankasTemp], quantity);
        ShowItemBox(playerid, FactionBrankas[playerid][factionBrankasTemp], sprintf("Removed_%dx", quantity), 2);

        new invstr[1028];
        mysql_format(dcrp_query, shstr, sizeof(shstr), "SELECT * FROM `polisi_brankas` WHERE `PID`=0 AND `Item` = '%s'", FactionBrankas[playerid][factionBrankasTemp]);
        mysql_query(dcrp_query, shstr);
        ...
        if(rows > 0)
        {
            mysql_format(dcrp_query, invstr, sizeof(invstr), "UPDATE `polisi_brankas` SET `Quantity` = `Quantity` + %d WHERE `PID`=0 AND `Item`='%s'", quantity, FactionBrankas[playerid][factionBrankasTemp]);
            mysql_tquery(dcrp_query, invstr, "OnPolisiDeposit", "i", playerid);
        }
        else 
        {
            mysql_format(dcrp_query, invstr, sizeof(invstr), "INSERT INTO `polisi_brankas` SET `PID`=0, `Item`='%s', `Model`=%d, `Quantity`=%d", FactionBrankas[playerid][factionBrankasTemp], FactionBrankas[playerid][factionBrankasModel], quantity);
            mysql_tquery(dcrp_query, invstr, "OnBengkelDeposit", "i", playerid);
        }
        AccountData[playerid][menuShowed] = false;
    }
    else if(dialogid == DIALOG_POLVAULT_WITHDRAW)
    {
        ...
        if(cache_num_rows() > 0)
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_POLVAULT_OUT, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Polisi",
            shstr, "Input", "Batal");
        }
        else 
        {
            AccountData[playerid][menuShowed] = false;
            ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Brankas Polisi",
    ...
    else if(dialogid == DIALOG_POLVAULT_OUT)
    {
        ...
        {
            mysql_format(dcrp_query, jts, sizeof(jts), "UPDATE `polisi_brankas` SET `Quantity`=%d WHERE `ID`=%d", FactionBrankas[playerid][factionBrankasQuant], FactionBrankas[playerid][factionBrankasID]);
            mysql_tquery(dcrp_query, jts);
        }
        else 
        {
            mysql_format(dcrp_query, jts, sizeof(jts), "DELETE FROM `polisi_brankas` WHERE `ID`=%d", FactionBrankas[playerid][factionBrankasID]);
            mysql_tquery(dcrp_query, jts);
        }
        ItemAdd(playerid, FactionBrankas[playerid][factionBrankasTemp], quantity);
        ShowItemBox(playerid, FactionBrankas[playerid][factionBrankasTemp], sprintf("Received_%dx", quantity), 2);
    ...
    else if(dialogid == DIALOG_POLICE_HELI_GARAGE)
    {
        ...
        switch(listitem)
        {
            case 0:// Keluarkan Kendaraan
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_POLICE_HELI_GARAGE_OUT, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Garasi Polda", list, "Pilih", "Batal");
            }
            case 1:// Simpan Kendaraan
            {
                ...
                if(!foundnearby) return ShowTDN(playerid, NOTIFICATION_ERROR, "Tidak ada kendaraan dari Kepolisian DCRP milik anda di sekitar!");
            }
            case 2:// ambil kendaraan
            {
                ShowPlayerDialog(playerid, DIALOG_POLICE_HELI_BUY, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Garasi Polda",
                "Model\tHarga\ 
            ...
            {
                new frmtdel[151];
                mysql_format(dcrp_query, frmtdel, sizeof(frmtdel), "SELECT * FROM `player_vehicles` WHERE `PVeh_Faction` = 1 AND `PVeh_OwnerID` = %d", AccountData[playerid][pID]);
                mysql_query(dcrp_query, frmtdel);
                ...
                if(rows)
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_POLICE_GARAGE_DEL, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Hapus Kendaraan", list, "Hapus", "Batal");
                }
                else 
                {
                    PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
                    return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Hapus Kendaraan", "Anda tidak memiliki kendaraan Polisi!", "Tutup", "");
                }
            }
        }
    }
    else if(dialogid == DIALOG_POLICE_HELI_BUY)
    {
        ...
        switch(listitem)
        {
            case 0: //maverick
            {
                // Money check removed, action is now free
                //VehicleFaction_Create(playerid, 497, FACTION_POLISI, 948.8585, -1733.0096, 26.2472, 88.4399, 36, 36);
    ...
    else if(dialogid == DIALOG_POLICE_GARAGE)
    {
        ...
        switch(listitem)
        {
            case 0:// Keluarkan Kendaraan
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_POLICE_GARAGE_OUT, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Garasi Polda", list, "Pilih", "Batal");
            }
            case 1:// Simpan Kendaraan
            {
                new carid = -1, bool: foundnearby = false;
            ...
            case 2:// ambil kendaraan
            {
                ShowPlayerDialog(playerid, DIALOG_POLICE_GARAGE_BUY, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- ambil kendaraan",
                "Model\
                \nSultan Sabhara\
                \nCheetah Speed Hunter\
                \nBullet Speed Hunter\
                \nSanchez Sabhara\
                \nFBI Rancher\
                \nMobil Tahanan\
    ...
    else if(dialogid == DIALOG_FEDERAL_GARAGE)
    {
        ...
        switch(listitem)
        {
            case 0:// Keluarkan Kendaraan
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_FEDERAL_GARAGE_OUT, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Garasi Federal", list, "Pilih", "Batal");
            }
            ...
            case 2:// ambil kendaraan
            {
                ShowPlayerDialog(playerid, DIALOG_FEDERAL_GARAGE_BUY, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Ambil Kendaraan",
                "Model\tHarga\
                \nSultan Sabhara\t$5000\
    ...
    else if(dialogid == DIALOG_POLICE_GARAGE_DEL)
    {
        ...
        mysql_format(dcrp_query, frmtdel, sizeof(frmtdel), "SELECT * FROM `player_vehicles` WHERE `PVeh_Faction` = 1 AND `PVeh_OwnerID` = %d", AccountData[playerid][pID]);
        execute = mysql_query(dcrp_query, frmtdel, true);
        ...
        if(rowscount)
        {
            ...
            if(listitem >= 0 && listitem < rowscount)
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Hapus Kendaraan", kckstr, "Tutup", "");
                ...
                mysql_format(dcrp_query, strgbg, sizeof(strgbg), "DELETE FROM `player_vehicles` WHERE `id`=%d", hvid);
                mysql_tquery(dcrp_query, strgbg);
    ...
    {
        if(!response) return ShowTDN(playerid, NOTIFICATION_INFO, "Anda telah membatalkan pilihan");
        if(AccountData[playerid][pFaction] != FACTION_POLISI) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan anggota Kepolisian DCRP!");
        if(AccountData[playerid][pInjured]) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda sedang pingsan!");
        ...
        new dbstr[200];
        mysql_format(dcrp_query, dbstr, sizeof(dbstr), "UPDATE `player_vehicles` SET `PVeh_Impounded`=0, `PVeh_ImpoundedPrice`=0, `PVeh_ImpoundedTime`=0, `PVeh_ImpoundedReason`='N/A' WHERE `id`=%d", PlayerVehicle[vehid][pVehID]);
        ...
        else 
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Impound",
    ...
    else if(dialogid == DIALOG_PDM)
    {
        if(response)
        {
            if(AccountData[playerid][pFaction] != FACTION_POLISI) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan anggota Kepolisan DCRP!");
            ...
            switch(listitem)
            {
                case 0: // Interaksi Kendaraan
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_PDM_VEHICLE, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Interaksi Kendaraan",
                ...
                {
                    ShowPlayerDialog(playerid, DIALOG_PDM_OBJECT, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Mengeluarkan Object",
                ...
                case 2: // cek tahanan
                {
                    ...
                    if(count == 0)
                        ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- List Tahanan", "Tidak ada pemain dalam masa hukuman penjara!", "Tutup", "");
                    else
                        ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- List Tahanan", frmxt, "Tutup", "");
                }
            }
        }
        else
    ...
    else if(dialogid == DIALOG_PDM_VEHICLE)
    {
        ...
        if(AccountData[playerid][pFaction] != FACTION_POLISI) 
        {
            ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan anggota Kepolisian DCRP!");
            NearestVehicleID[playerid] = INVALID_VEHICLE_ID;
            return 1;
        }
        ...
        switch(listitem)
        {
            case 0: //vehicle info
            {
                ...
                foreach(new player : Player) if (AccountData[player][IsLoggedIn])
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Informasi Kendaraan", sha, "Tutup", "");
                }
            }
            case 1: // Bobol
            {
            ...
            case 3: //Impound
            {
                ShowPlayerDialog(playerid, DIALOG_PDM_VEHICLE_IMPOUND, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Menyita Kendaraan",
    ...
    else if(dialogid == DIALOG_PDM_VEHICLE_IMPOUND)
    {
        ...
        if(isnull(inputtext)) return ShowPlayerDialog(playerid, DIALOG_PDM_VEHICLE_IMPOUND, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Menyita Kendaraan",
        "Error: Tidak dapat diisi kosong!\nHarap masukkan sesuai format yang ada dibawah ini!\n[Durasi sita [hari]] [denda] [alasan]", "Input", "Batal");

        new impoundtime, fee, reason[128];
        if(sscanf(inputtext, "dds[128]", impoundtime, fee, reason)) return ShowPlayerDialog(playerid, DIALOG_PDM_VEHICLE_IMPOUND, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Menyita Kendaraan",
        "Harap masukkan sesuai format yang ada dibawah ini!\n[Durasi sita [hari]] [denda] [alasan]", "Input", "Batal");

        if(impoundtime < 1 || impoundtime > 30) return ShowPlayerDialog(playerid, DIALOG_PDM_VEHICLE_IMPOUND, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Menyita Kendaraan",
        "Error: Maksimal waktu 30 hari!\nHarap masukkan sesuai format yang ada dibawah ini!\n[Durasi sita [hari]] [denda] [alasan]", "Input", "Batal");

        if(fee < 1) return ShowPlayerDialog(playerid, DIALOG_PDM_VEHICLE_IMPOUND, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Menyita Kendaraan",
...
CheckTargetWeapon(playerid, targetid)
{
    ...
    ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Kepemilikan Senjata",
    frmtgun, "Tutup", "");
    return 1;
}
...
Dialog:BrankasPolice(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        switch(listitem)
        {
            ...
            {
                new xjjs[512];
                format(xjjs, sizeof(xjjs), "Senjata\tAmmo\nDesert Eagle\t100\n"GRAY"M4A1\t500\nShotgun\t100\n"GRAY"MP-5\t150\nSniper\t80\n"GRAY"Spas\t100");
                Dialog_Show(playerid, BRGUNPOLICE, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Brankas Persenjataan", xjjs, "Ambil", "Tutup");
            }
            case 1:// 
            {
...
{
    if(!response) return ShowTDN(playerid, NOTIFICATION_INFO, "Anda telah membatalkan pilihan!");
    if(GetPlayerFaction(playerid) != FACTION_POLISI) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan bagian dari Polda DCRP!");
    
    // Check if player is Rank 11-15 (KOMBES to higher ranks)
    if(AccountData[playerid][pFactionRank] < 11 || AccountData[playerid][pFactionRank] > 15) return ShowTDN(playerid, NOTIFICATION_ERROR, "Hanya rank 11-15 yang dapat mengambil senjata!");
    
    new weaponid, maxammo;
    switch(listitem)
    {
        case 0: { weaponid = 24; maxammo = 100; } // Desert Eagle
        case 1: { weaponid = 31; maxammo = 500; } // M4A1
        case 2: { weaponid = 25; maxammo = 100; } // Shotgun
        case 3: { weaponid = 29; maxammo = 150; } // MP-5
        case 4: { weaponid = 34; maxammo = 80; }  // Sniper
        case 5: { weaponid = 27; maxammo = 100; } // Spas
    }

    AccountData[playerid][pTempWeaponID] = weaponid;
    AccountData[playerid][pTempMaxAmmo] = maxammo;

    new str[128];
    format(str, sizeof(str), "Masukkan jumlah amunisi yang ingin diambil (Maksimal: %d):", maxammo);
    Dialog_Show(playerid, BRGUNPOLICE_AMMO, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Pilih Amunisi", str, "Ambil", "Batal");
    return 1;
}

Dialog:BRGUNPOLICE_AMMO(playerid, response, listitem, inputtext[])
{
    if(!response) return ShowTDN(playerid, NOTIFICATION_INFO, "Anda telah membatalkan pilihan!");
    
    new ammo = strval(inputtext);
    if(ammo <= 0) return ShowTDN(playerid, NOTIFICATION_ERROR, "Jumlah amunisi harus lebih dari 0!");
    if(ammo > AccountData[playerid][pTempMaxAmmo]) 
    {
        new str[128];
        format(str, sizeof(str), "Jumlah amunisi tidak boleh lebih dari %d!", AccountData[playerid][pTempMaxAmmo]);
        return ShowTDN(playerid, NOTIFICATION_ERROR, str);
    }

    GivePlayerWeaponEx(playerid, AccountData[playerid][pTempWeaponID], ammo);
    new str[128];
    format(str, sizeof(str), "Berhasil mengambil %s dengan %d amunisi!", ReturnWeaponName(AccountData[playerid][pTempWeaponID]), ammo);
    ShowTDN(playerid, NOTIFICATION_SUKSES, str);

    AccountData[playerid][pTempWeaponID] = 0;
    AccountData[playerid][pTempMaxAmmo] = 0;
    return 1;
}
    ...
    switch(listitem)
    {
        ...
        case 1:// 
        {
            if(AccountData[playerid][pGender] == 1) {
                Dialog_Show(playerid, Clothes_MaleSAPD, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Baju Dinas", ""WHITE"Sabhara\
                \n"GRAY"Polantas 1\n"WHITE"Polantas 2\n"GRAY"Brimob 1\n"WHITE"Brimob 2\n"GRAY"Propam\n"WHITE"Jenderal 1\
                \n"GRAY"Jenderal 2\n"WHITE"FBI\n"GRAY"DIV Udara\n"WHITE"Casis\n"GRAY"Rompi Kepolisian", "Pilih", "Batal");
            } else {
                Dialog_Show(playerid, Clothes_FemaleSAPD, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Baju Dinas", ""WHITE"Sabhara\n"GRAY"Polantas\n"WHITE"Brimob\
                \n"GRAY"Rompi Kepolisian", "Pilih", "Batal");
            }
        }
    }
...
CMD:pdm(playerid, params[])
{
    ...
    ShowPlayerDialog(playerid, DIALOG_PDM, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Police Menu",
    "Interaksi Kendaraan\
...
CMD:takegunlic(playerid, params[])
{
    ...
    new cQuery[255];
    mysql_format(dcrp_query, cQuery, sizeof(cQuery), "UPDATE `player_characters` SET `Char_WeaponLic`=%d, `Char_WeaponLicTime`=%d WHERE `pID`=%d", AccountData[otherid][pGunLic], AccountData[otherid][pGunLicTime], AccountData[otherid][pID]);
...
{
    if(AccountData[playerid][pFaction] != FACTION_POLISI) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan anggota Polisi DCRP!");
    if(AccountData[playerid][pInjured]) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda sedang pingsan!");
...
CMD:giveplate(playerid, params[])
{
    ...
    foreach(new iterid : PvtVehicles)
    {
        if(PlayerVehicle[iterid][pVehPhysic] == vehid && IsValidVehicle(PlayerVehicle[iterid][pVehPhysic]))
        {
            ...
            PlayerTextDrawSetString(playerid, ProgressBar[playerid][1], "MEMASANG PLATE");
...
public ImpoundVehicles(playerid, VID)
{
    ...
    if(AccountData[playerid][ActivityTime] >= 15)
    {
        ...
        foreach(new i : PvtVehicles) if (PlayerVehicle[i][pVehExists] && PlayerVehicle[i][pVehPhysic] == VID)
        {
            ...
            new dbstr[258];
            mysql_format(dcrp_query, dbstr, sizeof(dbstr), "UPDATE `player_vehicles` SET `PVeh_Impounded`=1, `PVeh_ImpoundedTime`=%d, `PVeh_ImpoundedPrice`=%d, `PVeh_ImpoundedReason`='%s' WHERE `id`=%d", PlayerVehicle[i][pVehImpoundDuration], PlayerVehicle[i][pVehImpoundFee], PlayerVehicle[i][pVehImpoundReason], PlayerVehicle[i][pVehID]);
            mysql_tquery(dcrp_query, dbstr);
        }
        NearestVehicleID[playerid] = INVALID_VEHICLE_ID;
...
public MakePlate(playerid, vehicleid)
{
    ...
    if(AccountData[playerid][ActivityTime] >= 130)
    {
        ...
        ItemDelete(playerid, "Plat Besi");
        ShowItemBox(playerid, "Plat Besi", "Removed_1x", 2);
        ...
        foreach(new iterid : PvtVehicles)
        {
            if(PlayerVehicle[iterid][pVehPhysic] == vehicleid && IsValidVehicle(PlayerVehicle[iterid][pVehPhysic]))
            {
                ...
                format(PlayerVehicle[iterid][pVehPlate], 128, "DCRP-%d%d%d%d-%s%s%s", random(10), random(10), random(10), random(10), g_Alphabet[xd1], g_Alphabet[xd2], g_Alphabet[xd3]);
