...
Dialog:DIALOG_PEDAGANG_PANEL(playerid, response, listitem, inputtext[])
{
    ...
    if(AccountData[playerid][pFaction] != FACTION_PEDAGANG) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan bagian dari Pedagang DCRP!");
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        if(AccountData[playerid][pFaction] == FACTION_PEDAGANG)
        {
            ...
            {
                ShowPlayerDialog(playerid, DIALOG_LOCKERPEDAGANG, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Locker Pedagang",
                "Baju Biasa\
                \n"GRAY"Pedagang 1\
                \nPedagang 2\
            ...
            if((IsPlayerInDynamicArea(playerid, Pedagang_Stuff[PdgCooking]) || IsPlayerInDynamicArea(playerid, Pedagang_Stuff[PdgCookingEms])) && AccountData[playerid][pDutyPedagang])
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_LOUNGES_MASAK, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Dapur Pedagang",
            ...
            if((IsPlayerInDynamicArea(playerid, Pedagang_Stuff[PdgKulkas]) || IsPlayerInDynamicArea(playerid, Pedagang_Stuff[PdgKulkasEms])) && AccountData[playerid][pDutyPedagang])
            {
                ...
                Dialog_Show(playerid, BrankasLounge, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Brankas Pedagang", shstr, "Pilih", "Batal");
                HideShortKey(playerid);
            }
            ...
            {
                ShowPlayerDialog(playerid, DIALOG_PEDAGANG_GARAGE, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Garasi Pedagang",
                "Keluarkan Kendaraan\
            ...
            if(IsPlayerInDynamicArea(playerid, Pedagang_Stuff[PdgDesk]))
            {
                ...
                Dialog_Show(playerid, BosDesk_Pedagang, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Bos Desk",
                "Invite\
                \n"GRAY"Kelola Jabatan\
                \nKick\
                \n"GRAY"Lihat Anggota\
    ...
    if(response)
    {
        switch(listitem)
        {
            case 0:
            {
                ...
                Dialog_Show(playerid, Pedagang_Invite, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Bos Desk", frmxt, "Pilih", "Batal");
            }
            case 1: // Kelola Jabatan offline / online
			{
                mysql_query(dcrp_query, "SELECT * FROM player_characters WHERE Char_Faction = 6 ORDER BY Char_FactionRank DESC");
                ...
                if(rows)
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_PEDSETRANK, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Set Jabatan", shstr, "Pilih", "Batal");
                }
                else 
                {
                    PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
                    return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Set Jabatan", "Tidak ada Anggota Pedagang!", "Tutup", "");
                }
			}
			case 2:
			{
                ...
                if(rows)
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_PEDKICKMEMBER, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Kick Anggota", shstr, "Kick", "Batal");
                }
                else 
                {
                    PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
			...
			case 4:
			{
				...
				Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Saldo Finansial", sstr, "Close", "");
			}
			case 5:
			{
				new sstr[512];
				...
				Dialog_Show(playerid, SaldoPedagangDeposit, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Pedagang", sstr, "Input", "Batal");
			}
			case 6:
			{
				new sstr[512];
				format(sstr, sizeof sstr, "Mohon ikuti format berikut:\nGunakanlah format ambil [jumlah] untuk menarik saldo dari Brankas\nGunakan tanpa tanda ][ pada kolom dibawah ini:");
				Dialog_Show(playerid, SaldoPedagangWithdraw, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Pedagang", sstr, "Input", "Batal");
			}
        }
    }
...
Dialog:SaldoPedagangDeposit(playerid, response, listitem, inputtext[])
{
    ...
    if(response)
    {
        ...
        if(!strcmp(string, "depo", true))
        {
            ...
            ShowItemBox(playerid, "Uang", sprintf("Removed_$%s", FormatMoney(amount)), 2);
            
            new query[255];
            mysql_format(dcrp_query, query, sizeof(query), "UPDATE `stuffs` SET `restomoneyvault` = %d WHERE `ID` = 0", RestoMoneyVault);
...
Dialog:SaldoPedagangWithdraw(playerid, response, listitem, inputtext[])
{
    ...
    if(response)
    {
        ...
        if(!strcmp(string, "ambil", true))
        {
            ...
            ShowItemBox(playerid, "Uang", sprintf("Received_$%s", FormatMoney(amount)), 2);
...
Dialog:Pedagang_Invite(playerid, response, listitem, inputtext[])
{
    ...
    mysql_format(dcrp_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_Faction`= 6, `Char_FactionRank` = 1 WHERE `pID`=%d", AccountData[targetid][pID]);
...
Dialog:BrankasLounge(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        switch(listitem)
        {
            case 0:
            {
                Dialog_Show(playerid, BrankasSambal, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Pedagang", "Mohon ikuti format berikut\n[ambil] [jumlah] atau [depo] [jumlah]\nMasukkan tanpa tanda ][ pada kolom dibawah ini:", "Input", "Batal");
            }
            case 1:
            {
                Dialog_Show(playerid, BrankasBeras, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Pedagang", "Mohon ikuti format berikut\n[ambil] [jumlah] atau [depo] [jumlah]\nMasukkan tanpa tanda ][ pada kolom dibawah ini:", "Input", "Batal");
            }
            case 2:
            {
                Dialog_Show(playerid, BrankasGula, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Pedagang", "Mohon ikuti format berikut\n[ambil] [jumlah] atau [depo] [jumlah]\nMasukkan tanpa tanda ][ pada kolom dibawah ini:", "Input", "Batal");
            }
            case 3:
            {
                Dialog_Show(playerid, BrankasGaram, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Pedagang", "Mohon ikuti format berikut\n[ambil] [jumlah] atau [depo] [jumlah]\nMasukkan tanpa tanda ][ pada kolom dibawah ini:", "Input", "Batal");
            }
            case 4:
            {
                Dialog_Show(playerid, BrankasIkan, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Pedagang", "Mohon ikuti format berikut\n[ambil] [jumlah] atau [depo] [jumlah]\nMasukkan tanpa tanda ][ pada kolom dibawah ini:", "Input", "Batal");
            }
            case 5:
            {
                Dialog_Show(playerid, BrankasAyamFillet, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Pedagang", "Mohon ikuti format berikut\n[ambil] [jumlah] atau [depo] [jumlah]\nMasukkan tanpa tanda ][ pada kolom dibawah ini:", "Input", "Batal");
            }
            case 6:
            {
                Dialog_Show(playerid, BrankasSusuOlahan, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Pedagang", "Mohon ikuti format berikut\n[ambil] [jumlah] atau [depo] [jumlah]\nMasukkan tanpa tanda ][ pada kolom dibawah ini:", "Input", "Batal");
            }
            case 7:
            {
                Dialog_Show(playerid, BrankasAirMineral, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Pedagang", "Mohon ikuti format berikut\n[ambil] [jumlah] atau [depo] [jumlah]\nMasukkan tanpa tanda ][ pada kolom dibawah ini:", "Input", "Batal");
            }
            case 8:
            {
                Dialog_Show(playerid, BrankasNasgor, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Pedagang", "Mohon ikuti format berikut\n[ambil] [jumlah] atau [depo] [jumlah]\nMasukkan tanpa tanda ][ pada kolom dibawah ini:", "Input", "Batal");
            }
            case 9:
            {
                Dialog_Show(playerid, BrankasBakso, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Pedagang", "Mohon ikuti format berikut\n[ambil] [jumlah] atau [depo] [jumlah]\nMasukkan tanpa tanda ][ pada kolom dibawah ini:", "Input", "Batal");
            }
            case 10:
            {
                Dialog_Show(playerid, BrankasNasiPecel, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Pedagang", "Mohon ikuti format berikut\n[ambil] [jumlah] atau [depo] [jumlah]\nMasukkan tanpa tanda ][ pada kolom dibawah ini:", "Input", "Batal");
            }
            case 11:
            {
                Dialog_Show(playerid, BrankasBuburPedas, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Pedagang", "Mohon ikuti format berikut\n[ambil] [jumlah] atau [depo] [jumlah]\nMasukkan tanpa tanda ][ pada kolom dibawah ini:", "Input", "Batal");
            }
            case 12:
            {
                Dialog_Show(playerid, BrankasSusuFresh, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Pedagang", "Mohon ikuti format berikut\n[ambil] [jumlah] atau [depo] [jumlah]\nMasukkan tanpa tanda ][ pada kolom dibawah ini:", "Input", "Batal");
            }
            case 13:
            {
                Dialog_Show(playerid, BrankasEsTeh, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Pedagang", "Mohon ikuti format berikut\n[ambil] [jumlah] atau [depo] [jumlah]\nMasukkan tanpa tanda ][ pada kolom dibawah ini:", "Input", "Batal");
            }
            case 14:
            {
                Dialog_Show(playerid, BrankasKopiKenangan, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Pedagang", "Mohon ikuti format berikut\n[ambil] [jumlah] atau [depo] [jumlah]\nMasukkan tanpa tanda ][ pada kolom dibawah ini:", "Input", "Batal");
            }
            case 15:
            {
                Dialog_Show(playerid, BrankasCochoMatcha, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Pedagang", "Mohon ikuti format berikut\n[ambil] [jumlah] atau [depo] [jumlah]\nMasukkan tanpa tanda ][ pada kolom dibawah ini:", "Input", "Batal");
            }
        }
    }
    else 
...
Dialog:BrankasSambal(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        if(!strcmp(option, "ambil", true))
        {
            ...
            Sambal -= amount;
            ItemAdd(playerid, "Sambal", amount);
        ...
        else if(!strcmp(option, "depo", true))
        {
            ...
            Sambal += amount;
            ItemDelete(playerid, "Sambal", amount);
            ShowItemBox(playerid, "Sambal", sprintf("Removed_%dx", amount), 2);
...
Dialog:BrankasBeras(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        else if(!strcmp(option, "depo", true))
        {
            ...
            Beras += amount;
            ItemDelete(playerid, "Beras", amount);
            ShowItemBox(playerid, "Beras", sprintf("Removed_%dx", amount), 2);
...
Dialog:BrankasGula(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        else if(!strcmp(option, "depo", true))
        {
            ...
            Gula += amount;
            ItemDelete(playerid, "Gula", amount);
            ShowItemBox(playerid, "Gula", sprintf("Removed_%dx", amount), 2);
...
Dialog:BrankasGaram(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        else if(!strcmp(option, "depo", true))
        {
            ...
            Garam += amount;
            ItemDelete(playerid, "Garam", amount);
            ShowItemBox(playerid, "Garam", sprintf("Removed_%dx", amount), 2);
...
Dialog:BrankasIkan(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        else if(!strcmp(option, "depo", true))
        {
            ...
            Ikan += amount;
            ItemDelete(playerid, "Ikan", amount);
            ShowItemBox(playerid, "Ikan", sprintf("Removed_%dx", amount), 2);
...
Dialog:BrankasAyamFillet(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        if(!strcmp(option, "ambil", true))
        {
            ...
            AyamFillet -= amount;
            ItemAdd(playerid, "Ayam Kemas", amount);
            ShowItemBox(playerid, "Ayam Kemas", sprintf("Received_%dx", amount), 2);
        ...
        else if(!strcmp(option, "depo", true))
        {
            ...
            AyamFillet += amount;
            ItemDelete(playerid, "Ayam Kemas", amount);
            ShowItemBox(playerid, "Ayam Kemas", sprintf("Removed_%dx", amount), 2);
...
Dialog:BrankasSusuOlahan(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        if(!strcmp(option, "ambil", true))
        {
            ...
            SusuOlahan -= amount;
            ItemAdd(playerid, "Susu Olahan", amount);
        ...
        else if(!strcmp(option, "depo", true))
        {
            ...
            SusuOlahan += amount;
            ItemDelete(playerid, "Susu Olahan", amount);
            ShowItemBox(playerid, "Susu Olahan", sprintf("Removed_%dx", amount), 2);
...
Dialog:BrankasAirMineral(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        if(!strcmp(option, "ambil", true))
        {
            ...
            AirMineral -= amount;
            ItemAdd(playerid, "Air Mineral", amount);
        ...
        else if(!strcmp(option, "depo", true))
        {
            ...
            AirMineral += amount;
            ItemDelete(playerid, "Air Mineral", amount);
            ShowItemBox(playerid, "Air Mineral", sprintf("Removed_%dx", amount), 2);
...
Dialog:BrankasNasgor(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        if(!strcmp(option, "ambil", true))
        {
            ...
            NasiGoreng -= amount;
            ItemAdd(playerid, "Nasi Goreng", amount);
        ...
        else if(!strcmp(option, "depo", true))
        {
            ...
            NasiGoreng += amount;
            ItemDelete(playerid, "Nasi Goreng", amount);
            ShowItemBox(playerid, "Nasi Goreng", sprintf("Removed_%dx", amount), 2);
...
Dialog:BrankasBakso(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        else if(!strcmp(option, "depo", true))
        {
            ...
            Bakso += amount;
            ItemDelete(playerid, "Bakso", amount);
            ShowItemBox(playerid, "Bakso", sprintf("Removed_%dx", amount), 2);
...
Dialog:BrankasNasiPecel(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        if(!strcmp(option, "ambil", true))
        {
            ...
            NasiPecel -= amount;
            ItemAdd(playerid, "Nasi Pecel", amount);
        ...
        else if(!strcmp(option, "depo", true))
        {
            ...
            NasiPecel += amount;
            ItemDelete(playerid, "Nasi Pecel", amount);
            ShowItemBox(playerid, "Nasi Pecel", sprintf("Removed_%dx", amount), 2);
...
Dialog:BrankasBuburPedas(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        if(!strcmp(option, "ambil", true))
        {
            ...
            BuburPedas -= amount;
            ItemAdd(playerid, "Bubur Pedas", amount);
        ...
        else if(!strcmp(option, "depo", true))
        {
            ...
            BuburPedas += amount;
            ItemDelete(playerid, "Bubur Pedas", amount);
            ShowItemBox(playerid, "Bubur Pedas", sprintf("Removed_%dx", amount), 2);
...
Dialog:BrankasSusuFresh(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        if(!strcmp(option, "ambil", true))
        {
            ...
            SusuFresh -= amount;
            ItemAdd(playerid, "Susu Fresh", amount);
        ...
        else if(!strcmp(option, "depo", true))
        {
            ...
            SusuFresh += amount;
            ItemDelete(playerid, "Susu Fresh", amount);
            ShowItemBox(playerid, "Susu Fresh", sprintf("Removed_%dx", amount), 2);
...
Dialog:BrankasEsTeh(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        if(!strcmp(option, "ambil", true))
        {
            ...
            EsTeh -= amount;
            ItemAdd(playerid, "Es Teh", amount);
        ...
        else if(!strcmp(option, "depo", true))
        {
            ...
            EsTeh += amount;
            ItemDelete(playerid, "Es Teh", amount);
            ShowItemBox(playerid, "Es Teh", sprintf("Removed_%dx", amount), 2);
...
Dialog:BrankasKopiKenangan(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        if(!strcmp(option, "ambil", true))
        {
            ...
            KopiKenangan -= amount;
            ItemAdd(playerid, "Kopi Kenangan", amount);
        ...
        else if(!strcmp(option, "depo", true))
        {
            ...
            KopiKenangan += amount;
            ItemDelete(playerid, "Kopi Kenangan", amount);
            ShowItemBox(playerid, "Kopi Kenangan", sprintf("Removed_%dx", amount), 2);
...
Dialog:BrankasCochoMatcha(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        if(!strcmp(option, "ambil", true))
        {
            ...
            CochoMatcha -= amount;
            ItemAdd(playerid, "Choco Matcha", amount);
        ...
        else if(!strcmp(option, "depo", true))
        {
            ...
            CochoMatcha += amount;
            ItemDelete(playerid, "Choco Matcha", amount);
            ShowItemBox(playerid, "Chico Matcha", sprintf("Removed_%dx", amount), 2);
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    switch(dialogid)
    {
        ...
        {
            if(!response) return ShowTDN(playerid, NOTIFICATION_INFO, "Anda telah membatalkan pilihan");
            if(AccountData[playerid][pFaction] != FACTION_PEDAGANG) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan anggota Resto DCRP!");
        ...
        case DIALOG_PEDAGANG_GARAGE:
        {
            ...
            switch(listitem)
            {
                ...
                {   
                    ...
                    ShowPlayerDialog(playerid, DIALOG_PEDAGANG_GARAGE_TAKEOUT, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Garasi Pedagang", lstr, "Pilih", "Batal");
                }
                case 1:// Simpan Kendaraan
                {
                    ...
                    if(!foundnearby) return ShowTDN(playerid, NOTIFICATION_ERROR, "Tidak ada kendaraan dari Pedagang DCRP milik anda di sekitar!");
                }
                case 2:// Buy Kendaraan
                {
                    ShowPlayerDialog(playerid, DIALOG_PEDAGANG_GARAGE_BUY, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Beli Kendaraan", 
                    "Model\tHarga\
                    \nRancher Resto\t$6000\
                ...
                {
                    new frmtdel[158];
                    mysql_format(dcrp_query, frmtdel, sizeof(frmtdel), "SELECT * FROM `player_vehicles` WHERE `PVeh_Faction` = 6 AND `PVeh_OwnerID` = %d", AccountData[playerid][pID]);
                    ...
                    if(rows)
                    {
                        ...
                        ShowPlayerDialog(playerid, DIALOG_PEDAGANG_GARAGE_DELETE, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Hapus Kendaraan", list, "Hapus", "Batal");
                    }
                    else 
                    {
                        PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
                        return Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Hapus Kendaraan", "Anda tidak memiliki kendaraan Pedagang", "Tutup", "");
                    }
                }
            }
        }
        ...
        {
            if(!response) return ShowTDN(playerid, NOTIFICATION_INFO, "Anda telah membatalkan pilihan");
            if(AccountData[playerid][pFaction] != FACTION_PEDAGANG) return ShowTDN(playerid, NOTIFICATION_INFO, "Anda bukan anggota Pedagang DCRP");
            ...
            if(cache_num_rows())
            {
                ...
                if(listitem >= 0 && listitem < cache_num_rows())
                {
                    ...
                    mysql_format(dcrp_query, strgbg, sizeof(strgbg), "DELETE FROM `player_vehicles` WHERE `id` = %d", havpid);
        ...
        {
            if(!response) return ShowTDN(playerid, NOTIFICATION_INFO, "Anda telah membatalkan pilihan");
            if(AccountData[playerid][pFaction] != FACTION_PEDAGANG) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan anggota Pedagang DCRP!");
            if(AccountData[playerid][pFactionRank] < 5) return ShowTDN(playerid, NOTIFICATION_ERROR, "Minimal Rank Wakil CEO untuk akses Bos Desk!");

            if(isnull(inputtext)) return ShowPlayerDialog(playerid, DIALOG_RANK_SET_PEDAGANG, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Kelola Jabatan", \
            ...
            if(!IsNumeric(inputtext)) return ShowPlayerDialog(playerid, DIALOG_RANK_SET_PEDAGANG, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Kelola Jabatan", \
            ...
            if(strval(inputtext) < 1 || strval(inputtext) > AccountData[playerid][pFactionRank]) return ShowPlayerDialog(playerid, DIALOG_RANK_SET_PEDAGANG, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Kelola Jabatan", \
        ...
        case DIALOG_PEDKICKMEMBER:
        {
            ...
            if(cache_num_rows())
            {
                ...
                new strgbg[158];
                mysql_format(dcrp_query, strgbg, sizeof(strgbg), "DELETE FROM `player_vehicles` WHERE `PVeh_OwnerID`=%d AND `PVeh_Faction` = 6", pidrow);
                mysql_tquery(dcrp_query, strgbg);
                ...
                foreach(new i : Player)
                {
                    if(AccountData[i][IsLoggedIn] && AccountData[i][pSpawned] && pidrow == AccountData[i][pID])
                    {
                        ...
                        RefreshFactionMap(i);
                        ShowTDN(i, NOTIFICATION_WARNING, "Anda telah dikeluarkan dari faction Pedagang DCRP!");
                    }
                }
        ...
        case DIALOG_PEDSETRANK:
        {
            ...
            if(rows)
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_RANK_SET_PEDAGANG, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Kelola Jabatan", \
                "Silahkan pilih jabatan untuk ditetapkan (masukkan angka saja):\n\
                1. Magang\n\
                2. Junior\n\
                3. Senior\n\
