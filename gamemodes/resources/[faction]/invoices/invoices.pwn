...
GivePlayerInvoice(playerid, targetid)
{
    ...
    ShowPlayerDialog(playerid, DIALOG_INVOICE_NAME, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Invoice", "Mohon masukkan nama untuk tagihan ini:", "Input", "Batal");
    return 1;
}
...
{
    new shstr[200];
    mysql_format(dcrp_query, shstr, sizeof(shstr), "SELECT * FROM `invoices` WHERE `Owner`=%d ORDER BY Cost DESC", AccountData[targetid][pID]);
    new Cache:execute = mysql_query(dcrp_query, shstr);
    if(cache_num_rows() > 0)
    {
        ...
        ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Invoice Pending", affah, "Tutup", "");
    }
    else
        ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Invoice Pending", "Pemain tersebut tidak memiliki Tagihan.", "Tutup", "");
...
{
    new shstr[200];
    mysql_format(dcrp_query, shstr, sizeof(shstr), "SELECT * FROM `invoices` WHERE `Owner`=%d ORDER BY Cost DESC", AccountData[playerid][pID]);
    new Cache:execute = mysql_query(dcrp_query, shstr);
    ...
    if(rows)
    {
        ...
        ShowPlayerDialog(playerid, DIALOG_PAY_INVOICE, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Invoice Pending", affah, "Pilih", "Batal");
    }
    else
    {
        ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Invoice belum terbayar", 
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == DIALOG_INVOICE_NAME)
    {
        ...
        if(isnull(inputtext)) return ShowPlayerDialog(playerid, DIALOG_INVOICE_NAME, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Invoice", 
        "Error: Nama tidak boleh kosong!\nMohon masukkan nama untuk tagihan ini:", "Input", "Batal");

        if(IsNumeric(inputtext)) return ShowPlayerDialog(playerid, DIALOG_INVOICE_NAME, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Invoice", 
        "Error: Harus berformat nama, tidak boleh angka!\nMohon masukkan nama untuk tagihan ini:", "Input", "Batal");

        if(strlen(inputtext) < 1 || strlen(inputtext) > 128) return ShowPlayerDialog(playerid, DIALOG_INVOICE_NAME, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Invoice", 
        "Error: Nama tidak dapat kurang dari 1 huruf atau lebih dari 125!\nMohon masukkan nama untuk tagihan ini:", "Input", "Batal");

        if(!IsValidFormatText(inputtext)) return ShowPlayerDialog(playerid, DIALOG_INVOICE_NAME, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Invoice", 
        "Error: Tidak dapat menggunakan tanda kutip ' '\nMohon masukkan nama untuk tagihan ini:", "Input", "Batal");

        SetPVarString(playerid, "InvoiceName", inputtext);
        ShowPlayerDialog(playerid, DIALOG_INVOICE_COST, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Invoice", "Mohon masukkan nominal tagihan dikolom bawah ini:", "Input", "Batal");
    }
    else if(dialogid == DIALOG_INVOICE_COST)
    {
        ...
        if(isnull(inputtext)) return ShowPlayerDialog(playerid, DIALOG_INVOICE_COST, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Invoice", 
        "Error: Nominal tagihan tidak boleh kosong\nMohon masukkan nominal tagihan dikolom bawah ini:", "Input", "Batal");

        if(!IsNumeric(inputtext)) return ShowPlayerDialog(playerid, DIALOG_INVOICE_COST, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Invoice", 
        "Error: Hanya dapat diisi angka!\nMohon masukkan nominal tagihan dikolom bawah ini:", "Input", "Batal");

        if(strval(inputtext) < 1) return ShowPlayerDialog(playerid, DIALOG_INVOICE_COST, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Invoice", 
        ...
        mysql_format(dcrp_query, cQuery, sizeof(cQuery), "INSERT INTO `invoices` SET `Name`='%s', `Sender`=%d, `Faction`=%d, `Cost`=%d, `Owner`=%d", invoicename, AccountData[playerid][pID], AccountData[playerid][pFaction], cost, AccountData[targetid][pID]);
        mysql_tquery(dcrp_query, cQuery, "OnInvoiceCreated", "dd", playerid, targetid);
    }
    else if(dialogid == DIALOG_PAY_INVOICE)
    {
        ...
        new cQuery[200];
        mysql_format(dcrp_query, cQuery, sizeof(cQuery), "SELECT * FROM `invoices` WHERE `Owner`=%d ORDER BY Cost DESC", AccountData[playerid][pID]);
        new Cache:execute = mysql_query(dcrp_query, cQuery);
        if(cache_num_rows() > 0)
        {
            ...
            if(listitem >= 0 && listitem < cache_num_rows())
            {
                ...
                if(factid == 1) // Polisi Faction ID
                {
                    ...
                    mysql_format(dcrp_query, rasberry, sizeof(rasberry), "UPDATE `player_characters` SET `Char_BankMoney` = `Char_BankMoney` + %d WHERE `pID` = %d", (cost/100)*60, sender);
                    mysql_tquery(dcrp_query, rasberry);
                    ...
                    mysql_format(dcrp_query, sha, sizeof(sha), "DELETE FROM `invoices` WHERE `invoiceID`=%d", invid);
                    mysql_tquery(dcrp_query, sha);

                    mysql_format(dcrp_query, icsr, sizeof(icsr), "UPDATE `stuffs` SET `policemoneyvault`=%d WHERE `ID`=0", PoliceMoneyVault);
                ...
                else if(factid == 2) // Pemerintah Faction ID
                {
                    ...
                    mysql_format(dcrp_query, rasberry, sizeof(rasberry), "UPDATE `player_characters` SET `Char_BankMoney` = `Char_BankMoney` + %d WHERE `pID` = %d", (cost/100)*30, sender);
                    mysql_tquery(dcrp_query, rasberry);
                    ...
                    mysql_format(dcrp_query, icsr, sizeof(icsr), "UPDATE `stuffs` SET `pemerintahmoneyvault`=%d WHERE `ID`=0", PemerintahMoneyVault);
                    mysql_tquery(dcrp_query, icsr);
                }
                else if(factid == 3) // EMS Faction ID
                {
                    ...
                    mysql_format(dcrp_query, rasberry, sizeof(rasberry), "UPDATE `player_characters` SET `Char_BankMoney` = `Char_BankMoney` + %d WHERE `pID` = %d", (cost/100)*20, sender);
                    mysql_tquery(dcrp_query, rasberry);
                    ...
                    mysql_format(dcrp_query, icsr, sizeof(icsr), "UPDATE `stuffs` SET `emsmoneyvault`=%d WHERE `ID`=0", EMSMoneyVault);
                    mysql_tquery(dcrp_query, icsr);
                }
                else if(factid == 5) // Bengkel Faction ID
                {
                    ...
                    mysql_format(dcrp_query, icsr, sizeof(icsr), "UPDATE `stuffs` SET `bengkelmoneyvault`=%d WHERE `ID`=0", BengkelMoneyVault);
                    mysql_tquery(dcrp_query, icsr);
                }
                else if(factid == 6) // Pedagang Faction ID
                {
                    ...
                    mysql_format(dcrp_query, icsr, sizeof(icsr), "UPDATE `stuffs` SET `restomoneyvault`=%d WHERE `ID`=0", RestoMoneyVault);
                    mysql_tquery(dcrp_query, icsr);
                }
            }
        }
