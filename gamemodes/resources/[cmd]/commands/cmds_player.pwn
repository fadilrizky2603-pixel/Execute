...
CMD:dokterlokal(playerid, params[])
{
	...
	PlayerTextDrawSetString(playerid, ProgressBar[playerid][1], "DOKTER LOKAL");
...
CMD:settings(playerid, params[])
{
	...
	Dialog_Show(playerid, DialogToggle, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Settings", list, "Pilih", "Batal");
	return 1;
}
...
{
	new query[255];
	mysql_format(dcrp_query, query, sizeof(query), "SELECT * FROM `warninglogs` WHERE `pID`=%d", AccountData[playerid][pID]);
	...
	if(rowscount)
	{
		...
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Riwayat Peringatan", list, "Tutup", "");
	}
	else 
	{
		new list[255];
...
CMD:delay(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Waktu Delay", shstr, "Tutup", "");
	return 1;
}
...
CMD:sm(playerid, params[])
{
	...
	\n"GRAY"Ladang (Uranium/Kanabis)/Crafting/Cuci Uang\t"GRAY"4 Polisi & 2 EMS\t%s\
	\nCarsteal\t4 Polisi & 2 EMS\t%s\
	\n"GRAY"Rampok Warung\t"GRAY"4 Polisi & 2 EMS\t%s\
	\nPeperangan\t15 Polisi & 6 EMS\t%s", status1, status2, status3, status4, status5);
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Status Mendung", list, "Tutup", "");
	return 1;
}
...
CMD:evipname(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_VIP_NAME, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Vip Name Custom",
...
CMD:mprice(playerid, params[])
{
	...
	format(frmtprice, sizeof(frmtprice), ""YELLOW"========== [Dinas Pekerjaan Kota DCRP] ==========\
	...
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Harga Penjualan", frmtprice, "Tutup", "");
	return 1;
}
...
Dialog:VehicleAudioPlayer(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		if(strlen(inputtext) < 1 || strlen(inputtext) > 128)
			return 	Dialog_Show(playerid, VehicleAudioPlayer, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Vehicle Music", "ERROR, panjang link tidak boleh di bawah 1 maximal 128 karakter!\n\nSpotify - Cerahi Hidupmu Secerah Matahari Di Pagi Hari\n\n"GRAY"Kami sarankan anda untuk upload file mp3 ke discord terlebih dahulu.\n"RED_E"Note: Fitur ini tidak support link Youtube secara langsung!\nMasukan off untuk mematikan lagu!\n\n"YELLOW"(Apabila file mp3 telah di upload ke discord, silahkan copy linknya dan paste dibawah ini):", "Input", "Batal");
...
Dialog:DialogToggle(playerid, response, listitem, inputtext[])
{
	...
	if(response)
	{
		switch(listitem)
		{
			case 3: // Dynamic Render
			{
				ShowPlayerDialog(playerid, DIALOG_STREAMER_CONFIG, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Setting Render",
			...
			case 4:// Auto Engine
			{
				switch(AccountData[playerid][pTogAutoEngine])
				{
					case false:
					{
						...
						mysql_tquery(dcrp_query, sprintf("UPDATE player_characters SET Char_TogAutoEngine = '%d' WHERE pID = '%d'", AccountData[playerid][pTogAutoEngine], GetUCPSQLID(playerid)));
			...
			case 5:// Toggle Private Message
			{
				switch(AccountData[playerid][pTogPM])
				{
					case false:// Nyalakan
					{
						...
						mysql_tquery(dcrp_query, sprintf("UPDATE player_characters SET Char_TogPM = '%d' WHERE pID = '%d'", AccountData[playerid][pTogPM], GetUCPSQLID(playerid)));
					}
					case true://
					{
						AccountData[playerid][pTogPM] = AccountData[playerid][pTogPM] ? (false) : (true);
			...
			case 6:// Toggle Global Message
			{
				switch(AccountData[playerid][pTogGlobal])
				{
					case false:// Nyalakan
					{
						...
						mysql_tquery(dcrp_query, sprintf("UPDATE player_characters SET Char_TogPM = '%d' WHERE pID = '%d'", AccountData[playerid][pTogGlobal], GetUCPSQLID(playerid)));
					}
					case true://
					{
						AccountData[playerid][pTogGlobal] = AccountData[playerid][pTogGlobal] ? (false) : (true);
			...
			case 7:// Toggle Notif Style
			{
				switch(AccountData[playerid][pStyleNotif])
				{
					case 1:// Nyalakan
					{
						...
						mysql_tquery(dcrp_query, sprintf("UPDATE player_characters SET Char_NotifStyle = '%d' WHERE pID = '%d'", AccountData[playerid][pStyleNotif], GetUCPSQLID(playerid)));
					}
					case 2://
					{
...
CMD:accept(playerid, params[])
{
	...
	/*if(!strcmp(option, "buyveh", true))
	{
		...
		if((vehicle_index = Vehicle_ReturnID(vehicleid)) != RETURN_INVALID_VEHICLE_ID)
		{
			...
			new cQuery[598];
			mysql_format(dcrp_query, cQuery, sizeof(cQuery), "UPDATE `player_vehicles` SET `PVeh_OwnerID`=%d WHERE `id`=%d", PlayerVehicle[vehicle_index][pVehOwnerID], PlayerVehicle[vehicle_index][pVehID]);
...
CMD:elist(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DialogElist, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- List Animasi", "Elist\nEprop Anim\nInteraksi Anim", "Pilih", "Batal");
	return 1;
}
...
CMD:flist(playerid, params[])
{
	...
	\nKepolisian Daerah DCRP\t%d\
	\n"GRAY"EMS Kota DCRP\t"GRAY"%d\
	\nBengkel Kota DCRP\t%d\
	\n"GRAY"Transportasi Kota DCRP\t"GRAY"%d\
	\nPedagang Kota DCRP\t%d\
	\n"GRAY"Pemerintah Daerah Kota DCRP\t"GRAY"%d",
	countpd, countems, countbengkel, counttrans, countpdg, countpemer);
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- On Duty List", shstr, "Tutup", "");
	return 1;
}
...
CMD:fixme(playerid, params[])
{
	...
	Dialog_Show(playerid, DialogFixMe, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Fix Me", "Keluhan\tKeterangan\
...
CMD:help(playerid, params[])
{
	...
	Dialog_Show(playerid, DialogHelp, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Bantuan", "Jenis Bantuan\tKeterangan\
	...
	\n"GRAY"Petunjuk Dasar\tBerisi beberapa petunjuk dasar DCRP\
...
Dialog:DialogHelp(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		switch(listitem)
		{
			case 0:
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Perintah Dasar",
			...
			case 1:
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE" - Perintah Kendaraan",
				"Jenis Perintah\tKeterangan\
				\n/sellveh\tMenjual kendaraan kepada pemain lain\
				\n"GRAY"Buka radial kendraan\t"GRAY"Menyalakan/Mematikan mesin kendaraan secara manual\
				\nBuka smartphone\tMelihat daftar kendaraan milik anda\
				\n"GRAY"/windows\t"GRAY"Membuka jendela kendaraan\
				\n/sl\tMengatur Limit speed kendaraan", "Close", "");
			}
			case 2:
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE" - Perintah Roleplay",
				"Jenis Perintah\tKeterangan\
			...
			case 3:
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE" - Petunjuk Dasar",
				"Jenis Perintah\tKeterangan\
			...
			case 4:
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE" - Petunjuk Lapar Haus",
				""WHITE"Dikarenakan "DCRP"DCRP"WHITE" merupakan server dengan mode Roleplay, sudah pasti karakter anda\
...
CMD:saving(playerid, params[])
{
	...
	ShowTDN(playerid, NOTIFICATION_INFO, "Successfully updates your account data to "DCRP"DCRP Database");
	return 1;
}
...
{
	SelectTextDraw(playerid, Y_DCRP);
	return 1;
}
...
/*CMD:pay(playerid, params[])
{
	...
	// SendEmbedMessage("DCRP", dc, "Security RTM Money", "https://cdn.discordapp.com/attachments/1011319839366856806/1110744778250919976/20230426_210737-1_2.png", "https://cdn.discordapp.com/attachments/1124221192778285076/1124227665201725450/1.png", "1109630050174779472", 0xeeca0d);
	return 1;
}*/
...
CMD:gum(playerid, params[])
{
	...
	new tss[255];
	mysql_format(dcrp_query, tss, sizeof(tss), "UPDATE `player_characters` SET `Char_RedMoney` = `Char_RedMoney` + %d WHERE `pID`=%d", value, AccountData[otherid][pID]);
	mysql_tquery(dcrp_query, tss);
	
	mysql_format(dcrp_query, tss, sizeof(tss), "UPDATE `player_characters` SET `Char_RedMoney` = `Char_RedMoney` - %d WHERE `pID`=%d", value, AccountData[playerid][pID]);
	mysql_tquery(dcrp_query, tss);
	return 1;
}
...
CMD:rpgun(playerid, params[])
{
	...
	SendRPMeAboveHead(playerid, "Mengeluarkan senjata dan siap untuk menembak kapan saja", X11_PLUM1);
	return 1;
}