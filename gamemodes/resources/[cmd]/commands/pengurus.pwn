...
CMD:resetweapoff(playerid, params[])
{
	...
	new string[178];
	mysql_format(dcrp_query, string, sizeof(string), "SELECT * FROM `player_characters` WHERE `Char_Name`='%s'", playerName);
	mysql_tquery(dcrp_query, string, "OffResetWeapon", "ds", playerid, playerName);
	return 1;
}
...
public OffResetWeapon(playerid, name[])
{
	...
	else
	{
		...
		for(new i = 0; i < 13; i++) {
			mysql_format(dcrp_query, string, sizeof(string), "UPDATE `player_characters` SET `Gun%d`=0, `Ammo%d`=0 WHERE `pID`=%d", i+1, i+1, RegID);
...
CMD:setfaction(playerid, params[])
{
	...
	if(fid == FACTION_NONE)
	{
		...
		mysql_format(dcrp_query, tss, sizeof(tss), "UPDATE `player_characters` SET `Char_Faction`=0, `Char_FactionRank`=0 WHERE `pID`=%d", AccountData[otherid][pID]);
	...
	else 
	{
		...
		mysql_format(dcrp_query, tss, sizeof(tss), "UPDATE `player_characters` SET `Char_Faction`=%d, `Char_FactionRank`=%d WHERE `pID`=%d", fid, rank, AccountData[otherid][pID]);
		mysql_tquery(dcrp_query, tss);
	}
	static tmp[125];
...
CMD:updatedb(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" Telah membackup semua data pemain online ke "PINK1"DCRP Database", AccountData[playerid][pAdminname]);
	return 1;
}
...
CMD:achangename(playerid, params[])
{
	...
	new query[255];
	mysql_format(dcrp_query, query, sizeof(query), "SELECT `Char_Name` FROM `player_characters` WHERE `Char_Name`='%s'", frmtname);
	mysql_tquery(dcrp_query, query, "ChangedName", "dds", otherid, playerid, frmtname);
	return 1;
}
