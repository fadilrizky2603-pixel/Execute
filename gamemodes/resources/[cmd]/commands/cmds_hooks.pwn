...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == DIALOG_BAGASI)
    {
        ...
        switch(listitem)
        {
            case 0: //deposit
            {
                ...
                execute = mysql_query(dcrp_query, tss, true);
    ...
    else if(dialogid == DIALOG_BAGASI_IN)
    {
        ...
        SwitchVehicleBoot(PlayerVehicle[vehid][pVehPhysic], false);
        ItemDelete(playerid, VehicleBagasi[playerid][vehiclebagasiTemp], quantity);
        ShowItemBox(playerid, VehicleBagasi[playerid][vehiclebagasiTemp], sprintf("Removed_%dx", quantity), 2);
        
        AddCapacityVehicle(vehid, VehicleBagasi[playerid][vehiclebagasiTemp], quantity);
        new invstr[1028], Cache:execute;
        mysql_format(dcrp_query, shstr, sizeof(shstr), "SELECT * FROM `vehicle_bagasi` WHERE `vID`=%d AND `Item`='%s'", PlayerVehicle[vehid][pVehID], VehicleBagasi[playerid][vehiclebagasiTemp]);
        execute = mysql_query(dcrp_query, shstr, true);
        new rows = cache_num_rows();
        if(rows > 0)
        {
            mysql_format(dcrp_query, invstr, sizeof(invstr), "UPDATE `vehicle_bagasi` SET `Quantity` = `Quantity` + %d WHERE `vID` = %d AND `Item` = '%s'", quantity, PlayerVehicle[vehid][pVehID], VehicleBagasi[playerid][vehiclebagasiTemp]);
            mysql_tquery(dcrp_query, invstr, "OnBagasiDeposit", "i", playerid);
        }
        else 
        {
            mysql_format(dcrp_query, invstr, sizeof(invstr), "INSERT INTO `vehicle_bagasi` SET `vID`=%d, `Item`='%s', `Model`=%d, `Quantity`=%d", PlayerVehicle[vehid][pVehID], VehicleBagasi[playerid][vehiclebagasiTemp], VehicleBagasi[playerid][vehiclebagasiModel], quantity);
            mysql_tquery(dcrp_query, invstr, "OnBagasiDeposit", "i", playerid);
        }
    ...
    else if(dialogid == DIALOG_BAGASI_WITHDRAW)
    {
        ...
        new tss[100];
        mysql_format(dcrp_query, tss, sizeof(tss), "SELECT * FROM `vehicle_bagasi` WHERE `vID`=%d", PlayerVehicle[vehid][pVehID]);
    ...
    else if(dialogid == DIALOG_BAGASI_OUT)
    {
        ...
        {
            mysql_format(dcrp_query, jts, sizeof(jts), "UPDATE `vehicle_bagasi` SET `Quantity`=%d WHERE `ID`=%d", VehicleBagasi[playerid][vehiclebagasiQuant], VehicleBagasi[playerid][vehiclebagasiID]);
            mysql_tquery(dcrp_query, jts);
        }
        else 
        {
            mysql_format(dcrp_query, jts, sizeof(jts), "DELETE FROM `vehicle_bagasi` WHERE `ID`=%d", VehicleBagasi[playerid][vehiclebagasiID]);
            mysql_tquery(dcrp_query, jts);
        }
        SwitchVehicleBoot(PlayerVehicle[vehid][pVehPhysic], false);
        ItemAdd(playerid, VehicleBagasi[playerid][vehiclebagasiTemp], quantity);
        ShowItemBox(playerid, VehicleBagasi[playerid][vehiclebagasiTemp], sprintf("Received_%dx", quantity), 2);
            ...
            case 1: //kunci
            {
                foreach(new iterpv : PvtVehicles)
                {
                    if(PlayerVehicle[iterpv][pVehExists])
                    {
                        if(PlayerVehicle[iterpv][pVehPhysic] == vehid)
                        {
                            ...
                            ShowPlayerFooter(playerid, sprintf("~w~%s %s", GetVehicleName(PlayerVehicle[iterpv][pVehPhysic]), PlayerVehicle[iterpv][pVehLocked] ? ("~r~Terkunci") : ("~g~Terbuka")), 4000);
                            return 1;
                        }
                    }
                }
                ...
                if(AccountData[playerid][pJobVehicle] != 0)
                {
                    if(vehid == JobVehicle[AccountData[playerid][pJobVehicle]][Vehicle])
                    {
                        ...
                        ShowPlayerFooter(playerid, sprintf("~w~%s %s", GetVehicleName(JobVehicle[AccountData[playerid][pJobVehicle]][Vehicle]), JobVehicle[AccountData[playerid][pJobVehicle]][Locked] ? ("~r~Terkunci") : ("~g~Terbuka")), 4000, 4);
                    }
                    return 1;
                }
                ...
                if(PlayerElectricJob[playerid][ElectricVehicle] == vehid)
                {
                    ...
                    ShowPlayerFooter(playerid, sprintf("~w~%s %s", GetVehicleName(PlayerElectricJob[playerid][ElectricVehicle]), PlayerElectricJob[playerid][ElectricLocked] ? ("~r~Terkunci") : ("~g~Terbuka")), 4000, 4);
                    return 1;
                }
            }
            case 2: //lampu
    ...
    else if(dialogid == DIALOG_CHANGE_PASSWORD)
    {
        ...
        if(isnull(inputtext)) return ShowPlayerDialog(playerid, DIALOG_CHANGE_PASSWORD, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Ubah Password", 
        "Error: Tidak dapat diisi kosong!\nMohon masukkan kata sandi baru yang ingin anda terapkan:", "Change", "Cancel");

        if(IsNumeric(inputtext)) return ShowPlayerDialog(playerid, DIALOG_CHANGE_PASSWORD, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Ubah Password", 
        "Error: Tidak dapat diisi angka saja!\nMohon masukkan kata sandi baru yang ingin anda terapkan:", "Change", "Cancel");

        if(strlen(inputtext) < 7 || strlen(inputtext) > 24) return ShowPlayerDialog(playerid, DIALOG_CHANGE_PASSWORD, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Ubah Password", 
        ...
        new query[255];
        mysql_format(dcrp_query, query, sizeof(query), "UPDATE `playerucp` SET `password`='%s', `salt`='%e' WHERE `ucp`='%s'", AccountData[playerid][pPassword], AccountData[playerid][pSalt], AccountData[playerid][pUCP]);
...
AddCapacityVehicle(vehicleid, const item[], quantity)
{
    ...
    {
        PlayerVehicle[vehicleid][pVehCapacity] += quantity*0.040;
    }
    ...
    {
        PlayerVehicle[vehicleid][pVehCapacity] += quantity*0.100;
    }
    ...
    {
        PlayerVehicle[vehicleid][pVehCapacity] += quantity*0.45;
    }
    ...
    {
        PlayerVehicle[vehicleid][pVehCapacity] += quantity*0.500;
    }
    ...
    new sctr[596];
    mysql_format(dcrp_query, sctr, sizeof(sctr), "UPDATE `player_vehicles` SET `PVeh_Capacity`=%f WHERE `id`=%d", PlayerVehicle[vehicleid][pVehCapacity], PlayerVehicle[vehicleid][pVehID]);
