...
CMD:fhelp(playerid, params[])
{
	...
	switch(AccountData[playerid][pFaction])
	{
		case 1: // Polisi
		{
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Polisi Command", 
			"Perintah\tKeterangan\n\
		...
		case 2: // Pemerintah
		{
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Pemerintah Command",
			"Perintah\tKeterangan\n\
		...
		case 3: //ems
		{
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- EMS Command",
			"Perintah\tKeterangan\n\
		...
		case 5: // Bengkel
		{
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Bengkel Command",
			"Perintah\tKeterangan\n\
...
CMD:taser(playerid, params[])
{
	...
	if(!PlayerTaserOn[playerid])
	{
		...
		ShowPlayerFooter(playerid, "~w~TASER~n~~g~ON", 2500);
	...
	{
		SetWeapons(playerid);
		ShowPlayerFooter(playerid, "~w~TASER~n~~r~OFF", 2500);
...
CMD:makekta(playerid, params[])
{
	...
	ItemAdd(otherid, "KTA");
	ShowItemBox(otherid, "KTA", "Received_1x", 2);
...
// 	Dialog_Show(playerid, DialogTrace, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Tracking", ""WHITE"Lacak Nomor Telepon\n"GRAY"Lacak Plate Kendaraan\n"WHITE"Lacak Akun Twitter", "Pilih", "Batal");
// 	return 1;
// }
...
Dialog:DialogTrace(playerid, response, listitem, inputtext[])
{
	...
	switch(listitem)
	{
		case 0:// nomor hp
		{
			Dialog_Show(playerid, TrackNoPhone, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Track Telepon", "Mohon masukkan nomor telepon valid\nContoh: 55261, 02521\n"YELLOW"(Mohon masukkan dikolom bawah sini):", "Track", "Batal");
		}
		case 1:// Plate Nomor
		{
			Dialog_Show(playerid, TrackPlateNomor, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Track Plate", "Mohon masukkan nomor plate kendaraan valid\nContoh: VR 5233 BOS, VR 2122 DAN\n"YELLOW"(Mohon masukkan dikolom bawah sini):", "Track", "Batal");
		}
		case 2:// Akun Twitter
		{
			Dialog_Show(playerid, TrackAkunTwitter, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Track Sosial Media", "Mohon memasukkan username akun media Twitter yang ingin dilacak\n"YELLOW"(Masukkan dibawah sini dengan Username yang Valid):", "Track", "Batal");
		}
	}
	return 1;
}
Dialog:TrackAkunTwitter(playerid, response, listitem, inputtext[])
{
	...
	if(isnull(inputtext))
		return Dialog_Show(playerid, TrackAkunTwitter, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Track Sosial Media", "ERROR: Tidak dapat Kosong!\nMohon memasukkan username akun media Twitter yang ingin dilacak\n"YELLOW"(Masukkan dibawah sini dengan Username yang Valid):", "Track", "Batal");
...
Dialog:TrackNoPhone(playerid, response, listitem, inputtext[])
{
	if(!response) return 1;
	if(isnull(inputtext))
		return Dialog_Show(playerid, TrackNoPhone, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Track Telepon", "Mohon masukkan nomor telepon valid\nContoh: 55261, 02521\n"YELLOW"(Mohon masukkan dikolom bawah sini):", "Track", "Batal");
...
Dialog:TrackPlateNomor(playerid, response, listitem, inputtext[])
{
	...
	if(isnull(inputtext) || IsNumeric(inputtext))
		return Dialog_Show(playerid, TrackPlateNomor, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Track Plate", "Mohon masukkan nomor plate kendaraan valid\nContoh: VR 5233 BOS, VR 2122 DAN\n"YELLOW"(Mohon masukkan dikolom bawah sini):", "Track", "Batal");
...
CMD:makesks(playerid, params[])
{
	if(AccountData[playerid][pFaction] != FACTION_EMS)
		return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan bagian dari Mitra EMS DCRP!");
...
/*CMD:takeradio(playerid, params[])
{
	...
	new query[255];
	mysql_format(dcrp_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_Radio`=0 WHERE `pID`=%d", AccountData[otherid][pID]);
...
{
	new minsty[512];
	if(AccountData[playerid][pFaction] != FACTION_EMS) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda bukan bagian dari EMS Kota DCRP!");
	new string[128];
