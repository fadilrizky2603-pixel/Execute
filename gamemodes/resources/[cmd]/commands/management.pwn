...
CMD:rall(playerid, params[])
{
	...
	mysql_tquery(dcrp_query, "UPDATE `player_characters` SET `Char_AdminPoint`=0");
...
CMD:oresetharta(playerid, params[])
{
	...
	new cQuery[200];
	mysql_format(dcrp_query, cQuery, sizeof(cQuery), "SELECT `pID` FROM `player_characters` WHERE `Char_Name`='%s'", playerName);
	mysql_tquery(dcrp_query, cQuery, "ResetHartaOffline", "ds", playerid, playerName);
	return 1;
}
...
public ResetHartaOffline(playerid, name[])
{
	...
	else
	{
		...
		mysql_tquery(dcrp_query, sprintf("DELETE FROM `inventory` WHERE `ID`=%d", dbid));
		mysql_tquery(dcrp_query, sprintf("DELETE FROM `player_gudang` WHERE `ID`=%d", dbid));
		mysql_tquery(dcrp_query, sprintf("DELETE FROM `player_housestorage` WHERE `ID`=%d", dbid));
		mysql_tquery(dcrp_query, sprintf("DELETE FROM `player_rusunstorage` WHERE `ID`=%d", dbid));
		mysql_tquery(dcrp_query, sprintf("DELETE FROM `player_vehicles` WHERE `PVeh_OwnerID`=%d", dbid));
		mysql_tquery(dcrp_query, sprintf("UPDATE `player_characters` SET `Char_Money`=0, `Char_BankMoney`=0, `Char_RedMoney`=0 WHERE `pID`=%d", dbid));
...
Dialog:ResetHarta_Conf(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		TakeMoney(otherid, AccountData[otherid][pMoney]);
		AccountData[otherid][pBankMoney] -= AccountData[otherid][pBankMoney];
		mysql_tquery(dcrp_query, sprintf("DELETE FROM `player_gudang` WHERE `ID`=%d", AccountData[otherid][pID]));
		mysql_tquery(dcrp_query, sprintf("DELETE FROM `player_housestorage` WHERE `ID`=%d", AccountData[otherid][pID]));
		mysql_tquery(dcrp_query, sprintf("DELETE FROM `player_rusunstorage` WHERE `ID`=%d", AccountData[otherid][pID]));
		mysql_tquery(dcrp_query, sprintf("DELETE FROM `player_vehicles` WHERE `PVeh_OwnerID`=%d", AccountData[otherid][pID]));
...
CMD:aresign(playerid, params[])
{
	...
	mysql_format(dcrp_query, cQuery, sizeof(cQuery), "SELECT * FROM `player_characters` WHERE `Char_UCP`='%s'", ucpname);
	mysql_tquery(dcrp_query, cQuery, "ResignedStaff", "ds", playerid, ucpname);
	return 1;
}
...
public ResignedStaff(playerid, ucp[])
{
	...
	else
	{
		...
		new cQuery[200];
		mysql_format(dcrp_query, cQuery, sizeof(cQuery), "UPDATE `player_characters` SET `Char_Admin`=0, `Char_AdminName`='%s' WHERE `Char_UCP`='%s'", ucp, ucp);
...
task ScheduleMTExecute[1000]()
{
	...
	{
		new crr = gettime();
		if(g_ScheduleTime != 0 && g_ScheduleTime < crr)
		{
			...
			SaveAll();
			SendClientMessageToAllEx(X11_RED, "[AdmCmd]"ARWIN1" Server Telah membackup semua data pemain dan kendaraan ke "PINK1"DCRP Database");
...
CMD:deletechar(playerid, params[])
{
	...
	new query[200];
	mysql_format(dcrp_query, query, sizeof(query), "SELECT `Char_Name` FROM `player_characters` WHERE `Char_UCP`='%s'", ucpname);
	mysql_tquery(dcrp_query, query, "DeleteCharQuery", "ds", playerid, ucpname);
...
public DeleteCharQuery(playerid, UCP[])
{
	...
	else
	{
		...
		Dialog_Show(playerid, DeleteCharList, DIALOG_STYLE_LIST, sprintf(""PINK1"DCRP"WHITE"- Char %s", UCP), sha, "Delete", "Cancel");
	}
	return 1;
}
...
Dialog:DeleteCharList(playerid, response, listitem, inputtext[])
{
	...
	mysql_format(dcrp_query, frmtdel, sizeof(frmtdel), "SELECT * FROM `player_characters` WHERE `Char_UCP`='%s'", ucpname);
	...
	if(cache_num_rows())
	{
		...
		if(listitem >= 0 && listitem < cache_num_rows())
		{
			...
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Delete Char", kckstr, "Tutup", "");

			mysql_tquery(dcrp_query, sprintf("DELETE FROM `player_characters` WHERE `pID`=%d", playerID));
			mysql_tquery(dcrp_query, sprintf("DELETE FROM `player_vehicles` WHERE `PVeh_OwnerID`=%d", playerID));
			mysql_tquery(dcrp_query, sprintf("DELETE FROM `inventory` WHERE `ID`=%d", playerID));
			mysql_tquery(dcrp_query, sprintf("DELETE FROM `contacts` WHERE `ID`=%d", playerID));
			mysql_tquery(dcrp_query, sprintf("DELETE FROM `player_gudang` WHERE `ID`=%d", playerID));
			mysql_tquery(dcrp_query, sprintf("DELETE FROM `player_housestorage` WHERE `ID`=%d", playerID));
			mysql_tquery(dcrp_query, sprintf("DELETE FROM `player_rusunstorage` WHERE `ID`=%d", playerID));
