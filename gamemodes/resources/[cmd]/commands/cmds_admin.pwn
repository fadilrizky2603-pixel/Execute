...
CMD:ahealth(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_HEALTH, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE" - Kesehatan", string, "Tutup", "");
    return 1;
}
...
CMD:checkdiscorducp(playerid, params[])
{
	...
	mysql_format(dcrp_query, cQuery, sizeof(cQuery), "SELECT `ucp` FROM `playerucp` WHERE `DiscordID`='%s'", DCID);
	execute = mysql_query(dcrp_query, cQuery, true);
	if(cache_num_rows())
	{
		new playerUCP[24];
...
CMD:aweap(playerid, params[])
{
	...
	if(!found)
    {
        ...
        return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Kepemilikan Senjata",
        "Pemain tersebut tidak memiliki senjata apapun!", "Tutup", "");
    }

	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST, sprintf(""PINK1"DCRP"WHITE"- %s", ReturnName(otherid)),
	sha, "Tutup", "");
	return 1;
}
...
CMD:setthestars(playerid, params[])
{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" memberikan status "PINK1"The Stars"ARWIN1" kepada "YELLOW"%s(%d)"ARWIN1" selama 30 hari", AccountData[playerid][pAdminname], AccountData[otherid][pName], otherid);
	SendCustomMessage(otherid, "THE STARS", "%s telah menjadikan anda The Stars selama 30 hari!", AccountData[playerid][pAdminname]);

	mysql_format(dcrp_query, cQuery, sizeof(cQuery), "UPDATE `player_characters` SET `Char_TheStars`=%d, `Char_TheStarsTime`=%d WHERE `pID`=%d", AccountData[otherid][pTheStars], AccountData[otherid][pTheStarsTime], AccountData[otherid][pID]);
...
CMD:osetstars(playerid, params[])
{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah memberikan status the stars (offline) selama 30 Hari"ARWIN1" kepada account "YELLOW"%s", AccountData[playerid][pAdminname], playerName);

	new duration = gettime() + (30 * 86400); // 30 Hari
	mysql_format(dcrp_query, cQuery, sizeof(cQuery), "UPDATE `player_characters` SET `Char_TheStars`=1, `Char_TheStarsTime`=%d WHERE `Char_Name`='%s'", duration, playerName);
	mysql_tquery(dcrp_query, cQuery);
	return 1;
}
...
CMD:removestars(playerid, params[])
{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah mencabut status "PINK1"The Stars"ARWIN1" pemain "YELLOW"%s(%d)", AccountData[playerid][pAdminname], AccountData[otherid][pName], otherid);
...
CMD:makewargaoff(playerid, params[]) 
{
	...
	new query[255];
	mysql_format(dcrp_query, query, sizeof(query), "SELECT `pID` FROM `player_characters` WHERE `Char_Name`='%e'", playerName);
	mysql_tquery(dcrp_query, query, "MakeWargaOffline", "ds", playerid, playerName);
	return 1;
}
...
CMD:resetdelay(playerid, params[])
{
	...
	if(!strcmp(type, "mower", true))
	{
		...
		SendClientMessageEx(playerid, X11_ARWIN, "[AdmCmd] Anda telah mereset delay sidejob mowing "YELLOW"%s(%d).", AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah mereset delay sidejob mowing anda.", GetAdminName(playerid));

		mysql_format(dcrp_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_MowerTime`=0 WHERE `pID`=%d", AccountData[otherid][pID]);
		mysql_tquery(dcrp_query, icsr);
	}
	else if(!strcmp(type, "delivery", true))
	{
		AccountData[otherid][pDeliveryTime] = 0;
		SendClientMessageEx(playerid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah mereset delay sidejob delivery "YELLOW"%s(%d).", GetAdminName(playerid), AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah mereset delay sidejob delivery anda.", GetAdminName(playerid));

		mysql_format(dcrp_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_DeliveryDelay`=0 WHERE `pID`=%d", AccountData[otherid][pID]);
		mysql_tquery(dcrp_query, icsr);
	}
	else if(!strcmp(type, "sweeper", true))
	{
		AccountData[otherid][pSweeperTime] = 0;
		SendClientMessageEx(playerid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah mereset delay sidejob sweeper "YELLOW"%s(%d).", GetAdminName(playerid), AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah mereset delay sidejob sweeper anda.", GetAdminName(playerid));

		mysql_format(dcrp_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_SweeperDelay`=0 WHERE `pID`=%d", AccountData[otherid][pID]);
		mysql_tquery(dcrp_query, icsr);
	}
	else if(!strcmp(type, "forklift", true))
	{
		AccountData[otherid][pForkliftTime] = 0;
		SendClientMessageEx(playerid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah mereset delay sidejob forklift "YELLOW"%s(%d).", GetAdminName(playerid), AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah mereset delay sidejob forklift anda.", GetAdminName(playerid));

		mysql_format(dcrp_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_ForkliftDelay`=0 WHERE `pID`=%d", AccountData[otherid][pID]);
		mysql_tquery(dcrp_query, icsr);
	}
...
CMD:fightstyle(playerid, params[])
{
	...
	{
		Dialog_Show(playerid, DIALOG_SELECT_FSTYLE, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Fight Style",
		"Fight Normal\
		\n"GRAY"Fight Boxing\
		\nFight Kungfu\
		\n"GRAY"Fight KneeHead\
...
CMD:onlinelist(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- List Online", string, "Tutup", "");
	return 1;
}
...
CMD:listafk(playerid, params[])
{
	...
	if(!find)
		Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Player AFK", "Tidak ada pemain yang sedang AFK", "Tutup", "");
	else 
		Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Player AFK", string, "Tutup", "");
	return 1;		
}
...
CMD:customnumber(playerid, params[])
{
	...
	new query[598];
	mysql_format(dcrp_query, query, sizeof(query), "SELECT Char_PhoneNum FROM player_characters WHERE Char_PhoneNum='%s'", number);
	mysql_tquery(dcrp_query, query, "PhoneNumberCustom", "iis", playerid, otherid, number);
	return 1;
}
...
CMD:customrek(playerid, params[])
{
	...
	new icsr[255];
	mysql_format(dcrp_query, icsr, sizeof(icsr), "SELECT `Char_BankRek` FROM `player_characters` WHERE `Char_BankRek`=%d", newrek);
	mysql_tquery(dcrp_query, icsr, "RekeningCustom", "iid", playerid, otherid, newrek);
	return 1;
}
...
FUNC::RekeningCustom(playerid, otherid, rekening)
{
	...
	else
	{
		...
		SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah mengubah nomor rekening "YELLOW"%s(%d)"DARKORANGE" [%d] ~> [%d]", AccountData[playerid][pAdminname], ReturnName(otherid), otherid, oldrek, rekening);
		SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah mengubah nomor rekening anda. Sebelum "YELLOW"%d"ARWIN1" Sesudah "YELLOW"%d", AccountData[playerid][pAdminname], oldrek, rekening);
		...
		new query[255];
		mysql_format(dcrp_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_BankRek`= %d WHERE `pID`=%d", rekening, AccountData[otherid][pID]);
...
public PhoneNumberCustom(playerid, otherid, const number[])
{
	...
	else
	{
		...
		SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah customisasi nomor HP "YELLOW"%s(%d) - %s", GetAdminName(playerid), GetRPName(otherid), otherid, number);
		SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] Admin "RED"%s"ARWIN1" telah mengganti nomor HP anda menjadi "YELLOW"%s.", GetAdminName(playerid), number);
		...
		new query[255];
		mysql_format(dcrp_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_PhoneNum`='%s' WHERE `pID`=%d", number, AccountData[otherid][pID]);
		mysql_tquery(dcrp_query, query);
	}
	return 1;
}
...
CMD:changegender(playerid, params[])
{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah mengganti gender "YELLOW"%s(%d) - %s", GetAdminName(playerid), AccountData[id][pName], id, (gender == 1) ? "Laki-Laki" : "Perempuan");
	...
	new icsr[200];
	mysql_format(dcrp_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_Gender`=%d WHERE `pID`=%d", gender, AccountData[id][pID]);
	mysql_tquery(dcrp_query, icsr);
	return 1;
}
...
CMD:respawnallveh(playerid, params[])
{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah memulai waktu reloading player vehicle!");
	return 1;
}
...
CMD:torture(playerid, params[])
{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah menggunakan /torture kepada pemain "YELLOW"%s(%d)", AccountData[playerid][pAdminname], AccountData[otherid][pName], otherid);
	return 1;
}
...
CMD:checkmask(playerid, params[])
{
	...
	execute = mysql_query(dcrp_query, sprintf("SELECT `Char_Name` FROM `player_characters` WHERE `Char_MaskID` = '%d';", strval(params)));
	if(cache_num_rows()) {
...
CMD:makewarga(playerid, params[])
{
	...
	new query[200];
	mysql_format(dcrp_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_Faction`=%d, `Char_FactionRank`=%d, `Char_Family`=%d, `Char_FamilyRank`=%d WHERE `pID`=%d", AccountData[otherid][pFaction], AccountData[otherid][pFactionRank], AccountData[otherid][pFamily], AccountData[otherid][pFamilyRank], AccountData[otherid][pID]);
	mysql_tquery(dcrp_query, query);
...
CMD:giveredmoney(playerid, params[])
{
	...
	SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd]"RED" %s"ARWIN1" memberikan anda Red Money sebesar "RED"%s", AccountData[playerid][pAdminname], FormatMoney(value));
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" memberikan Red Money sebesar "RED"%s"ARWIN1" kepada pemain "YELLOW"%s(%d)", AccountData[playerid][pAdminname], FormatMoney(value), AccountData[otherid][pName], otherid);
...
CMD:panel(playerid, const params[])
{
	...
	if(g_ServerLocked)
		Dialog_Show(playerid, ServerPanel, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Server Panel", "Unlock Server\n"GRAY"Set Hostname\nSet Weburl\n"GRAY"Server Info", "Pilih", "Batal");

	else Dialog_Show(playerid, ServerPanel, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Server Panel", "Lock Server\n"GRAY"Set Hostname\nSet Weburl\n"GRAY"Server Info", "Pilih", "Batal");
	return 1;
}
	...
	if(response)
	{
		switch(listitem)
		{
			case 0:// Lock/Unlock Server
			{
				...
				else Dialog_Show(playerid, LockServer, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Lock Server", "Harap masukan password untuk mengunci server\n"YELLOW"(Masukan dikolom bawah ini):", "Lock", "Cancel");
			}
            case 1: Dialog_Show(playerid, SetHostname, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Set Hostname", "Harap masukkan hostname baru server\n"YELLOW"(Masukan dikolom bawah ini):", "Submit", "Back");
            case 2: Dialog_Show(playerid, SetWeburl, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Set Weburl", "Harap masukkan Weburl / MOTD Server yang akan digunakan\nCth: dsc.gg/DCRP", "Submit", "Batal");
			case 3: Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Server Information", "Category\tValue\nDynamic Object:\t%d\nVehicle's Count:\t%d", "Close", "", Streamer_CountItems(STREAMER_TYPE_OBJECT), GetVehiclePoolSize());
		}
	}
...
Dialog:LockServer(playerid, response, listitem, inputtext[])
{
    ...
    if(response)
    {
        if(isnull(inputtext) || !strcmp(inputtext, "0"))
            return Dialog_Show(playerid, LockServer, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Lock Server", "Harap masukan password untuk mengunci server\n"YELLOW"(Masukan dikolom bawah ini):", "Lock", "Back");
        ...
        SendRconCommand(str);
        SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" has locked the server (password: %s).", GetAdminName(playerid), inputtext);
    }
...
Dialog:SetHostname(playerid, response, listitem, inputtext[])
{
    ...
    {
        if(isnull(inputtext))
            return Dialog_Show(playerid, SetHostname, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Set Hostname", "Harap masukkan hostname baru server\n"YELLOW"(Masukan dikolom bawah ini):", "Submit", "Back");
        ...
        SendRconCommand(str);
        SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" has set the hostname to \"%s\".", GetAdminName(playerid), inputtext);
    }
    else callcmd::panel(playerid, "\1");
    return 1;
}
...
Dialog:SetWeburl(playerid, response, listitem, inputtext[])
{
	...
	if(isnull(inputtext)) return Dialog_Show(playerid, SetWeburl, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Set Weburl", 
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah mengubah MOTD Server menjadi \"%s\".", AccountData[playerid][pAdminname], inputtext);
	return 1;
}
...
CMD:restart(playerid, params[])
{
	...
	if(g_RestartServer)
	{
		...
		return SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd]: Waktu restart telah ditunda oleh "RED"%s.", GetAdminName(playerid));
	}
...
new g_WorldTime = 12; // Default to noon

CMD:asuransikeliling(playerid, params[])
{
	new times;
	...
	if(sscanf(params, "d", times))
		return ShowTDN(playerid, NOTIFICATION_SYNTAX, "/asuransikeliling [waktu]");
	
	if(times < 5 || times > 120)
		return ShowTDN(playerid, NOTIFICATION_ERROR, "Waktu tidak bisa kurang dari 5 atau lebih dari 120!");

	if(g_AsuransiAll == 1)
		return Error(playerid, "Insurance keliling sudah dinyalakan mohon tunggu sampai selesai!");
	
	// Use the global world time variable
	new hour = g_WorldTime;
	
	// Play appropriate audio based on time
	foreach(new i : Player)
	{
		if(hour >= 5 && hour < 15) // pagi (5 AM - 2:59 PM)
		{
			PlayAudioStreamForPlayer(i, "https://b.top4top.io/m_3426ymq5d1.mp3"); // Morning audio
		}
		else if(hour >= 15 && hour < 19) // sore (3 PM - 6:59 PM)
		{
			PlayAudioStreamForPlayer(i, "https://l.top4top.io/m_3426utyvr1.mp3"); // Afternoon audio
		}
		else if(hour >= 19 || hour < 23) // malem (7 PM - 4:59 AM)
		{
			PlayAudioStreamForPlayer(i, "https://d.top4top.io/m_3426mh2xx1.mp3"); // Night audio
		}
	}
	
	SendClientMessageToAllEx(ARWIN, "[AdmCmd]: "RED"%s "ARWIN1"telah mengaktifkan asuransi keliling dalam %d detik.", GetAdminName(playerid), times);
	SendClientMessageToAllEx(ARWIN, "[Asuransi]: Mohon masuk kedalam kendaraan anda!, terima kasih");
	...
	foreach(new i : Player)
	{
		forex(a, 7)
		{
			PlayerTextDrawShow(i, InsuranceTD[i][a]);
		}
	}
	return 1;
}
...
CMD:respawnjobvehicle(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" Semua kendaraan job yang tidak dinaiki atau terpakai akan dihancurkan dalam 20 detik!", AccountData[playerid][pAdminname]);
	SetTimerEx("RespawnVehicleJobs", 15000, false, "d", playerid);
	return 1;
}

JadenDCRP: RespawnVehicleJobs(playerid)
{
...
{
	if(AccountData[playerid][pAdmin] < 1 && AccountData[playerid][pTheStars] < 1) return PermissionError(playerid);
	ShowPlayerDialog(playerid, DIALOG_ADMIN_HELP, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Admin Help",
...
CMD:dynhelp(playerid, params[])
{
	...
	ShowPlayerDialog(playerid, DIALOG_DYNAMIC_HELP, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Dynamic Help", shstr, "Pilih", "Batal");
	return 1;
}
...
CMD:makequiz(playerid, params[])
{
	...
	if(!strcmp(type, "question", true))
	{
		...
		SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah membuat Quiz server.", AccountData[playerid][pAdminname]);
		SendClientMessageToAllEx(X11_ARWIN, "~> %s? - "GREEN"%s.", frmxt, FormatMoney(QuizPrice));
		SendClientMessageToAllEx(X11_ARWIN, "~> Gunakan "RED"'/qa'"ARWIN1" untuk menjawab quiz.");
		Quiz = true;
	}
	else if(!strcmp(type, "answer", true))
	{
		...
		if(Quiz) return ShowTDN(playerid, NOTIFICATION_ERROR, "Sudah ada Quiz sebelumnya akhiri dengan /makequiz end");
		SendClientMessageEx(playerid, X11_ARWIN, "[AdmCmd] Anda telah menetapkan jawaban untuk Quiz ~> "YELLOW"%s", frmxt);
	...
	else if(!strcmp(type, "prize", true))
	{
		...
		SendClientMessageEx(playerid, X11_ARWIN, "[AdmCmd] Anda telah menetapkan hadiah quiz sebesar "GREEN"%s.", FormatMoney(price));
		QuizPrice = price;
	}
	else if(!strcmp(type, "end", true))
	{
		...
		QuizPrice = 0;
		QuizAnswer = "";
		SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd] Quiz telah diakhiri oleh "RED"%s.", GetAdminName(playerid));
	}
	return 1;
}
...
CMD:qa(playerid, params[])
{
	...
	if(!strcmp(frmtanswer, QuizAnswer, true)) 
	{
		...
		SendClientMessageToAllEx(X11_ARWIN, shstr);
...
CMD:stafflist(playerid, params[])
{
	...
	if(count == 0)
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Staff List", "Tidak ada administrator yang online!", "Tutup", "");
	else
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Staff List", shstr, "Tutup", "");
	return 1;
}
...
CMD:checknetwork(playerid, params[])
{
	if (GetAdminLevel(playerid))
	{
		...
		Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Network Stats", stats, "Close", "");
	}
	return 1;
}
...
CMD:admins(playerid, params[])
{
	...
	{
		format(list, sizeof(list), "Rank\tAdmin Name(ID)\tDuty\n");
		foreach(new i : Player) if (IsPlayerConnected(i) && AccountData[i][pSpawned])
		{
			if(AccountData[i][pAdmin] >= 1 || AccountData[i][pTheStars] >= 1)
			{
	...
	if(count == 0)
		Error(playerid, "Tidak ada administrator online dikota!"); // ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Online Administrator", "Tidak ada administrator online dikota!", "Tutup", "");
	else
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Online Administrator", list, "Close", "");
	return 1;
}
...
CMD:ajail(playerid, params[])
{
	...
	if(count == 0)
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Player Jail", "Tidak ada pemain dalam masa jail admin!", "Close", "");
	else 
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Player Jail", shstr, "Close", "");
	return 1;
}
...
CMD:aduty(playerid, params[])
{
	...
	if(!AccountData[playerid][pAdminDuty])
	{
		...
		{
			SendAdminMessage(X11_ARWIN, "[Staff Duty]"GREEN" %s %s telah on duty The Stars dengan nama %s.", GetStaffRank(playerid), AccountData[playerid][pName], GetAdminName(playerid));
		}
		else SendAdminMessage(X11_ARWIN, "[Staff Duty]"GREEN" %s %s telah on duty admin dengan nama %s.", GetStaffRank(playerid), AccountData[playerid][pName], GetAdminName(playerid));
	...
	else 
	{
		...
		if(AccountData[playerid][pTheStars])
		{
			SendAdminMessage(X11_ARWIN, "[Staff Duty]"GREEN" %s %s telah off duty The Stars dengan nama %s.", GetStaffRank(playerid), AccountData[playerid][pName], GetAdminName(playerid));
		}
		else SendAdminMessage(X11_ARWIN, "[Staff Duty]"GREEN" %s %s telah off duty admin dengan nama %s.", GetStaffRank(playerid), AccountData[playerid][pName], GetAdminName(playerid));
...
CMD:togooc(playerid, params[])
{
	...
	if(!TogOOC)
	{
		SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah mengaktifkan Global Chat OOC!", AccountData[playerid][pAdminname]);
		TogOOC = true;
	}
	else 
	{
		SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd]"RED"%s"ARWIN1" telah mematikan Global Chat OOC!", AccountData[playerid][pAdminname]);
		TogOOC = false;
	}
	return 1;
}
...
CMD:gotosf(playerid, params[])
{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" teleportasi ke San Fiero", AccountData[playerid][pAdminname]);
	return 1;
}
...
CMD:gotols(playerid, params[])
{
	...
	SetPlayerVirtualWorldEx(playerid, 0);
	SetPlayerInteriorEx(playerid, 0);
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" Teleportasi ke Los Santos", GetAdminName(playerid));
	AccountData[playerid][pInDoor] = -1;
    AccountData[playerid][pInHouse] = -1;
    AccountData[playerid][pInBiz] = -1;
    AccountData[playerid][pInFamily] = -1;
...
CMD:gotoasur(playerid, params[])
{
	...
	SetPlayerVirtualWorldEx(playerid, 0);
	SetPlayerInteriorEx(playerid, 0);
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" Teleportasi ke Insuranced", GetAdminName(playerid));
	AccountData[playerid][pInDoor] = -1;
    AccountData[playerid][pInHouse] = -1;
    AccountData[playerid][pInBiz] = -1;
    AccountData[playerid][pInFamily] = -1;
...
CMD:gotocarnaval(playerid, params[])
{
	...
	SetPlayerVirtualWorldEx(playerid, 0);
	SetPlayerInteriorEx(playerid, 0);
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" Teleportasi ke Karnaval", GetAdminName(playerid));
	AccountData[playerid][pInDoor] = -1;
    AccountData[playerid][pInHouse] = -1;
    AccountData[playerid][pInBiz] = -1;
    AccountData[playerid][pInFamily] = -1;
...
CMD:goto(playerid, params[])
{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" teleportasi ke player "YELLOW"%s(%d)", AccountData[playerid][pAdminname], ReturnName(otherid), otherid);
	SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd]:"RED"%s "ARWIN1"teleportasi ke posisi anda.", AccountData[playerid][pAdminname]);
	return 1;
}
...
CMD:sendtoplayer(playerid, params[])
{
	...
	SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" mengirimkan anda kepada "YELLOW"%s(%d)", AccountData[playerid][pAdminname], AccountData[toother][pName], toother);
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" Mengirimkan "YELLOW"%s(%d)"ARWIN1" kepada "YELLOW"%s(%d)", AccountData[playerid][pAdminname], AccountData[otherid][pName], otherid, AccountData[toother][pName], toother);
	return 1;
}
...
CMD:sendto(playerid, params[])
{
	...
	if(!strcmp(option, "ls", true))
	{
        ...
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
		SendClientMessageEx(playerid, X11_ARWIN, "[AdmCmd] "YELLOW"%s(%d)"ARWIN1" telah berhasil di teleportasikan ke Los Santos.", AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah menteleportasikan anda ke Los Santos.", AccountData[playerid][pAdminname]);
	...
	else if(!strcmp(option, "lv", true))
	{
        ...
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
		SendClientMessageEx(playerid, X11_ARWIN, "[AdmCmd] "YELLOW"%s(%d)"ARWIN1" telah berhasil di teleportasikan ke Las Vegas.", AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah menteleportasikan anda ke Las Vegas.", AccountData[playerid][pAdminname]);
		AccountData[otherid][pInDoor] = -1;
		AccountData[otherid][pInHouse] = -1;
		AccountData[otherid][pInRusun] = -1;
		AccountData[otherid][pInBiz] = -1;
	...
	else if(!strcmp(option, "sf", true))
	{
        ...
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
		SendClientMessageEx(playerid, X11_ARWIN, "[AdmCmd] "YELLOW"%s(%d)"ARWIN1" telah berhasil di teleportasikan ke San Fiero.", AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah menteleportasikan anda ke San Fiero.", AccountData[playerid][pAdminname]);
		AccountData[otherid][pInDoor] = -1;
		AccountData[otherid][pInHouse] = -1;
		AccountData[otherid][pInRusun] = -1;
		AccountData[otherid][pInBiz] = -1;
	...
	else if(!strcmp(option, "asuransi", true))
	{
        ...
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
		SendClientMessageEx(playerid, X11_ARWIN, "[AdmCmd] "YELLOW"%s(%d)"ARWIN1" telah berhasil di teleportasikan ke Asuransi.", AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah menteleportasikan anda ke Asuransi.", AccountData[playerid][pAdminname]);
		AccountData[otherid][pInDoor] = -1;
		AccountData[otherid][pInHouse] = -1;
		AccountData[otherid][pInRusun] = -1;
		AccountData[otherid][pInBiz] = -1;
	...
	else if(!strcmp(option, "carnaval", true))
	{
        ...
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
		SendClientMessageEx(playerid, X11_ARWIN, "[AdmCmd] "YELLOW"%s(%d)"ARWIN1" telah berhasil di teleportasikan ke Carnaval.", AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah menteleportasikan anda ke Carnaval.", AccountData[playerid][pAdminname]);
		AccountData[otherid][pInDoor] = -1;
		AccountData[otherid][pInHouse] = -1;
		AccountData[otherid][pInRusun] = -1;
		AccountData[otherid][pInBiz] = -1;
	...
	else if(!strcmp(option, "bandara", true))
	{
        ...
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
		SendClientMessageEx(playerid, X11_ARWIN, "[AdmCmd] "YELLOW"%s(%d)"ARWIN1" telah berhasil di teleportasikan ke Bandara.", AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah menteleportasikan anda ke Bandara.", AccountData[playerid][pAdminname]);
		AccountData[otherid][pInDoor] = -1;
		AccountData[otherid][pInHouse] = -1;
		AccountData[otherid][pInRusun] = -1;
		AccountData[otherid][pInBiz] = -1;
	...
	else if(!strcmp(option, "rs", true))
	{
        ...
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
		SendClientMessageEx(playerid, X11_ARWIN, "[AdmCmd] "YELLOW"%s(%d)"ARWIN1" telah berhasil di teleportasikan ke Rumah Sakit.", AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah menteleportasikan anda ke Rumah Sakit.", AccountData[playerid][pAdminname]);
		AccountData[otherid][pInDoor] = -1;
		AccountData[otherid][pInHouse] = -1;
		AccountData[otherid][pInRusun] = -1;
		AccountData[otherid][pInBiz] = -1;
	...
	else if(!strcmp(option, "balkot", true))
	{
        ...
        SetPlayerInterior(otherid, 0);
        SetPlayerVirtualWorld(otherid, 0);
		SendClientMessageEx(playerid, X11_ARWIN, "[AdmCmd] "YELLOW"%s(%d)"ARWIN1" telah berhasil di teleportasikan ke Balai Kota.", AccountData[otherid][pName], otherid);
		SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah menteleportasikan anda ke Balai Kota.", AccountData[playerid][pAdminname]);
		AccountData[otherid][pInDoor] = -1;
		AccountData[otherid][pInHouse] = -1;
		AccountData[otherid][pInRusun] = -1;
		AccountData[otherid][pInBiz] = -1;
...
CMD:gethere(playerid, params[])
{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah menarik pemain "YELLOW"%s(%d)"ARWIN1" ke posisinya.", AccountData[playerid][pAdminname], ReturnName(otherid), otherid);
	return 1;
}
...
CMD:arevive(playerid, params[])
{
	...
	if(!isnull(params) && !strcmp(params, "me", true))
	{
		...
		return SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah menghidupkan "YELLOW"diri sendiri", GetAdminName(playerid));
	}
	new 
		otherid
	;
	...
	SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah menghidupkan anda", GetAdminName(playerid));
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah menghidupkan "YELLOW"%s(%d)", GetAdminName(playerid), AccountData[otherid][pName], otherid);
	return 1;
}
...
CMD:spec(playerid, const params[])
{
    ...
    if(IsPlayerInAnyVehicle(otherid))
	{
		...
		if(GetPlayerState(otherid) == PLAYER_STATE_DRIVER)
	    {
			SendClientMessageEx(playerid, X11_ARWIN, "[AdmCmd] Anda sekarang Spectating"YELLOW" %s(%d)"ARWIN1" yang sedang mengemudikan "YELLOW"%s(%d)", AccountData[otherid][pName], otherid, GetVehicleModelName(GetVehicleModel(vID)), vID);
		}
		else
		{
			SendClientMessageEx(playerid, X11_ARWIN, "[AdmCmd] Anda sekarang Spectating"YELLOW" %s(%d)"ARWIN1" yang merupakan penumpang di "YELLOW"%s(%d).", AccountData[otherid][pName], otherid, GetVehicleModelName(GetVehicleModel(vID)), vID);
		}
	}
    else
	{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" sedang memantau "YELLOW"%s(%d).", GetAdminName(playerid), GetRPName(otherid), otherid);
	return 1;
}
...
CMD:slap(playerid, params[])
{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah melakukan slaped ke pemain "YELLOW"%s(%d).", AccountData[playerid][pAdminname], AccountData[otherid][pName], otherid);
...
CMD:ostats(playerid, params[])
{
	...
	mysql_format(dcrp_query, cQuery, sizeof(cQuery), "SELECT %s FROM player_characters WHERE Char_Name='%e' LIMIT 1;", shstr, playerName);
	mysql_tquery(dcrp_query, cQuery, "OfflineStats", "ds", playerid, playerName);
	return 1;
}
...
CMD:jetpack(playerid, params[])
{
	...
	if(sscanf(params, "u", otherid))
	{
		...
		SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" Menggunakan Jetpack", AccountData[playerid][pAdminname]);
	}
	else 
	{
		AccountData[otherid][pJetpack] = true;
		SetPlayerSpecialAction(otherid, SPECIAL_ACTION_USEJETPACK);
		SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" memberikan jetpack kepada "YELLOW"%s(%d).", AccountData[playerid][pAdminname], AccountData[otherid][pName], otherid);
	}
	return 1;
}
...
CMD:getip(playerid, params[]) 
{
	...
	if(AccountData[otherid][pAdmin] > 5)
	{
		...
		SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s(%d)"ARWIN1" mencoba mengambil data ip anda.", AccountData[playerid][pAdminname], playerid);
		return 1;
	}
	GetPlayerName(otherid, givePlayer, sizeof(givePlayer));
	GetPlayerIp(otherid, PlayerIP, sizeof(PlayerIP));
	
	SendClientMessageEx(playerid, X11_ARWIN, "[AdmCmd] Details IP Pemain "YELLOW"%s(%d) ~> "GREEN"`%s`.", givePlayer, otherid, PlayerIP);
	return 1;
}
...
CMD:aka(playerid, params[])
{
	...
	GetPlayerIp(otherid, PlayerIP, sizeof(PlayerIP));
	mysql_format(dcrp_query, query, sizeof(query), "SELECT Char_Name FROM player_characters WHERE Char_IP='%s'", PlayerIP);
	mysql_tquery(dcrp_query, query, "CheckPlayerIP", "is", playerid, PlayerIP);
	return true;
}
...
CMD:akaip(playerid, params[])
{
	...
	mysql_format(dcrp_query, query, sizeof(query), "SELECT Char_Name FROM player_characters WHERE IP='%s'", params);
	mysql_tquery(dcrp_query, query, "CheckPlayerIP2", "is", playerid, params);
	return true;
}
...
CMD:owarn(playerid, params[]) 
{
	...
	new query[255];
	mysql_format(dcrp_query, query, sizeof(query), "SELECT `pID`, `Char_Warn` FROM `player_characters` WHERE `Char_Name`='%e'", player);
	mysql_tquery(dcrp_query, query, "OfflineWarnPlayer", "iss", playerid, player, reason);
	return 1;
}
...
public OfflineWarnPlayer(playerid, name[], reason[])
{
	...
	else 
	{
		...
		SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd] Akun "YELLOW"%s"ARWIN1" telah diberikan warning(offline) oleh "RED"%s.", name, AccountData[playerid][pAdminname]);
		SendClientMessageToAllEx(X11_ARWIN, "~> Alasan: %s.", reason);

		new query[255];
		mysql_format(dcrp_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_Warn`=%d WHERE `pID`=%d", Warn + 1, RegID);
		mysql_tquery(dcrp_query, query);

		mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `warninglogs` SET `pID`=%d, `WarnType`=1, `WarnSender`='%s', `WarnTime`=%d, `WarnReason`='%s'", RegID, AccountData[playerid][pAdminname], gettime(), reason);
		mysql_tquery(dcrp_query, query);
	}
	return 1;
}
...
CMD:oreco(playerid, params[])
{
	...
	new query[512];
	mysql_format(dcrp_query, query, sizeof(query), "SELECT pID FROM player_characters WHERE Char_Name='%e'", player);
	mysql_tquery(dcrp_query, query, "ORecoPlayer", "iss", playerid, player, reason);
...
CMD:ojail(playerid, params[])
{
	...
	new query[512];
	mysql_format(dcrp_query, query, sizeof(query), "SELECT `pID` FROM `player_characters` WHERE `Char_Name`='%e'", player);
	mysql_tquery(dcrp_query, query, "OfflineJailed", "issi", playerid, player, reason, datez);
	return 1;
}
...
public OfflineJailed(adminid, const NameJail[], const Reason[], time)
{
	...
	else 
	{
		...
		cache_get_value_index_int(0, 0, RegID);
		SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd] Pemain dengan nama "YELLOW"%s"ARWIN1" telah dijail (offline) selama "RED"%d menit "ARWIN1"oleh"RED" %s", NameJail, time, GetAdminName(adminid));
		SendClientMessageToAllEx(X11_ARWIN, "~> Alasan: %s", Reason);

		new query[522], tmp[200], date;
		date = gettime();
		mysql_format(dcrp_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_Jail`=1, `Char_JailTime`=%d, `Char_JailReason`='%s' WHERE `pID`=%d", Jailtime, Reason, RegID);
		mysql_tquery(dcrp_query, query);

		mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `warninglogs` SET `pID`=%d, `WarnType`=2, `WarnTime`=%d, `WarnSender`='%s', `WarnReason`='%s'", RegID, date, GetAdminName(adminid), Reason);
		mysql_tquery(dcrp_query, query);
...
public ORecoPlayer(adminid, NameToReco[], RecoReason[])
{
	...
	else 
	{
		...
		mysql_tquery(dcrp_query, sprintf("UPDATE player_characters SET Char_BankMoney=0,Char_Money=0 WHERE pID='%d'", RegID));
	}
	return 1;
}
...
CMD:jail(playerid, params[])
{
	...
	date = gettime();
	mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `warninglogs` SET `pID`=%d, `WarnType`=2, `WarnTime`=%d, `WarnSender`='%s', `WarnReason`='%s'", AccountData[otherid][pID], date, GetAdminName(playerid), reason);
	mysql_tquery(dcrp_query, query);
...
CMD:aunarrest(playerid, params[])
{
	...
	AccountData[otherid][pArrest] = 0;
	AccountData[otherid][pArrestTime] = 0;
	SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd]"RED" %s"ARWIN1" Membebaskan anda dari Arrest (Penjara Polisi)", AccountData[playerid][pAdminname]);
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" Membebaskan player "YELLOW"%s(%d)"ARWIN1" dari Arrest (Penjara Polisi)", AccountData[playerid][pAdminname], AccountData[otherid][pName], otherid);
...
CMD:unjail(playerid, params[])
{
	...
	SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah membebaskan anda dari penjara!", GetAdminName(playerid));
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah membebaskan "YELLOW"%s(%d)"ARWIN1" dari hukuman jail!", GetAdminName(playerid), GetRPName(otherid), otherid);
	
	new cQuery[178];
	mysql_format(dcrp_query, cQuery, sizeof(cQuery), "DELETE FROM `warninglogs` WHERE `WarnType` = 2 AND `pID` = '%d'", AccountData[otherid][pID]);
...
CMD:kick(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd]"YELLOW" %s(%d)"ARWIN1" telah ditendang dari server oleh "RED"%s.", AccountData[otherid][pName], otherid, AccountData[playerid][pAdminname]);
	SendClientMessageToAllEx(X11_ARWIN, "~> Alasan: %s", reason);
	KickEx(otherid);
	return 1;
}
...
CMD:blockucp(playerid, params[])
{
	...
	if(datez != 0)
	{
		...
		SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd]"RED" %s"ARWIN1" telah memblokir Akun UCP "YELLOW"%s"ARWIN1" dari server selama %d hari.", AccountData[playerid][pAdminname], username, datez);
		SendClientMessageToAllEx(X11_ARWIN, "Reason ~> %s", reason);
	}
	else
	{
		banTime = datez;
		SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd]"RED" %s"ARWIN1" telah memblokir "RED"Permanent"ARWIN1" Akun UCP "YELLOW"%s"ARWIN1" dari server.", AccountData[playerid][pAdminname], username, datez);
		SendClientMessageToAllEx(X11_ARWIN, "Reason ~> %s", reason);
	}

	new query[248];
	mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `player_bans` (`name`, `ip`, `admin`, `reason`, `ban_date`, `ban_expire`) VALUES ('%s', ' ', '%s', '%s', '%d', '%d')", username, AccountData[playerid][pAdminname], reason, gettime(), banTime);
	mysql_tquery(dcrp_query, query);
	return 1;
}
...
CMD:ban(playerid, params[])
{
	...
	if(datez != 0)
	{
		SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah membanned akun player"YELLOW" %s(%d)"ARWIN1" selama "YELLOW"%d"ARWIN1" hari", GetAdminName(playerid), playerName, otherid, datez);
		SendClientMessageToAllEx(X11_ARWIN, "~> Alasan: %s", reason);
	...
	{
		SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah membanned "RED"Permanent"ARWIN1" player"YELLOW" %s(%d)", GetAdminName(playerid), playerName, otherid, datez);
		SendClientMessageToAllEx(X11_ARWIN, "~> Alasan: %s", reason);
    ...
    {
		banTime = gettime() + (datez * 86400);
		SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"Temporary Banned"ARWIN1" kepada anda selama %d hari.", datez);
	}
	else
	{
		SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd]"RED" Permanent Banned"ARWIN1" harap kontak admin yang bersangkutan jika anda merasa tidak bersalah!");
		banTime = datez;
	}
	new query[248], date;
	date = gettime();
	mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `player_bans` (`name`, `ip`, `admin`, `reason`, `ban_date`, `ban_expire`) VALUES ('%s', '%s', '%s', '%s', '%d', '%d')", AccountData[otherid][pName], PlayerIP, AccountData[playerid][pAdminname], reason, gettime(), banTime);
	mysql_tquery(dcrp_query, query);

	mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `warninglogs` SET `pID`=%d, `WarnTime`=%d, `WarnType`=3, `WarnSender`='%s', `WarnReason`='%s'", AccountData[otherid][pID], date, AccountData[playerid][pAdminname], reason);
	mysql_tquery(dcrp_query, query);
...
CMD:unban(playerid, params[])
{
	...
	new query[128];
	mysql_format(dcrp_query, query, sizeof(query), "SELECT name,ip FROM player_bans WHERE name = '%s'", tmp);
	mysql_tquery(dcrp_query, query, "OnUnbanQueryData", "is", playerid, tmp);
	return 1;
}
...
public OnUnbanQueryData(adminid, BannedName[])
{
	...
	{
		new query[128];
		mysql_format(dcrp_query, query, sizeof(query), "DELETE FROM `player_bans` WHERE `name` = '%s'", BannedName);
		mysql_tquery(dcrp_query, query);

		SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah mencabut status banned dari akun "YELLOW"%s", AccountData[adminid][pAdminname], BannedName);
...
/*CMD:unbanucp(playerid, params[])
{
	...
	new query[128];
	mysql_format(dcrp_query, query, sizeof(query), "SELECT name FROM player_bans WHERE name = '%e'", tmp);
	mysql_tquery(dcrp_query, query, "UnbanUCP", "is", playerid, tmp);
	return 1;
}
...
function UnbanUCP(adminid, BannedName[])
{
	if(cache_num_rows() > 0)
	{
		...
		SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah melepas hukuman banned "YELLOW"%s"ARWIN1" dari server", GetAdminName(adminid), BannedName);
	}
	else
	{
...
CMD:awarn(playerid, params[])
{
	...
	if(!IsPlayerConnected(otherid)) return ShowTDN(playerid, NOTIFICATION_ERROR, "Pemain tersebut tidak terkoneksi ke server!");
	mysql_format(dcrp_query, dbsr, sizeof(dbsr), "SELECT * FROM `warninglogs` WHERE `pID`=%d", AccountData[otherid][pID]);
	new Cache:execute = mysql_query(dcrp_query, dbsr, true);
	if(cache_num_rows())
	{
		...
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, sprintf(""PINK1"DCRP"WHITE"- %s(%d)", AccountData[otherid][pName], otherid), list, "Tutup", "");
	}
	else
	{
		new list[255];
...
CMD:warn(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd] "YELLOW"%s(%d)"ARWIN1" telah diberikan warning oleh "RED"%s.", AccountData[otherid][pName], otherid, GetAdminName(playerid));
	SendClientMessageToAllEx(X11_ARWIN, "~> Alasan: %s", reason);

	new query[255], currentTime;
	currentTime = gettime();
	mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `warninglogs` SET `pID`=%d, `WarnType`=1, `WarnSender`='%s', `WarnTime`=%d, `WarnReason`='%s'", AccountData[otherid][pID], GetAdminName(playerid), currentTime, reason);
	mysql_tquery(dcrp_query, query);
	return 1;
}
...
CMD:unwarn(playerid, params[])
{
	...
	SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah menghapus 1 point warning anda!", GetAdminName(playerid));
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah menghapus 1 point warning dari %s", GetAdminName(playerid), AccountData[otherid][pName]);
	return 1;
}
        ...
        if(GetVehicleModel(i))
        {
            ...
            if (((tempposx < radi) && (tempposx > -radi)) && ((tempposy < radi) && (tempposy > -radi)) && ((tempposz < radi) && (tempposz > -radi)))
            {
				if(IsVehicleEmpty(i))
				{
					...
					SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah merespawn kendaraan disekitarnya dengan radius"YELLOW" %.1f", GetAdminName(playerid), radi);
				}
			}
        }
    }
...
CMD:respawnsidejob(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah memulai timer selama 10 detik untuk merespawn kendaraan sidejob yang tidak terpakai / dinaiki.", AccountData[playerid][pAdminname]);
	return 1;
}
...
public RespawnPVSideJob(playerid)
{
	...
	SendClientMessageToAll(X11_ARWIN, "[AdmCmd] Semua kendaraan sidejob yang tidak terpakai/dinaiki telah direspawn kembali");
	return 1;
}
...
CMD:sethp(playerid, params[])
{
	...
	SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd]"RED" %s"ARWIN1" telah menetapkan health anda menjadi %d", GetAdminName(playerid), value);
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah menetapkan health "YELLOW"%s(%d) - %d%", GetAdminName(playerid), GetRPName(otherid), otherid, value);
	return 1;
}
...
CMD:sethpall(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd]"RED" %s"ARWIN1" telah menetapkan health bagi seluruh pemain menjadi %d", GetAdminName(playerid), value);
	return 1;
}
...
CMD:setbone(playerid, params[])
{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah men set jumlah Kondisi tulang pemain "YELLOW"%s(%d).", AccountData[playerid][pAdminname], AccountData[otherid][pName], otherid);
...
CMD:setboneall(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah men set jumlah Kondisi tulang seluruh pemain online menjadi %d%%.", AccountData[playerid][pAdminname], jumlah);
	return 1;
}
...
CMD:setam(playerid, params[])
{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah menetapkan armour "YELLOW"%s(%d) - %d.00", GetAdminName(playerid), GetRPName(otherid), otherid, jumlah);
	SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd]"RED" %s"ARWIN1" telah menetapkan armour anda menjadi %d.00", GetAdminName(playerid), jumlah);
	return 1;
}
...
CMD:vsethealth(playerid, params[])
{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" menetapkan "YELLOW"%.2f"ARWIN1" Health untuk kendaraan"YELLOW" [VID: %d] [%s]", AccountData[playerid][pAdminname], health, vehid, GetVehicleName(vehid));
	return 1;
}
...
CMD:afix(playerid, params[])
{
    ...
    if(index == RETURN_INVALID_VEHICLE_ID) 
    ...
    if(vehicleid > 0 && isnull(params))
    {
        ...
        SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" memperbaiki kendaraan "YELLOW"[VID: %d] [%s]", AccountData[playerid][pAdminname], vehicleid, GetVehicleName(vehicleid));
    }
    else
    {
...
CMD:setskin(playerid, params[])
{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" menetapkan skin"YELLOW" %s(%d) - %d.", GetAdminName(playerid), AccountData[otherid][pName], otherid, skin);
	return 1;
}
...
CMD:settime(playerid, params[])
{
	...
	SetWorldTime(time);
	g_WorldTime = time; // Store the world time
	foreach(new ii : Player)
	{
		SetPlayerTime(ii, time, 0);
	}
	SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd]"RED" %s"ARWIN1" telah mengubah waktu in-game server", GetAdminName(playerid));
	return 1;
}
...
CMD:setweather(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah mengubah cuaca in-game server.", AccountData[playerid][pAdminname]);
    return 1;
}
...
CMD:oban(playerid, params[])
{
	...
	new query[200];
	mysql_format(dcrp_query, query, sizeof(query), "SELECT Char_IP FROM player_characters WHERE Char_Name='%e'", player);
	mysql_tquery(dcrp_query, query, "OnOBanQueryData", "dssd", playerid, player, reason, datez);
	return 1;
}
...
CMD:resetweap(playerid, params[])
{
    ...
    SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] Admin "RED"%s"ARWIN1" telah mereset senjata anda.", AccountData[playerid][pAdminname]);
    SendStaffMessage(X11_ARWIN, ""RED"%s "ARWIN1"telah mereset senjata "YELLOW"%s(%d).", GetAdminName(playerid), AccountData[otherid][pName], otherid);
	return 1;
}
...
CMD:setlevel(playerid, params[])
{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah menetapkan level kepada "YELLOW"%s(%d) - %d", GetAdminName(playerid), GetRPName(otherid), otherid, jumlah);
	return 1;
}
...
CMD:setstress(playerid, params[])
{
	...
	SendClientMessageEx(userid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah menetapkan stress anda menjadi %d%%", AccountData[playerid][pAdminname], jumlah);
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah menetapkan stress "YELLOW"%s(%d)"ARWIN1" menjadi %d%%", AccountData[playerid][pAdminname], AccountData[userid][pName], userid, jumlah);
	return 1;
}
...
CMD:reco(playerid, params[])
{
	...
	GiveMoney(userid, -AccountData[playerid][pMoney]);
	AccountData[userid][pBankMoney] = 0;
	SendStaffMessage(X11_ARWIN, ""RED"%s "ARWIN1" telah mereset ekonomi "YELLOW"%s(%d)", GetAdminName(playerid), GetRPName(userid), userid);
	SendClientMessageEx(userid, X11_GRAY, "[i] Admin "RED"%s"ARWIN1" telah mereset ekonomi anda", GetAdminName(playerid));
	
	new shstr[125];
	mysql_format(dcrp_query, shstr, sizeof(shstr), "UPDATE `player_characters` SET `Char_BankMoney`=0, `Char_Money`=0 WHERE `pID`=%d", AccountData[userid][pID]);
...
CMD:goodmood(playerid, params[])
{
	...
	if(!AccountData[playerid][pGoodMood])
	{
		...
		SendStaffMessage(X11_ARWIN, "%s"ARWIN1" menggunakan cmd '/goodmood'", GetAdminName(playerid));
	}
	else
	{
		AccountData[playerid][pGoodMood] = false;
		SendStaffMessage(X11_ARWIN, "%s"ARWIN1" berhenti dari mode '/goodmood'", GetAdminName(playerid));
	}
	return 1;
}
...
CMD:sethbe(playerid, params[])
{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah menetapkan HBE kepada "YELLOW"%s(%d) - %d", GetAdminName(playerid), GetRPName(targetid), targetid, value);
	return 1;
}
...
CMD:sethbeall(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd]"RED" %s"ARWIN1" telah menetapkan HBE untuk seluruh player menjadi %d", GetAdminName(playerid), value);
	return 1;
}
...
CMD:setvip(playerid, params[])
{
	...
	if(dayz == 0)
	{
		...
		SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah menetapkan VIP %s"ARWIN1" kepada "YELLOW"%s(%d)."ARWIN1" Durasi: Permanent.", GetAdminName(playerid), GetPlayerVipName(otherid), GetRPName(otherid), otherid);
		SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] Admin "RED"%s "ARWIN1"telah menetapkan VIP Kepada anda ke level %s"ARWIN1" Permanent time!", GetAdminName(playerid), GetPlayerVipName(otherid));
	}
	else
	{
		AccountData[otherid][pVipTime] = gettime() + (dayz * 86400);
		SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah menetapkan VIP %s"ARWIN1" kepada "YELLOW"%s(%d)."ARWIN1" Durasi: %d hari.", GetAdminName(playerid), GetPlayerVipName(otherid), GetRPName(otherid), otherid, dayz);
		SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] Admin "RED"%s "ARWIN1"telah menetapkan VIP Kepada anda ke level %s"ARWIN1" selama %d hari!", GetAdminName(playerid), GetPlayerVipName(otherid), dayz);
	}
...
CMD:giveweapall(playerid, params[])
{
	...
	SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd]"RED" %s"ARWIN1" telah memberikan "YELLOW"%s(%d ammo)"ARWIN1" kepada seluruh pemain lebih dari level %d", AccountData[playerid][pAdminname], ReturnWeaponName(weaponid), ammo, level);
...
CMD:giveweap(playerid, params[])
{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s "ARWIN1"memberikan "YELLOW"%s(%d Ammo)"ARWIN1" kepada "YELLOW"%s(%d).", GetAdminName(playerid), ReturnWeaponName(weaponid), ammo, AccountData[otherid][pName], otherid);
...
CMD:takemoney(playerid, params[])
{
	...
	TakeMoney(otherid, value);
	SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] Admin "RED"%s"ARWIN1" telah menarik uang anda sejumlah "DARKGREEN"%s.", GetAdminName(playerid), FormatMoney(value));
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah menarik uang milik "YELLOW"%s(%d) -> %s.", GetAdminName(playerid), AccountData[otherid][pName], otherid, FormatMoney(value));
...
CMD:veh(playerid, params[])
{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s "ARWIN1"memunculkan kendaraan %s dengan warna (%d, %d).", GetAdminName(playerid), GetVehicleModelName(model[0]), color1, color2);
	return 1;
}
		...
		if(vehid == PlayerVehicle[i][pVehPhysic])
		{
			foreach(new pid : Player)
			{
				if(PlayerVehicle[i][pVehOwnerID] == AccountData[pid][pID]) {
					...
					SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" menggunakan cmd '/customplate' pada kendaraan milik"YELLOW"%s(%d) - Plat %s.", GetAdminName(playerid), AccountData[pid][pName], pid, string);
				}
			}
		}
	}
...
CMD:adveh(playerid, params[])
{
	...
	if(GetPlayerState(playerid) == PLAYER_STATE_ONFOOT) //jika player sedang di tanah
	{
		...
		SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah menghancurkan semua kendaraan static tidak terpakai milik Admin.", GetAdminName(playerid));
	}
	else //jika player tidak di tanah atau sedang didalam kendaraan
	{
		...
		SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" menghancurkan kendaraan static admin VID: %d.", GetAdminName(playerid), vehicleid);
	}
	return 1;
}
//-----------------------------[ Admin Level 5 ]------------------
...
CMD:setadminname(playerid, params[])
{
	...
	new query[125];
	mysql_format(dcrp_query, query, sizeof(query), "SELECT `Char_AdminName` FROM `player_characters` WHERE `Char_AdminName`='%s'", aname);
	mysql_tquery(dcrp_query, query, "a_ChangeAdminName", "iis", otherid, playerid, aname);
	return 1;
}
...
CMD:setmoney(playerid, params[])
{
	...
	SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd]"RED" %s"ARWIN1" telah menetapkan uang anda menjadi "LIGHTGREEN"%s", GetAdminName(playerid), FormatMoney(money));
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah menetapkan uang "YELLOW"%s(%d) - %s", AccountData[playerid][pAdminname], GetRPName(otherid), otherid, FormatMoney(money));
...
CMD:givemoney(playerid, params[])
{
	...
	GiveMoney(otherid, value);
	SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] Admin "RED"%s"ARWIN1" memberikan anda uang sejumlah "DARKGREEN"%s.", GetAdminName(playerid), FormatMoney(value));
	SendStaffMessage(X11_ARWIN, ""RED"%s "ARWIN1"memberikan uang kepada "YELLOW"%s(%d) - %s", GetAdminName(playerid), AccountData[otherid][pName], otherid, FormatMoney(value));
...
CMD:givemoneyall(playerid, params[])
{
	...
	{
		GiveMoney(i, value);
		PlayAudioStreamForPlayer(playerid, "http://j.top4top.io/m_34316m9yj1.mp3");
	}
	SendClientMessageToAllEx(X11_ARWIN, "[AdmCmd]"RED" %s"ARWIN1" telah memberikan uang kepada player online dengan jumlah "DARKGREEN"%s.", GetAdminName(playerid), FormatMoney(value));
...
CMD:setbankmoney(playerid, params[])
{
	...
	SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] Admin "RED"%s"ARWIN1" menetapkan Saldo Rekening anda menjadi "DARKGREEN"%s.", GetAdminName(playerid), FormatMoney(value));
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" menetapkan Saldo Rekening "YELLOW"%s(%d) - %s.", GetAdminName(playerid), AccountData[otherid][pName], otherid, FormatMoney(value));

	format(tmp, sizeof(tmp), "Menggunakan cmd /setbankmoney untuk %s dengan jumlah %s.", AccountData[otherid][pName], FormatMoney(value));
	AddAdminLog(AccountData[playerid][pName], AccountData[playerid][pUCP], GetStaffRank(playerid), tmp);

	mysql_format(dcrp_query, tss, sizeof(tss), "UPDATE `player_characters` SET `Char_BankMoney`=%d WHERE `pID`=%d", value, AccountData[otherid][pID]);
	mysql_tquery(dcrp_query, tss);
	return 1;
}
...
CMD:setvw(playerid, params[])
{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" menetapkan WWID "YELLOW"%s(%d) - %d", GetAdminName(playerid), AccountData[otherid][pName], otherid, wwid);
	SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah men set VWID anda - %d.", AccountData[playerid][pAdminname], wwid);
	return 1;
}
...
CMD:setint(playerid, params[])
{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" menetapkan INT ID "YELLOW"%s(%d) - %d", GetAdminName(playerid), AccountData[otherid][pName], otherid, intid);
	SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah men set Int ID anda - %d.", AccountData[playerid][pAdminname], intid);
	return 1;
}
...
CMD:explode(playerid, params[])
{
	...
	GetPlayerPos(otherid, POS[0], POS[1], POS[2]);
	CreateExplosion(POS[0], POS[1], POS[2], 7, 5.0);
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" menggunakan explode kepada "YELLOW"%s(%d).", GetAdminName(playerid), AccountData[otherid][pName], otherid);
...
CMD:setadmin(playerid, params[])
{
	...
	SendClientMessageEx(otherid, X11_ARWIN, "[i] Admin "RED"%s"ARWIN1" telah menetapkan anda sebagai %s", GetAdminName(playerid), GetStaffRank(otherid));
	SendStaffMessage(X11_ARWIN, ""RED"%s "ARWIN1"telah menetapkan admin kepada "YELLOW"%s(%d) - %s.", GetAdminName(playerid), AccountData[otherid][pName], otherid, GetStaffRank(otherid));

	format(tmp, sizeof(tmp), "Menggunakan cmd /setadmin kepada %s menjadi admin level %d.", AccountData[otherid][pName], value);
	AddAdminLog(AccountData[playerid][pName], AccountData[playerid][pUCP], GetStaffRank(playerid), tmp);

	mysql_format(dcrp_query, tss, sizeof(tss), "UPDATE `player_characters` SET `Char_Admin`=%d WHERE `pID`=%d", value, AccountData[otherid][pID]);
	mysql_tquery(dcrp_query, tss);
	return 1;
}
...
{
	if(AccountData[playerid][pAdmin] < 1) return PermissionError(playerid);
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Job List",
	"0\tPengangguran\
	\n"GRAY"1\tSupir Bus\
	\n2\tPenambang\
	\n"GRAY"3\tTukang Kayu\
...
CMD:setjob(playerid, params[])
{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" telah set job "YELLOW"%s(%d) - %s", GetAdminName(playerid), AccountData[otherid][pName], otherid, GetPlayerJobName(otherid));
	SendClientMessageEx(otherid, X11_ARWIN, "[AdmCmd] "RED"%s"ARWIN1" telah menetapkan pekerjaan"YELLOW" %s "ARWIN1"untuk anda", GetAdminName(playerid), GetPlayerJobName(otherid));
	return 1;
}
...
CMD:clearallchat(playerid)
{
	...
	SendStaffMessage(X11_ARWIN, ""RED"%s"ARWIN1" menghapus semua chatlog", AccountData[playerid][pAdminname]);
	return 1;
}
...
CMD:checkucp(playerid, params[])
{
	...
	new query[596];
	mysql_format(dcrp_query, query, sizeof(query), "SELECT `Char_Name` FROM `player_characters` WHERE `Char_UCP`='%e'", name);
	mysql_tquery(dcrp_query, query, "CheckUCP", "ds", playerid, name);
	return 1;
}
...
CMD:fixes(playerid, params[])
{
	...
	if(count == 0)
	{
		...
		return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Fix Request",
		"Tidak ada player request fix untuk saat ini!", "Tutup", "");
	}

	ShowPlayerDialog(playerid, DIALOG_FIXMEACC, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE" - Fix Request", list, "Pilih", "Batal");
	return 1;
}
...
public OfflineStats(playerid, const name[])
{
	...
	else 
	{
		...
		mysql_format(dcrp_query, query, sizeof(query), "SELECT * FROM `families` WHERE `F_ID`=%d", Fams);
		CheckFamilies = mysql_query(dcrp_query, query);
		...
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Offline Stats", shstr, "Tutup", "");
