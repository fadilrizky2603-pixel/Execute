        ...
        switch(g_AsuransiTime)
        {
			case 0:
            {
				...
				foreach(new b : Player)
				{
					forex(a, 7)
					{
						PlayerTextDrawHide(b, InsuranceTD[b][a]);
					}
				}
            ...
            default:
			{
				...
				{
					format(str, sizeof(str), "%d Menit %02d Detik", times[1], times[2]);
					PlayerTextDrawSetString(i, InsuranceTD[i][6], str);
				}
...
ptask PlayerDelay[1000](playerid)
{
	...
	if(AccountData[playerid][pGunLic] > 0)
	{
		if(AccountData[playerid][pGunLicTime] != 0 && AccountData[playerid][pGunLicTime] <= timejir)
		{
			...
			Info(playerid, "Masa berlaku Lisensi Senjata anda sudah habis, silakan perpanjang ke Kepolisian DCRP!");
		}
	}
	if(AccountData[playerid][pHuntingLic] > 0)
	{
		if(AccountData[playerid][pHuntingLicTime] != 0 && AccountData[playerid][pHuntingLicTime] <= timejir)
		{
			...
			Info(playerid, "Masa berlaku Lisensi Hunting anda sudah habis, silakan perpanjang ke Pemerintahan DCRP!");
		}
	}
	// Sim Expired
	if(AccountData[playerid][pSimA] > 0)
	{
		if(AccountData[playerid][pSimATime] != 0 && AccountData[playerid][pSimATime] <= timejir)
		{
			...
			SendClientMessageEx(playerid, -1, ""YELLOW"INFORMATION:"WHITE" Masa Waktu Sim A Anda sudah tidak berlaku, anda bisa memperpanjangnya di Kepolisian DCRP");
		}
	}
	if(AccountData[playerid][pSimB] > 0)
	{
		if(AccountData[playerid][pSimBTime] != 0 && AccountData[playerid][pSimBTime] <= timejir)
		{
			...
			SendClientMessageEx(playerid, -1, ""YELLOW"INFORMATION:"WHITE" Masa Waktu Sim B Anda sudah tidak berlaku, anda bisa memperpanjangnya di Kepolisian DCRP");
		}
	}
	if(AccountData[playerid][pSimC] > 0)
	{
		if(AccountData[playerid][pSimCTime] != 0 && AccountData[playerid][pSimCTime] <= timejir)
		{
			...
			SendClientMessageEx(playerid, -1, ""YELLOW"INFORMATION:"WHITE" Masa Waktu Sim C Anda sudah tidak berlaku, anda bisa memperpanjangnya di Kepolisian DCRP");
		}
	}
	...
	if(AccountData[playerid][pSKS] > 0)
	{
		...
		{
			Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Surat Keterangan Sehat", "Masa aktif Surat Keterangan Sehat Anda Sudah Habis\
			\nAnda diharuskan untuk memperpanjang ke Mitra EMS DCRP Jika berkenan", "Tutup", "");
	...
	if(AccountData[playerid][pSKCK] > 0)
	{
		...
		{
			Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Surat Keterangan Cacatan Kepolisian", "Masa aktif Surat Keterangan Cacatan Kepolisian/SKCK Anda Sudah Habis\
			\nAnda diharuskan untuk memperpanjang ke Kepolisian DCRP Jika berkenan", "Tutup", "");
	...
	if(AccountData[playerid][pBPJS] > 0)
	{
		...
		{
			Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Kartu DCRP Sehat", "Masa aktif Kartu DCRP Sehat/BPJS Anda Sudah Habis\
			\nAnda diharuskan untuk memperpanjang ke Mitra EMS DCRP Jika berkenan", "Tutup", "");
	...
	if(AccountData[playerid][pWarn] >= 20)
	{
		...
		mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO player_bans(name, ip, admin, reason, ban_date, ban_expire) VALUES ('%s', '%s', 'Server Ban', '20 Total Warning', %i, '0')", playerName, PlayerIP, timejir);
