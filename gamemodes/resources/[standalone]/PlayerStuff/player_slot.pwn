...
hook OnGameModeInitEx()
{
	...
	VerText = TextDrawCreate(194.0, SLOT_MACHINE_POS_Y-21, "~r~DCRP ~w~SLOT MACHINE");
...
PutPlayerInSlotMachine(playerid,  firstBet=MIN_BET,   startBalance=0)
{
	...
	ItemDelete(playerid, "Chip", startBalance);
	ShowItemBox(playerid, "Chip", sprintf("Removed_%dx", startBalance), 2);
...
ExitPlayerFromSlotMachine(playerid)
{
    ...
    if(Balance[playerid] > 0) 
	{
		...
		ItemAdd(playerid, "Chip", Balance[playerid]);
	}
...
CMD:changebet(playerid, params[])
{
	if(Gambling[playerid] == G_STATE_READY)
	{
		Dialog_Show(playerid, ChangeBetSpin, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Change Bet", 
...
CMD:cvc(playerid, params[])
{
	...
	ItemDelete(otherid, "Chip", value);
	ShowItemBox(otherid, "Chip", sprintf("Removed_%dx", value), 2);
...
Dialog:ChangeBetSpin(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		...
		if(bet < MIN_BET) return Dialog_Show(playerid, ChangeBetSpin, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Change Bet", 
		""WHITE"ERROR: Minimal Bet $10!\n"WHITE"Bet anda saat ini: "GREEN"%s\n\n"WHITE"Mohon masukkan berapa jumlah yang ingin anda pasang dalam taruhan ini:\nMin Bet: "GREEN"$10\n"WHITE"Max Bet: "GREEN"$10",
		"Bet", "Cancel", FormatMoney(Bet[playerid]));

		if(bet > MAX_BET) return Dialog_Show(playerid, ChangeBetSpin, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Change Bet", 
