...
Button_Rebuild(id)
{
    if(id != -1)
    {
        ...
        {
            ButtonData[id][DoorObjectID] = CreateDynamicObject(ButtonData[id][DoorModelID], ButtonData[id][DoorcPos][0], ButtonData[id][DoorcPos][1], ButtonData[id][DoorcPos][2], ButtonData[id][DoorDCRPos][0], ButtonData[id][DoorDCRPos][1], ButtonData[id][DoorDCRPos][2], ButtonData[id][bWorld], ButtonData[id][bInterior], -1);
        }
    }
    return 1;
}
...
{
    new query[1218];
    mysql_format(dcrp_query, query, sizeof(query), "UPDATE `buttons` SET `DoorModel`=%d, `Faction`=%d, `Faction2`=%d, `Family`=%d, `Owner`=%d, `Speed`=%f, `ButtonX`=%f, `ButtonY`=%f, `ButtonZ`=%f, `ButtonRX`=%f, `ButtonRY`=%f, `ButtonRZ`=%f,\
    ...
    ButtonData[id][DoorcPos][0], ButtonData[id][DoorcPos][1], ButtonData[id][DoorcPos][2], ButtonData[id][DoorDCRPos][0], ButtonData[id][DoorDCRPos][1], ButtonData[id][DoorDCRPos][2], ButtonData[id][DooroPos][0], ButtonData[id][DooroPos][1], ButtonData[id][DooroPos][2], ButtonData[id][DooroRPos][0], ButtonData[id][DooroRPos][1], ButtonData[id][DooroRPos][2],
...
public LoadButtons()
{
    ...
    if(rows)
    {
        for(new i = 0; i < rows; i ++)
        {
            ...
            cache_get_value_name_float(i, "DoorCRX", ButtonData[id][DoorDCRPos][0]);
            cache_get_value_name_float(i, "DoorCRY", ButtonData[id][DoorDCRPos][1]);
            cache_get_value_name_float(i, "DoorCRZ", ButtonData[id][DoorDCRPos][2]);
...
CMD:addbutton(playerid, params[])
{
    ...
    new query[596];
    mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `buttons` SET `ID`=%d, `ButtonX`=%f, `ButtonY`=%f, `ButtonZ`=%f, `ButtonRX`=%f, `ButtonRY`=%f, `ButtonRZ`=%f, `Faction`=0, `Faction2`=0, `Family`=-1, `World`=%d, `Interior`=%d",
    ...
    mysql_tquery(dcrp_query, query, "OnButtonCreated", "dd", playerid, id);
    return 1;
}
...
CMD:removebutton(playerid, params[])
{
    ...
    ButtonData[id][DoorDCRPos][0] = ButtonData[id][DoorDCRPos][1] = ButtonData[id][DoorDCRPos][2] = 0.0;
    ...
    new query[255];
    mysql_format(dcrp_query, query, sizeof(query), "DELETE FROM `buttons` WHERE `ID`=%d", id);
    mysql_tquery(dcrp_query, query);
...
public PressButon(playerid, buttonid)
{
    ...
    else
    {
        ...
        SetDynamicObjectRot(ButtonData[buttonid][DoorObjectID], ButtonData[buttonid][DoorDCRPos][0], ButtonData[buttonid][DoorDCRPos][1], ButtonData[buttonid][DoorDCRPos][2]);
    }
    return 1;
}
    ...
    if(AccountData[playerid][bEditID] != -1 && Iter_Contains(Buttons, AccountData[playerid][bEditID]))
    {
        ...
        else if(response == EDIT_RESPONSE_FINAL)
        {
            ...
            else if(AccountData[playerid][bEdit] == 2) // close
            {
                ...
                ButtonData[id][DoorDCRPos][0] = rx;
                ButtonData[id][DoorDCRPos][1] = ry;
                ButtonData[id][DoorDCRPos][2] = rz;
                Button_Rebuild(id);
