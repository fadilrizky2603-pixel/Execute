...
CMD:adddeer(playerid, params[])
{
    ...
    mysql_format(dcrp_query, twsmk, sizeof(twsmk), "INSERT INTO `hunting` SET `DeerID`=%d, `DeerX`=%f, `DeerY`=%f, `DeerZ`=%f, `DeerRX`=%f, `DeerRY`=%f, `DeerRZ`=%f, \
    ...
    mysql_tquery(dcrp_query, twsmk, "OnDeerCreated", "dd", playerid, id);
    return 1;
}
...
CMD:removedeer(playerid, params[])
{
    ...
    mysql_format(dcrp_query, icsr, sizeof(icsr), "DELETE FROM `hunting` WHERE `DeerID`=%d", id);
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        ...
        if(id > -1) 
        {
            if(IsPlayerInRangeOfPoint(playerid, 1.5, HuntData[id][DeerPOS][0], HuntData[id][DeerPOS][1], HuntData[id][DeerPOS][2]))
            {
                if(HuntData[id][DeerShoot])
                {
                    ...
                    PlayerTextDrawSetString(playerid, ProgressBar[playerid][1], "MEMOTONG RUSA");
        ...
        if(IsPlayerInRangeOfPoint(playerid, 2.5, -1692.5179, -88.5486, 3.5670))
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_HUNTING_SELL, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Jual Hasil Buru",
            shstr, "Jual", "Batal");
        }
    }
    return 1;
        ...
        case DIALOG_HUNTING_SELL:
        {
            ...
            switch(listitem)
            {
                case 0: //daging
                {
                    ...
                    ItemDelete(playerid, "Daging", value);
                    ShowItemBox(playerid, "Uang", sprintf("Recieved_$%s", FormatMoney((value*15))), 2);
                ...
                case 1: // tandu
                {
                    ...
                    ItemDelete(playerid, "Tanduk", value);
                    ShowItemBox(playerid, "Uang", sprintf("Recieved_$%s", FormatMoney((value*20))), 2);
                ...
                case 2: // Kulit
                {
                    ...
                    GiveMoney(playerid, (value*20));
                    ItemDelete(playerid, "Kulit", value);
                    ShowItemBox(playerid, "Uang", sprintf("Recieved_$%s", FormatMoney((value*20))), 2);
...
task HuntingUpdate[1000]()
{
    ...
    foreach (new i : Hunt)
    {
        ...
        if (HuntData[i][DeerTime] == 0)
        {
            ...
            mysql_format(dcrp_query, icsr, sizeof(icsr), "UPDATE `hunting` SET `DeerTime`=0 WHERE `DeerID`=%d", i);
            mysql_tquery(dcrp_query, icsr);
        }
        else
        {
...
public ProcessRusa(playerid)
{
    ...
    if(AccountData[playerid][ActivityTime] >= 10)
    {
        ...
        switch(rand)
        {
            case 0..2:
            {
                ItemAdd(playerid, "Daging");
                ShowItemBox(playerid, "Daging", "Received_1x", 2);
            }
            case 3..5:
            {
                ItemAdd(playerid, "Kulit");
                ShowItemBox(playerid, "Kulit", "Received_1x", 2);
            }
            case 6..8:
            {
                ItemAdd(playerid, "Tanduk");
                ShowItemBox(playerid, "Tanduk", "Received_1x", 2);
            }
            case 9..10:
            {
                new randkulit = RandomEx(1, 5);
                ItemAdd(playerid, "Kulit", randkulit);
                ShowItemBox(playerid, "Kulit", sprintf("Received_%dx", randkulit), 2);
            }
        }
