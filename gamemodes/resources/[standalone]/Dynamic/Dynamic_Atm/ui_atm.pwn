...
CMD:addatm(playerid, params[])
{
	...
	mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO atms SET id='%d', posx='%f', posy='%f', posz='%f', posrx='%f', posry='%f', posrz='%f', interior='%d', world='%d'", id, AtmData[id][atmX], AtmData[id][atmY], AtmData[id][atmZ], AtmData[id][atmRX], AtmData[id][atmRY], AtmData[id][atmRZ], GetPlayerInterior(playerid), GetPlayerVirtualWorld(playerid));
	mysql_tquery(dcrp_query, query, "OnAtmCreated", "ii", playerid, id);
	return 1;
}
...
CMD:removeatm(playerid, params[])
{
	...
	mysql_format(dcrp_query, query, sizeof(query), "DELETE FROM atms WHERE id=%d", id);
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	switch(dialogid)
	{
		case DIALOG_ATM_TRANSFER:
		{
			...
			if(isnull(inputtext)) return ShowPlayerDialog(playerid, DIALOG_ATM_TRANSFER, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Fleeca Bank", 
			"Error: Harus diisi tidak dapat kosong!\nMohon masukkan nomor rekening yang ingin anda transfer:", "Submit", "Batal");

			if(!IsNumeric(inputtext)) return ShowPlayerDialog(playerid, DIALOG_ATM_TRANSFER, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Fleeca Bank", 
			...
			mysql_tquery(dcrp_query, query, "SearchRekening", "dd", playerid, strval(inputtext));
			return 1;	
		}
		case DIALOG_ATM_TRANSFER1:
		{
			...
			if(isnull(inputtext)) 
			{
				return ShowPlayerDialog(playerid, DIALOG_ATM_TRANSFER1, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Fleeca Bank", 
		...
		case DIALOG_ATM_DEPOSIT:
		{
			...
			if(isnull(inputtext))
			{
				ShowPlayerDialog(playerid, DIALOG_ATM_DEPOSIT, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Fleeca Bank", 
			...
			PlayerTextDrawSetString(playerid, ATMTD[playerid][64], sprintf("~g~%s", FormatMoney(AccountData[playerid][pBankMoney])));
			PlayerTextDrawSetString(playerid, ATMTD[playerid][84], sprintf("~g~%s~w~", FormatMoney(AccountData[playerid][pMoney])));
			PlayerTextDrawShow(playerid, ATMTD[playerid][64]);
			PlayerTextDrawShow(playerid, ATMTD[playerid][84]);
			ShowTDN(playerid, NOTIFICATION_SUKSES, sprintf("Berhasil deposit ke atm dengan jumlah ~r~%s", FormatMoney(value)));

			new query[200];
			mysql_format(dcrp_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_BankMoney`=%d, `Char_Money`=%d WHERE `pID`=%d", AccountData[playerid][pBankMoney], AccountData[playerid][pMoney], AccountData[playerid][pID]);
			mysql_tquery(dcrp_query, query);
		}
		case DIALOG_ATM_WITHDRAW:
		{
			...
			if(isnull(inputtext))
			{
				ShowPlayerDialog(playerid, DIALOG_ATM_WITHDRAW, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Fleeca Bank", 
			...
			PlayerTextDrawShow(playerid, ATMTD[playerid][64]);
			PlayerTextDrawShow(playerid, ATMTD[playerid][84]);
			ShowTDN(playerid, NOTIFICATION_SUKSES, sprintf("Berhasil withdraw dengan jumlah ~g~%s", FormatMoney(value)));
...
FUNC::SearchRekening(playerid, norek)
{
	...
	else 
	{
		...
		ShowPlayerDialog(playerid, DIALOG_ATM_TRANSFER1, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Fleeca Bank", "Mohon masukkan nominal uang yang ingin anda transfer:", "Submit", "Batal");
	}
	return 1;
}
