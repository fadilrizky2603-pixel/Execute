...
Gudang_Refresh(id)
{
    if(id != -1)
    {
        ...
        if(GudangData[id][gudangPOS][0] != 0.0 || GudangData[id][gudangPOS][1] != 0.0 || GudangData[id][gudangPOS][2] != 0.0)
        {
            ...
            format(frmtname, sizeof(frmtname), "| Gudang %s |\n[Tekan "GREEN"ALT"WHITE" untuk akses gudang]", GudangData[id][gudangName]);
...
CMD:addgudang(playerid, params[])
{
    ...
    new tss[596];
    mysql_format(dcrp_query, tss, sizeof(tss), "INSERT INTO `gudang` SET `GudangID`=%d, `GudangName`='%s', `GudangPrice`=%d, `GudangX`=%f, `GudangY`=%f, `GudangZ`=%f, `GudangInterior`=%d, `GudangWorld`=%d",
    ...
    mysql_tquery(dcrp_query, tss, "OnGudangCreated", "dd", playerid, id);
    return 1;
}
...
CMD:removegudang(playerid, params[])
{
	...
	mysql_format(dcrp_query, icsr, sizeof(icsr), "DELETE FROM `gudang` WHERE `GudangID` = %d", id);
        ...
        if(id > -1)
        {
            if(IsPlayerInRangeOfPoint(playerid, 2.0, GudangData[id][gudangPOS][0], GudangData[id][gudangPOS][1], GudangData[id][gudangPOS][2]))
            {
                if(AccountData[playerid][pHasGudangID] == id)
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_GUDANG_OPTION, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Gudang",
                ...
                else 
                {
                    ...
                    AccountData[playerid][pTempValue] = id;
                    ShowPlayerDialog(playerid, DIALOG_GUDANG_BUY, DIALOG_STYLE_LIST, sprintf(""PINK1"DCRP"WHITE"- Gudang %s", GudangData[id][gudangName]),
                    shstr, "Pilih", "Batal");
                }
            }
        }
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == DIALOG_GUDANG_BUY)
    {
        ...
        switch(listitem)
        {
            case 0: //sewa
            {
                ...
                new icsr[255];
                mysql_format(dcrp_query, icsr, sizeof(icsr), "UPDATE `player_characters` SET `Char_HasGudangID`=%d WHERE `pID`=%d", AccountData[playerid][pHasGudangID], AccountData[playerid][pID]);
                mysql_tquery(dcrp_query, icsr);
    ...
    else if(dialogid == DIALOG_GUDANG_OPTION)
    {
        ...
        switch(listitem)
        {
            ...
            {
                AccountData[playerid][menuShowed] = true;
                ShowPlayerDialog(playerid, DIALOG_GUDANGVAULT, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Gudang",
                "Simpan Barang\n"GRAY"Ambil Barang", "Pilih", "Batal");
            }
            case 1: //berhenti sewa
            {
                Dialog_Show(playerid, GUDANG_STOPCONF, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP", 
            ...
            case 2: //status sewa
            {
                ...
                if(gdid != -1)
                {
                    ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, sprintf(""PINK1"DCRP"WHITE"- %s", GudangData[gdid][gudangName]),
                    shstr, "Tutup", "");
                }
            }
        }
        ...
        switch(listitem)
        {
            case 0: //deposit
            {
                ...
                if(rows)
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_GUDANGVAULT_DEPOSIT, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Gudang", str, "Pilih", "Batal");
                }
                else
                {
                    PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
                    return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Gudang",
            ...
            case 1: //withdraw
            {
                ...
                mysql_format(dcrp_query, jss, sizeof(jss), "SELECT * FROM `player_gudang` WHERE `ID`=%d", AccountData[playerid][pID]);
                mysql_query(dcrp_query, jss);
                if(cache_num_rows() > 0)
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_GUDANGVAULT_WITHDRAW, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Gudang", shstr, "Pilih", "Batal");
                }
                else 
                {
                    return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Gudang",
    ...
    else if(dialogid == DIALOG_GUDANGVAULT_DEPOSIT)
    {
        ...
        if(cache_num_rows() > 0)
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_GUDANGVAULT_IN, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Gudang", shstr, "Input", "Batal");
        }
    }
    else if(dialogid == DIALOG_GUDANGVAULT_IN)
    {
        ...
        if(isnull(inputtext))
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_GUDANGVAULT_IN, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Gudang", 
            shstr, "Input", "Batal");
            return 1;
        }
        ...
        ItemDelete(playerid, GudangBrankas[playerid][gudangBrankasTemp], quantity);
        ShowItemBox(playerid, GudangBrankas[playerid][gudangBrankasTemp], sprintf("Removed_%dx", quantity), 2);

        new invstr[1028];
        mysql_format(dcrp_query, shstr, sizeof(shstr), "SELECT * FROM `player_gudang` WHERE `ID`=%d AND `itemName`='%s'", AccountData[playerid][pID], GudangBrankas[playerid][gudangBrankasTemp]);
        mysql_query(dcrp_query, shstr);
        if(cache_num_rows() > 0)
        {
            mysql_format(dcrp_query, invstr, sizeof(invstr), "UPDATE `player_gudang` SET `itemQuantity` = `itemQuantity` + %d WHERE `ID`=%d AND `itemName`='%s'", quantity, AccountData[playerid][pID], GudangBrankas[playerid][gudangBrankasTemp]);
            mysql_tquery(dcrp_query, invstr, "OnGudangDeposit", "d", playerid);
        }
        else 
        {
            mysql_format(dcrp_query, invstr, sizeof(invstr), "INSERT INTO `player_gudang` SET `ID`=%d, `itemName`='%s', `itemModel`=%d, `itemQuantity`=%d", AccountData[playerid][pID], GudangBrankas[playerid][gudangBrankasTemp], GudangBrankas[playerid][gudangBrankasModel], quantity);
            mysql_tquery(dcrp_query, invstr, "OnGudangDeposit", "d", playerid);
        }
    }
    else if(dialogid == DIALOG_GUDANGVAULT_WITHDRAW)
    {
        ...
        mysql_format(dcrp_query, tss, sizeof(tss), "SELECT * FROM `player_gudang` WHERE `ID`=%d", AccountData[playerid][pID]);
        mysql_query(dcrp_query, tss);
        if(cache_num_rows() > 0)
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_GUDANGVAULT_OUT, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Gudang",
            shstr, "Input", "Batal");
        }
    }
    else if(dialogid == DIALOG_GUDANGVAULT_OUT)
    {
        ...
        {
            mysql_format(dcrp_query, jts, sizeof(jts), "UPDATE `player_gudang` SET `itemQuantity`=%d WHERE `itemID`=%d", GudangBrankas[playerid][gudangBrankasQuant], GudangBrankas[playerid][gudangBrankasID]);
            mysql_tquery(dcrp_query, jts);
        }
        else
        {
            mysql_format(dcrp_query, jts, sizeof(jts), "DELETE FROM `player_gudang` WHERE `itemID`=%d", GudangBrankas[playerid][gudangBrankasID]);
            mysql_tquery(dcrp_query, jts);
        }
        ItemAdd(playerid, GudangBrankas[playerid][gudangBrankasTemp], quantity);
        ReduceCapacityGudang(playerid, GudangBrankas[playerid][gudangBrankasTemp], quantity);
        ShowItemBox(playerid, GudangBrankas[playerid][gudangBrankasTemp], sprintf("Received_%dx", quantity), 2);
...
Dialog:GUDANG_STOPCONF(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        new tss[255];
        mysql_format(dcrp_query, tss, sizeof(tss), "UPDATE `player_characters` SET `Char_HasGudangID`=-1, `Char_GudangRentTime`=0 WHERE `pID`=%d", AccountData[playerid][pID]);
        mysql_tquery(dcrp_query, tss);
    }
    else
    {
        return ShowPlayerDialog(playerid, DIALOG_GUDANG_OPTION, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Gudang",
        "Buka Gudang\n"GRAY"Berhenti menyewa gudang\nCek status sewa", "Pilih", "Batal");
    }
    return 1;
}
...
AddCapacityGudang(playerid, const item[], quantity)
{
    ...
    {
        AccountData[playerid][pGudangCapacity] += quantity*0.040;
    }
    ...
    {
        AccountData[playerid][pGudangCapacity] += quantity*0.100;
    }
    ...
    {
        AccountData[playerid][pGudangCapacity] += quantity*0.45;
    }
    ...
    {
        AccountData[playerid][pGudangCapacity] += quantity*0.500;
    }
    ...
    new sctr[596];
    mysql_format(dcrp_query, sctr, sizeof(sctr), "UPDATE `player_characters` SET `Char_GudangStorage`=%f WHERE `pID`=%d", AccountData[playerid][pGudangCapacity], AccountData[playerid][pID]);
