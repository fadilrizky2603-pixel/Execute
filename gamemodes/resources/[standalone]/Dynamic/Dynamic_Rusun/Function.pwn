...
JadenDCRP: OnRusunsDataCreated(id) {
...
RusunRefresh(id)
{
    ...
    if (RusunsData[id][rOwnerID] > 0)
    {
        ...
        RusunsData[id][rLabel] = CreateDynamic3DTextLabel("[Tekan "GREEN"ALT"WHITE" untuk membuka menu]", -1, RusunsData[id][rExtPos][0], RusunsData[id][rExtPos][1], RusunsData[id][rExtPos][2] + 0.35, 2.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, RusunsData[id][rVW], RusunsData[id][rInterior], -1, 2.0, -1, 0);
    }
    else
    {
        RusunsData[id][rPickup] = CreateDynamicPickup(19524, 23, RusunsData[id][rExtPos][0], RusunsData[id][rExtPos][1], RusunsData[id][rExtPos][2], RusunsData[id][rVW], RusunsData[id][rInterior], -1, 2.0);
    ...
    RusunsData[id][rIntLabel] = CreateDynamic3DTextLabel(""RED"[ALT]"WHITE" Keluar", -1, RusunsData[id][rIntPos][0], RusunsData[id][rIntPos][1], RusunsData[id][rIntPos][2] + 0.35, 2.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1);
    return 1;
}
...
Rusun_Create(playerid, price, name[])
{
    ...
    if ((id = Iter_Free(RusunDatas)) != -1) {
        ...
        mysql_tquery(dcrp_query, cQuery, "OnRusunsDataCreated", "d", id);
...
Rusun_Delete(id) {
    if (Iter_Contains(RusunDatas, id)) {
        ...
        mysql_tquery(dcrp_query, sprintf("DELETE FROM `rusuns` WHERE `ID` = '%d'", RusunsData[id][rID]));
...
JadenDCRP: OnRusunDepo(playerid)
{
...
JadenDCRP: EnterToRusun(playerid, id)
{
    if (!Iter_Contains(RusunDatas, id))
        return 0;
