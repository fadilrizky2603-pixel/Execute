...
RemainingTime(playerid)
{
    ...
    if(rsid != -1)
    {
        ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, sprintf(""PINK1"DCRP"WHITE"- %s", RusunData[rsid][rusunName]),
        shstr, "Tutup", "");
    }
    return 1;
}
...
{
    new query[1218];
    mysql_format(dcrp_query, query, sizeof(query), "UPDATE `dynamic_rusun` SET `RusunOwner`='%s', `RusunOwnerID`=%d, `RusunPrice`=%d, `RusunName`='%s', `RusunInterior`=%d, `RusunExtInterior`=%d, `RusunWorld`=%d,\
...
Rusun_Refresh(id)
{
    if(id != -1)
    {
        ...
        if(RusunData[id][rusunExtPos][0] != 0.0)
        {
            ...
            RusunData[id][rusunExtLabel] = CreateDynamic3DTextLabel("[Tekan "GREEN"ALT"WHITE" untuk membuka menu]", -1, RusunData[id][rusunExtPos][0], RusunData[id][rusunExtPos][1], RusunData[id][rusunExtPos][2] + 0.35, 2.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, RusunData[id][rusunWorld], RusunData[id][rusunInterior], -1, 2.0, -1, 0);
        }

        if(RusunData[id][rusunIntPos][0] != 0.0)
        {
            RusunData[id][rusunIntLabel] = CreateDynamic3DTextLabel(""RED"[ALT]"WHITE" Keluar", -1, RusunData[id][rusunIntPos][0], RusunData[id][rusunIntPos][1], RusunData[id][rusunIntPos][2] + 0.35, 2.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1);
        }
    }
    return 1;
}

JadenDCRP: Rusun_Load()
{
...
CMD:addrusun(playerid, params[])
{
    ...
    new query[596];
    mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `dynamic_rusun` SET `rID`=%d, `RusunOwner`='%s', `RusunOwnerID`=%d, `RusunPrice`=%d, `RusunName`='%s', `ExtPosX`=%f, `ExtPosY`=%f, `ExtPosZ`=%f, `ExtPosA`=%f,\
    ...
    mysql_tquery(dcrp_query, query, "OnRusunCreated", "dd", playerid, id);
    return 1;
}
...
CMD:editrusun(playerid, params[])
{
    ...
    else if(!strcmp(option, "reset", true))
    {
        ...
        mysql_tquery(dcrp_query, sprintf("UPDATE `player_characters` SET `Char_HasRusunID`=-1 WHERE `pID`=%d", RusunData[id][rusunOwnerID]));
        Rusun_Reset(id);
        Rusun_Refresh(id);
        Rusun_Save(id);
    ...
    else if(!strcmp(option, "delete", true))
    {
        ...
        new query[178];
        mysql_format(dcrp_query, query, sizeof(query), "DELETE FROM `dynamic_rusun` WHERE `rID`=%d", id);
        mysql_tquery(dcrp_query, query);
    }
    return 1;
}
...
hook OnGameModeInit()
{
    ...
    CreateDynamic3DTextLabel("[Tekan "GREEN"ALT"WHITE" untuk membuka brankas]", -1, 2234.7104, -1106.1246, 1050.8828, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1);
}
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        ...
        {
            ShowPlayerDialog(playerid, DIALOG_RUSUN_BRANKAS, DIALOG_STYLE_LIST, sprintf(""PINK1"DCRP"WHITE"- %s", RusunData[AccountData[playerid][pInRusun]][rusunName]),
            "Undang\
            \n"GRAY"Pakaian\
            \nMembuang Pakaian\
        ...
        if(rsid != -1)
        {
            if(RusunData[rsid][rusunOwnerID] != 0)
            {
                ...
                else
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_RUSUN_OWNED, DIALOG_STYLE_LIST, sprintf(""PINK1"DCRP"WHITE"- %s", RusunData[rsid][rusunName]), 
                    "Masuk\
            ...
            else
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_RUSUN, DIALOG_STYLE_TABLIST, sprintf(""PINK1"DCRP"WHITE"- %s", RusunData[rsid][rusunName]),
                srss, "Pilih", "Batal");
            }
        }
    }
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    else if(dialogid == DIALOG_RUSUN_BRANKAS)
    {
        ...
        switch(listitem)
        {
            case 0:
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_RUSUN_INVITE, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Undang Teman", frmxt, "Pilih", "Batal");
            }
            case 1:
            {
            ...
            case 3:
            {
                ShowPlayerDialog(playerid, DIALOG_RUSUN_BROPTION, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Brankas",
        ...
        switch(listitem)
        {
            case 0: //deposit
            {
                ...
                if(cache_num_rows() > 0)
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_RUSUNVAULT_DEPOSIT, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Brankas", str, "Pilih", "Batal");
                }
                else 
                {
                    PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
                    return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Brankas",
            ...
            case 1: //withdraw
            {
                ...
                format(str, sizeof(str), "Nama Item\tJumlah\tBerat (%.3f/80kg)\n", AccountData[playerid][pRusunCapacity]);
                mysql_format(dcrp_query, icsr, sizeof(icsr), "SELECT * FROM `player_rusunstorage` WHERE `ID`=%d", AccountData[playerid][pID]);
                ...
                if(cache_num_rows() > 0)
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_RUSUNVAULT_WITHDRAW, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Brankas", str, "Pilih", "Batal");
                }
                else 
                {
                    return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Brankas",
    ...
    else if(dialogid == DIALOG_RUSUN_INVITE)
    {
        ...
        if(rsid > -1)
        {
            ...
            ShowPlayerDialog(targetid, DIALOG_RUSUN_INVITECONF, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Undang Teman",
    ...
    else if(dialogid == DIALOG_RUSUNVAULT_DEPOSIT)
    {
        ...
        if(cache_num_rows() > 0)
        {
            if(listitem >= 0 && listitem < cache_num_rows())
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_RUSUNVAULT_IN, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas",
                shstr, "Input", "Batal");
            }
        }
    }
    else if(dialogid == DIALOG_RUSUNVAULT_IN)
    {
        ...
        ItemDelete(playerid, RusunBrankas[playerid][rusunbrankasTemp], quantity);
        ShowItemBox(playerid, RusunBrankas[playerid][rusunbrankasTemp], sprintf("Removed_%dx", quantity), 2);
        AddCapacityRusun(playerid, RusunBrankas[playerid][rusunbrankasTemp], quantity);

        new invstr[1218];
        mysql_format(dcrp_query, shstr, sizeof(shstr), "SELECT * FROM `player_rusunstorage` WHERE `ID`=%d AND `rsItemName`='%s'", AccountData[playerid][pID], RusunBrankas[playerid][rusunbrankasTemp]);
        mysql_query(dcrp_query, shstr);
        if(cache_num_rows() > 0)
        {
            mysql_format(dcrp_query, invstr, sizeof(invstr), "UPDATE `player_rusunstorage` SET `rsItemQuantity` = `rsItemQuantity` + %d WHERE `ID`=%d AND `rsItemName`='%s'", quantity, AccountData[playerid][pID], RusunBrankas[playerid][rusunbrankasTemp]);
            mysql_tquery(dcrp_query, invstr, "OnRusunDeposit", "d", playerid);
        }
        else
        {
            mysql_format(dcrp_query, invstr, sizeof(invstr), "INSERT INTO `player_rusunstorage` SET `ID`=%d, `rsItemName`='%s', `rsItemModel`=%d, `rsItemQuantity`=%d", AccountData[playerid][pID], RusunBrankas[playerid][rusunbrankasTemp], RusunBrankas[playerid][rusunbrankasModel], quantity);
            mysql_tquery(dcrp_query, invstr, "OnRusunDeposit", "d", playerid);
        }
    }
    else if(dialogid == DIALOG_RUSUNVAULT_WITHDRAW)
    {
        ...
        new tss[1218];
        mysql_format(dcrp_query, tss, sizeof(tss), "SELECT * FROM `player_rusunstorage` WHERE `ID`=%d", AccountData[playerid][pID]);
        mysql_query(dcrp_query, tss);
        if(cache_num_rows() > 0)
        {
            if(listitem >= 0 && listitem < cache_num_rows())
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_RUSUNVAULT_OUT, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas",
                shstr, "Input", "Batal");
            }
        }
    }
    else if(dialogid == DIALOG_RUSUNVAULT_OUT)
    {
        ...
        {
            mysql_format(dcrp_query, tss, sizeof(tss), "UPDATE `player_rusunstorage` SET `rsItemQuantity`=%d WHERE `rsItemID`=%d", RusunBrankas[playerid][rusunbrankasQuant], RusunBrankas[playerid][rusunbrankasID]);
            mysql_tquery(dcrp_query, tss);
        } 
        else 
        {
            mysql_format(dcrp_query, tss, sizeof(tss), "DELETE FROM `player_rusunstorage` WHERE `rsItemID`=%d", RusunBrankas[playerid][rusunbrankasID]);
            mysql_tquery(dcrp_query, tss);
        }
        ItemAdd(playerid, RusunBrankas[playerid][rusunbrankasTemp], quantity);
        ShowItemBox(playerid, RusunBrankas[playerid][rusunbrankasTemp], sprintf("Received_%dx", quantity), 2);
...
AddCapacityRusun(playerid, const item[], quantity)
{
    ...
    {
        AccountData[playerid][pRusunCapacity] += quantity*0.040;
    }
    ...
    {
        AccountData[playerid][pRusunCapacity] += quantity*0.100;
    }
    ...
    {
        AccountData[playerid][pRusunCapacity] += quantity*0.45;
    }
    ...
    {
        AccountData[playerid][pRusunCapacity] += quantity*0.500;
    }
    ...
    new sctr[596];
    mysql_format(dcrp_query, sctr, sizeof(sctr), "UPDATE `player_characters` SET `Char_RusunStorage`=%f WHERE `pID`=%d", AccountData[playerid][pRusunCapacity], AccountData[playerid][pID]);
    mysql_tquery(dcrp_query, sctr);
    return 1;
}
