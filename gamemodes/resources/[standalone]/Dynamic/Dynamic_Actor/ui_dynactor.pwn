...
stock Actor_Create(skin, Float:x, Float:y, Float:z, Float:a, world, interior, anim = 0, const name[] = "")
{
    ...
    if(id != INVALID_ITERATOR_SLOT)
    {
        ...
        Actor_Spawn(id);
        mysql_tquery(dcrp_query, "INSERT INTO `actors` (`Skin`) VALUES(0)", "OnActorCreated", "d", id);
		return id;
    }
...
stock Actor_Save(id)
{
    ...
    new query[1012];
	mysql_format(dcrp_query, query, sizeof(query), "UPDATE `actors` SET ");
	mysql_format(dcrp_query, query, sizeof(query), "%s`Skin` = '%d', ", query, aData[id][aSkin]);
	mysql_format(dcrp_query, query, sizeof(query), "%s`PosX` = '%f', ", query, aData[id][aPos][0]);
	mysql_format(dcrp_query, query, sizeof(query), "%s`PosY` = '%f', ", query, aData[id][aPos][1]);
	mysql_format(dcrp_query, query, sizeof(query), "%s`PosZ` = '%f', ", query, aData[id][aPos][2]);
	mysql_format(dcrp_query, query, sizeof(query), "%s`PosA` = '%f', ", query, aData[id][aPos][3]);
	mysql_format(dcrp_query, query, sizeof(query), "%s`Anim` = '%d', ", query, aData[id][aAnim]);
	mysql_format(dcrp_query, query, sizeof(query), "%s`World` = '%d', ", query, aData[id][aWorld]);
	mysql_format(dcrp_query, query, sizeof(query), "%s`Interior` = '%d', ", query, aData[id][aInterior]);
	mysql_format(dcrp_query, query, sizeof(query), "%s`Name` = '%s' ", query, aData[id][aName]);
	mysql_format(dcrp_query, query, sizeof(query), "%sWHERE `ID` = '%d'", query, aData[id][aID]);
...
stock Actor_Delete(id)
{
    if(Iter_Contains(DynamicActor, id))
    {
        mysql_tquery(dcrp_query, sprintf("DELETE FROM `actors` WHERE `ID` = '%d'", aData[id][aID]));
