...
{
    new cQuery[522];
    mysql_format(dcrp_query, cQuery, sizeof(cQuery), "UPDATE `uranium` SET `UraniumX`='%f', `UraniumY`='%f', `UraniumZ`='%f', `UraniumRX`='%f', `UraniumRY`='%f', `UraniumRZ`='%f', `Interior`=%d, `World`=%d, `Timer`=%d WHERE `ID`=%d",
...
CMD:adduranium(playerid, params[])
{
    ...
    new query[298];
    mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `uranium` (`ID`, `UraniumX`, `UraniumY`, `UraniumZ`, `UraniumRX`, `UraniumRY`, `UraniumRZ`, `Interior`, `World`, `Timer`) VALUES ('%d', '%f', '%f', '%f', '%f', '%f', '%f', '%d', '%d', '%d')",
    ...
    mysql_tquery(dcrp_query, query, "OnUraniumCreated", "ii", playerid, id);
    return 1;
}
...
CMD:edituranium(playerid, params[])
{
    ...
    if(!strcmp(option, "delete", true))
    {
        ...
        mysql_tquery(dcrp_query, sprintf("DELETE FROM `uranium` WHERE `ID`=%d", id));
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        ...
        if(id > -1 && UraniumData[id][UraniumArea] != STREAMER_TAG_AREA: INVALID_STREAMER_ID)
        {
            ...
            UraniumData[id][UraniumExists] = false;
            PlayerTextDrawSetString(playerid, ProgressBar[playerid][1], "MENGAMBIL URANIUM");
        ...
        if(IsPlayerInRangeOfPoint(playerid, 2.0, -953.216, -494.536, 25.960))
        {
            ...
            Dialog_Show(playerid, MixerUraniumConf, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Make Uranium",
        ...
        /*if(IsPlayerInRangeOfPoint(playerid, 2.0, -953.216, -494.536, 25.960))
        {
            ...
            AccountData[playerid][ActivityTime] = 1;
            PlayerTextDrawSetString(playerid, ProgressBar[playerid][1], "MENGOLAH URANIUM");
            ShowProgressBar(playerid);
...
public MixUranium(playerid)
{
    ...
    if(AccountData[playerid][ActivityTime] >= 15)
    {
        ...
        ItemDelete(playerid, "Uranium ACD", 5);
        ItemDelete(playerid, "Garam", 15);
        ItemDelete(playerid, "Air Mineral", 5);
        ItemDelete(playerid, "Botol");
        ItemAdd(playerid, "Uranium");
        ShowItemBox(playerid, "Uranium", "Received_1x", 2);
        ShowItemBox(playerid, "Uranium ACD", "Removed_5x", 2);
        ShowItemBox(playerid, "Garam", "Removed_15x", 2);
        ShowItemBox(playerid, "Air Mineral", "Removed_5x", 2);
...
public TakeUranium(playerid, id)
{
    ...
    if(AccountData[playerid][ActivityTime] >= 15)
    {
        ...
        ItemAdd(playerid, "Uranium ACD");
        ShowItemBox(playerid, "Uranium ACD", "Received_1x", 2);
...
{
    CreateDynamic3DTextLabel(""LIGHTGREY"[Uranium Lab]\n"YELLOW"[ALT]"WHITE" To Make Uranium", -1, -953.216, -494.536, 25.960, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 0, -1, 10.0, -1, 0);
}
