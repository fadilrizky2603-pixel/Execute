...
{
    new query[1218];
    mysql_format(dcrp_query, query, sizeof(query), "UPDATE `iconmaps` SET `IconX`=%f, `IconY`=%f, `IconZ`=%f, `IconID`=%d, `IconInterior`=%d, `IconWorld`=%d WHERE `ID`=%d",
...
CMD:addicon(playerid, params[])
{
    ...
    new shstr[596];
    mysql_format(dcrp_query, shstr, sizeof(shstr), "INSERT INTO `iconmaps` SET `ID`=%d, `IconX`=%f, `IconY`=%f, `IconZ`=%f, `IconID`=%d, `IconInterior`=%d, `IconWorld`=%d",
    ...
    mysql_tquery(dcrp_query, shstr, "OnIconAdded", "dd", playerid, id);
    return 1;
}
...
CMD:editicon(playerid, params[])
{
    ...
    else if(!strcmp(type, "delete", true))
    {
        ...
        new query[200];
        mysql_format(dcrp_query, query, sizeof(query), "DELETE FROM `iconmaps` WHERE `ID`=%d", id);
