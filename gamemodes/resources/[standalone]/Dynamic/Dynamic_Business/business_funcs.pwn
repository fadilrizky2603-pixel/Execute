...
CMD:createbisnis(playerid, params[])
{
	...
	mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO bisnis SET ID='%d', owner='%s', price='%d', type='%d', extposx='%f', extposy='%f', extposz='%f', extposa='%f', name='%s'", bid, BisnisData[bid][bOwner], BisnisData[bid][bPrice], BisnisData[bid][bType], BisnisData[bid][bExtposX], BisnisData[bid][bExtposY], BisnisData[bid][bExtposZ], BisnisData[bid][bExtposA], BisnisData[bid][bName]);
	mysql_tquery(dcrp_query, query, "OnBisnisCreated", "i", bid);
	return 1;
}
...
CMD:editbisnis(playerid, params[])
{
	...
	else if(!strcmp(type, "delete", true))
    {
		...
		new query[128];
		mysql_format(dcrp_query, query, sizeof(query), "DELETE FROM bisnis WHERE ID=%d", bid);
	...
	if((newkeys & KEY_SECONDARY_ATTACK) && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
	{
		foreach(new bid : Bisnis)
		{
			if(IsPlayerInRangeOfPoint(playerid, 2.8, BisnisData[bid][bExtposX], BisnisData[bid][bExtposY], BisnisData[bid][bExtposZ]))
			{
				...
				if(BisnisData[bid][bLocked])
					return ShowPlayerFooter(playerid, "~w~Biz ~r~Terkunci!", 1000);
