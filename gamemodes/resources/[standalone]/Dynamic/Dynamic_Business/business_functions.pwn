...
{
    new cQuery[1048];
    mysql_format(dcrp_query, cQuery, sizeof(cQuery), "UPDATE `business` SET `Owner`='%s', ",  BisnisData[id][bizOwner]);
    mysql_format(dcrp_query, cQuery, sizeof(cQuery), "%s`Name`='%s', " cQuery, BisnisData[id][bizName]);
    mysql_format(dcrp_query, cQuery, sizeof(cQuery), "%s`Type`='%d', ", cQuery, BisnisData[id][bizType]);
    mysql_format(dcrp_query, cQuery, sizeof(cQuery), "%s`Locked`='%d', ", cQuery, BisnisData[id][bizLocked]);
    mysql_format(dcrp_query, cQuery, sizeof(cQuery), "%s`Money`='%d', ", cQuery, BisnisData[id][bizMoney]);
    mysql_format(dcrp_query, cQuery, sizeof(cQuery), "%s`PointX`='%f', ", cQuery, BisnisData[id][bizPoint][0]);
    mysql_format(dcrp_query, cQuery, sizeof(cQuery), "%s`PointY`='%f', ", cQuery, BisnisData[id][bizPoint][1]);
    mysql_format(dcrp_query, cQuery, sizeof(cQuery), "%s`PointZ`='%f', ", cQuery, BisnisData[id][bizPoint][2]);
    mysql_format(dcrp_query, cQuery, sizeof(cQuery), "%s`Visit`='%d', ", cQuery, BisnisData[id][bizVisit]);
    mysql_format(dcrp_query, cQuery, sizeof(cQuery), "%s`ExtVW`='%d', ", cQuery, BisnisData[id][bizExtVw]);
    mysql_format(dcrp_query, cQuery, sizeof(cQuery), "%s`ExtInt`='%d' ", cQuery, BisnisData[id][bizExtInt]);
    mysql_format(dcrp_query, cQuery, sizeof(cQuery), "%sWHERE `ID` = '%d'", cQuery, id);
...
Bisnis_Refresh(id)
{
    if(id != -1)
    {
        ...
        {
            format(sha, sizeof(sha), ""LIGHTGREY"[%s]\n"YELLOW"Unknows\n"WHITE"Tekan "GREEN"[ALT]"WHITE" untuk Membeli", type);
        }
        else
        {
            format(sha, sizeof(sha), ""LIGHTGREY"[%s]\n"YELLOW"%s\n"WHITE"%s\nTekan "GREEN"[ALT]"WHITE" untuk Membeli", type, BisnisData[id][bizName], BisnisData[id][bizOwner]);
        }
