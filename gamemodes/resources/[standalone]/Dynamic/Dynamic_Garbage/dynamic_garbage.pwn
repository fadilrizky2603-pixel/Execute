...
{
    new query[596];
    mysql_format(dcrp_query, query, sizeof(query), "UPDATE `trash` SET `Model`=%d, `Cooldown`=%d, `PosX`=%f, `PosY`=%f, `PosZ`=%f, `RotX`=%f, `RotY`=%f, `RotZ`=%f, `Interior`=%d, `World`=%d WHERE `ID`=%d",
...
CMD:addtrash(playerid, params[])
{
    ...
    new query[598];
    mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `trash` SET `ID`=%d, `Model`=%d, `Cooldown`=%d, `PosX`=%f, `PosY`=%f, `PosZ`=%f, `RotX`=%f, `RotY`=%f, `RotZ`=%f,\
    ...
    mysql_tquery(dcrp_query, query, "OnTrashCreated", "d", id);
    return 1;
}
...
CMD:edittrash(playerid, params[])
{
    ...
    else if(!strcmp(option, "delete", true))
    {
        ...
        new query[125];
        mysql_format(dcrp_query, query, sizeof(query), "DELETE FROM `trash` WHERE `ID`=%d", id);
        mysql_tquery(dcrp_query, query); 
    }
    return 1;
}
...
public TakingTrash(playerid, id)
{
    ...
    if(AccountData[playerid][ActivityTime] >= 3)
    {
        ...
        switch(rands)
        {
            case 1..24:
            {
                ItemAdd(playerid, "Botol", randbotol);
                ShowItemBox(playerid, "Botol", sprintf("Received_%dx", randbotol), 2);
            }
            case 25..30:
            {
    ...
    foreach(new id : Trash)
    {
        if(--TrashData[id][trashCooldown] > 0)
        {
            if(!TrashData[id][trashCooldown])
            {
                ...
                TrashData[id][trashCooldown] = 0;
                new query[255];
                mysql_format(dcrp_query, query, sizeof(query), "UPDATE `trash` SET `Cooldown`=%d WHERE `ID`=%d", TrashData[id][trashCooldown], id);
                mysql_tquery(dcrp_query, query);
            }
        }
