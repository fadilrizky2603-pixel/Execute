...
ShowKeysHouseList(playerid)
{
    ...
    ShowPlayerDialog(playerid, DIALOG_HKEYS, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Pemegang Kunci", playerName, "Pilih", "Batal");
    return 1;
}
...
House_Typee(houseid)
{
    ...
    new query[512];
    mysql_format(dcrp_query, query, sizeof query, "UPDATE house SET HS_IntPosX='%f', HS_IntPosY='%f', HS_IntPosZ='%f', HS_IntPosA='%f', HS_Interior='%d' WHERE ID='%d'", HouseData[houseid][hsIntPos][0], HouseData[houseid][hsIntPos][1], HouseData[houseid][hsIntPos][2], HouseData[houseid][hsIntPos][3], HouseData[houseid][hsInt], houseid);
...
CMD:customint(playerid, params[])
{
    ...
    new query[255];
    mysql_format(dcrp_query, query, sizeof(query), "UPDATE `house` SET `HS_IntPosX`=%f, `HS_IntPosY`=%f, `HS_IntPosZ`=%f, `HS_IntPosA`=%f, `HS_Interior`=%d WHERE `ID`=%d", HouseData[hid][hsIntPos][0], HouseData[hid][hsIntPos][1], HouseData[hid][hsIntPos][2], HouseData[hid][hsIntPos][3], HouseData[hid][hsInt], hid);
    mysql_tquery(dcrp_query, query);
    return 1;
}
...
CMD:edithouse(playerid, params[])
{
    ...
    {   
        ...
        mysql_format(dcrp_query, jskc, sizeof(jskc), "UPDATE `player_characters` SET `Char_OwnedHouse` = -1, `Char_FriendHouse`= -1 WHERE `pID`=%d", HouseData[did][hsOwnerID]);
        mysql_tquery(dcrp_query, jskc);  

        mysql_format(dcrp_query, tss, sizeof(tss), "UPDATE `player_characters` SET `Char_FriendHouse`=-1 WHERE `Char_FriendHouse`=%d", did);
        mysql_tquery(dcrp_query, tss);

        mysql_format(dcrp_query, jskc, sizeof(jskc), "DELETE FROM `house` WHERE `ID`=%d", did);
        mysql_tquery(dcrp_query, jskc);
...
ShowHouseWeapons(playerid, hid) 
{
    ...
    ShowPlayerDialog(playerid, DIALOG_WEAPON_CHEST, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Weapon Chest", shstr, "Pilih", "Batal");
    return 1;
}
...
hook OnPlayerEnterDynArea(playerid, STREAMER_TAG_AREA: areaid)
{
    foreach(new i : House)
    {
        ...
        {
            ShowKey(playerid, "Helipad House");
        }
        ...
        if(areaid == HouseData[i][housegarageArea])
        {
            ...
            {
                ShowPlayerFooter(playerid, "~y~Garasi Rumah~n~~w~Gunakan Klakson!", 2000);
            }
            else if(GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
            {
                ShowKey(playerid, "Garasi Rumah");
            }
        }
    }
    return 1;
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(newkeys & KEY_SECONDARY_ATTACK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        ...
        if(hid > -1)
        {
            if(IsPlayerInRangeOfPoint(playerid, 1.0, HouseData[hid][hsExtPos][0], HouseData[hid][hsExtPos][1], HouseData[hid][hsExtPos][2]))
            {
                ...
                {
                    new title[100];
                    format(title, sizeof(title), ""PINK1"DCRP"WHITE"- %s", HouseData[hid][hsOwner]);
                ...
                PlayerTextDrawSetString(playerid, ProgressBar[playerid][1], "MASUK");
        ...
        if(AccountData[playerid][pInHouse] != -1)
        {
            if(IsPlayerInRangeOfPoint(playerid, 1.0, HouseData[inhouseid][hsIntPos][0], HouseData[inhouseid][hsIntPos][1], HouseData[inhouseid][hsIntPos][2]))
            {
                ...
                PlayerTextDrawSetString(playerid, ProgressBar[playerid][1], "KELUAR");
                ShowProgressBar(playerid);
                ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 1, 0, 0, 0, 0, 1);
            }
        }
        ...
        if(hid > -1)
        {
            if(IsPlayerInRangeOfPoint(playerid, 2.0, HouseData[hid][housebrankasPos][0], HouseData[hid][housebrankasPos][1], HouseData[hid][housebrankasPos][2]))
            {
                if(HouseData[hid][hsType] > 3) 
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_HOUSE_BRANKAS, DIALOG_STYLE_LIST, sprintf(""PINK1"DCRP"WHITE"- %s", HouseData[hid][hsOwner]),
                    shstr, "Pilih", "Batal");
                } 
                else 
                {
    ...
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        ...
        if(ghid != -1)
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_HOUSEGARAGE_OUT, DIALOG_STYLE_TABLIST_HEADERS, sprintf(""PINK1"DCRP"WHITE"- Garasi Rumah %d", ghid), list, "Pilih", "Batal");
            HideShortKey(playerid);
        }
        ...
        if(hpid != -1)
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_HOUSEHELIPAD_OUT, DIALOG_STYLE_TABLIST_HEADERS, sprintf(""PINK1"DCRP"WHITE"- Garasi Helipad %d", hpid), list, "Pilih", "Batal");
        }
    }
    ...
    switch(dialogid)
    {
        case DIALOG_HOUSEVAULT:
        {
            ...
            if(!response)
            {
                if(HouseData[id][hsType] > 3) 
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_HOUSE_BRANKAS, DIALOG_STYLE_LIST, sprintf(""PINK1"DCRP"WHITE"- %s", HouseData[id][hsOwner]),
                    shstr, "Pilih", "Batal");
                } 
                else 
                {
            ...
            switch(listitem)
            {
                case 0: //deposit
                {
                    ...
                    if(rows)
                    {
                        ...
                        ShowPlayerDialog(playerid, DIALOG_HOUSEVAULT_DEPOSIT, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Brankas Rumah", str, "Pilih", "Batal");
                    }
                    else 
                    {
                        PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
                        return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Brankas Rumah",
                ...
                case 1: //withdraw
                {
                    ...
                    format(str, sizeof(str), "Nama Item\tJumlah\tBerat (%.3f/%s)\n", HouseData[id][houseMaxCapacity], frmtcapacity);
                    mysql_format(dcrp_query, tss, sizeof(tss), "SELECT * FROM `player_housestorage` WHERE `ID`=%d", AccountData[playerid][pID]);
                    mysql_query(dcrp_query, tss);
                    if(cache_num_rows())
                    {
                        ...
                        ShowPlayerDialog(playerid, DIALOG_HOUSEVAULT_WITHDRAW, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Brankas Rumah", str, "Pilih", "Batal");
                    }
                    else 
                    {
                        PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
        ...
        case DIALOG_HOUSEVAULT_DEPOSIT:
        {
            ...
            if(cache_num_rows() > 0)
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_HOUSEVAULT_IN, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Rumah", 
                shstr, "Input", "Batal");
            }
        }
        case DIALOG_HOUSEVAULT_IN:
        {
            ...
            ItemDelete(playerid, HouseBrankas[playerid][housebrankasTemp], quantity);
            ShowItemBox(playerid, HouseBrankas[playerid][housebrankasTemp], sprintf("Removed_%dx", quantity), 2);
            ...
            new invstr[1028];
            mysql_format(dcrp_query, shstr, sizeof(shstr), "SELECT * FROM `player_housestorage` WHERE `ID`=%d AND `hsItemName`='%s'", AccountData[playerid][pID], HouseBrankas[playerid][housebrankasTemp]);
            mysql_query(dcrp_query, shstr);
            if(cache_num_rows() > 0)
            {
                mysql_format(dcrp_query, invstr, sizeof(invstr), "UPDATE `player_housestorage` SET `hsItemQuantity` = `hsItemQuantity` + %d WHERE `ID` = %d AND `hsItemName` = '%s'", quantity, AccountData[playerid][pID], HouseBrankas[playerid][housebrankasTemp]);
                mysql_tquery(dcrp_query, invstr, "OnHouseDeposit", "d", playerid);
            }
            else 
            {
                mysql_format(dcrp_query, invstr, sizeof(invstr), "INSERT INTO `player_housestorage` SET `ID`=%d, `hsItemName`='%s', `hsItemModel`=%d, `hsItemQuantity`=%d", AccountData[playerid][pID], HouseBrankas[playerid][housebrankasTemp], HouseBrankas[playerid][housebrankasModel], quantity);
                mysql_tquery(dcrp_query, invstr, "OnHouseDeposit", "d", playerid);
            }
        }
        case DIALOG_HOUSEVAULT_WITHDRAW:
        {
            ...
            if(cache_num_rows())
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_HOUSEVAULT_OUT, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Bengkel",
                shstr, "Input", "Batal");
            }
        }
        case DIALOG_HOUSEVAULT_OUT:
        {
            ...
            if(isnull(inputtext))
            {
                AccountData[playerid][menuShowed] = true;
                format(shstr, sizeof(shstr), "Anda akan mengambil item:\nNama: %s\nJumlah di brankas: %d\nError: Tidak dapat diisi kosong!\nMohon masukkan berapa jumlah yang ingin anda ambil:", HouseBrankas[playerid][housebrankasTemp], HouseBrankas[playerid][housebrankasQuant]);
                ShowPlayerDialog(playerid, DIALOG_HOUSEVAULT_OUT, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Brankas Rumah", 
                shstr, "Input", "Batal");
                return 1;
            }
            ...
            {
                mysql_format(dcrp_query, jts, sizeof(jts), "UPDATE `player_housestorage` SET `hsItemQuantity`= %d WHERE `hsItemID`=%d", HouseBrankas[playerid][housebrankasQuant], HouseBrankas[playerid][housebrankasID]);
                mysql_tquery(dcrp_query, jts);
            }
            else 
            {
                mysql_format(dcrp_query, jts, sizeof(jts), "DELETE FROM `player_housestorage` WHERE `hsItemID`=%d", HouseBrankas[playerid][housebrankasID]);
                mysql_tquery(dcrp_query, jts);
            }
            ItemAdd(playerid, HouseBrankas[playerid][housebrankasTemp], quantity);
            ShowItemBox(playerid, HouseBrankas[playerid][housebrankasTemp], sprintf("Received_%dx", quantity), 2);
            ...
            switch(listitem)
            {
                case 2:
                {
                    ...
                    if(count == 0)
                    {
                        ...
                        return ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Undang Teman", 
                        "Tidak ada player yang berada dekat dengan pintu rumah anda!", "Tutup", "");
                    }

                    ShowPlayerDialog(playerid, DIALOG_HOUSE_INVITE, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Undang Teman", frmxt, "Undang", "Batal");
                }
                case 3:
                {
                ...
                {
                    if(!House_HaveAccess(playerid, id)) return ShowTDN(playerid, NOTIFICATION_ERROR, "Hanya pemilik rumah dan pemegang kunci yang dapat mengakses!");
                    ShowPlayerDialog(playerid, DIALOG_HOUSEVAULT, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Brankas Rumah",
                    "Simpan Barang\n"GRAY"Ambil Barang", "Pilih", "Batal");
                    AccountData[playerid][menuShowed] = true;
                }
                case 7:// Putar Musik
                {
                    Dialog_Show(playerid, DialogHouseMusik, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- House Musik",
                ...
                case 8: // Pemegang Kunci
                {
                    ...
                    new query[255];
                    mysql_format(dcrp_query, query, sizeof(query), "SELECT * FROM `player_characters` WHERE `Char_FriendHouse`= '%d'", id);
                    mysql_tquery(dcrp_query, query, "LoadKeysList", "d", playerid);
                }
            }
        }
        ...
        case DIALOG_HOUSE_INVITE:
        {
            if(!response)
            {
                if(HouseData[AccountData[playerid][pInHouse]][hsType] > 3) 
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_HOUSE_BRANKAS, DIALOG_STYLE_LIST, sprintf(""PINK1"DCRP"WHITE"- %s", HouseData[AccountData[playerid][pInHouse]][hsOwner]),
                    shstr, "Pilih", "Batal");
                } 
                else 
                {
            ...
            ShowPlayerDialog(targetid, DIALOG_HOUSE_INVITECONF, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Undang Teman", 
        ...
        case DIALOG_HKEYS:
        {
            ...
            if(!response) 
            {
                if(HouseData[houseid][hsType] > 3) 
                {
                    ...
                    ShowPlayerDialog(playerid, DIALOG_HOUSE_BRANKAS, DIALOG_STYLE_LIST, sprintf(""PINK1"DCRP"WHITE"- %s", HouseData[houseid][hsOwner]),
                    shstr, "Pilih", "Batal");
                } 
                else 
                {
            ...
            if(HouseMemberName[playerid][listitem][0] == EOS) return ShowPlayerDialog(playerid, DIALOG_HKEYS_ADD, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Bagikan Kunci",
            ...
            ShowPlayerDialog(playerid, DIALOG_HKEYS_REMOVE, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Pemegang Kunci", shstr, "Iya", "Tidak");
        ...
        case DIALOG_HKEYS_REMOVE:
        {
            ...
            {
                new query[255];
                mysql_format(dcrp_query, query, sizeof(query), "SELECT `Char_Name` FROM `player_characters` WHERE `Char_FriendHouse`=%d", AccountData[playerid][pInHouse]);
                mysql_tquery(dcrp_query, query, "LoadKeysList", "d", playerid);
                return 1;
            }
            ...
            foreach(new i : Player) if (IsPlayerConnected(i))
            {
                ...
                if(strfind(playerNameOnline, playerName, true) != -1)
                {
                    ...
                    new query[256];
                    mysql_format(dcrp_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_FriendHouse`=-1 WHERE `Char_Name`= '%e'", playerName);
                    mysql_tquery(dcrp_query, query);
                }
            }
            ShowTDN(playerid, NOTIFICATION_SUKSES, sprintf("Anda menarik kunci rumah dari %s", playerName));
            new query[256];
            mysql_format(dcrp_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_FriendHouse`=-1 WHERE `Char_Name`= '%s'", playerName);
            mysql_tquery(dcrp_query, query);
        }
        case DIALOG_HKEYS_ADD:
        {
            ...
            if(isnull(inputtext)) return ShowPlayerDialog(playerid, DIALOG_HKEYS_ADD, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Bagikan Kunci",
            "Error: Tidak dapat diisi kosong!\nMohon masukkan playerid yang ingin diberikan kunci rumah:", "Input", "Kembali");

            if(!IsNumeric(inputtext)) return ShowPlayerDialog(playerid, DIALOG_HKEYS_ADD, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Bagikan Kunci",
            "Error: Hanya dapat diisi angka!\nMohon masukkan playerid yang ingin diberikan kunci rumah:", "Input", "Kembali");
            ...
            foreach(new i : Player) if (IsPlayerConnected(i))
            {
                if(i == targetid)
                {
                    ...
                    new query[255];
                    mysql_format(dcrp_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_FriendHouse` = %d WHERE `pID` = %d", id, AccountData[i][pID]);
                    mysql_tquery(dcrp_query, query);
                }
            }
        }
...
Dialog:DialogHouseMusik(playerid, response, listitem, inputtext[])
{
    ...
    if(isnull(inputtext))
    {
        ...
        Dialog_Show(playerid, DialogHouseMusik, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- House Musik", jskc, "Input", "Cancel");
        return 1;
    }
...
public MasukRumahAccept(playerid, houseid)
{
    ...
    else
    {
        ...
        progressvalue = AccountData[playerid][ActivityTime] * 67/4;
...
AddCapacityHouse(houseid, const item[], quantity)
{
    ...
    {
        HouseData[houseid][houseMaxCapacity] += quantity*0.040;
    }
    ...
    {
        HouseData[houseid][houseMaxCapacity] += quantity*0.100;
    }
    ...
    {
        HouseData[houseid][houseMaxCapacity] += quantity*0.45;
    }
    ...
    {
        HouseData[houseid][houseMaxCapacity] += quantity*0.500;
    }
