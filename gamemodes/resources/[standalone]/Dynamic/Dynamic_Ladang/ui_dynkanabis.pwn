...
CMD:removekanabis(playerid, params[])
{
	...
	mysql_format(dcrp_query, query, sizeof(query), "DELETE FROM ladang WHERE id=%d", id);
...
hook OnPlayerEnterDynArea(playerid, STREAMER_TAG_AREA:areaid)
{
	foreach(new i : Ladang)
	{
		if(areaid == LadangData[i][kanabisArea])
		{
			...
			{
				ShowKey(playerid, "Ambil Kanabis");
			}
		}
	}
	return 1;
	...
	if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
	{
		...
		if(kid != -1 && LadangData[kid][kanabisArea] != STREAMER_TAG_AREA: INVALID_STREAMER_ID)
		{
			...
			if(count >= 4 && ems >= 2) 
			{
				...
				PlayerTextDrawSetString(playerid, ProgressBar[playerid][1], "MEMETIK KANABIS");
