    ...
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        if(IsPlayerInDynamicArea(playerid, kanabisOlah))
        {
            ...
            if(count >= 4 && ecount >= 2)
            {
                ...
                PlayerTextDrawSetString(playerid, ProgressBar[playerid][1], "MENGOLAH KANABIS");
...
public MengolahKanabis(playerid)
{
    ...
    if(AccountData[playerid][ActivityTime] >= 15)
    {
        ...
        ItemDelete(playerid, "Kanabis", 10);
        ItemDelete(playerid, "Plastik", 20);
        ItemAdd(playerid, "Marijuana", 1);
        ShowItemBox(playerid, "Plastik", "Removed_20x", 2);
        ShowItemBox(playerid, "Kanabis", "Removed_10x", 2);
        ShowItemBox(playerid, "Marijuana", "Received_1x", 2);
    }
    else 
    {
