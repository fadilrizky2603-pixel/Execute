...
{
    new cQuery[598];
    mysql_format(dcrp_query, cQuery, sizeof(cQuery), "UPDATE `objects` SET `Model`=%d, `ObjectX`=%f, `ObjectY`=%f, `ObjectZ`=%f, `ObjectRX`=%f, `ObjectRY`=%f, `ObjectRZ`=%f, `ObjectInterior`=%d, `ObjectWorld`=%d WHERE `ID`=%d", \
...
CMD:createobject(playerid, params[])
{
    ...
    new cQuery[598];
    mysql_format(dcrp_query, cQuery, sizeof(cQuery), "INSERT INTO `objects` SET `ID`=%d, `Model`=%d, `ObjectX`=%f, `ObjectY`=%f, `ObjectZ`=%f, `ObjectRX`=%f, `ObjectRY`=%f, `ObjectRZ`=%f, `ObjectInterior`=%d, `ObjectWorld`=%d",
    ...
    mysql_tquery(dcrp_query, cQuery, "OnObjectAdded", "d", id);
    return 1;
}
...
CMD:clone(playerid, params[])
{
    ...
    mysql_tquery(dcrp_query, cQuery, "OnObjectAdded", "d", newID);
    return 1;
}
...
CMD:deleteobject(playerid, params[])
{
    ...
    mysql_tquery(dcrp_query, sprintf("DELETE FROM `objects` WHERE `ID`=%d", id));
    return 1;
}
