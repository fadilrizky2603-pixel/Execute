...
public Underground_Load()
{
    if(cache_num_rows())
    {
        for(new i = 0; i != cache_num_rows(); i ++)
        {
            ...
            UndergroundData[i][underExitSpawn][0] = cache_get_field_float(i, "enterX");
            UndergroundData[i][underExitSpawn][1] = cache_get_field_float(i, "enterY");
            UndergroundData[i][underExitSpawn][2] = cache_get_field_float(i, "enterZ");
            UndergroundData[i][underExitSpawn][3] = cache_get_field_float(i, "enterRZ");

            Underground_Sync(i);
        }
        printf("[Dynamic Basement]: Jumlah total Dynamic Basemet yang dimuat %d", cache_num_rows());
    }
    return 1;
}
...
Underground_Create(playerid)
{
    ...
    if((index = Iter_Free(Underground)) != cellmin)
    {
        ...
        mysql_tquery(dcrp_query, "INSERT INTO `underground` (`enterX`) VALUES ('0');", "UndergroundCreated", "d", index);
        return index;
    }
    return -1;
}
...
Underground_Sync(index)
{
	if(Underground_IsExists(index))
	{
		...
		if(IsValidDynamic3DTextLabel(UndergroundData[index][enterLabel]))
		{
			UpdateDynamic3DTextLabelText(UndergroundData[index][enterLabel], -1, sprintf(""BLUEJEGE"[Basement Park, %d]\n"WHITE"Press "RED"'H' "WHITE"to enter", index));
		...
		else UndergroundData[index][enterLabel] = CreateDynamic3DTextLabel(sprintf(""BLUEJEGE"[Basement Park, %d]\n"WHITE"Press "RED"'H' "WHITE"to enter", index), -1, UndergroundData[index][underEnter][0], UndergroundData[index][underEnter][1], UndergroundData[index][underEnter][2] + 0.75, 5, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, 0, 0);

		if(IsValidDynamic3DTextLabel(UndergroundData[index][exitLabel]))
		{
			UpdateDynamic3DTextLabelText(UndergroundData[index][exitLabel], -1, sprintf(""BLUEJEGE"[Basement Park, %d]\n"WHITE"Press "RED"'H' "WHITE"to exit", index));
		...
		else UndergroundData[index][exitLabel] = CreateDynamic3DTextLabel(sprintf(""BLUEJEGE"[Basement Park, %d]\n"WHITE"Press "RED"'H' "WHITE"to exit", index), -1, -1745.4021,987.9285,17.6099 + 0.75, 5, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, (index + 2), 0);
...
Underground_Save(index)
{
    ...
    if(Underground_IsExists(index))
    {
        mysql_format(dcrp_query, query, sizeof(query), "UPDATE `underground` SET `enterX`='%.4f', `enterY`='%.4f', `enterZ`='%.4f', `exitX`='%.4f', `exitY`='%.4f', `exitZ`='%.4f', `exitRZ`='%.4f' WHERE `id`=%d", 
...
{
    new id = -1, Float:playerDist, Float:tmpdist = 9999.0;

    foreach(new i : Underground)
    {
        playerDist = GetPlayerDistanceFromPoint(playerid, UndergroundData[i][underEnter][0], UndergroundData[i][underEnter][1], UndergroundData[i][underEnter][2]);

        if(playerDist > range) continue;

        if(playerDist <= tmpdist) {
            tmpdist = playerDist;
            id = i;
            break;
        }
