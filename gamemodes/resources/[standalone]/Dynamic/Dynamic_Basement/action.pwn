...
hook OnGameModeInit()
{
    mysql_pquery(dcrp_query, "SELECT * FROM `underground` ORDER BY `id` ASC LIMIT "#MAX_DYNAMIC_BASEMENT"", "Underground_Load", "");
    return 1;
}
    ...
    if(newkeys & KEY_CROUCH && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
    {
        ...
        if((index = Underground_Nearest(playerid)) != -1 && IsPlayerInDynamicCP(playerid, UndergroundData[index][enterCP]))
        {
            if(UndergroundData[index][underExitSpawn][0] == 0.0)
                return ShowTDN(playerid, NOTIFICATION_ERROR, "Basement ini belum dapat digunakan, silahkan kontak admin dengan perintah '/report'");
        ...
        else
        {
            ...
            if(IsPlayerInDynamicCP(playerid, UndergroundData[index][enterCP]) && IsPlayerInRangeOfPoint(playerid, 3.0, -1745.4021, 987.9285, 17.6099))
            {
                SetCameraBehindPlayer(playerid);
