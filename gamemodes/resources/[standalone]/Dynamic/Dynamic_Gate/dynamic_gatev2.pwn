...
{
    new query[2012];
    mysql_format(dcrp_query, query, sizeof(query), "UPDATE `gate` SET `Model`=%d, `Faction`=%d, `Family`=%d, `Owner`=%d, `Speed`=%f, `cX`=%f, `cY`=%f, `cZ`=%f, `cRX`=%f, `cRY`=%f, `cRZ`=%f,\
...
CMD:addgate(playerid, params[])
{
    ...
    new query[555];
    mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `gate` SET ID=%d, `model`=%d, `cX`=%f, `cY`=%f, `cZ`=%f, `cRX`=%f, `cRY`=%f, `cRZ`=%f, `oX`=%f, `oY`=%f, `oZ`=%f, `oRX`=%f, `oRY`=%f, `oRZ`=%f",
    ...
    mysql_tquery(dcrp_query, query, "OnGatesCreated", "d", id);
    return 1;
}
...
CMD:editgate(playerid, params[])
{
    ...
    else if(!strcmp(option, "delete", true))
    {
        ...
        new query[125];
        mysql_format(dcrp_query, query, sizeof(query), "DELETE FROM `gate` WHERE `ID`=%d", id);
        mysql_tquery(dcrp_query, query);
    }
    return 1;
}
