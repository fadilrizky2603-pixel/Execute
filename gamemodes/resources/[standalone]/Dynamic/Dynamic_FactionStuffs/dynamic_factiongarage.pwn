...
{
    new query[1218];
    mysql_format(dcrp_query, query, sizeof(query), "UPDATE `factiongarage` SET `Name`='%s', `Type`=%d, `Interior`=%d, `World`=%d, `PosX`=%f, `PosY`=%f, `PosZ`=%f, `SpawnX`=%f, `SpawnY`=%f, `SpawnZ`=%f, `SpawnA`=%f WHERE `ID`=%d",
...
GetFactNameGarage(id)
{
    ...
    if(id != -1)
    {
        ...
        {
            format(frmtname, sizeof(frmtname), ""ROYALBLUE1"[Kepolisian Kota DCRP]");
        }
        else if(FactionGarage[id][fgType] == 2) // pemerintah
        {
            format(frmtname, sizeof(frmtname), ""LIGHTSKYBLUE1"[Pemerintah Daerah DCRP]");
        }
        else if(FactionGarage[id][fgType] == 3) // EMS
        {
            format(frmtname, sizeof(frmtname), ""VIOLETRED1"[EMS Kota DCRP]");
        }
        else if(FactionGarage[id][fgType] == 4) // Trans
        {
            format(frmtname, sizeof(frmtname), ""YELLOW"[TRANS Kota DCRP]");
        }
        else if(FactionGarage[id][fgType] == 5) // Bengkel
        {
            format(frmtname, sizeof(frmtname), ""LIGHTGREEN"[Bengkel Kota DCRP]");
        }
        else if(FactionGarage[id][fgType] == 6) // EMS
        {
            format(frmtname, sizeof(frmtname), ""TOMATO1"[Pedagang Kota DCRP]");
        }
    }
    return frmtname;
}
...
CMD:addfgarage(playerid, params[])
{
    ...
    new query[712];
    mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `factiongarage` SET `ID`=%d, `Name`='%s', `Type`=%d, `Interior`=%d, `World`=%d, `PosX`=%f, `PosY`=%f, `PosZ`=%f, `SpawnX`=%f, `SpawnY`=%f, `SpawnZ`=%f, `SpawnA`=%f",
    ...
    mysql_tquery(dcrp_query, query, "OnFactionGarageCreated", "dd", playerid, id);
    return 1;
}
...
CMD:removefgarage(playerid, params[])
{
    ...
    new query[125];
    mysql_format(dcrp_query, query, sizeof(query), "DELETE FROM `factiongarage` WHERE `ID`=%d", id);
    mysql_tquery(dcrp_query, query);
    return 1;
}
...
CMD:fgarage(playerid, params[])
{
    ...
    if(fgid != -1)
    {
        ...
        ShowPlayerDialog(playerid, DIALOG_FACTION_GARAGE_MENU, DIALOG_STYLE_LIST, sprintf(""PINK1"DCRP"WHITE"- %s Garage", GetFactGarageType(fgid)), "Keluarkan Kendaraan\n"GRAY"Simpan Kendaraan", "Pilih", "Batal");
    }
            ...
            switch(listitem)
            {
                case 0: //keluarkan
                {
                    switch(FactionGarage[fgid][fgType])
                    {
                        case 1: //polisi
                        {
                            ShowPlayerDialog(playerid, DIALOG_FACTION_GARAGE1, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Keluarkan Kendaraan",
                            "Sultan Sabhara\
                        ...
                        case 2: //pemerintah
                        {
                            ShowPlayerDialog(playerid, DIALOG_FACTION_GARAGE2, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Keluarkan Kendaraan",
                            "Mobil Gubernur\
                            \n"GRAY"Stratum\
                            \nRomero", "Pilih", "Batal");
                        }
                        case 3: //EMS
                        {
                            ShowPlayerDialog(playerid, DIALOG_FACTION_GARAGE3, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Keluarkan Kendaraan",
                            "Ambulance\
                            \n"GRAY"FBI Rancher\
                            \nSanchez EMS\
                        ...
                        case 4: //Trans
                        {
                            ShowPlayerDialog(playerid, DIALOG_FACTION_GARAGE4, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Keluarkan Kendaraan",
                            "Taxi\
                            \n"GRAY"Cabbie\
                        ...
                        case 5: // Bengkel
                        {
                            ShowPlayerDialog(playerid, DIALOG_FACTION_GARAGE5, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Keluarkan Kendaraan",
                            "Mobil Derek\
                            \n"GRAY"Sanchez", "Pilih", "Batal");
                        }
                        case 6: //pedagang
                        {
                            ShowPlayerDialog(playerid, DIALOG_FACTION_GARAGE6, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Keluarkan Kendaraan",
                            "Rancher\
                            \n"GRAY"Sanchez", "Pilih", "Batal");
                        }
                    }
