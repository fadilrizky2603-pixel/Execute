...
{
    new query[1218];
    mysql_format(dcrp_query, query, sizeof(query), "UPDATE `label_fivem` SET `LabelText`='%s', `LabelX`=%f, `LabelY`=%f, `LabelZ`=%f, `LabelRX`=%f, `LabelRY`=%f, `LabelRZ`=%f, `LabelInterior`=%d, `LabelWorld`=%d WHERE `LabelID`=%d",
...
CMD:addlabel(playerid, params[])
{
    ...
    new query[596];
    mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `label_fivem` SET `LabelID`=%d, `LabelX`=%f, `LabelY`=%f, `LabelZ`=%f, `LabelRX`=0, `LabelRY`=0, `LabelRZ`=%f, `LabelInterior`=%d, `LabelWorld`=%d, `LabelText`='%s'",
    ...
    mysql_tquery(dcrp_query, query, "OnLabelAdded", "dd", playerid, id);
    return 1;
}
...
CMD:removelabel(playerid, params[])
{
    ...
    mysql_format(dcrp_query, query, sizeof(query), "DELETE FROM `label_fivem` WHERE `LabelID`=%d", id);
