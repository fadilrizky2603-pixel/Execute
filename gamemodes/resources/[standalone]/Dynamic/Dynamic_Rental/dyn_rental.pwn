...
RentalRefresh(rentid)
{
    if(rentid != -1)
    {
        ...
        RentalData[rentid][RentalLabel] = CreateDynamic3DTextLabel(sprintf(""LIGHTGREY"[%s]\n"BLUEJEGE"Rental Sepedah\n"GREEN"[ALT]"WHITE" untuk mengakses rental sepedah", string), COLOR_WHITE, RentalData[rentid][RentalPos][0], RentalData[rentid][RentalPos][1], RentalData[rentid][RentalPos][2] + 0.55, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1);
    }
    return 1;
}
...
CMD:editrent(playerid, params[])
{
    ...
    else if(!strcmp(type, "delete", true))
    {
        ...
        mysql_tquery(dcrp_query, sprintf("DELETE FROM bike_rentals WHERE RentID='%d'", rentid));
    ...
    if(newstate == PLAYER_STATE_DRIVER)
    {
        ...
        if((iterid = Vehicle_Nearest2(playerid)) != -1)
        {
            if(Vehicle_IsOwner(playerid, iterid) && PlayerVehicle[iterid][pVehRental] != -1)
            {
                ...
                Info(playerid, "Sisa waktu %d jam %d menit %d detik", hours, minutes, seconds);
            }
        }
    }
    return 1;
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        ...
        if(rentid != -1)
        {
            if(RentalData[rentid][RentalSpawn][0] == 0 && RentalData[rentid][RentalSpawn][1] == 0 && RentalData[rentid][RentalSpawn][2] == 0) return ShowTDN(playerid, NOTIFICATION_ERROR, "Rental ini belum memiliki lokasi spawn kendaraannya");
            switch(RentalData[rentid][RentalType])
            {
                case 1: // Motorcycle
                {
        ...
        case 1: // Motor
        {
            switch(listitem)
            {
                case 0://
                {
                    ...
                    TakeMoney(playerid, 230);
                    VehicleRental_Create(playerid, 481, RentalData[id][RentalTimer], id, RentalData[id][RentalSpawn][0], RentalData[id][RentalSpawn][1], RentalData[id][RentalSpawn][2], RentalData[id][RentalSpawn][3], random(225), random(225), 1000, GetPlayerVirtualWorld(playerid), GetPlayerInterior(playerid));
                }
                case 1:
                {
                    ...
                    TakeMoney(playerid, 270);
                    VehicleRental_Create(playerid, 510, RentalData[id][RentalTimer], id, RentalData[id][RentalSpawn][0], RentalData[id][RentalSpawn][1], RentalData[id][RentalSpawn][2], RentalData[id][RentalSpawn][3], random(225), random(225), 1500, GetPlayerVirtualWorld(playerid), GetPlayerInterior(playerid));
                }
                case 2: //Kembalikan
                {
                    ...
                    foreach(new i : PvtVehicles) if(PlayerVehicle[i][pVehExists]) if(IsValidVehicle(PlayerVehicle[i][pVehPhysic]) && !IsABoat(PlayerVehicle[i][pVehPhysic]))
                    {
                        if(PlayerVehicle[i][pVehOwnerID] == AccountData[playerid][pID])
                        {
                            if(PlayerVehicle[i][pVehRental] != -1 && PlayerVehicle[i][pVehRentTime] != 0)
                            {
                                ...
                                mysql_tquery(dcrp_query, sprintf("DELETE FROM player_vehicles WHERE id = '%d'", PlayerVehicle[i][pVehID]));
...
VehicleRental_Create(playerid, modelid, rentaltime, rentalid, Float:x, Float:y, Float:z, Float:angle, color1, color2, price, worldid, interiorid)
{
    ...
    mysql_format(dcrp_query, cQuery, sizeof(cQuery), "INSERT INTO `player_vehicles` (`PVeh_OwnerID`, `PVeh_ModelID`, `PVeh_Parked`, `PVeh_Housed`, `PVeh_RentTime`, `PVeh_Rental`, `PVeh_Plate`, `PVeh_Health`, `PVeh_Fuel`, `PVeh_PosX`, `PVeh_PosY`, `PVeh_PosZ`, `PVeh_PosA`, \
    ...
    mysql_tquery(dcrp_query, cQuery, "OnVehRentalCreated", "ii", playerid, i);
    return 1;
}
