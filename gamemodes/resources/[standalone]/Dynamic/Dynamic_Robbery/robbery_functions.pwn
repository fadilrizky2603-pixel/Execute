...
{
    new cQuery[1048];
    mysql_format(dcrp_query, cQuery, sizeof(cQuery), "UPDATE `robbery` SET `RobberyX`=%f, `RobberyY`=%f, `RobberyZ`=%f, `RobberyRX`=%f, `RobberyRY`=%f, `RobberyRZ`=%f, `RobberyInterior`=%d, `RobberyWorld`=%d WHERE `RobberyID`=%d",
...
CMD:addrobbery(playerid, params[])
{
    ...
    new cQuery[1048];
    mysql_format(dcrp_query, cQuery, sizeof(cQuery), "INSERT INTO `robbery` SET `RobberyID`=%d, `RobberyX`=%f, `RobberyY`=%f, `RobberyZ`=%f, `RobberyRX`=%f, `RobberyRY`=%f, `RobberyRZ`=%f, `RobberyInterior`=%d, `RobberyWorld`=%d",
    ...
    mysql_tquery(dcrp_query, cQuery, "OnRobberyAdded", "dd", playerid, id);
    return 1;
}
...
CMD:editrobbery(playerid, params[])
{
    ...
    else if(!strcmp(option, "delete", true))
    {
        ...
        new query[200];
        mysql_format(dcrp_query, query, sizeof(query), "DELETE FROM `robbery` WHERE `RobberyID` = %d", id);
...
FUNC::RobberyShowTD(playerid, const text[])
{
