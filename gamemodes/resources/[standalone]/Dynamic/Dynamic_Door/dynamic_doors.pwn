...
{
	new dquery[2048];
	mysql_format(dcrp_query, dquery, sizeof(dquery), "UPDATE doors SET name='%s', password='%s', icon='%d', locked='%d', admin='%d', owner='%d', vip='%d', faction='%d', family='%d', garage='%d', custom='%d', extvw='%d', extint='%d', extposx='%f', extposy='%f', extposz='%f', extposa='%f', intvw='%d', intint='%d', intposx='%f', intposy='%f', intposz='%f', intposa='%f', mapicon='%d' WHERE ID='%d'",
	...
	mysql_tquery(dcrp_query, dquery);
	return 1;
}
		...
		if (IsPlayerInRangeOfPoint(playerid, 3, DoorData[did][dExtposX], DoorData[did][dExtposY], DoorData[did][dExtposZ]))
		{
			if(DoorData[did][dOwner] == AccountData[playerid][pID])
			{
				if(!DoorData[did][dLocked])
				{
					...
					mysql_tquery(dcrp_query, sprintf("UPDATE doors SET locked=%d WHERE ID=%d", DoorData[did][dLocked], did));
				}
				else
				{
...
CMD:createdoor(playerid, params[])
{
	...
	mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO doors SET ID=%d, extvw=%d, extint=%d, extposx=%f, extposy=%f, extposz=%f, extposa=%f, name='%s'", did, DoorData[did][dExtvw], DoorData[did][dExtint], DoorData[did][dExtposX], DoorData[did][dExtposY], DoorData[did][dExtposZ], DoorData[did][dExtposA], name);
	mysql_tquery(dcrp_query, query, "OnDoorsCreated", "ii", playerid, did);
	return 1;
}
...
/*CMD:gotodoor(playerid, params[])
{
	...
	Dialog_Show(playerid, DIALOG_GOTO_DOOR, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Dynamic Door", shstr, "Select", "Cancel");
	return 1;
}
...
CMD:editdoor(playerid, params[])
{
	...
	else if(!strcmp(type, "delete", true))
    {
		...
		new query[128];
		mysql_format(dcrp_query, query, sizeof(query), "DELETE FROM doors WHERE ID=%d", did);
