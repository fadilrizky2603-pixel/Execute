...
Slot_Refresh(id)
{
    if(id != -1)
    {
        ...
        if(SlotData[id][SlotPos][0] != 0.0)
        {
            ...
            SlotData[id][SlotLabel] = CreateDynamic3DTextLabel(""GREEN"[ALT]"WHITE" Using Slot Machine", -1, SlotData[id][SlotPos][0], SlotData[id][SlotPos][1], SlotData[id][SlotPos][2] + 0.55, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, SlotData[id][SlotWorld], SlotData[id][SlotInterior], -1, 10.0, -1, 0);
        }
    }
    return 1;
}
...
{
    new query[596];
    mysql_format(dcrp_query, query, sizeof(query), "UPDATE `slotmachine` SET `PosX`=%f, `PosY`=%f, `PosZ`=%f, `RotX`=%f, `RotY`=%f, `RotZ`=%f, `Interior`=%d, `World`=%d WHERE `ID`=%d",
...
CMD:addslot(playerid, params[])
{
    ...
    new query[255];
    mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `slotmachine` SET `ID`=%d, `PosX`=%f, `PosY`=%f, `PosZ`=%f, `RotX`=%f, `RotY`=%f, `RotZ`=%f, `Interior`=%d, `World`=%d",
    ...
    mysql_tquery(dcrp_query, query, "OnSlotCreated", "dd", playerid, id);
    return 1;
}
...
CMD:removeslot(playerid, params[])
{
    ...
    new query[200];
    mysql_format(dcrp_query, query, sizeof(query), "DELETE FROM `slotmachine` WHERE `ID`=%d", id);
    mysql_tquery(dcrp_query, query);
    return 1;
}
...
//             if(isnull(inputtext))
//             {
//                 format(title, sizeof(title), ""PINK1"DCRP"WHITE"- Mesin %02d", id+1);
