public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	if(dialogid == DIALOG_REGISTER)
    {
		...
		if (strlen(inputtext) <= 5) return ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "Register - "PINK1"DCRP", ""WHITE"Buatlah kata sandi anda dengan benar\n"WHITE"NOTE: Kata sandi minimal 5 Karakter!\n\n"YELLOW"(Mohon isi dan selalu ingat Password anda di bawah ini):", "Register", "Tolak");
		
		if(!IsValidPassword(inputtext))
		{
			Error(playerid, "Sandi valid : A-Z, a-z, 0-9, _, [ ], ( )");
			ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "Register - "PINK1"DCRP", ""WHITE"Kata sandi yang anda gunakan mengandung karakter yang tidak valid!\n\n"YELLOW"(Mohon isi dan selalu ingat Password anda di bawah ini):", "Register", "Tolak");
			return 1;
		}
		...
		mysql_format(dcrp_query, query, sizeof query, "UPDATE playerucp SET password = '%s', salt = '%e', extrac = '%d' WHERE ucp = '%e'", AccountData[playerid][pPassword], AccountData[playerid][pSalt], AccountData[playerid][pExtraChar], AccountData[playerid][pUCP]);
		mysql_tquery(dcrp_query, query, "CheckCharacters", "d", playerid);
		return 1;
	}
	if(dialogid == DIALOG_LOGIN)
	{
		...
		if (strcmp(hashed_pass, AccountData[playerid][pPassword]) == 0)
		{
			if(!Blacklist_Check(playerid, "name", AccountData[playerid][pUCP])) {
				...
				Info(playerid, "Validating your account data...");
				Info(playerid, "Getting info of your character, please wait...");
				Info(playerid, "Silahkan pilih karakter yang akan dimainkan");
			}
		}
		else
		{
	...
	/*if(dialogid == DIALOG_CHARLIST)
	{
		...
		{
			if(ucp::data[playerid][listitem][ucp::Name][0] == EOS) return ShowPlayerDialog(playerid, DIALOG_MAKE_CHAR, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Pembuatan Karakter",
			""WHITE"Selamat Datang di "DCRP"DCRP\n"WHITE"Sebelum bermain anda harus membuat karakter terlebih dahulu\
			...
			SetPlayerName(playerid, ucp::data[playerid][listitem][ucp::Name]);
			StopStream(playerid);
			
			if(!Blacklist_Check(playerid, "name", ucp::data[playerid][listitem][ucp::Name])) {
				mysql_tquery(dcrp_query, sprintf("SELECT * FROM `player_characters` WHERE `Char_Name` = '%s' LIMIT 1;", ucp::data[playerid][AccountData[playerid][pChar]][ucp::Name]), "LoadPlayerData", "d", playerid);
			}
		}
	}*/
	...
	{
		new shstr[596];
		format(shstr, sizeof(shstr), ""WHITE"Selamat datang di "PINK1"DCRP\nSebelum bermain anda harus membuat karakter terlebih dahulu\
		\nMasukkan nama karakter hanya dengan nama orang indonesia\n\nCth: Ucok_Siregar, Dadang_Sucipto");
		if(!response) return KickEx(playerid);

		if(strlen(inputtext) < 1 || strlen(inputtext) > 24)
		{
			ShowPlayerDialog(playerid, DIALOG_MAKE_CHAR, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Pembuatan Karakter", 
		...
		new cQuery[225];
		mysql_format(dcrp_query, cQuery, sizeof(cQuery), "SELECT * FROM `player_characters` WHERE `Char_Name` = '%s'", inputtext);
		mysql_tquery(dcrp_query, cQuery, "InsertPlayerName", "ds", playerid, inputtext);
	...
	if(dialogid == DIALOG_AGE)
    {
		if(!response) 
		{
			return ShowPlayerDialog(playerid, DIALOG_AGE, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Tanggal Lahir", "Mohon masukkan tanggal lahir sesuai format hh/bb/tttt cth: (25/09/2001)", "Input", "");
		}
		if(response)
		{
			new
			...
			if(sscanf(inputtext, "p</>ddd", iDay, iMonth, iYear)) {
				ShowPlayerDialog(playerid, DIALOG_AGE, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Tanggal Lahir", "Mohon masukkan tanggal lahir sesuai format hh/bb/tttt cth: (25/09/2001)", "Input", "");
			}
			else if(iYear < 1900 || iYear > year) {
				ShowPlayerDialog(playerid, DIALOG_AGE, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Tahun Lahir", "ERROR: Invalid Tahun Lahir\nMohon masukkan tanggal lahir sesuai format hh/bb/tttt cth: (25/09/2001)", "Input", "");
			}
			else if(iMonth < 1 || iMonth > 12) {
				ShowPlayerDialog(playerid, DIALOG_AGE, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Bulan Lahir", "ERROR: Invalid Bulan Lahir\nMohon masukkan tanggal lahir sesuai format hh/bb/tttt cth: (25/09/2001)", "Input", "");
			}
			else if(iDay < 1 || iDay > arrMonthDays[iMonth - 1]) {
				ShowPlayerDialog(playerid, DIALOG_AGE, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Tanggal Lahir", "ERROR: Invalid Tanggal Lahir\nMohon masukkan tanggal lahir sesuai format hh/bb/tttt cth: (25/09/2001)", "Input", "");
			}
			else 
			{
				SetPVarInt(playerid, "CreateAge", 1);
				format(AccountData[playerid][pAge], 50, inputtext);
				ShowPlayerDialog(playerid, DIALOG_ORIGIN, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Negara Kelahiran", "Mohon masukkan kembali negara asal kelahiran karakter.\nPerhatian: Masukkan nama negara yang valid (cth: Indonesia).", "Input", "");
			}
		}
		return 1;
	}
	if(dialogid == DIALOG_ORIGIN)
	{
		if(response)
		{
			if(isnull(inputtext) || IsNumeric(inputtext))
			{
				...
				return ShowPlayerDialog(playerid, DIALOG_ORIGIN, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Negara Kelahiran", "Mohon masukkan kembali negara asal kelahiran karakter.\nPerhatian: Masukkan nama negara yang valid (cth: Indonesia).", "Input", "");
			}
			...
			if (isnull(inputtext) || strlen(inputtext) > 50) 
			{
				...
				else return ShowPlayerDialog(playerid, DIALOG_ORIGIN, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Negara Kelahiran", "Mohon masukkan kembali negara asal kelahiran karakter.\nPerhatian: Masukkan nama negara yang valid (cth: Indonesia).", "Input", "");
			}
			new query[512];
			format(AccountData[playerid][pOrigin], 32, inputtext);
			SetPVarInt(playerid, "CreateOrigin", 1);

			mysql_format(dcrp_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_Origin` = '%s' WHERE `pID` = '%d'", inputtext, AccountData[playerid][pID]);
			mysql_tquery(dcrp_query, query);
			ShowPlayerDialog(playerid, DIALOG_TINGGIBADAN, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE" - Tinggi Badan (cm)", "Mohon masukkan tinggi badan (cm) karakter!\nPerhatian: Format hanya berupa angka satuan cm (cth: 163).", "Input", "");
		}
		else ShowPlayerDialog(playerid, DIALOG_ORIGIN, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Negara Kelahiran", "Mohon masukkan kembali negara asal kelahiran karakter.\nPerhatian: Masukkan nama negara yang valid (cth: Indonesia).", "Input", "");
	}
	if(dialogid == DIALOG_TINGGIBADAN)
	{
		if(response)
		{
			...
			if(isnull(inputtext) || !IsNumeric(inputtext))
			{
				...
				return ShowPlayerDialog(playerid, DIALOG_TINGGIBADAN, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE" - Tinggi Badan (cm)", "Mohon masukkan tinggi badan (cm) karakter!\nPerhatian: Format hanya berupa angka satuan cm (cth: 163).", "Input", "");
			}
			...
			new strgbg[200];
			mysql_format(dcrp_query, strgbg, sizeof(strgbg), "UPDATE `player_characters` SET `Char_BodyHeight`='%d' WHERE `pID`='%d'", AccountData[playerid][pTinggiBadan], AccountData[playerid][pID]);
			mysql_tquery(dcrp_query, strgbg);
			ShowPlayerDialog(playerid, DIALOG_BERATBADAN, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE" - Berat Badan (kg)", "Mohon masukkan berat badan (kg) karakter!\nPerhatian: Format hanya berupa angka satuan kg (cth: 75).", "Input", "");
		}
		else return ShowPlayerDialog(playerid, DIALOG_TINGGIBADAN, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE" - Tinggi Badan", "Mohon masukkan tinggi badan karakter!\nPerhatian: format berupa angka minimal 165-180cm (cth: 160)", "Input", "Back");
	}
	if(dialogid == DIALOG_BERATBADAN)
	{
		if(response)
		{
			...
			if(isnull(inputtext) || !IsNumeric(inputtext))
				return ShowPlayerDialog(playerid, DIALOG_BERATBADAN, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE" - Berat Badan (kg)", "Mohon masukkan berat badan (kg) karakter!\nPerhatian: Format hanya berupa angka satuan kg (cth: 75).", "Input", "");
			
			if(berat <= 40)
				return ShowPlayerDialog(playerid, DIALOG_BERATBADAN, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE" - Berat Badan (kg)", "Tidak dapat dibawah 40kg!", "Input", "");

			if(berat >= 95)
				return ShowPlayerDialog(playerid, DIALOG_BERATBADAN, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE" - Berat Badan (kg)", "Tidak dapat diatas 95kg!", "Input", "");

			AccountData[playerid][pBeratBadan] = berat;
			SetPVarInt(playerid, "CreateWeight", 1);
			new strgbg[200];
			mysql_format(dcrp_query, strgbg, sizeof(strgbg), "UPDATE `player_characters` SET `Char_BodyWeight`='%d' WHERE `pID`='%d'", AccountData[playerid][pBeratBadan], AccountData[playerid][pID]);
			mysql_tquery(dcrp_query, strgbg);
			ShowPlayerDialog(playerid, DIALOG_GENDER, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Jenis Kelamin", ""WHITE"Laki-Laki\n"GRAY"Perempuan", "Pilih", "");
		}
	}
	if(dialogid == DIALOG_GENDER)
    {
		if(response)
		{
			...
			// ShowPlayerDialog(playerid, DIALOG_SELECT_SPAWN, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Pilih Lokasi Spawn",
		...
		{
			mysql_format(dcrp_query, characterQuery, sizeof(characterQuery), "INSERT INTO `player_characters` (`Char_Name`, `Char_UCP`, `Char_RegisterDate`) VALUES ('%s', '%s', CURRENT_TIMESTAMP())", AccountData[playerid][pTempName], AccountData[playerid][pUCP]);
			mysql_tquery(dcrp_query, characterQuery, "OnPlayerRegister", "d", playerid);
	...
	if(dialogid == DialogTrackMyVeh)
	{
		...
		if(isnull(inputtext))
		{
			ShowPlayerDialog(playerid, DialogTrackMyVeh, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Lacak Kendaraan", 
			...
			switch(listitem)
			{
				case 0: //slot 1
				{
					...
					else
					{
						...
						ShowPlayerDialog(playerid, DialogToyEdit, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Edit Fashion (Topi/Helmet)", string, "Select", "Cancel");
					}
				}
				case 1: //slot 2
				{
					...
					else
					{
						...
						format(string, sizeof string, "%s"WHITE"Hapus\t(Pilih jika anda ingin menghapus permanent)\n", string);
						format(string, sizeof string, "%s"VERONA_ARWIN"%s\t%s\n", string, status, status2);
						ShowPlayerDialog(playerid, DialogToyEdit, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Edit Fashion (Kacamata)", string, "Select", "Cancel");
					}
				}
				case 2: //slot 3
				{
					...
					else
					{
						...
						format(string, sizeof string, "%s"WHITE"Hapus\t(Pilih jika anda ingin menghapus permanent)\n", string);
						format(string, sizeof string, "%s"VERONA_ARWIN"%s\t%s\n", string, status, status2);
						ShowPlayerDialog(playerid, DialogToyEdit, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Edit Fashion (Aksesoris)", string, "Select", "Cancel");
					}
				}
				case 3: //slot 4
				{
					...
					else
					{
						...
						format(string, sizeof string, "%s"WHITE"Hapus\t(Pilih jika anda ingin menghapus permanent)\n", string);
						format(string, sizeof string, "%s"VERONA_ARWIN"%s\t%s\n", string, status, status2);
						ShowPlayerDialog(playerid, DialogToyEdit, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Edit Fashion (Tas/Koper)", string, "Select", "Cancel");
					}
				}
				case 4:
				{
					if(AccountData[playerid][PurchasedToy] == true)
					{
						...
						new string[128];
						mysql_format(dcrp_query, string, sizeof(string), "DELETE FROM toys WHERE Owner = '%s'", AccountData[playerid][pName]);
						mysql_tquery(dcrp_query, string);
						AccountData[playerid][PurchasedToy] = false;
						ShowPlayerFooter(playerid, "~r~~h~All Toy Rested!~y~!", 3000);
					}
				}
			}
		}
	...
	if(dialogid == DialogToyEdit)
	{
		...
		if(response)
		{
			switch(listitem)
			{
				...
				case 2: // change bone
				{
					ShowPlayerDialog(playerid, DIALOG_TOYPOSISI, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Ubah Tulang(Bone)", 
					"Spine\
					\n"GRAY"Head\
					\nLeft Upper Arm\
				...
				case 3:
				{
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSX, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Edit Sumbu X", mstr, "Edit", "Cancel");
				}
				case 4:
				{
					new mstr[128];
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSY, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Edit Sumbu X", mstr, "Edit", "Cancel");
				}
				case 5:
				{
					new mstr[128];
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSZ, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Edit Sumbu Z", mstr, "Edit", "Cancel");
				}
				case 6:
				{
					new mstr[128];
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSRX, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Edit Rotasi X", mstr, "Edit", "Cancel");
				}
				case 7:
				{
					new mstr[128];
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSRY, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Edit Rotasi Y", mstr, "Edit", "Cancel");
				}
				case 8:
				{
					new mstr[128];
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSRZ, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Edit Rotasi Z", mstr, "Edit", "Cancel");
				}
				case 9:
				{
					new mstr[128];
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSSX, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Edit Skala X", mstr, "Edit", "Cancel");
				}
				case 10:
				{
					new mstr[128];
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSSY, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Edit Skala Y", mstr, "Edit", "Cancel");
				}
				case 11:
				{
					new mstr[128];
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSSZ, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Edit Skala Z", mstr, "Edit", "Cancel");
				}
				case 12: // remove toy
				{
		...
		else
		{
			...
			ShowPlayerDialog(playerid, DIALOG_TOY, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Edit Fashion", string, "Select", "Cancel");
		}
		return 1;
	}
	...
	if(dialogid == DIALOG_TOYPOSISIBUY)
	{
		if(response)
		{
			...
			{
				new query[255];
				mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `toys` (`Owner`) VALUES ('%s')" , AccountData[playerid][pName]);
				mysql_tquery(dcrp_query, query);
			}
	...
	if(dialogid == DIALOG_TOYPOSX)
	{
		if(response)
		{
			...
			format(toys, sizeof(toys), ""PINK1"DCRP"WHITE" - Edit Fashion (%s)", minsty);
			format(string, sizeof string, "Pengaturan\tParameter\n");
			format(string, sizeof string, "%s"WHITE"Edit Posisi\t(Khusus PC)\n", string);
			format(string, sizeof string, "%s"VERONA_ARWIN"Edit Posisi\t(Khusus Android)\n", string);
			format(string, sizeof string, "%s"WHITE"Ubah Posisi Tulang\t(Bone)\n", string);
	...
	if(dialogid == DIALOG_EDITBONE)
	{
		if(response)
		{
			...
			mysql_format(dcrp_query, string, sizeof(string), "INSERT INTO weaponsettings (Owner, WeaponID, Bone) VALUES ('%d', %d, %d) ON DUPLICATE KEY UPDATE Bone = VALUES(Bone)", AccountData[playerid][pID], weaponid, listitem + 1);
			mysql_tquery(dcrp_query, string);
		}
	...
	if(dialogid == DIALOG_DYNAMIC_HELP)
	{
		...
		switch(listitem)
		{
			case 0:// Actor
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Dynamic Actor",
				"Perintah\tKeterangan\n\
			...
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Dynamic Rents",
				"Perintah\tKeterangan\n\
			...
			case 2: // Dynamic Door
			{
				...
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Dynamic Door", str, "Tutup", "");
			}
			case 3: // Fivem Label
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Dynamic Label",
				"Perintah\tKeterangan\n\
			...
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Dynamic Public Garage",
				"Perintah\tKeterangan\n\
			...
			case 5: //Tong Sampah
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Dynamic Trash",
				"Perintah\tKeterangan\n\
			...
			case 6: //Dynamic ATM
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Dynamic ATM", 
				"Perintah\tKeterangan\n\
			...
			case 7: //Kanabis
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Dynamic Ladang", 
				"Perintah\tKeterangan\n\
			...
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Dynamic Robbery", 
				"Perintah\tKeterangan\n\
			...
			case 10: //Hunting
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Dynamic Hunting", 
				"Perintah\tKeterangan\n\
			...
			case 11: //Gudang
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Dynamic Gudang", 
				"Perintah\tKeterangan\n\
			...
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Dynamic Houses", 
				"Perintah\tKeterangan\n\
			...
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Dynamic Icon",
				"Perintah\tKeterangan\n\
			...
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Dynamic Buttons",
				"Perintah\tKeterangan\n\
			...
			case 16: // Object
			{
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Dynamic Object",
				"Perintah\tKeterangan\n\
	...
	if(dialogid == DIALOG_ADMIN_HELP)
	{
		...
		switch(listitem)
		{
			case 0: // Trial & The Stars
			{
				...
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Trial Admin", 
				"Perintah\tKeterangan\n\
				/a\tAkses Chat Staff\n\
			...
			case 1: //AL2
			{
				...
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Helper", 
				"Perintah\tKeterangan\n\
			...
			case 2:// AL3
			{
				...
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Admin I", 
				"Perintah\tKeterangan\n\
			...
			case 3: //AL4
			{
				...
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Admin II", 
				"Perintah\tKeterangan\n\
			...
			case 4: // AL5
			{
				...
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Admin III", 
				"Perintah\tKeterangan\n\
			...
			case 5: //
			{
				...
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Pengurus", 
				"Perintah\tKeterangan\n\
			...
			case 6: //management
			{
				...
				ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Management", 
				"Perintah\tKeterangan\n\
	...
	if(dialogid == DIALOG_AMOUNT)
	{
		...
		if (response)
		{
			...
			if(sscanf(inputtext, "p</>d", jumlah)) {
				ShowPlayerDialog(playerid, DIALOG_AMOUNT, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Set Amount", "Kesalahan! Tidak sah\nMasukan jumlah item minimal 1 sampai 100k", "Oke", "Batal");
			}
			...
			else
			{
				...
				PlayerTextDrawSetString(playerid, INVINFO[playerid][22], str);
				PlayerTextDrawShow(playerid, INVINFO[playerid][22]);
	...
	if(dialogid == DIALOG_GIVE)
	{
		if (response)
		{
			new p2 = NearestPlayer[playerid][listitem];
			new itemid = AccountData[playerid][pSelectItem];
			new value = AccountData[playerid][pAmountInv];
			SelectTextDraw(playerid, COLOR_SYNTAX);
	...
	if(dialogid == DIALOG_GIVECASH)
	{
		if (response)
		{	
			new p2 = NearestPlayer[playerid][listitem];
			AccountData[playerid][pPilihID] = p2;
			ShowPlayerDialog(playerid, DIALOG_INPUTCASH, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"Give Money", "Masukkan jumlah uang yang ingin anda berikan:", "Input", "Batal");
		}
	}
	if(dialogid == DIALOG_INPUTCASH)
	{
		if(response)
		{
			new userid = AccountData[playerid][pPilihID];
			...
			if(money > AccountData[playerid][pMoney] || AccountData[playerid][pMoney] < 1)
				return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda tidak memiliki cukup uang!");

			if(userid == INVALID_PLAYER_ID || userid == playerid || !NearPlayer(playerid, userid, 4.0))
				return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda harus berada di dekat player tersebut.");

			if(money < 1 || money > 100000)
				return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda hanya dapat memberikan uang sebesar 1 - 100000");

			format(str, sizeof(str), "Removed_%sx", FormatMoney(money));
			ShowItemBox(playerid, "Uang", str, 3);

			format(str, sizeof(str), "Received_%sx", FormatMoney(money));
			ShowItemBox(userid, "Uang", str, 3);
			...
			foreach(new i : Player) if (IsPlayerConnected(i)) if (AccountData[i][pAdmin] > 0 || AccountData[i][pTheStars] > 0)
			{
				if(money > 5000)
				{
					SendClientMessageEx(i, -1, ""YELLOW"[RTM Alert]: %s (%d) memberikan %s kepada %s (%d)", AccountData[playerid][pName], playerid, FormatMoney(money), AccountData[userid][pName], userid);
				}
				else 
				{
					SendClientMessageEx(i, -1, ""YELLOW"[Pay]: %s (%d) memberikan %s kepada %s (%d)", AccountData[playerid][pName], playerid, FormatMoney(money), AccountData[userid][pName], userid);
				}
			}
			
			TakeMoney(playerid, money);
			GiveMoney(userid, money);

			new tstr[128];
			format(tstr, sizeof(tstr), "Anda berhasil memberi uang sebanyak ~g~%s ~w~kepada ~y~%s", FormatMoney(money), ReturnName(userid));
			ShowTDN(playerid, NOTIFICATION_SUKSES, tstr);
			ShowTDN(userid, NOTIFICATION_INFO, sprintf("Anda menerima uang sebanyak ~g~%s", FormatMoney(money)));

			new frmxt[255];
			format(frmxt, sizeof(frmxt), "Memberikan uang kepada %s sebesar $%d", AccountData[userid][pName], money);
	...
	/*if(dialogid == DIALOG_SETAMOUNT)
	{
		...
		if(isnull(inputtext))
		{
			ShowPlayerDialog(playerid, DIALOG_SETAMOUNT, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Set Amount",
		...
		AccountData[playerid][pAmountInv] = strval(inputtext);
		PlayerTextDrawSetString(playerid, inv::string[playerid][0], inputtext);
		PlayerTextDrawShow(playerid, inv::string[playerid][0]);
	}
	...
	if(dialogid == DIALOG_GOPAYWITHDRAW)
	{
		...
		if(amount > AccountData[playerid][pSaldoGopay]) return ShowPlayerDialog(playerid, DIALOG_GOPAYWITHDRAW, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE" - Tarik Saldo Gopay", "ERROR: Anda tidak memiliki saldo sebanyak itu\n\nInput jumlah saldo gopay yang ingin kamu tarik:", "Submit", "Cancel");
		if(amount < 1) return ShowPlayerDialog(playerid, DIALOG_GOPAYWITHDRAW, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE" - Tarik Saldo Gopay", "ERROR: Anda memasukan angka yang tidak Valid!\n\nInput jumlah saldo gopay yang ingin kamu tarik:", "Submit", "Cancel");
		...
		else
		{
			...
			mysql_format(dcrp_query, query, sizeof(query), "UPDATE player_characters SET Char_Money=%d,Char_Gopay=%d WHERE pID=%d", AccountData[playerid][pMoney], AccountData[playerid][pSaldoGopay], AccountData[playerid][pID]);
			mysql_tquery(dcrp_query, query);
		...
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					...
					strcat(str, "duduk\nduduk2\nduduk3\nduduk4\nduduk5\nduduk6\njarteng\n");
					strcat(str, "pusing\nped\nped2\nped3\nped4\nped5\nped6\nped7\nped8\nped9\nped10\nped11\nped12\nped13\nped14\nped15\nped15\nkencing\nx\n");
					ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE" - List Animasi", str, "Close", "");
				}
				case 1:
				{
					ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Emote Property",
					"TV\
					\nBan\
					\nBesbol\
					\nBasket\
				...
				case 2:
				{
					...
					ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- List Animasi Interaksi", str, "Close", "");
				}
			}
		}
	}
	if(dialogid == DIALOG_VERIFYCODE)
	{
		...
		if(isnull(inputtext))
		{
			...
			\n"WHITE"Version: "DCRP""TEXT_GAMEMODE"\n"YELLOW"(Silahkan masukkan PIN yang dikirimkan oleh Bot DCRP ke Discord anda dibawah ini):", AccountData[playerid][pUCP]);
			ShowPlayerDialog(playerid, DIALOG_VERIFYCODE, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Verifikasi Akun",
			shstr, "Input", "Batal");
			return 1;
		}
		...
		{
			format(shstr, sizeof(shstr), ""WHITE"Selamat datang di "DCRP"DCRP"WHITE"\nUCP ini telah terdaftar!\nNama UCP: "LIGHTGREEN"%s\n"WHITE"Version: "DCRP""TEXT_GAMEMODE"\n"YELLOW"(Silahkan untuk membuat password baru dibawah ini):", AccountData[playerid][pUCP]);
			ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, ""PINK1"DCRP"WHITE"- Pembuatan Password",
			shstr, "Input", "Batal");
			printf("[debug] OnPlayerVerifyCode(PID: %d UCP: %s) terpanggil", playerid, ReturnName(playerid));
			return 1;
		}

		format(shstr, sizeof(shstr), ""WHITE"Selamat datang di "DCRP"DCRP\n"WHITE"UCP Ini telah terdaftar!\nNama UCP: "LIGHTGREEN"%s\
		\n"WHITE"Version: "DCRP""TEXT_GAMEMODE"\n"YELLOW"(Silahkan masukkan PIN yang dikirimkan oleh Bot DCRP ke Discord anda dibawah ini):", AccountData[playerid][pUCP]);
		return ShowPlayerDialog(playerid, DIALOG_VERIFYCODE, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Verifikasi Akun",
		shstr, "Input", "Batal");
	}
	...
	if(dialogid == DialogMyVeh)
	{
		...
		ShowPlayerDialog(playerid, DIALOG_MYV_MENU, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Kendaraan Saya", 
		"Cari Kendaraan\
	...
	if(dialogid == DIALOG_MYV_MENU)
	{
		...
		switch(listitem)
		{
			...
			case 1: // Detail kendaraan
			{
				...
				ShowPlayerDialog(playerid, DIALOG_VEHICLE_DETAIL, DIALOG_STYLE_INPUT, sprintf(""PINK1"DCRP"WHITE"- %s Vehicle Details", AccountData[playerid][pName]), strings, "Submit", "Batal");
			}
		}
	}
	if(dialogid == DIALOG_VEHICLE_DETAIL)
	{
		...
		(PlayerVehicle[vehid][pVehEngineUpgrade] != 1) ? ""RED"None" : ""GREEN"Upgrade", (PlayerVehicle[vehid][pVehBodyUpgrade] != 3) ? ""RED"None" : ""GREEN"Upgrade",
		GetFuel(PlayerVehicle[vehid][pVehPhysic]), vHealth, (PlayerVehicle[vehid][pVehEngineUpgrade] != 1) ? "1000.0" : "2000.0", PlayerVehicle[vehid][pVehBodyRepair]);
		ShowPlayerDialog(otherid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, sprintf(""PINK1"DCRP"WHITE"- %s Vehicle Details", AccountData[playerid][pName]), strings, "Tutup", "");
	}
	...
	if(dialogid == DIALOG_BANNEDREASON)
	{
		...
		new query[248], date;
		date = gettime();
		mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `player_bans` (`name`, `ip`, `admin`, `reason`, `ban_date`, `ban_expire`) VALUES ('%s', '%s', '%s', '%s', '%d', '%d')", AccountData[ClickPlayerID[playerid]][pUCP], PlayerIP, AccountData[playerid][pAdminname], inputtext, gettime(), banTime);
		mysql_tquery(dcrp_query, query);

		mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `warninglogs` SET `pID`=%d, `WarnTime`=%d, `WarnType`=3, `WarnSender`='%s', `WarnReason`='%s'", AccountData[ClickPlayerID[playerid]][pID], date, AccountData[playerid][pAdminname], inputtext);
		mysql_tquery(dcrp_query, query);
		...
		// SendEmbedMessage("DCRP", shstr, "Kemanan DCRP #1", "", "", "1152229209498980392", 0xff8fd5);
	...
	if(dialogid == DIALOG_VIP_NAME)
	{
		...
		if(isnull(inputtext)) return ShowPlayerDialog(playerid, DIALOG_VIP_NAME, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Vip Name Custom",
    ...
    if(dialogid == DIALOG_GIVEGUN_AMMO)
    {
        if(!response) return 1; // Player clicked cancel

        new targetid = GetPVarInt(playerid, "GiveGunTarget");
        new weaponid = GetPVarInt(playerid, "GiveGunWeapon");
        new maxammo = GetPVarInt(playerid, "GiveGunMaxAmmo");
        
        // Clear the variables
        DeletePVar(playerid, "GiveGunTarget");
        DeletePVar(playerid, "GiveGunWeapon");
        DeletePVar(playerid, "GiveGunMaxAmmo");

        // Validate if target is still valid
        if(!IsPlayerConnected(targetid) || !NearPlayer(playerid, targetid, 5.0))
            return ShowTDN(playerid, NOTIFICATION_ERROR, "Target player tidak valid atau terlalu jauh!");

        // Validate ammo input
        new ammo_to_give;
        if(sscanf(inputtext, "d", ammo_to_give))
            return ShowTDN(playerid, NOTIFICATION_ERROR, "Jumlah peluru tidak valid!");

        if(ammo_to_give < 1)
            return ShowTDN(playerid, NOTIFICATION_ERROR, "Jumlah peluru minimal 1!");

        if(ammo_to_give > maxammo)
            return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda tidak memiliki peluru sebanyak itu!");

        // Calculate remaining ammo for the original player
        new remaining_ammo = maxammo - ammo_to_give;

        // Give the weapon with specified ammo to target
        GivePlayerWeaponEx(targetid, weaponid, ammo_to_give);
        
        // Reset and give remaining ammo to original player
        ResetWeapon(playerid, weaponid);
        if(remaining_ammo > 0) {
            GivePlayerWeaponEx(playerid, weaponid, remaining_ammo);
        }
        
        SendClientMessageEx(playerid, -1, ""YELLOW"[!] Anda telah memberikan senjata %s dengan %d peluru kepada %s.", ReturnWeaponName(weaponid), ammo_to_give, ReturnName(targetid));
        SendClientMessageEx(targetid, -1, ""YELLOW"[!] %s telah memberikan senjata %s dengan %d peluru kepada anda.", ReturnName(playerid), ReturnWeaponName(weaponid), ammo_to_give);
        
        ApplyAnimation(playerid, "BOMBER", "BOM_Plant", 4.1, 0, 0, 0, 0, 0, 1);
        SendRPMeAboveHead(playerid, sprintf("Memberikan %s dengan %d peluru kepada %s", ReturnWeaponName(weaponid), ammo_to_give, ReturnName(targetid)), X11_PLUM1);
    }
