...
{
	new query[256];
	mysql_format(dcrp_query, query, sizeof(query), "SELECT `Char_Name`, `Char_Level`, `Char_Money`, `Char_BankMoney` FROM `player_characters` WHERE `Char_UCP`= '%s' LIMIT %d;", AccountData[playerid][pUCP], MAX_CHARS + AccountData[playerid][pExtraChar]);
...
public LoadCharacter(playerid)
{
	...
	{
		ucp::data[playerid][x][ucp::Name][0] = EOS;
	}
	for(new x = 0; x < cache_num_rows(); x ++)
	{
		cache_get_value_name(x, "Char_Name", ucp::data[playerid][x][ucp::Name]);
		cache_get_value_name_int(x, "Char_Level", ucp::data[playerid][x][ucp::Level]);
		cache_get_value_name_int(x, "Char_Money", ucp::data[playerid][x][ucp::Money]);
		cache_get_value_name_int(x, "Char_BankMoney", ucp::data[playerid][x][ucp::Bank]);
	}
...
ShowCharacterList(playerid)
{
	...
	strcat(frmtname, "Name\tLevel\tMoney\tBank\n");
	for (new i = 0; i < MAX_CHARS; i++) if (ucp::data[playerid][i][ucp::Name][0] != EOS)
	{
		format(strgbg, sizeof(strgbg), ""WHITE"%s\t%d\t%s\t%s\n", ucp::data[playerid][i][ucp::Name], ucp::data[playerid][i][ucp::Level], FormatMoney(ucp::data[playerid][i][ucp::Money]), FormatMoney(ucp::data[playerid][i][ucp::Bank]));
		strcat(frmtname, strgbg);
		count++;
	}
	if (count < MAX_CHARS)	
		strcat(frmtname, ""GREEN"+ New Character");
	ShowPlayerDialog(playerid, DIALOG_CHARLIST, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Character List", frmtname, "Pilih", "Keluar");
	return 1;
}

stock ResetUcpEnum(playerid)
{
	for(new x; x<MAX_CHARS; x++)
	{
		ucp::data[playerid][x][ucp::Name] = EOS;
		ucp::data[playerid][x][ucp::Money] = 0;
		ucp::data[playerid][x][ucp::Bank] = 0;
		ucp::data[playerid][x][ucp::Level] = 0;
	}
	return 1;
}*/
forward DoubleUCPChecker(playerid, name[]);
public DoubleUCPChecker(playerid, name[])
{
	new bool:active = false;

	foreach(new i : Player) if(AccountData[i][pSpawned] && !strcmp(AccountData[i][pUCP], name, true) && i != playerid) {
		active = true;
		break;
	}
	return active;
...
public CheckPlayerUCP(playerid, rcc)
{
    ...
    if(!DoubleUCPChecker(playerid, ReturnName(playerid))) {
		for(new x = 0; x < 1; x ++)
		{
			TextDrawShowForPlayer(playerid, DCRPLOGO[x]);
		}
		...
		if(cache_num_rows())
		{
			...
			if(!Blacklist_Check(playerid, "name", AccountData[playerid][pUCP])) {
				...
				{
					format(frmxtdialog, sizeof(frmxtdialog), ""WHITE"Selamat datang di "DCRP"DCRP\n"WHITE"UCP Ini telah terdaftar!\nNama UCP: "LIGHTGREEN"%s\
		...
		{
			new shstr[596];
			format(shstr, sizeof(shstr), ""WHITE"Dari: DCRP Bot\nKepada: Calon Aktor (pemain peran) di DCRP kami, "RED"%s\
			\n\n"WHITE"Silahkan terlebih dahulu mengambil tiket DCRP di discord sebelum dapat memasuki DCRP.\
			\nLink Discord: "YELLOW"dsc.gg/DCRP", AccountData[playerid][pUCP]);
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Checking Tiket", shstr, "Keluar", "");
			KickEx(playerid);
		}
	}
	else
...
public CheckBanUCP(playerid)
{
	if(cache_num_rows())
	{
		...
		{
			new query[248];
			mysql_format(dcrp_query, query, sizeof(query), "DELETE FROM player_bans WHERE name = '%s'", AccountData[playerid][pUCP]);
		...
		else
		{
			new query[248], PlayerIP[16], strgbg[712];
  			mysql_format(dcrp_query, query, sizeof query, "UPDATE `player_bans` SET `last_activity_timestamp` = '%d' WHERE `name` = '%s'", gettime(), AccountData[playerid][pUCP]);
			mysql_tquery(dcrp_query, query);
			...
			if(banTime_Int == 0)
			{
				format(strgbg, sizeof(strgbg), ""WHITE"UCP Anda diblokir dari DCRP!\n\nBerdasarkan informasi:\n"YELLOW"Nama UCP:"WHITE" %s\n"YELLOW"IP Address:"WHITE" %s\n"YELLOW"Admin On Duty:"WHITE" %s\n"YELLOW"Tanggal Blokir: "WHITE"%s\n"YELLOW"Reason:"WHITE" %s\n"YELLOW"Durasi Blokir: "WHITE"Permanent\
				\n\n"VERONA_G"INGAT:"WHITE" Jika anda ingin bermain lagi dan tidak mengulangi lagi, masuk ke Discord: "YELLOW"dsc.gg/DCRP"WHITE" kemudian pilih channel #req-unbanned", BannedName, PlayerIP, PlayerName, ReturnDate(banDate), Reason);
				ShowPlayerDialog(playerid, DIALOG_DISPLAYBANNED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- UCP Blocked", strgbg, "Keluar", "");
			}
			else
			{
				format(strgbg, sizeof(strgbg), ""WHITE"UCP Anda diblokir dari DCRP!\n\nBerdasarkan informasi:\n"YELLOW"Nama UCP:"WHITE" %s\n"YELLOW"IP Address:"WHITE" %s\n"YELLOW"Admin On Duty:"WHITE" %s\n"YELLOW"Tanggal Blokir: "WHITE"%s\n"YELLOW"Reason:"WHITE" %s\n"YELLOW"Durasi Blokir: "WHITE"%s\
				\n\n"VERONA_G"INGAT:"WHITE" Jika anda ingin bermain lagi dan tidak mengulangi lagi, masuk ke Discord: "YELLOW"dsc.gg/DCRP"WHITE" kemudian pilih channel #req-unbanned", BannedName, PlayerIP, PlayerName, ReturnDate(banDate), Reason, RemainingTimelapse(banTime_Int));
				ShowPlayerDialog(playerid, DIALOG_DISPLAYBANNED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- UCP Blocked", strgbg, "Keluar", "");
			}
  		}
	}
...
JadenDCRP: LoadPlayerData(playerid)
{	
	...
	new cQuery[1048];
	mysql_format(dcrp_query, cQuery, sizeof(cQuery), "SELECT * FROM `inventory` WHERE `ID`=%d", AccountData[playerid][pID]);
	mysql_tquery(dcrp_query, cQuery, "LoadInventory", "d", playerid);
	
	mysql_format(dcrp_query, cQuery, sizeof(cQuery), "SELECT * FROM `weaponsettings` WHERE `Owner`=%d", AccountData[playerid][pID]);
	mysql_tquery(dcrp_query, cQuery, "OnWeaponsLoaded", "d", playerid);
	...
	mysql_format(dcrp_query, cQuery, sizeof(cQuery), "SELECT * FROM `blacklist_players` WHERE `PID` = %d", AccountData[playerid][pID]);
	mysql_tquery(dcrp_query, cQuery, "LoadBlacklistInfo", "d", playerid);

	mysql_format(dcrp_query, cQuery, sizeof(cQuery), "SELECT * FROM `toys` WHERE `Owner` = '%s' LIMIT 1", AccountData[playerid][pName]);
	mysql_tquery(dcrp_query, cQuery, "LoadPlayerToys", "d", playerid);
	...
	/*if(gettime() > AccountData[playerid][LastSpawn])
	{
		if(AccountData[playerid][pInjured])
		{
			...
			ShowPlayerDialog(playerid, DIALOG_SELECT_SPAWNEXPIRED, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Pilih Lokasi Spawn",
	...
	else if(AccountData[playerid][pInjured] == 1 && AccountData[playerid][pInjuredTime] != 0)
	{
		...
		//Syntax(playerid, "Anda Pingsan tetapi Jika Tidak Ada Ems Yang Bertugas Gunakan {FFFF00}'/dokterlokal'");
		//AccountData[playerid][playerClickSpawn] = 1;
	}
	else
	{
		...
		//AccountData[playerid][playerClickSpawn] = 0;
		PlayerTextDrawSetString(playerid, SpawnTD[playerid][18], sprintf("%s", GetFactName(playerid)));
		ShowSpawnTextdraws(playerid);
	}
	...
	new vQuery[300];
	mysql_format(dcrp_query, vQuery, sizeof(vQuery), "SELECT * FROM `player_vehicles` WHERE `PVeh_OwnerID` = '%d' ORDER BY `id` ASC", AccountData[playerid][pID]);
	mysql_tquery(dcrp_query, vQuery, "Vehicle_Load", "d", playerid);
	...
	{
		new str[32];
		format(str, sizeof(str), "%d Menit %02d Detik", (g_AsuransiTime / 60) % 60, g_AsuransiTime % 60);
		PlayerTextDrawSetString(playerid, InsuranceTD[playerid][6], str);

		forex(a, 7)
		{
			PlayerTextDrawShow(playerid, InsuranceTD[playerid][a]);
		}
	}
	return 1;
}
...
public MakeWargaOffline(playerid, name[]) 
{
	...
	else
	{
		...
		new query[155];
		mysql_format(dcrp_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_Family`=-1, `Char_FamilyRank`=0, `Char_Faction`=0, `Char_FactionRank`=0 WHERE `pID`=%d", RegID);
		mysql_tquery(dcrp_query, query);
...
public InsertPlayerName(playerid, const name[])
{
	...
	if(rows > 0)
	{
		ShowPlayerDialog(playerid, DIALOG_MAKE_CHAR, DIALOG_STYLE_INPUT, ""PINK1"DCRP Roleplay "WHITE"- Pembuatan Karakter",
...
public OnPlayerRegister(playerid)
{
	...
	Inventory_Clear(playerid);
	ItemAdd(playerid, "Backpack");
	ShowItemBox(playerid, "Backpack", "Received_1x", 2);
	...
	mysql_format(dcrp_query, query, sizeof(query), "SELECT `Char_BankRek` FROM `player_characters` WHERE `Char_BankRek`=%d", rekening);
	mysql_tquery(dcrp_query, query, "BankRekening", "id", playerid, rekening);
	...
	mysql_format(dcrp_query, query, sizeof(query), "SELECT Char_PhoneNum FROM player_characters WHERE Char_PhoneNum='%s'", nope);
	mysql_tquery(dcrp_query, query, "PhoneNumber", "is", playerid, nope);

	mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `blacklist_players` SET `PID` = %d", AccountData[playerid][pID]);
	mysql_tquery(dcrp_query, query);	
...
public BankRekening(playerid, rekening)
{
	if(cache_num_rows())
	{
		...
		mysql_format(dcrp_query, query, sizeof(query), "SELECT `Char_BankRek` FROM `player_characters` WHERE `Char_BankRek`=%d", rekening);
		mysql_tquery(dcrp_query, query, "BankRekening", "id", playerid, rek);
		SendClientMessageEx(playerid, -1, "[i] Ada rekening yang sama dengan rekeningmu. Kami sedang mencarikan ulang untuk anda...");
	}
	else 
	{
		new query[125];
		mysql_format(dcrp_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_BankRek`=%d WHERE `pID`=%d", rekening, AccountData[playerid][pID]);
		mysql_tquery(dcrp_query, query);
...
public PhoneNumber(playerid, phone[])
{
	if(cache_num_rows() > 0)
	{
		...
		mysql_format(dcrp_query, query, sizeof(query), "SELECT `Char_PhoneNum` FROM `player_characters` WHERE `Char_PhoneNum`='%s'", phones);
		mysql_tquery(dcrp_query, query, "PhoneNumber", "is", playerid, phones);
		SendClientMessageEx(playerid, -1, "[i] Sudah ada yang menggunakan nomor telepon ini. kami akan mencarikan yang baru");
	}
	else 
	{
		new query[128];
		mysql_format(dcrp_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_PhoneNum`='%s' WHERE `pID`='%d'", phone, AccountData[playerid][pID]);
		mysql_tquery(dcrp_query, query);
...
public a_ChangeAdminName(otherplayer, playerid, nname[])
{
	...
	else
	{
		...
		new query[512];
	    mysql_format(dcrp_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_AdminName`='%s' WHERE `pID`=%d", nname, AccountData[otherplayer][pID]);
		mysql_tquery(dcrp_query, query);
...
public OnOBanQueryData(adminid, UCPToBan[], banReason[], banTime)
{
	...
	else 
	{
		...
		new twsmk[512];
		mysql_format(dcrp_query, twsmk, sizeof(twsmk), "INSERT INTO player_bans (name, ip, admin, reason, ban_date, ban_expire) VALUES ('%s', '%s', '%s', '%s', UNIX_TIMESTAMP(), %d)", UCPToBan, PlayerIP, AccountData[adminid][pAdminname], banReason, datez);
		mysql_tquery(dcrp_query, twsmk);
	}
	return 1;
}

JadenDCRP: PressJumpReset(playerid)
{
...
public CheckUCP(playerid, nameucp[])
{
	...
	if(count)
	{
		...
		tstr = ""PINK1"DCRP"WHITE"- UCP: {FFFF00}", strcat(tstr, nameucp);
...
public FactDutyHour(playerid)
{
	...
	if(AccountData[playerid][pFactDutyTimer] == 3600)
	{
		...
		ShowPlayerFooter(playerid, "+ 5,000 XP", 2000);
...
public UsingDokterLokal(playerid)
{
	...
	if(AccountData[playerid][ActivityTime] >= 15)
	{
		...
		ShowItemBox(playerid, "Uang", "Removed_$2000", 2);
	}
	else
	{
...
stock LoadServerPickup()
{
	...
	format(strings, sizeof(strings), ""PINK1"[Mount Chilliad Skydiving Point]\n\n"YELLOW"`/skydive`"WHITE"-> Mulai terjun payung.\n"WHITE"[Harga]:"GREEN"$50");
...
public ChangeMPrice()
{
	...
	SendClientMessageToAll(-1, ""ORANGE"[Pasar] : "WHITE"Harga Pasar telah berubah, gunakan "YELLOW"'/mprice' "WHITE"untuk melihat harga jual terbaru");
	return 1; 
}

task UpdateRusunTime[1000]()
{
	new currentTime = gettime();
	...
	if(currentTime >= g_RusunTime)
	{
		foreach(new rsid : Rusun)
		{
			...
			mysql_tquery(dcrp_query, sprintf("UPDATE `player_characters` SET `Char_HasRusunID`=-1 WHERE `pID`=%d", RusunData[rsid][rusunOwnerID]));
			...
			new query[258];
			mysql_format(dcrp_query, query, sizeof(query), "UPDATE `stuffs` SET `globaltime_rusun`=%d WHERE `ID`=0", g_RusunTime);
			mysql_tquery(dcrp_query, query);
		}
	}
	return 1;
...
JadenDCRP: DestroyLabelOut(playerid)
{
...
Blacklist_Check(playerid, const type[], target[])
{
	...
	execute = mysql_query(dcrp_query, sprintf("SELECT * FROM `player_bans` WHERE `%s` = '%s' LIMIT 1;", type, target));
	...
	if(cache_num_rows())
	{
		...
		if(time != 0 && time <= currentTime) // melepas status banned akun
		{
			...
			mysql_tquery(dcrp_query, sprintf("SELECT * FROM `player_characters` WHERE `Char_Name` = '%s' LIMIT 1;", pbanname), "LoadPlayerData", "d", playerid);
		}
		else
		{
			new PlayerIP[16];
			GetPlayerIp(playerid, PlayerIP, sizeof(PlayerIP));
			mysql_tquery(dcrp_query, sprintf("UPDATE `player_bans` SET `last_activity_timestamp` = '%d' WHERE `name` = '%s'", gettime(), AccountData[playerid][pUCP]));
			...
			{
				format(strgbg, sizeof(strgbg), ""WHITE"UCP Anda diblokir dari DCRP!\n\nBerdasarkan informasi:\n"YELLOW"Nama UCP:"WHITE" %s\n"YELLOW"IP Address:"WHITE" %s\n"YELLOW"Admin On Duty:"WHITE" %s\n"YELLOW"Tanggal Blokir: "WHITE"%s\n"YELLOW"Reason:"WHITE" %s\n"YELLOW"Durasi Blokir: "WHITE"Permanent\
				\n\n"VERONA_G"INGAT:"WHITE" Jika anda ingin bermain lagi dan tidak mengulangi lagi, masuk ke Discord: "YELLOW"dsc.gg/DCRP"WHITE" kemudian pilih channel #req-unbanned", username, PlayerIP, banby, ReturnDate(date), reason);
				ShowPlayerDialog(playerid, DIALOG_DISPLAYBANNED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- UCP Blocked", strgbg, "Keluar", "");
			}
			else
			{
				format(strgbg, sizeof(strgbg), ""WHITE"UCP Anda diblokir dari DCRP!\n\nBerdasarkan informasi:\n"YELLOW"Nama UCP:"WHITE" %s\n"YELLOW"IP Address:"WHITE" %s\n"YELLOW"Admin On Duty:"WHITE" %s\n"YELLOW"Tanggal Blokir: "WHITE"%s\n"YELLOW"Reason:"WHITE" %s\n"YELLOW"Durasi Blokir: "WHITE"%s\
				\n\n"VERONA_G"INGAT:"WHITE" Jika anda ingin bermain lagi dan tidak mengulangi lagi, masuk ke Discord: "YELLOW"dsc.gg/DCRP"WHITE" kemudian pilih channel #req-unbanned", username, PlayerIP, banby, ReturnDate(date), reason, RemainingTimelapse(time));
				ShowPlayerDialog(playerid, DIALOG_DISPLAYBANNED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- UCP Blocked", strgbg, "Keluar", "");
			}
		}
		// KickEx(playerid);
...
{
	mysql_tquery(dcrp_query, sprintf("DELETE FROM `player_bans` WHERE `name` = '%s';", SQL_ReturnEscaped(username)));
	return 1;
}
...
Blacklist_UCPExists(const type[], const target[])
{
	...
	execute = mysql_query(dcrp_query, sprintf("SELECT `%s` FROM `player_characters` WHERE `%s` = '%s' LIMIT 1;", type, type, target));
...
Blacklist_Exists(const type[], const target[])
{
	...
	execute = mysql_query(dcrp_query, sprintf("SELECT `%s` FROM `player_bans` WHERE `%s` = '%s' LIMIT 1;", type, type, target));
