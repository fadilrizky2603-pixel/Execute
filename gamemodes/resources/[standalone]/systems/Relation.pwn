    ...
    if(response)
    {
        switch(listitem)
        {
            case 0:
            {
                ...
                new Cache:checkname = mysql_query(dcrp_query, sprintf("SELECT `PartnerName` FROM `users` WHERE `pID` = '%d' LIMIT 1;",AccountData[playerid][pID]));
                if(cache_num_rows())
                {
                ...
                new Cache:checklove = mysql_query(dcrp_query, sprintf("SELECT `Love` FROM `users` WHERE `pID` = '%d' LIMIT 1;", AccountData[playerid][pPartnerID]));
                if(cache_num_rows())
                {
                ...
                Dialog_Show(playerid, ShowOnly, DIALOG_STYLE_MSGBOX, ""LIGHTBLUE""SERVER_NAME""WHITE" - Status Hubungan", output, "Close", "");
            }
            case 1:
            {
                ...
                if(IsPlayerConnected(targetid))
                {
                    ...
                    { 
                        ...
                        mysql_tquery(dcrp_query, sprintf("UPDATE `users` SET `PartnerGift` = '%d' WHERE pID = %d;", gift_date, AccountData[playerid][pID]));
                        mysql_tquery(dcrp_query, sprintf("UPDATE `users` SET `PartnerGift` = '%d' WHERE pID = %d;", gift_date, AccountData[targetid][pID]));
                ...
                {
                    new partnerlove;
                    new Cache:checkaccount = mysql_query(dcrp_query, sprintf("SELECT `Love` FROM `users` WHERE `pID` = '%d' LIMIT 1;",AccountData[playerid][pPartnerID]));
                    if(cache_num_rows())
                    {
                    ...
                    mysql_tquery(dcrp_query, sprintf("UPDATE `users` SET `PartnerGift` = '%d' WHERE pID = %d;", gift_date, AccountData[playerid][pPartnerID]));
                    //Ngeupdate love si partnernya kalau offline
                    mysql_tquery(dcrp_query, sprintf("UPDATE `users` SET `Love` = '%d' WHERE pID = %d;", partnerlove+10, AccountData[playerid][pPartnerID]));
                }                
            }
            case 2:
            {
                Dialog_Show(playerid, RelationInvites, DIALOG_STYLE_INPUT, ""LIGHTBLUE""SERVER_NAME""WHITE" - Relation Invite", ""RED"[PERINGATAN] "WHITE"Kamu tidak bisa menjalin hubungan dengan sesama jenis!!!\n\n"YELLOW"(Masukan id/kantong player yang ingin kamu ajak menjalin hubungan)", "Select", "Close");
            }
            case 3:
            {   
                new targetid = IsPlayerNearPartner(playerid);
                ...
                {
                    mysql_tquery(dcrp_query, sprintf("UPDATE `users` SET `Status` = '0', `Love` = '0.0', `PartnerID` = '0', `PartnerName` = 'Single' WHERE pID = %d", AccountData[playerid][pPartnerID]));
...
CMD:relationcheck(playerid, params[])
{
    ...
    new Cache:checkname = mysql_query(dcrp_query, sprintf("SELECT `PartnerName` FROM `users` WHERE `pID` = '%d' LIMIT 1;",AccountData[userid][pID]));
    if(cache_num_rows())
    {
        cache_get_value_name(0, "PartnerName", partnername, 32);
        cache_delete(checkname);
    ...
    new Cache:checklove = mysql_query(dcrp_query, sprintf("SELECT `Love` FROM `users` WHERE `pID` = '%d' LIMIT 1;", AccountData[userid][pPartnerID]));
    if(cache_num_rows())
    {
        cache_get_value_name_float(0, "Love", partnerlove);
        cache_delete(checklove);
...
Dialog:RelationInvites(playerid, response, listitem, inputtext[])
{
    ...
    Dialog_Show(playerid, RelationMenu, DIALOG_STYLE_LIST, ""LIGHTBLUE""SERVER_NAME""WHITE" - Relationship Invitation", "Invite to date\nMarry her!", "Select", "Cancel");
    return 1;
}
    ...
    if(response)
    {
        ...
        switch(listitem)
        {
            case 0:
            {
                ...
                Dialog_Show(userid, RelationInvite, DIALOG_STYLE_MSGBOX, ""LIGHTBLUE""SERVER_NAME""WHITE" - Relationship Invitation", ""RED"%s "WHITE"has invited you to date them", "Accept", "Decline", name);
            }
            case 1:
            {
                if(IsPlayerInRangeOfPoint(playerid, 10.0, 1220.0037, -1446.7375, 45.2224))
                {
                    if(AccountData[playerid][pLove] == 100 && AccountData[userid][pLove] == 100)
                    {
                        if(AccountData[playerid][pPartnerID] == AccountData[userid][pID])
                        {
                            ...
                            Dialog_Show(userid, RelationInviteMarry, DIALOG_STYLE_MSGBOX, ""LIGHTBLUE""SERVER_NAME""WHITE" - Relationship Invitation", ""RED"%s "WHITE"has proposed you to marry them", "Accept", "Decline", name);               
                        }
