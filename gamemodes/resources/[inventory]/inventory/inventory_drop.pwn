...
DropPlayerItem(playerid, itemid, quantity = 1)
{
    ...
    new tstr[16];
    format(tstr, sizeof(tstr), "Removed_%dx", quantity);
    ShowItemBox(playerid, string, tstr, 2);
    
    DropItem(string, InventoryData[playerid][itemid][invModel], quantity, x, y, z - 0.9, GetPlayerInterior(playerid), GetPlayerVirtualWorld(playerid));
    ItemDelete(playerid, string, quantity);
...
DropItem(const item[], model, quantity, Float:x, Float:y, Float:z, interior, world)
{
    for (new i = 0; i != MAX_DROPPED_ITEMS; i ++) if(!DroppedItems[i][droppedModel])
    {
        ...
        new tstr[256];
        format(tstr, sizeof(tstr), ""PINK1"[%s]\n{FFFFFF}| %dx | %.3f KG", DroppedItems[i][droppedItem], DroppedItems[i][droppedQuantity], GetItemKG(DroppedItems[i][droppedItem], DroppedItems[i][droppedQuantity]));
        DroppedItems[i][droppedLabel] = CreateDynamic3DTextLabel(tstr, -1, DroppedItems[i][droppedPos][0], DroppedItems[i][droppedPos][1], DroppedItems[i][droppedPos][2], 5, _, _, _, DroppedItems[i][droppedWorld],DroppedItems[i][droppedInt]);
        return i;
    }
    return -1;
}
...
CMD:deletedrop(playerid)
{
    ...
    new
        count = 0;
    for (new i = 0; i < MAX_DROPPED_ITEMS; i ++) if(count < MAX_LISTED_ITEMS && DroppedItems[i][droppedModel] && IsPlayerInRangeOfPoint(playerid, 10.5, DroppedItems[i][droppedPos][0], DroppedItems[i][droppedPos][1], DroppedItems[i][droppedPos][2]) && GetPlayerInterior(playerid) == DroppedItems[i][droppedInt] && GetPlayerVirtualWorld(playerid) == DroppedItems[i][droppedWorld]) 
    {
...
PickupItem(playerid, itemid)
{
    if(itemid != -1 && DroppedItems[itemid][droppedModel])
    {
        if(IsInventoryFull(playerid, DroppedItems[itemid][droppedItem], DroppedItems[itemid][droppedQuantity])) return ShowTDN(playerid, NOTIFICATION_ERROR, "Tas kamu tidak kuat menampung lebih banyak item lagi.");

        new tstr[16];
        format(tstr, sizeof(tstr), "Received_%dx", DroppedItems[itemid][droppedQuantity]);
        ShowItemBox(playerid, DroppedItems[itemid][droppedItem], tstr, 2);

        if(ItemAdd(playerid, DroppedItems[itemid][droppedItem], DroppedItems[itemid][droppedQuantity]) == -1)
            return 1;
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == DROP_ITEM)
    {
        ...
        {
            if(isnull(inputtext))
                return ShowPlayerDialog(playerid, DROP_ITEM, DIALOG_STYLE_INPUT, "Drop Item", "Berapa jumlah item yang akan anda buang:", "Drop", "Cancel");

            if(strval(inputtext) < 1 || strval(inputtext) > InventoryData[playerid][itemid][invQuantity])
                return ShowPlayerDialog(playerid, DROP_ITEM, DIALOG_STYLE_INPUT, "Drop Item", "Jumlah yang anda input tidak valid.\nBerapa jumlah item yang akan anda buang:", "Drop", "Cancel");

            DropPlayerItem(playerid, itemid, strval(inputtext));
        }
    }
    if(dialogid == PICKUP_ITEM)
    {
        if(!response) return true;
        {
            ...
            {
                if(IsInventoryFull(playerid, DroppedItems[id][droppedItem], DroppedItems[id][droppedQuantity])) return ShowTDN(playerid, NOTIFICATION_ERROR, "Tas kamu tidak kuat menampung lebih banyak item lagi.");

                if(PickupItem(playerid, id))
                {
                    SendRPMeAboveHead(playerid, "Mengambil barang dibawah", X11_PLUM1);
                    ApplyAnimation(playerid, "BOMBER", "BOM_Plant", 4.1, 0, 0, 0, 0, 0, 1);
                }
                else ShowTDN(playerid, NOTIFICATION_ERROR, "Anda tidak memiliki slot yang cukup di inventory!");
            }
            else ShowTDN(playerid, NOTIFICATION_ERROR, "Barang telah diambil oleh seseorang!");
        }
    }
    return 1;
}
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        ...
        if(GetPlayerSpecialAction(playerid) == SPECIAL_ACTION_DUCK)
        {
            ...
            {
                String = "Nama Barang\tJumlah\tBerat\n";
             
                for (new i = 0; i < MAX_DROPPED_ITEMS; i ++) if(count < MAX_LISTED_ITEMS && DroppedItems[i][droppedModel] && IsPlayerInRangeOfPoint(playerid, 2.5, DroppedItems[i][droppedPos][0], DroppedItems[i][droppedPos][1], DroppedItems[i][droppedPos][2]) && GetPlayerInterior(playerid) == DroppedItems[i][droppedInt] && GetPlayerVirtualWorld(playerid) == DroppedItems[i][droppedWorld]) 
                {
                    format(str, sizeof(str), "%s\t%d\t%.3f KG\n", DroppedItems[i][droppedItem], DroppedItems[i][droppedQuantity], GetItemKG(DroppedItems[i][droppedItem], DroppedItems[i][droppedQuantity]));
                    strcat(String, str);
                    NearestItems[playerid][count++] = i;
                }
                if(count == 1)
                ...
                {
                    return ShowPlayerDialog(playerid, PICKUP_ITEM, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"Pickup Items", String, "Pickup", "Cancel");
                }
            }
        }
    }
...
stock Item_Delete(itemid)
{
    ...
    if (itemid != -1 && DroppedItems[itemid][droppedModel])
	{
	    ...
	    mysql_format(dcrp_query, query, sizeof(query), "DELETE FROM `dropped` WHERE `ID` = '%d'", DroppedItems[itemid][droppedID]);
...
DropItem(item[], player[], model, quantity, Float:x, Float:y, Float:z, interior, world)
{
	...
	forex(i, MAX_DROPPED_ITEMS) if (!DroppedItems[i][droppedModel])
	{
		...
		mysql_tquery(dcrp_query, query, "OnDroppedItem", "d", i);
		return i;
	}
	return -1;
}
	...
	switch(dialogid)
	{
		case DialogDropItem:
		{
			...
			if(response)
			{
				...
				ShowPlayerDialog(playerid, DialogDropItem, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Drop Barang", str, "Drop", "Cancel");
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(newkeys & KEY_WALK && GetPlayerSpecialAction(playerid) == SPECIAL_ACTION_DUCK)
    {
        ...
        if(id != -1)
        {
            ...
            else ShowPlayerDialog(playerid, DIALOG_ITEM_PICKUP, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Pickup Item", shstr, "Ambil", "Batal");
        }
    }
	return 1;
}