...
ShowContactList(playerid)
{
	...
	if(count)
	{
		...
		ShowPlayerDialog(playerid, DialogContact, DIALOG_STYLE_TABLIST_HEADERS, sprintf(""PINK1"DCRP"WHITE"- Daftar Kontak (%d/100)", CountPlayerContact(playerid)), sha, "Pilih", "Batal");
		return 1;
	}
	else ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, sprintf(""PINK1"DCRP"WHITE"- Daftar Kontak (%d/100)", CountPlayerContact(playerid)), "Anda tidak memiliki kontak tersimpan!", "Tutup", ""), PlayerContactPage[playerid] = 0;
	return 1;
}
...
/*ShowContactList(playerid)
{
	...
	mysql_format(dcrp_query, frmtstring, sizeof(frmtstring), "SELECT * FROM `contacts` WHERE `ID`=%d ORDER BY ID ASC LIMIT %d, 15", AccountData[playerid][pID], page * 15);
	result = mysql_query(dcrp_query, frmtstring, true);
	...
	if(count)
	{
		...
		if(page != 0) strcat(sha, ""RED"<< Halaman Sebelumnya\t\n");
		ShowPlayerDialog(playerid, DialogContact, DIALOG_STYLE_TABLIST_HEADERS, sprintf(""PINK1"DCRP"WHITE"- Daftar Kontak | Halaman (%d)", PlayerContactPage[playerid] + 1), sha, "Pilih", "Batal");
		return 1;
	}
	else ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, sprintf(""PINK1"DCRP"WHITE"- Daftar Kontak | Halaman (%d)", PlayerContactPage[playerid] + 1), "Anda tidak memiliki kontak tersimpan!", "Tutup", ""), PlayerContactPage[playerid] = 0;
	return 1;
}
...
ShowContactList(playerid)
{
	...
	if(count != 0)
		ShowPlayerDialogPages(playerid, "ContactListDialog", DIALOG_STYLE_TABLIST_HEADERS, sprintf(""PINK1"DCRP"WHITE"- Daftar Kontak (%d/100)", CountPlayerContact(playerid)), "Pilih", "Batal", 15, ""LIGHTGREEN">> Next Page", ""RED"<< Previous Page");
	else
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Daftar Kontak (0/100)", "Anda tidak memiliki kontak tersimpan!", "Tutup", "");
	return 1;
}*/
...
/*ShowContactList(playerid)
{
	...
	if(count > 0)
		ShowPlayerDialog(playerid, DialogContact, DIALOG_STYLE_TABLIST_HEADERS, sprintf(""PINK1"DCRP"WHITE"- Daftar Kontak (%d/100)", CountPlayerContact(playerid)), shstr, "Pilih", "Batal");
	else 
		ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Daftar Kontak (0/100)", "Anda tidak memiliki kontak tersimpan!", "Tutup", "");
	return 1;
}*/
...
public OnContactEditName(playerid, contactIndex, const newname[])
{
	...
	mysql_tquery(dcrp_query, sprintf("UPDATE `contacts` SET `contactName`='%s' WHERE `contactID`=%d", newname, ContactData[playerid][contactIndex][contactID]));
	return 1;
}
...
public OnContactEditNumber(playerid, contactIndex, const newnumber[])
{
	...
	mysql_tquery(dcrp_query, sprintf("UPDATE `contacts` SET `contactNumber`='%s' WHERE `contactID`=%d", newnumber, ContactData[playerid][contactIndex][contactID]));
	return 1;
}
...
public OnWhatsappSent(playerid, targetid, contactSelected, serverid)
{
	...
	if(AccountData[targetid][CurrentlyReadWA])
	{
		if(!strcmp(ContactData[targetid][cidt][contactNumber], AccountData[playerid][pPhone], false))
		{
			...
			mysql_format(dcrp_query, query, sizeof(query), "SELECT * FROM `whatsapp_chats` WHERE `ID`=%d ORDER BY `chatTimestamp` ASC LIMIT 30", serverid);
			execute = mysql_query(dcrp_query, query, true);
			new rows = cache_num_rows();
			if(rows)
			{
				if(rows >= 30)
				{
					mysql_format(dcrp_query, harlem, sizeof(harlem), "DELETE FROM `whatsapp_chats` WHERE `ID`=%d", serverid);
					mysql_tquery(dcrp_query, harlem);
				}

				ContactData[targetid][cidt][contactUnread] = 0;
				mysql_format(dcrp_query, query, sizeof(query), "UPDATE `contacts` SET `contactUnread`=0 WHERE `contactID`=%d", ContactData[targetid][cidt][contactID]);
	...
	if(rows)
	{
		if(rows >= 30)
		{
			mysql_format(dcrp_query, query, sizeof(query), "DELETE FROM `whatsapp_chats` WHERE `ID`=%d", serverid);
			mysql_tquery(dcrp_query, query);
		}

		ContactData[playerid][contactSelected][contactUnread] = 0;
		mysql_format(dcrp_query, query, sizeof(query), "UPDATE `contacts` SET `contactUnread`=0 WHERE `contactID`=%d", ContactData[playerid][contactSelected][contactID]);
		new list[2500], date[64], issuer[24], watext[128];
