		...
		if(killerid != INVALID_PLAYER_ID)
		{
				...
				if(EventData[eventScore][eventTeams[killerid]-1] >= EventData[eventTarget]) 
				{
					...
					{ 
						if(IsPlayerInEvent(i))
						{
							...
							{
								GiveMoney(i, EventData[eventWinnerPrice]);
								SendCustomMessage(i, "EVENT", "Team anda menang dalam event ini dan mendapat hadiah sebanyak "GREEN"%s.", FormatMoney(EventData[eventWinnerPrice]));
							}
							else
							{
								GiveMoney(i, EventData[eventBonus]);
			...
			else if(EventData[eventType] == TYPE_DM)
			{
					...
					{ 
						if(IsPlayerInEvent(i))
						{
							if(i == EventData[eventWinner]){
								GiveMoney(i, EventData[eventBonus]*3);
				...
				else 
				{
					if(GetPlayerState(playerid) == PLAYER_STATE_WASTED)
					{
						SetSpawnInfo(playerid, eventTeams[playerid], AccountData[playerid][pSkin], EventData[eventSpawnX_DM], EventData[eventSpawnY_DM], EventData[eventSpawnZ_DM], 0.0, 0, 0, 0, 0, 0, 0);
					}
					eventWaitingSpawn[playerid] = 1;
				}
			}
...
hook OnPlayerEnterRaceCP(playerid)
{
	if(IsPlayerInEvent(playerid) && EventData[eventType] == TYPE_JETPACK)
	{
		...
		if(eventCheckpoint[playerid] == (EventData[eventJetpackType]*16)) 
		{
			...
			SendEventMessageAll(COLOR_CLIENT, sprintf("EVENT: "WHITE"Event "RED"Jetpack "WHITE"telah selesai dan dimenangkan oleh "YELLOW"%s", ReturnNames(playerid, 0)));
			SendAdminMessage(X11_TOMATO_1, "AdmWarn: Event Jetpack dimenangkan oleh %s", ReturnNames(playerid, 0));

		    foreach(new i : Player) if(IsPlayerInEvent(i))
		    {
		    	if(EventData[eventWinner] == playerid)
					{
						GiveMoney(playerid, EventData[eventBonus]);
...
hook OnScriptInit()
{
    eventTextdraws[0] = TextDrawCreate(503.306, 147.999, "box");
    TextDrawLetterSize(eventTextdraws[0], 0.000, 8.594);
    TextDrawTextSize(eventTextdraws[0], 631.395, 0.000);
    ...
    eventTextdraws[1] = TextDrawCreate(567.464, 150.716, "- tdm event score -");
    TextDrawLetterSize(eventTextdraws[1], 0.251, 1.349);
    TextDrawTextSize(eventTextdraws[1], 0.000, 123.000);
    TextDrawAlignment(eventTextdraws[1], 2);
    TextDrawColor(eventTextdraws[1], -1061109505);
    TextDrawUseBox(eventTextdraws[1], 1);
    TextDrawBoxColor(eventTextdraws[1], -260013825);
    TextDrawSetShadow(eventTextdraws[1], 0);
    TextDrawSetOutline(eventTextdraws[1], 1);
    TextDrawBackgroundColor(eventTextdraws[1], 255);
    TextDrawFont(eventTextdraws[1], 3);
    TextDrawSetProportional(eventTextdraws[1], 1);

    eventTextdraws[2] = TextDrawCreate(565.522, 166.834, "Target:_250");
    TextDrawLetterSize(eventTextdraws[2], 0.296, 1.732);
    ...
    eventTextdraws[3] = TextDrawCreate(505.045, 190.785, "Team_A:_0");
    TextDrawLetterSize(eventTextdraws[3], 0.270, 1.399);
    ...
    eventTextdraws[4] = TextDrawCreate(635.461, 332.133, "_");
    TextDrawLetterSize(eventTextdraws[4], 0.203, 1.174);
    ...
    eventTextdraws[5] = TextDrawCreate(635.461, 343.133, "_");
    TextDrawLetterSize(eventTextdraws[5], 0.203, 1.174);
    ...
    eventTextdraws[6] = TextDrawCreate(635.461, 354.133, "_");
    TextDrawLetterSize(eventTextdraws[6], 0.203, 1.174);
    ...
    eventTextdraws[7] = TextDrawCreate(635.461, 365.133, "_");
    TextDrawLetterSize(eventTextdraws[7], 0.203, 1.174);
