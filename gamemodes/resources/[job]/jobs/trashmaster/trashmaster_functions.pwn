    ...
    if(newstate == PLAYER_STATE_DRIVER)
    {
        if(!AccountData[playerid][pAdminDuty] && !IsTrashmasterWorking(playerid) && IsTrashmasterVehicles(GetPlayerVehicleID(playerid)))
        {
            if(!AccountData[playerid][pTrashmasterDelay])
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_TRASHMASTER_START, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Trashmaster Sidejob",
...
hook OnPlayerEnterRaceCP(playerid)
{
    if(IsTrashmasterWorking(playerid) && GetPVarInt(playerid, "SelesaiTrashmaster") == 1)
    {
        ...
        ShowItemBox(playerid, "Uang", "Received_$2,000", 2);
...
ptask RespawnTrashmasterVehicle[1000](playerid)
{
    if (SQL_IsCharacterLogged(playerid) && IsTrashmasterWorking(playerid))
    {
        ...
        if (counter > 0)
        {
            ...
            ShowPlayerFooter(playerid, sprintf("~y~Warning ~n~~w~Anda harus naik ke kendaraan sebelum ~r~%d detik ~w~berakhir atau pekerjaan akan gagal", TrashmasterRespawnCounter[playerid]), 1000);
        }
    }
    return 1;
}
