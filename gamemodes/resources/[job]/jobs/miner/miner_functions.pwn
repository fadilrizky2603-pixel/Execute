...
NERO::MinerLabelMap(playerid)
{
	UnloadMinerLabelMap(playerid);

	PlayerMinerVars[playerid][LeburTextLabel][0] = CreateDynamic3DTextLabel(""GREEN"[ALT]"WHITE" Meleburkan Batu", -1, 2152.3289, -2263.5745, 13.3000+0.4, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, -1, -1, playerid);
	PlayerMinerVars[playerid][LeburTextLabel][1] = CreateDynamic3DTextLabel(""GREEN"[ALT]"WHITE" Meleburkan Batu", -1, 2147.9492, -2263.9167, 13.2982+0.4, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, -1, -1, playerid);
	PlayerMinerVars[playerid][LeburTextLabel][2] = CreateDynamic3DTextLabel(""GREEN"[ALT]"WHITE" Meleburkan Batu", -1, 2152.1406, -2267.9675, 13.3071+0.4, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, -1, -1, playerid);
...
NERO::UnloadMinerLabelMap(playerid)
{
	for(new i = 0; i < 3; i ++)
	{
...
NERO::TakeBatu(playerid)
{
	...
	if(AccountData[playerid][ActivityTime] >= 120)
	{
		...
		switch(rands)
		{
			...
			{
				ItemAdd(playerid, "Batu Kotor", randbatu);
				ItemAdd(playerid, "Material", 1);
				ShowItemBox(playerid, "Batu Kotor", sprintf("Received_%dx", randbatu), 2);
				ShowItemBox(playerid, "Material", "Received_1x", 2);
			}
		}
		pCount1[playerid] = 25;
		HideShortKey(playerid);
	...
	else 
	{
		...
		PlayerTextDrawTextSize(playerid, ProgressBar[playerid][1], progressvalue, 19.000000);
...
NERO::CuciBatu(playerid)
{
	...
	if(AccountData[playerid][ActivityTime] >= 126)
	{
		...
		ItemDelete(playerid, "Batu Kotor", 2);
		ItemAdd(playerid, "Batu Bersih", 2);
		ShowItemBox(playerid, "Batu Bersih", "Received_2x", 2);
	}
	else 
	{
	...
	if(AccountData[playerid][ActivityTime] >= 126)
	{
		...
		switch(rands) 
		{
			case 1..50:
			{
				...
				ItemDelete(playerid, "Batu Bersih", 1);
			...
			case 51..55:
			{
				...
				ItemAdd(playerid, "Besi", 5);
				ItemDelete(playerid, "Batu Bersih", 1);
				ShowItemBox(playerid, "Besi", "Received_5x", 2);
				ShowItemBox(playerid, "Emas", sprintf("Received_%dx", randemas), 2);
			}
			case 56..58:
			{
				ItemAdd(playerid, "Berlian", randberlian);
				ItemDelete(playerid, "Batu Bersih", 1);
				ShowItemBox(playerid, "Berlian", sprintf("Received_%dx", randberlian), 2);
