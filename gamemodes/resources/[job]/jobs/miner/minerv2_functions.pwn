...
stock MinerJobStuffs(playerid) 
{
    ...
    PlayerMinerVars[playerid][MinerLeburText][0] = CreateDynamic3DTextLabel(""GREEN"[ALT]"WHITE" Meleburkan Batu", -1, 2152.3289, -2263.5745, 13.3000+0.4, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, -1, -1, playerid);
    PlayerMinerVars[playerid][MinerLeburText][1] = CreateDynamic3DTextLabel(""GREEN"[ALT]"WHITE" Meleburkan Batu", -1, 2147.9492, -2263.9167, 13.2982+0.4, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, -1, -1, playerid);
    PlayerMinerVars[playerid][MinerLeburText][2] = CreateDynamic3DTextLabel(""GREEN"[ALT]"WHITE" Meleburkan Batu", -1, 2152.1406, -2267.9675, 13.3071+0.4, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, -1, -1, playerid);
        ...
        if(AccountData[playerid][pJob] == JOB_MINER) 
        {
            for(new x = 0; x < 23; x++) 
            {
                if(IsPlayerInDynamicArea(playerid, PlayerMinerVars[playerid][MinerStoneArea][x])) 
                {
                    ...
                    if(Inventory_Count(playerid, "Batu Kotor") >= 100) return ShowTDN(playerid, NOTIFICATION_ERROR, "Tidak dapat membawa Batu Kotor lebih dari 100!");
            ...
            if(IsPlayerInDynamicArea(playerid, PlayerMinerVars[playerid][MinerStoneWash]))
            {
                if(AccountData[playerid][ActivityTime] != 0) return ShowTDN(playerid, NOTIFICATION_WARNING, "Anda sedang melakukan sesuatu, harap tunggu!");
                if(AccountData[playerid][pInjured]) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda sedang pingsan!");
                if(Inventory_Count(playerid, "Batu Bersih") >= 100) return ShowTDN(playerid, NOTIFICATION_ERROR, "Tidak dapat membawa Batu Bersih lebih dari 100!");
                ...
                PlayerTextDrawSetString(playerid, ProgressBar[playerid][1], "MENCUCI BATUAN");
                ShowProgressBar(playerid);
                HideShortKey(playerid);
            ...
            for(new x = 0; x < 3; x++)
            {
                if(IsPlayerInDynamicArea(playerid, PlayerMinerVars[playerid][MinerStoneLebur][x]))
                {
                    ...
                    PlayerTextDrawSetString(playerid, ProgressBar[playerid][1], "MELEBUR BATUAN");
                    ShowProgressBar(playerid);
                    ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 1, 0, 0, 0, 0, 1);
                }
            }
...
public MinerStoneTake(playerid, t) 
{
    ...
    if(AccountData[playerid][ActivityTime] >= 10)
    {
        ...
        switch(rands) 
        {
            case 1..79:
            {
                ItemAdd(playerid, "Batu Kotor", randbatu);
                ShowItemBox(playerid, "Batu Kotor", sprintf("Received_%dx", randbatu), 2);
            }
            case 80..100:
            {
                ItemAdd(playerid, "Batu Kotor", 1);

                new randv = RandomEx(1, 2);
                ItemAdd(playerid, "Material", randv);
                ShowItemBox(playerid, "Material", sprintf("Received_%dx", randv), 2);
                ShowItemBox(playerid, "Batu Kotor", "Received_1x", 2);
            }
        }
...
public MinerWashing(playerid) 
{
    ...
    if(AccountData[playerid][ActivityTime] >= 8)
    {
        ...
        ItemDelete(playerid, "Batu Kotor", 5);
        ItemAdd(playerid, "Batu Bersih", 5);
        ShowItemBox(playerid, "Batu Kotor", "Removed_5x", 2);
        ShowItemBox(playerid, "Batu Bersih", "Received_5x", 2);
...
public MinerLebur(playerid, t)
{
    ...
    if(AccountData[playerid][ActivityTime] >= 6)
    {
        ...
        switch(rands) 
        {
            case 1..50:
            {
                ItemDelete(playerid, "Batu Bersih");
                ItemAdd(playerid, "Besi", randbesi);
                ItemAdd(playerid, "Tembaga", randtembaga);
                ShowItemBox(playerid, "Besi", sprintf("Received_%dx", randbesi), 2);
                ShowItemBox(playerid, "Tembaga", sprintf("Received_%dx", randtembaga), 2);
                ShowItemBox(playerid, "Batu Bersih", "Removed_1x", 2);
            }
            case 51..55:
            {
                ItemDelete(playerid, "Batu Bersih");
                ItemAdd(playerid, "Emas", randemas);
                ShowItemBox(playerid, "Emas", sprintf("Received_%dx", randemas), 2);
                ShowItemBox(playerid, "Batu Bersih", "Removed_1x", 2);
            }
            case 56..58:
            {
                ItemDelete(playerid, "Batu Bersih");
                ItemAdd(playerid, "Berlian", 1);
                ShowItemBox(playerid, "Berlian", "Received_1x", 2);
                ShowItemBox(playerid, "Batu Bersih", "Removed_1x", 2);
            }
            case 59..70:
            {
    ...
    else 
    {
        ...
        static Float: progressvalue;
        progressvalue = AccountData[playerid][ActivityTime] * 67/6;
        PlayerTextDrawTextSize(playerid, ProgressBar[playerid][2], progressvalue, 2.000000);
        PlayerTextDrawShow(playerid, ProgressBar[playerid][2]);
        return 0;
    }
