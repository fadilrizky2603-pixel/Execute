    ...
    if(newstate == PLAYER_STATE_DRIVER)
    {
        ...
        if(!AccountData[playerid][pAdminDuty] && !IsDeliveryWorking(playerid) && IsDeliveryVehicles(vehicleid))
        {
            if(AccountData[playerid][pDeliveryTime] == 0)
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_DELIVERY_START, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Delivery Sidejob",
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    ...
    {
        new vehicle_nearest = GetNearestVehicleToPlayer(playerid, 5.0, false);
...
ptask RespawnDeliveryVehicle[1000](playerid)
{
    if(SQL_IsCharacterLogged(playerid) && IsDeliveryWorking(playerid))
    {
        if(DeliveryRespawnCounter[playerid] != 0)
        {
            ...
            ShowPlayerFooter(playerid, sprintf("~y~Warning ~n~~w~Anda harus naik ke kendaraan sebelum ~r~%d detik ~w~berakhir atau pekerjaan akan gagal", DeliveryRespawnCounter[playerid]), 1000);
        }
    }
    return 1;
}
