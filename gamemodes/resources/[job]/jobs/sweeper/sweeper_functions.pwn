    ...
    if(newstate == PLAYER_STATE_DRIVER)
    {
        ...
        if(IsSweeperVehicle(vehicleid))
        {
            if(!DurringSweeping[playerid])
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_SWEEPER_START, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Sweeper Sidejob", "Apakah anda ingin memulai tugas sweeper?\
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    switch(dialogid)
    {
        case DIALOG_SWEEPER_START:
        {
            ...
            new query[255];
            mysql_format(dcrp_query, query, sizeof(query), "UPDATE `player_characters` SET `Char_SweeperDelay`=1800 WHERE `pID`=%d", AccountData[playerid][pID]);
...
hook OnPlayerEnterCP(playerid)
{
    if(DurringSweeping[playerid] && IsSweeperVehicle(GetPlayerVehicleID(playerid)))
    {
        if(SweeperIndex[playerid] < 13)
        {
            ...
            ShowItemBox(playerid, "Uang", "Received_$80", 2);
        }
        else 
        {
            ...
            ShowItemBox(playerid, "Uang", "Received_$100", 2);
        }
    }
    return 1;
}

ptask VehicleSweeperSpeed[1000](playerid) 
{
    new vehicleid = GetPlayerVehicleID(playerid);
    if(DurringSweeping[playerid] && IsSweeperVehicle(vehicleid)) 
    {
