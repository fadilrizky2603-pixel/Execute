...
LoadVarsDaur(playerid)
{
    ...
    PlayerRecyclerVars[playerid][RecyclerStartLabel] = CreateDynamic3DTextLabel(""GREEN"[ALT]"WHITE" Mulai Recycler", -1, -1485.7316, 2594.8167, 28.9015, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 6, playerid);
    ...
    forex(i, 4)
    {
        ...
        PlayerRecyclerVars[playerid][RecyclerOlahLabel][i] = CreateDynamic3DTextLabel(""GREEN"[ALT]"WHITE" Daur Ulang", -1, OlahPoint[i][0], OlahPoint[i][1], OlahPoint[i][2], 15.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 0, playerid);
    }
    return 1;
}
    ...
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        if(AccountData[playerid][pJob] == JOB_RECYCLER)
        {
            if(IsPlayerInDynamicArea(playerid, PlayerRecyclerVars[playerid][RecyclerStartArea]))
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_RECYCLER_START, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Recycler Job", 
            ...
            for(new x = 0; x < 4; x++)
            {
                if(IsPlayerInDynamicArea(playerid, PlayerRecyclerVars[playerid][RecyclerSortirArea][x]) && PlayerRecyclerVars[playerid][DurringRecycler])
                {
                    ...
                    PlayerTextDrawSetString(playerid, ProgressBar[playerid][1], "MENYORTIR");
                    ShowProgressBar(playerid);
                    ApplyAnimationEx(playerid, "BD_FIRE", "wash_up", 4.1, 1, 0, 0, 0, 0, 1);
                ...
                if(IsPlayerInDynamicArea(playerid, PlayerRecyclerVars[playerid][RecyclerOlahArea][x]))
                {
                    ...
                    PlayerTextDrawSetString(playerid, ProgressBar[playerid][1], "DAUR ULANG");
                    ShowProgressBar(playerid);
                    ApplyAnimationEx(playerid, "BD_FIRE", "wash_up", 4.1, 1, 0, 0, 0, 0, 1);
        ...
        case DIALOG_RECYCLER_START:
        {
            ...
            switch(listitem)
            {
                case 0: //mulai
                {
                    ...
                    PlayerRecyclerVars[playerid][RecyclerTakeLabel] = CreateDynamic3DTextLabel(""GREEN"[ALT]"WHITE" Mengambil", -1, TakePoint[rand][0], TakePoint[rand][1], TakePoint[rand][2], 100.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, 0, 6, playerid);
    ...
    if(AccountData[playerid][ActivityTime] >= 8)
    {
        ...
        switch(rand)
        {
            case 1..9:
            {
                ...
                PlayerRecyclerVars[playerid][RecyclerSortirLabel][0] = CreateDynamic3DTextLabel(""GREEN"[ALT]"WHITE" Sortir", -1, -1447.6829, 2583.7949, 28.9015, 100.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, 0, 6, playerid);
            }
            case 10..19:
            {
                PlayerRecyclerVars[playerid][RecyclerSortirArea][1] = CreateDynamicSphere(-1461.1429, 2583.5684, 28.9015, 2.0, 0, 6, playerid);
                PlayerRecyclerVars[playerid][RecyclerSortirLabel][1] = CreateDynamic3DTextLabel(""GREEN"[ALT]"WHITE" Sortir", -1, -1461.1429, 2583.5684, 28.9015, 100.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, 0, 6, playerid);
            }
            case 20..29:
            {
                PlayerRecyclerVars[playerid][RecyclerSortirArea][2] = CreateDynamicSphere(-1472.1234, 2583.6997, 28.9015, 2.0, 0, 6, playerid);
                PlayerRecyclerVars[playerid][RecyclerSortirLabel][2] = CreateDynamic3DTextLabel(""GREEN"[ALT]"WHITE" Sortir", -1, -1472.1234, 2583.6997, 28.9015, 100.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, 0, 6, playerid);
            }
            case 30..40:
            {
                PlayerRecyclerVars[playerid][RecyclerSortirArea][3] = CreateDynamicSphere(-1485.2805, 2583.6887, 28.9015, 2.0, 0, 6, playerid);
                PlayerRecyclerVars[playerid][RecyclerSortirLabel][3] = CreateDynamic3DTextLabel(""GREEN"[ALT]"WHITE" Sortir", -1, -1485.2805, 2583.6887, 28.9015, 100.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, 0, 6, playerid);
            }
        }
    }
    else
...
public SortirRecycler(playerid, t)
{
    ...
    if(AccountData[playerid][ActivityTime] >= 15)
    {
        ...
        switch(rands)
        {
            case 1..21:
            {
                ItemAdd(playerid, "Boxmats", randbox);
                ShowItemBox(playerid, "Boxmats", sprintf("Received_%dx", randbox), 2);
            }
            case 22..51:
            {
                ItemAdd(playerid, "Boxmats", randbox);
        ...
        PlayerRecyclerVars[playerid][RecyclerTakeLabel] = CreateDynamic3DTextLabel(""GREEN"[ALT]"WHITE" Mengambil", -1, TakePoint[randtakes][0], TakePoint[randtakes][1], TakePoint[randtakes][2], 100.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, 0, 6, playerid);
    }
    else
    {
        AccountData[playerid][ActivityTime] ++;
...
public RecyclerOlah(playerid, t)
{
    ...
    if(AccountData[playerid][ActivityTime] >= 15)
    {
        ...
        switch(rands)
        {
            case 1..11:
            {
                ItemDelete(playerid, "Boxmats", 5);
                ItemAdd(playerid, "Baja", randbaja);
                ItemAdd(playerid, "Plat Besi", randplatbesi);
                ShowItemBox(playerid, "Baja", sprintf("Received_%dx", randbaja), 2);
                ShowItemBox(playerid, "Plat Besi", sprintf("Received_%dx", randplatbesi), 2);
                ShowItemBox(playerid, "Boxmats", "Removed_5x", 2);
            }
            case 12..35:
            {
                ItemDelete(playerid, "Boxmats", 5);
                ItemAdd(playerid, "Plastik", randplastik);
                ItemAdd(playerid, "Plat Besi", randplatbesi);
                ShowItemBox(playerid, "Plastik", sprintf("Received_%dx", randplastik), 2);
                ShowItemBox(playerid, "Plat Besi", sprintf("Received_%dx", randplatbesi), 2);
                ShowItemBox(playerid, "Boxmats", "Removed_5x", 2);
            }
            case 36..56:
            {
                ItemDelete(playerid, "Boxmats", 5);
                ItemAdd(playerid, "Karet", randkaret);
                ItemAdd(playerid, "Plat Besi", randplatbesi);
                ShowItemBox(playerid, "Karet", sprintf("Received_%dx", randkaret), 2);
                ShowItemBox(playerid, "Plat Besi", sprintf("Received_%dx", randplatbesi), 2);
                ShowItemBox(playerid, "Boxmats", "Removed_5x", 2);
            }
            case 57..71:
            {
                ItemDelete(playerid, "Boxmats", 5);
                ItemAdd(playerid, "Alumunium", randalumu);
                ItemAdd(playerid, "Plat Besi", randplatbesi);
                ShowItemBox(playerid, "Alumunium", sprintf("Received_%dx", randalumu), 2);
                ShowItemBox(playerid, "Plat Besi", sprintf("Received_%dx", randplatbesi), 2);
                ShowItemBox(playerid, "Boxmats", "Removed_5x", 2);
            }
            case 72..89:
            {
                ItemDelete(playerid, "Boxmats", 5);
                ItemAdd(playerid, "Kaca", randkaca);
                ItemAdd(playerid, "Plat Besi", randplatbesi);
                ShowItemBox(playerid, "Kaca", sprintf("Received_%dx", randkaca), 2);
                ShowItemBox(playerid, "Plat Besi", sprintf("Received_%dx", randplatbesi), 2);
                ShowItemBox(playerid, "Boxmats", "Removed_5x", 2);
            }
        }
