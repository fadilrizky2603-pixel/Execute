    ...
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        if(AccountData[playerid][pJob] == JOB_RECYCLER)
        {
            if(IsPlayerInDynamicArea(playerid, PlayerDaurVars[playerid][DaurStartArea]))
            {
                ...
                {
                    Dialog_Show(playerid, DialogStartRecycler, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Recycler Job", ""VERONA_ARWIN"Mulai Pekerjaan\n"WHITE"Selesaikan Pekerjaan\n", "Pilih", "Batal");
                }
            }
