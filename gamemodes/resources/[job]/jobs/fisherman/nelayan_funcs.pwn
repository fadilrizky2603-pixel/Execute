...
LoadVarsFisherman(playerid)
{
    ...
    if(AccountData[playerid][pJob] == JOB_FISHERMAN)
    {
        ...
        FishermanBoatLabel[playerid] = CreateDynamic3DTextLabel(""LIGHTGREY"[Vehicle Rental]\n"BLUEJEGE"Rental Perahu\n"GREEN"[ALT]"WHITE" untuk mengeluarkan perahu nelayan", -1, 111.3999, -1895.6553, 2.9408 + 0.5, 15.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 0, playerid, 15.0, -1, 0);
    }
    return 1;
}
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(newkeys & KEY_WALK)
    {
        if(AccountData[playerid][pJob] == JOB_FISHERMAN)
        {
            ...
            {
                Dialog_Show(playerid, FishermanBoatSpawn, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Sewa Perahu",
                "Jenis Perahu\tHarga\
                \nReefer\t$150\
            ...
            if(IsPlayerInDynamicArea(playerid, NetFisherArea[playerid]))
            {
                ...
                PlayerTextDrawSetString(playerid, ProgressBar[playerid][1], "NET FISHING");
...
public NetFishing(playerid)
{
    ...
    if(AccountData[playerid][ActivityTime] >= 15)
    {
        ...
        ItemAdd(playerid, "Ikan", 1);
        ShowItemBox(playerid, "Ikan", "Received_1x", 2);
        new rands = RandomEx(1, 51), randgaram = RandomEx(1, 3);
        switch(rands)
        {
            case 1..11:
            {
                ItemAdd(playerid, "Garam Kristal", randgaram);
                Info(playerid, "Anda mendapatkan "YELLOW"Garam Kristal"WHITE" sebanyak "YELLOW"%dx", randgaram);
            }
            case 12..31:
            {
                ItemAdd(playerid, "Garam Kristal", 2);
