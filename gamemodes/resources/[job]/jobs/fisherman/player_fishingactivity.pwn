...
new STREAMER_TAG_AREA:FishingArea[14];
...
hook OnPlayerDisconnect(playerid, reason)
{
    ...
    // Destroy JadenFish UI on disconnect
    for(new i = 0; i < 59; i++)
    {
        if(JadenFish[playerid][i] != PlayerText:INVALID_TEXT_DRAW)
        {
            PlayerTextDrawDestroy(playerid, JadenFish[playerid][i]);
            JadenFish[playerid][i] = PlayerText:INVALID_TEXT_DRAW;
        }
    }
    return 1;
}
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        ...
        CleanupFishingActivity(playerid);
        ...
        if(IsPlayerInDynamicArea(playerid, SellFishArea))
        {
            ...
            if(GetPlayerState(playerid) != PLAYER_STATE_ONFOOT) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda harus berada di darat!");
            if(IsPlayerInAnyVehicle(playerid)) return ShowTDN(playerid, NOTIFICATION_ERROR, "Anda tidak bisa melakukan ini di dalam kendaraan!");
        ...
        if(IsPlayerInDynamicArea(playerid, SellFishIlegalArea))
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_SELLFISH_ILEGAL, DIALOG_STYLE_LIST, ""PINK1"DreamCountry"WHITE"- Sell Ilegal Fish",
            "Ikan Hiu\nPenyu", "Pilih", "Batal");
        }
        ...
        for(new x = 0; x < 14; x ++)
        {
            if(IsPlayerInDynamicArea(playerid, FishingArea[x]))
            {
                ...
                return 1; // Prevent multiple fishing spots activation
            }
        }
    }
    return 1;
...
public UpdateJadenFish(playerid)
{
    ...
    // Check if player is too far from any fishing spot
    new bool:isNearFishingSpot = false;
    for(new x = 0; x < 14; x++)
    {
        if(IsPlayerInDynamicArea(playerid, FishingArea[x]))
        {
    ...
    if(JadenFish[playerid][21] != PlayerText:INVALID_TEXT_DRAW)
    {
    ...
    if(JadenFish[playerid][22] != PlayerText:INVALID_TEXT_DRAW)
    {
    ...
    // Reapply animation to prevent it from stopping
    ApplyAnimationEx(playerid, "SWORD", "sword_block", 50.0, 0, 1, 0, 1, 1, 1);
...
public FishingTime(playerid, x)
{
    ...
    if(!IsValidDynamicArea(FishingArea[x]) || !IsPlayerInDynamicArea(playerid, FishingArea[x]))
    {
        CleanupFishingActivity(playerid);
        AccountData[playerid][ActivityTime] = 0;
        return 0;
...
CleanupFishingActivity(playerid)
{
    ...
    // Kill existing timers if any
    if(pTimerMancing[playerid] != -1)
    {
        KillTimer(pTimerMancing[playerid]);
        pTimerMancing[playerid] = -1;
    }
    if(pTimerJadenFish[playerid] != -1)
    {
        KillTimer(pTimerJadenFish[playerid]);
        pTimerJadenFish[playerid] = -1;
    }
    if(pTimerSellFish[playerid] != -1)
    {
        KillTimer(pTimerSellFish[playerid]);
        pTimerSellFish[playerid] = -1;
    }

    // Hide existing JadenFish UI if any
    for(new i = 0; i < 59; i++)
    {
        if(JadenFish[playerid][i] != PlayerText:INVALID_TEXT_DRAW)
        {
    ...
    // Cleanup animations and objects
    ClearAnimations(playerid, 1);
    StopLoopingAnim(playerid);
    SetPlayerSpecialAction(playerid, SPECIAL_ACTION_NONE);
    RemovePlayerAttachedObject(playerid, 9);
    
    // Reset activity time if it was set
    if(AccountData[playerid][ActivityTime] != 0)
    {
        AccountData[playerid][ActivityTime] = 0;
        HideProgressBar(playerid);
