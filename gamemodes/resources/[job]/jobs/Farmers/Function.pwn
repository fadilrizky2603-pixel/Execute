...
JadenDCRP: CreatePlant(playerid, type, time)
{
...
timer CreatePlant[3000](playerid, type, time)
{
    ...
    if (GetPlayerPos(playerid, x, y, z) && GetPlayerFacingAngle(playerid, ang))
    {
        if ((id = Iter_Free(Plants)) != -1)
        {
            ...
            mysql_tquery(dcrp_query, "INSERT INTO `plants` (`PlantInt`) VALUES(0)", "OnPlantCreated", "d", id);
            RefreshPlant(id);
            return id;
        }
    }
...
timer HarvestPlant[3000](playerid, plantid)
{
    ...
    switch (PlantData[plantid][PlantType])
    {
        case PADI:
        {
            ...
            ItemAdd(playerid, "Padi", rand);
            ShowItemBox(playerid, "Padi", sprintf("Received_%dx", rand), 2);
        ...
        {
            new rand = Random(10, 15);
            ItemAdd(playerid, "Tebu", rand);
            ShowItemBox(playerid, "Tebu", sprintf("Received_%dx", rand), 2);
        ...
        {
            new rand = Random(10, 15);
            ItemAdd(playerid, "Cabe", rand);
            ShowItemBox(playerid, "Cabe", sprintf("Received_%dx", rand), 2);
