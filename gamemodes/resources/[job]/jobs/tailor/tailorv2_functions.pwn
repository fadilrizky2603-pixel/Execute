    ...
    if(AccountData[playerid][pJob] == JOB_TAILOR)
    {
        for(new i = 0; i < 4; i ++) {
            if(areaid == PlayerTailorVars[playerid][TailorMakeBenang][i]) {
                ShowKey(playerid, "Membuat Benang");
            }
        }

        for(new i = 0; i < 7; i ++) {
            if(areaid == PlayerTailorVars[playerid][TailorMakeKaC][i]) {
                ShowKey(playerid, "Olah Bahan");
            }
        }
    }
    return 1;
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        if(TailorBenang_Area(playerid))
        {
            ...
            PlayerTextDrawSetString(playerid, ProgressBar[playerid][1], "MENGOLAH BENANG");
        ...
        if(TailorKaC_Area(playerid))
        {
            ...
            Dialog_Show(playerid, DialogOlahTailor, DIALOG_STYLE_TABLIST_HEADERS, "Olah Bahan -"DCRP" DCRP",
            "Bahan\tKebutuhan\
            \nKain\t1 Benang\
...
Dialog:DialogOlahTailor(playerid, response, listitem, inputtext[])
{
    ...
    switch(listitem)
    {
        ...
        case 1:// 
        {
            ...
            PlayerTextDrawSetString(playerid, ProgressBar[playerid][1], "MEMBUAT PAKAIAN");
            ShowProgressBar(playerid);
            ApplyAnimation(playerid, "BD_FIRE", "wash_up", 4.1, 1, 0, 0, 0, 0, 1);  
        }
    }
