...
hook OnPlayerStateChange(playerid, newstate, oldstate)
{
    ...
    if(newstate == PLAYER_STATE_DRIVER && !IsMowerWorking(playerid) && IsMowerVehicles(vehicleid))
    {
        if(!AccountData[playerid][pMowerTime])
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_MOWER_START, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Mower Sidejob",
...
ptask RespawnMowerVehicle[1000](playerid)
{
    if(IsMowerWorking(playerid))
    {
        if(MowerRespawnCounter[playerid] > 0)
        {
            ...
            ShowPlayerFooter(playerid, sprintf("~y~Mowing Job ~n~~w~Anda harus memotong rumput selama ~r~%d detik ~w~untuk mendapatkan hasil kerja", MowerRespawnCounter[playerid]), 1000);
        }
    }
    return 1;
}
...
task MowingVehicle[1000]()
{
    ...
    {
        if(GetVehicleSpeedCap(MowerVehicles[0]) > 30.0)
            SetVehicleSpeedCap(MowerVehicles[0], 30.0);
    }
    if(IsValidVehicle(MowerVehicles[1]))
    {
        if(GetVehicleSpeedCap(MowerVehicles[1]) > 30.0)
            SetVehicleSpeedCap(MowerVehicles[1], 30.0);
    }
    if(IsValidVehicle(MowerVehicles[2]))
    {
        if(GetVehicleSpeedCap(MowerVehicles[2]) > 30.0)
            SetVehicleSpeedCap(MowerVehicles[2], 30.0);
    }
    return 1;
}