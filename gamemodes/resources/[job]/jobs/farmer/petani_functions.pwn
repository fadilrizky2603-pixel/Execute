...
LoadVarsFarmer(playerid)
{
    ...
    PlayerFarmerVars[playerid][FarmerOlahLabel] = CreateDynamic3DTextLabel(""GREEN"[ALT]"WHITE" Olah Tanaman", -1, 3.9224, 66.8390, 3.1172 + 0.25, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, -1, -1, playerid);
    PlayerFarmerVars[playerid][FarmerStoreLabel] = CreateDynamic3DTextLabel(""GREEN"[ALT]"WHITE" Membeli bibit", -1, -547.7806, -185.1288, 78.4063, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, -1, -1, playerid);
}
...
stock Weed_Delete(id)
{
    ...
    mysql_tquery(dcrp_query, sprintf("DELETE FROM `weeds` WHERE `ID` = '%d'", WeedData[id][weedID]));
...
public HarvestWeed(playerid, wid)
{
    ...
    if(AccountData[playerid][ActivityTime] >= 15)
    {
        ...
        {
            ItemAdd(playerid, "Padi", rands);
            ShowItemBox(playerid, "Padi", sprintf("Received_%dx", rands), 2);
        }
        else if(WeedData[wid][weedType] == 2) // Tebu
        {
            ItemAdd(playerid, "Tebu", rands);
            ShowItemBox(playerid, "Tebu", sprintf("Received_%dx", rands), 2);
        }
        else if(WeedData[wid][weedType] == 3) // Cabai
        {
            ItemAdd(playerid, "Cabe", rands);
            ShowItemBox(playerid, "Cabe", sprintf("Received_%dx", rands), 2);
        }
        Weed_Delete(wid);
    }
    else 
    {
        ...
        PlayerTextDrawTextSize(playerid, ProgressBar[playerid][2], progressvalue, 2.000000);
...
stock Weed_Create(playerid, type)
{
    ...
    if(GetPlayerPos(playerid, x, y, z))
    {
        forex(i, MAX_WEED) if(!WeedData[i][weedExists])
        {
            ...
            Iter_Add(Weeds, i);
            new query[178];
            mysql_format(dcrp_query, query, sizeof(query), "INSERT INTO `weeds` (`Time`) VALUES ('%d')", WeedData[i][weedTime]);
            mysql_tquery(dcrp_query, query, "OnWeedCreated", "d", i);    
            return i;
        }
    }
    return -1;
...
{
    new query[596];
    mysql_format(dcrp_query, query, sizeof(query), "UPDATE `weeds` SET ");
    mysql_format(dcrp_query, query, sizeof(query), "%s`PosX`='%f', ", query, WeedData[id][weedPos][0]);
    mysql_format(dcrp_query, query, sizeof(query), "%s`PosY`='%f', ", query, WeedData[id][weedPos][1]);
    mysql_format(dcrp_query, query, sizeof(query), "%s`PosZ`='%f', ", query, WeedData[id][weedPos][2]);
    mysql_format(dcrp_query, query, sizeof(query), "%s`Time`='%d', ", query, WeedData[id][weedTime]);
    mysql_format(dcrp_query, query, sizeof(query), "%s`Type`='%d' ", query, WeedData[id][weedType]);
    mysql_format(dcrp_query, query, sizeof(query), "%sWHERE `ID` = '%d'", query, WeedData[id][weedID]);
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        ...
        {
            ShowPlayerDialog(playerid, DIALOG_BUY_SEEDS, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE" - Beli Bibit",
            "Jenis Bibit\tHarga\
            \nCabe\t"DARKGREEN"$7\
            \nTebu\t"DARKGREEN"$10\
        ...
        if(IsPlayerInDynamicArea(playerid, PlayerFarmerVars[playerid][FarmerOlahArea]))
        {
            ...
            {
                ShowPlayerDialog(playerid, DIALOG_FARMER_OLAH, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Olah Tanaman",
                "Item\tReqruitment\
    ...
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT && AccountData[playerid][pJob] == JOB_FARMER)
    {
    ...
    switch(dialogid)
    {
        case DIALOG_BUY_SEEDS:
        {
            ...
            switch(listitem)
            {
                case 0:// cabe
                {
                    ShowPlayerDialog(playerid, DIALOG_BIBIT_CABE, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Bibit Cabe", 
                ...
                case 1:// tebu
                {
                    ShowPlayerDialog(playerid, DIALOG_BIBIT_TEBU, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Bibit Tebu", 
                    ""WHITE"Anda akan membeli "YELLOW"bibit tebu"WHITE" seharga "DARKGREEN"$10/bibit\
                    \n"YELLOW"(Masukkan berapa banyak bibit yang ingin anda beli):", "Input", "Batal");
                }
                case 2:// padi
                {
                    ShowPlayerDialog(playerid, DIALOG_BIBIT_PADI, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Bibit Padi", 
        ...
        case DIALOG_BIBIT_CABE:
        {
            ...
            if(isnull(inputtext))
            {
                return ShowPlayerDialog(playerid, DIALOG_BIBIT_CABE, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Bibit Cabe", 
            ...
            ItemAdd(playerid, "Bibit Cabe", quantity);
            ShowItemBox(playerid, "Bibit Cabe", sprintf("Received_%dx", quantity), 2);
        }
        case DIALOG_BIBIT_TEBU:
        {
            ...
            if(isnull(inputtext))
            {
                return ShowPlayerDialog(playerid, DIALOG_BIBIT_TEBU, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Bibit Tebu", 
            ...
            ItemAdd(playerid, "Bibit Tebu", quantity);
            ShowItemBox(playerid, "Bibit Tebu", sprintf("Received_%dx", quantity), 2);
        }
        case DIALOG_BIBIT_PADI:
        {
            ...
            if(isnull(inputtext))
            {
                return ShowPlayerDialog(playerid, DIALOG_BIBIT_PADI, DIALOG_STYLE_INPUT, ""PINK1"DCRP"WHITE"- Bibit Padi", 
            ...
            ItemAdd(playerid, "Bibit Padi", quantity);
            ShowItemBox(playerid, "Bibit Padi", sprintf("Received_%dx", quantity), 2);
        }
        case DIALOG_FARMER_OLAH:
        {
            ...
            switch(listitem)
            {
                case 0: //sambal
                {
                    ...
                    PlayerTextDrawSetString(playerid, ProgressBar[playerid][1], "MENGOLAH TANAMAN");
                    ShowProgressBar(playerid);
...
public PlantProcessed(playerid)
{
    ...
    if(AccountData[playerid][ActivityTime] >= 145)
    {
        ...
        switch(AccountData[playerid][pTempOlah])
        {
            case 0: //Sambal
            {
                ItemDelete(playerid, "Cabe", 4);
                ItemDelete(playerid, "Botol");
                ItemAdd(playerid, "Sambal");
                ShowItemBox(playerid, "Cabe", "Removed_4x", 2);
                ShowItemBox(playerid, "Botol", "Removed_1x", 2);
                ShowItemBox(playerid, "Sambal",  "Received_1x",2);
            }
            case 1: // beras
            {
                ItemDelete(playerid, "Padi", 4);
                ItemDelete(playerid, "Kain");
                ItemAdd(playerid, "Beras");
                ShowItemBox(playerid, "Padi", "Removed_4x", 2);
                ShowItemBox(playerid, "Kain", "Removed_1x", 2);
                ShowItemBox(playerid, "Beras", "Received_1x", 2);
            }
            case 2: //Tebu
            {
                ItemDelete(playerid, "Tebu", 4);
                ItemDelete(playerid, "Kain");
                ItemAdd(playerid, "Gula");
                ShowItemBox(playerid, "Tebu", "Removed_4x", 2);
                ShowItemBox(playerid, "Kain", "Removed_1x", 2);
                ShowItemBox(playerid, "Gula", "Received_1x", 2);
            }
            case 3: // Garam
            {
                ItemDelete(playerid, "Garam Kristal", 4);
                ItemDelete(playerid, "Botol");
                ItemAdd(playerid, "Garam");
                ShowItemBox(playerid, "Garam Kristal", "Removed_4x", 2);
                ShowItemBox(playerid, "Botol", "Removed_1x", 2);
                ShowItemBox(playerid, "Garam", "Received_1x", 2);
            }
        }
