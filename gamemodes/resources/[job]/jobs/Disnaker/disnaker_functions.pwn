...
hook OnGameModeInit()
{
    CreateDynamic3DTextLabel(""GREEN"[ALT]"WHITE" Akses Disnaker", -1, 1430.7720, 1541.8113, 16.3877, 15.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1);
    // CreateDynamic3DTextLabel(""GREEN"[ALT]"WHITE" Membuat KTP", -1, 1376.7292, 1573.6344, 17.0003, 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1);
    return 1;
}
    ...
    if(response)
    {
        ...
        switch(listitem)
        {
            case 0: // Ambil Job
            {
                ...
                countrecycler,
                countmixer);
                ShowPlayerDialog(playerid, DIALOG_DISNAKER, DIALOG_STYLE_TABLIST_HEADERS, ""PINK1"DCRP"WHITE"- Disnaker", string, "Pilih", "Batal");
            }
            case 1: // Buat KTP
            {
...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(newkeys & KEY_WALK && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
    {
        ...
        {
            Dialog_Show(playerid, DISNAKER_MENU, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Disnaker Menu",
