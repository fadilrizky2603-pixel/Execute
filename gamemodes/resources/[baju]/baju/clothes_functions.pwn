...
CMD:sc(playerid, params[])
{
    ...
    new cQuery[2118];
    mysql_format(dcrp_query, cQuery, sizeof(cQuery), "INSERT INTO `clothes_wardrobe` SET `IDs`=%d, `clothesName`='%s', `clothesModel`=%d,\
    ...
    mysql_tquery(dcrp_query, cQuery, "OnClothesDeposit", "d", playerid);
    return 1;
}
...
{
    new dbstr[255];
    mysql_format(dcrp_query, dbstr, sizeof(dbstr), "SELECT * FROM `clothes_wardrobe` WHERE `IDs`=%d", AccountData[playerid][pID]);
    mysql_query(dcrp_query, dbstr);
...
ShowPlayerClothes(playerid)
{
    ...
    if(rows)
    {
        ...
        ShowPlayerDialog(playerid, DIALOG_CLOTHES, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Lemari Pakaian", list, "Pilih", "Batal");
    }
    else 
    {
        PlayerPlaySound(playerid, 5206, 0.0, 0.0, 0.0);
        ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""PINK1"DCRP"WHITE"- Lemari Pakaian", 
...
DropClothesPlayer(playerid)
{
    ...
    if(cache_num_rows())
    {
        ...
        ShowPlayerDialog(playerid, DIALOG_CLOTHES_DELETE, DIALOG_STYLE_LIST, ""PINK1"DCRP"WHITE"- Buang Pakaian", list, "Buang", "Batal");
    }
    else 
    {
    ...
    switch(dialogid)
    {
        ...
        case DIALOG_CLOTHES_DELETE:
        {
            ...
            if(cache_num_rows())
            {
                ...
                new query[200];
                mysql_format(dcrp_query, query, sizeof(query), "DELETE FROM `clothes_wardrobe` WHERE `clothesModel` = %d", clothesmodel);
