...
YCMD:setgravityforme(playerid, params[])
{
    new Float:fGravity;
...
YCMD:serversettings(playerid, params[])
{
...
YCMD:setrecdir(playerid, params[])
{
...
YCMD:ysftickrate(playerid, params[])
{
	new ticks;
...
YCMD:restarttime(playerid, params[])
{
	new Float:fTime;
...
YCMD:maxplayers(playerid, params[])
{
	new maxplayers;
...
YCMD:allownamechar(playerid, params[])
{
...
YCMD:isnamechar(playerid, params[])
{
	new character;
...
YCMD:classinfo(playerid, params[])
{
...
YCMD:scoreupdate(playerid, params[])
{
	new update;
...
YCMD:setnameinquery(playerid, params[])
{
...
YCMD:timeouttime(playerid, params[])
{
	new playertwo, time;
...
YCMD:myteamforplayer(playerid, params[])
{
...
YCMD:nameforme(playerid, params[])
{
...
YCMD:skinforme(playerid, params[])
{
...
YCMD:styleforme(playerid, params[])
{
	new playertwo, skinid;
...
YCMD:quatforme(playerid, params[])
{
...
YCMD:showplayer(playerid, params[])
{
	new playertwo;
...
YCMD:hideplayer(playerid, params[])
{
	new playertwo;
...
YCMD:addplayer(playerid, params[])
{
	new playertwo, isnpc;
...
YCMD:removeplayer(playerid, params[])
{
	new playertwo;
...
YCMD:spawnforworld(playerid, params[])
{
	new playertwo;
...
YCMD:broadcastdeath(playerid, params[])
{
	new playertwo;
...
YCMD:consolemsgs(playerid, params[])
{
	new enable, color;
...
YCMD:psyncdata(playerid, params[])
{
	new player1;
...
YCMD:actorinfo(playerid, params[])
{
...
YCMD:setplayerver(playerid, params[])
{
	SetPlayerVersion(playerid, params);
	return 1;
}

YCMD:disablekey(playerid, params[])
{
	new keys, ud, lr;
...
YCMD:vspawninfo(playerid, params[])
{
...
YCMD:bubble(playerid, params[]) // illustrate how SendRPC works. this is equal to SetPlayerChatBubbleForPlayer
{
...
YCMD:attachobject(playerid, params[])
{
...
YCMD:attachpobject(playerid, params[])
{
...
YCMD:attachdynamic(playerid, params[])
{
...
YCMD:attachedobj(playerid, params[])
{
	new slot;
...
YCMD:attachobjforplayer(playerid, params[])
{
...
YCMD:objectinfo(playerid, params[])
{
	new objectid;
...
YCMD:pobjectinfo(playerid, params[])
{
	new objectid;
	if(sscanf(params, "n", objectid)) return SendClientMessage(playerid, 0xFF0000AA, "USAGE: /objectinfo <Object ID>");
...
YCMD:objecttype(playerid, params[])
{
	new objectid;
...
YCMD:objmovespeed(playerid, params[])
{
...
YCMD:ismaterial(playerid, params[])
{
...
YCMD:getmaterial(playerid, params[])
{
	new objectid, materialindex;
...
YCMD:getpmaterial(playerid, params[])
{
	new objectid, materialindex;
...
YCMD:getmaterialtext(playerid, params[])
{
...
YCMD:getpmaterialtext(playerid, params[])
{
	new objectid, materialindex, text[144], materialsize, fontface[32], fontsize, bold, fontcolor, backcolor, textalignment;
...
YCMD:3dtextinfo(playerid, params[])
{
	if(!strcmp(params, "all"))
	{
		for(new i; i != 1024; i++)
...
YCMD:p3dtextinfo(playerid, params[])
{
	if(!strcmp(params, "all"))
	{
		for(new i; i != 1024; i++)
...
*/
YCMD:pickupinfo(playerid, params[])
{
	new id, Float:fPos[3];
	if(!strcmp(params, "all"))
	{
...
/*
YCMD:ppickupinfo(playerid, params[])
{
	new id, Float:fPos[3];
	if(!strcmp(params, "all"))
	{
...
*/
YCMD:modelinfo(playerid, params[])
{
...
YCMD:formatmsg(playerid, params[])
{
    SendClientMessagef(playerid, 0x33FF33AA, "Kurva %s %d %.3f", "any�d", -1547, 2654.6542);
    SendClientMessageToAllf(0x33FF33AA, "lofasz a %s", "f�ledbe");

    GameTextForPlayerf(playerid, 3000, 3, "Kis %s", "kecske");
    GameTextForAllf(3000, 3, "Kis %s", "kecske");

    SendPlayerMessageToPlayerf(playerid, playerid, "megbasztam %s %s h�t�n", "ap�dat", "any�d");
    SendPlayerMessageToAllf(playerid, "megbasztam %s %s h�t�n", "ap�dat", "any�d");
    return 1;
}

YCMD:netstats(playerid, params[])
{
...
YCMD:ysf_netstats(playerid, params[])
{
...
YCMD:gotoplayer(playerid, params[])
{
	new toplayer;
...

{
    ...
    printf("l�fasz - %d", limitglobalchat);
}
/*
